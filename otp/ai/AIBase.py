...
class AIBase:
    ...
    def __init__(self):
        ...
        self.wantTk = ConfigVariableBool('want-tk', 0).value
        ...
        if self.wantPets:
            if game.name == 'toontown':
                ...
                self.petMoodTimescale = ConfigVariableDouble(
                    'pet-mood-timescale', 1.).value
                self.petMoodDriftPeriod = ConfigVariableDouble(
                    'pet-mood-drift-period', PetConstants.MoodDriftPeriod).value
                self.petThinkPeriod = ConfigVariableDouble(
                    'pet-think-period', PetConstants.ThinkPeriod).value
                self.petMovePeriod = ConfigVariableDouble(
                    'pet-move-period', PetConstants.MovePeriod).value
        ...
        self.newDBRequestGen = ConfigVariableBool(
            'new-database-request-generate', 1).value
    ...
    def setupCpuAffinities(self, minChannel):
        ...
        if affinityMask != -1:
            ...
            if game.name == 'uberDog':
                ...
                affinity = ConfigVariableInt('ai-cpu-affinity', -1).value
                if autoAffinity and (affinity == -1):
                    affinity = 1
            if affinity != -1:
