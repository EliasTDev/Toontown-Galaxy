...
class AIZoneDataObj:
    ...
    def __init__(self, parentId, zoneId):
        assert self.notify.debug(
            f'AIZoneDataObj.__init__({parentId}, {zoneId})')
        self._parentId = parentId
        self._zoneId = zoneId
        self._refCount = 0
        self._collTravs = {}
    ...
    def destroy(self):
        assert self.notify.debug(
            f'AIZoneDataObj.destroy({self._parentId}, {self._zoneId})')
    ...
    def getRender(self):
        if not hasattr(self, '_render'):
            self._render = NodePath(
                f'render-{self._parentId}-{self._zoneId}')
            if ConfigVariableBool('leak-scene-graph', 0).value:
    ...
    def getCollTrav(self, name=None):
        ...
        if name not in self._collTravs:
            self._collTravs[name] = CollisionTraverser(
                f'cTrav-{name}-{self._parentId}-{self._zoneId}')
    ...
    def _doCollisions(self, task=None, topNode=None, cTravName=None):
        ...
        if topNode is not None:
            # make sure the topNode for collision traversal is a descendant of
            # our render node
    ...
    def doCollTrav(self, topNode=None, cTravName=None):
        ...
        assert self.notify.debug(
            f'doCollTrav({self._parentId}, {self._zoneId})')
    ...
    def startCollTrav(self, respectPrevTransform=1, cTravName=None):
        ...
        if cTravName is None:
            cTravName = AIZoneDataObj.DefaultCTravName
        assert self.notify.debug(
            'startCollTrav(%s, (%s, %s), %s)' %
            (cTravName, self._parentId, self._zoneId, respectPrevTransform))
        if cTravName not in self._collTravsStarted:
            ...
            assert self.notify.debug(
                'adding %s collision traversal for (%s, %s)' %
    ...
    def stopCollTrav(self, cTravName=None):
        ...
        if cTravName is None:
            cTravName = AIZoneDataObj.DefaultCTravName
        self.notify.debug(
            f'stopCollTrav({cTravName}, {self._parentId}, {self._zoneId})')
...
class AIZoneDataStore:
    ...
    def printStats(self):
        self.notify.debug(
            f'{len(self._zone2data)} zones have zone data allocated')
