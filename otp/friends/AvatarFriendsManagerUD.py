...
class AvatarFriendsManagerUD(DistributedObjectGlobalUD):
    ...
    def avatarOnlinePlusAccountInfo(self, avatarId, accountId, playerName,
        ...
        if avatarId and avatarId not in self.isAvatarOnline:
            ...
            self.avatarId2Info[avatarId] = AvatarFriendInfo(
                avatarName=str(avatarId),
            ...
            if not hasattr(friends, "keys"):  # check for error
                self.notify.warning(
                    f"self.db.getFriends(avatarId) has no keys {friends}")
                return
                ...
                if avatarId in avatarId2info:
                    ...
                    for friendId in friends:
                        if friendId in self.isAvatarOnline:
                            if (friendId in self.avatarId2FriendsList) and (
                                    avatarId in self.avatarId2FriendsList[friendId]):
                                self.sendUpdateToAvatarId(
                                    friendId, "updateAvatarFriend", [
                                        avatarId, self.getFriendView(
                                            friendId, avatarId)])
            ...
            for friend in list(friends.keys()):
                ...
                if friendId not in self.isAvatarOnline:
                    if friendId not in self.avatarId2Info:
                        ...
                        # fetch this friend's name from the gameDB since we
                        # don't have it yet
                        context = self.air.allocateContext()
                        dclassName = "DistributedAvatarUD"
                        self.air.contextToClassName[context] = dclassName
                        self.acceptOnce(
                            f"doFieldResponse-{context}", setName, [
                                friendId, self.avatarId2Info, [
                                    avatarId, ]])
                        self.air.queryObjectField(
                            dclassName, "setName", friendId, context)
                    else:
                        # print "AFMUD warning: info entry found for offline
                        # friend"
                        self.sendUpdateToAvatarId(
                            avatarId, "updateAvatarFriend", [
                                friendId, self.getFriendView(
                                    avatarId, friendId)])
    ...
    def getFriendView(self, viewerId, friendId):
        ...
        assert friendId in self.avatarId2FriendsList[viewerId], "avatarId2FriendsList[%d] has no key %d" % (
            viewerId, friendId)
        ...
        if avatarId:
            ...
            if friendsList is not None and avatarId in self.avatarId2Info:
                for friend in friendsList:
                    self.sendUpdateToAvatarId(
                        friend, "updateAvatarFriend", [
                            avatarId, self.avatarId2Info[avatarId]])
...
# ----------------------------------------------------------------------
    ...
    def requestInvite(self, otherAvatarId):
        ...
        assert self.notify.debugCall(f"avatarId:{avatarId}")
        ...
        # check for unvitation
        elif avatarId in self.avatarId2Unvitations.setdefault(otherAvatarId, []):
            ...
            reject(RejectCode.ALREADY_YOUR_FRIEND)
            self.notify.error(
                f"Friends lists out of sync {avatarId} {otherAvatarId}")
        ...
        # should be adding player friends list?
            ...
            self.air.writeServerEvent(
                'friendAccept', avatarId, f'{otherAvatarId}')

            # tell them they're friends and give presence info, includes online
            # status
            self.sendUpdateToAvatarId(
                otherAvatarId, "updateAvatarFriend", [
                    avatarId, self.getFriendView(
                        otherAvatarId, avatarId)])
            self.sendUpdateToAvatarId(
                avatarId, "updateAvatarFriend", [
                    otherAvatarId, self.getFriendView(
            ...
            self.air.writeServerEvent(
                'friendInvite', avatarId, f'{otherAvatarId}')
            self.sendUpdateToAvatarId(
                avatarId, "friendConsidering", [otherAvatarId])
            self.sendUpdateToAvatarId(
                otherAvatarId, "invitationFrom", [
                    avatarId, self.avatarId2Info[avatarId].avatarName])
    ...
    def requestRemove(self, otherAvatarId):
        ...
        avatarId = self.air.getAvatarIdFromSender()
        self.air.writeServerEvent(
            'friendRemove', avatarId, f'{otherAvatarId}')
        friendsList = self.avatarId2FriendsList.get(avatarId, None)
        ...
        if otherAvatarId in invitations:
            ...
            self.sendUpdateToAvatarId(
                otherAvatarId, "retractInvite", [avatarId])
        ...
        if otherAvatarId not in friendsList:
            ...
            self.sendUpdateToAvatarId(
                avatarId, "removeAvatarFriend", [otherAvatarId])
            self.sendUpdateToAvatarId(
                otherAvatarId, "removeAvatarFriend", [avatarId])
