...
class FriendManagerAI(DistributedObjectAI.DistributedObjectAI):
    ...
    friendDataFolder = simbase.config.GetString(
        'server-data-folder', 'dependencies/backups/')
    ...
    def friendQuery(self, inviteeId):
        ...
        if not invitee:
            self.air.writeServerEvent(
                'suspicious',
                inviteeId,
                'FriendManagerAI.friendQuery not on list')
            return
    ...
    def cancelFriendQuery(self, context):
        ...
        try:
            ...
            # FriendManagerAI.notify.warning('Message for unknown context ' +
            # `context`)
            return
    ...
    def inviteeFriendConsidering(self, response, context):
        ...
        """
        self.notify.debug(
            "AI: inviteeFriendConsidering(%d, %d)" %
            (response, context))
        avId = self.air.getAvatarIdFromSender()
        ...
        try:
            ...
            self.air.writeServerEvent(
                'suspicious',
                avId,
                'FriendManagerAI.inviteeFriendConsidering unknown context')
            FriendManagerAI.notify.warning(
                'Message for unknown context ' + context)
            return
    ...
    def inviteeFriendResponse(self, yesNoMaybe, context):
        ...
        try:
            ...
            self.air.writeServerEvent(
                'suspicious',
                avId,
                'FriendManagerAI.inviteeFriendResponse unknown context')
            FriendManagerAI.notify.warning(
                'Message for unknown context ' + context)
            return
        ...
        if yesNoMaybe == 1:
            ...
            dg = PyDatagram()
            dg.addServerHeader(
                self.GetPuppetConnectionChannel(
                    invitee.getDoId()),
                self.air.ourChannel,
            ...
            dg.addUint16(
                self.air.dclassesByName['DistributedToonAI'].getNumber())
            self.air.send(dg)
            invitee.extendFriendsList(inviter.getDoId())
            inviter.extendFriendsList(invitee.getDoId())
    ...
    def down_friendConsidering(self, recipient, yesNoAlready, context):
        ...
        self.sendUpdateToAvatarId(
            recipient, "friendConsidering", [
                yesNoAlready, context])
        self.notify.debug(
            "AI: friendConsidering(%d, %d)" %
            (yesNoAlready, context))
    ...
    def down_inviteeFriendQuery(self, recipient, inviterId, inviterName,
        ...
        self.notify.debug(
            "AI: inviteeFriendQuery(%d, %s, dna, %d)" %
    ...
    def down_inviteeCancelFriendQuery(self, recipient, context):
        ...
        self.sendUpdateToAvatarId(
            recipient, "inviteeCancelFriendQuery", [context])
    ...
    def trueFriendCodesTask(self, task):
        for trueFriendCode in list(self.trueFriendCodes.keys()):
            ...
            trueFriendCodeInfo = self.trueFriendCodes[trueFriendCode]
            # get the day of the code
            trueFriendCodeDay = trueFriendCodeInfo[1]
    ...
    def requestSecret(self):
        ...
        if not av:
            self.notify.warning(f'no av from avid: {avId}')
            return
        ...
        if len(av.getFriendsList()) < OTPGlobals.MaxFriends:
            ...
            self.air.writeServerEvent(
                'true-friend-code-requested',
                avId=avId,
                trueFriendCode=trueFriendCode)
        else:
    ...
    def down_requestSecretResponse(self, recipient, result, secret):
        ...
        """
        self.sendUpdateToAvatarId(
            recipient, 'requestSecretResponse', [
                result, secret])
    ...
    def createTrueFriendCode(self):
        ...
        trueFriendCode = '{0}{1}{2} {3}{4}{5}'.format(
            randomChar(),
            randomChar(),
            randomChar(),
            randomChar(),
    ...
    def submitSecret(self, secret):
            ...
            if friend:
                ...
                def handleAvatar(dclass, fields):
                    ...
                    self.air.dbInterface.updateObject(
                        self.air.dbId, friendId, self.air.dclassesByName['DistributedToonAI'], {
        ...
        self.air.writeServerEvent(
            'truefriend-code-submitted',
            avId=avId,
            friendId=friendId,
            trueFriendCode=secret)
    ...
    def down_submitSecretResponse(self, recipient, result, avId):
        ...
        """
        self.sendUpdateToAvatarId(
            recipient, 'submitSecretResponse', [
                result, avId])
...
#            self.beginInvite(invite)
    ...
    def cancelInvite(self, invite):
        if invite.inviteeKnows:
            self.down_inviteeCancelFriendQuery(
                invite.inviteeId, invite.context)
    ...
    def makeFriendsReply(self, result, context):
        ...
        if result:
            ...
            if invitee is not None:
                invitee.extendFriendsList(
                    invite.inviterId, invite.sendSpecialResponse)
                self.air.questManager.toonMadeFriend(invitee, inviter)

            #inviter = self.air.doId2do.get(invite.inviterId)
            if inviter is not None:
                inviter.extendFriendsList(
                    invite.inviteeId, invite.sendSpecialResponse)
    ...
    def __requestSecretReply(self, result, secret, requesterId):
        self.notify.debug(
            "request secret result = %d, secret = '%s', requesterId = %d" %
            (result, secret, requesterId))
        self.down_requestSecretResponse(requesterId, result, secret)

    def __submitSecretReply(self, result, secret, requesterId, avId):
        self.notify.debug(
            "submit secret result = %d, secret = '%s', requesterId = %d, avId = %d" %
