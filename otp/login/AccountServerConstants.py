...
class AccountServerConstants(RemoteValueSet):
    ...
    def __init__(self, cr):
        ...
        # rurbino the minPw and minName used to be zero, that sounds bad and
        # upped it to 1
        self.defaults = {
            'minNameLength': '1',
            'minPwLength': '1',
...
# (only used for customer service number for the US)
        ...
        if cr.productName == 'DisneyOnline-US':
            if base.config.GetBool('tt-specific-login', 0):
                # the new website does not have constants.php, don't try to
                # query it
                pass
            else:
                noquery = 0
        ...
        self.notify.debug(f'grabbing account server constants from {url.cStr()}')
