...
def checkName(name, otherCheckFuncs=[], font=None):
    ...
    def printableChars(name):
        for char in name:
            ...
            if ord(char) < 0x80 and char not in string.printable:
                notify.info(f'name contains non-printable char #{ord(char)}')
        ...
        for char in name:
            if not _validCharacter(char):
                if char in string.digits:
                    ...
                    notify.info(
                        f'name contains bad char: {TextEncoder().encodeWtext(char)}')
        ...
        if font:
            ...
            for c in name:
                if not tn.hasCharacter(str(c)):
                    notify.info(
                        f'name contains bad char: {TextEncoder().encodeWtext(c)}')
    ...
    def hasLetters(name):
        ...
        for word in words:
            ...
            if len(letters) == 0:
                notify.info(
                    f'word "{TextEncoder().encodeWtext(word)}" has no letters')
    ...
    def hasVowels(name):
        ...
        def perWord(word):
            ...
            if len(letters) > 2:
                ...
                if len(vowels) == 0:
                    notify.info(
                        f'word "{TextEncoder().encodeWtext(word)}" has no vowels')
    ...
    def checkPeriods(name):
        ...
        for word in words:
            ...
            if numPeriods > 2:
                notify.info(
                    f'word "{TextEncoder().encodeWtext(word)}" has too many periods')
                return OTPLocalizer.NCPeriodUsage
    ...
    def checkJapanese(name):
        ...
        for char in (ord(char) for char in te.decodeText(name)):
            if char not in allowedUtf8:
                ...
                if char in asciiDigits:
                    ...
                    notify.info(
                        f'name contains not allowed utf8 char: 0x{char:04x}')
        ...
        if (dc < 2):
            notify.info(f'name is too short: {dc:0.1f}')
            return OTPLocalizer.NCTooShort
        elif (dc > 8):
            notify.info(
                f'name has been occupied more than eight display cells: {dc:0.1f}')
            return OTPLocalizer.NCGeneric
    ...
    notify.info(
        f"checking name \"{TextEncoder().encodeWtext(name).decode('utf-8')}\"...")
...
# these tests can be removed or replaced for international versions of the
# codebase
