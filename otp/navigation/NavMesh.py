...
class NavMesh(object):
    ...
    def visualize(
        ...
        self.visVertexData = GeomVertexData(
            "OMGVERTEXDATA2", gFormat, Geom.UHDynamic)
        self.visVertexWriter = GeomVertexWriter(self.visVertexData, "vertex")
        self.visVertexColorWriter = GeomVertexWriter(
            self.visVertexData, "color")
    ...
    def _discoverInitialConnectivity(self):
        ...
        for pId in list(self.polyToVerts.keys()):
            ...
            for v in verts:
                candidates += [p for p in self.vertToPolys[v]
    ...
    def _attemptToMergePolys(self, polyA, polyB):
        ...
        # Convexity Check.
        # Verify that removing the edge preserves convexity and bail out if
        # not.
        ...
        # Now walk A clockwise until we hit the CCW-most vert of the shared
        # edge.
        ...
        # Walk B clockwise until we get back to the CW-most vert of the shared
        # edge.
        ...
        # Make sure we have vertex->poly pointers for all the new verts we
        # added to A.
        for v in newVerts:
    ...
    def _pruneExtraVerts(self):
        ...
        print(f"Starting verts: {len(self.vertToPolys)}")
        for polyId in list(self.connectionLookup.keys()):
            self._cleanPoly(polyId)
        print(f"Ending verts: {len(self.vertToPolys)}")

    def _compactPolyIds(self):
        polyList = sorted(self.polyToVerts.keys())
...
# return ord(str[pos+2])
    ...
    def makeNodeLocator(self, environment):
        ...
        for pId in self.polyToAngles:
            ...
            # Unfortunately we can only make triangle or square
            # CollisionPolygons on the fly
    ...
    def findNodeFromPos(self, environment, x, y):
        ...
        if self.chq.getNumEntries() != 1:
            self.notify.warning(
                f"No node found at position: {x}, {y} in {environment}")
            return 0
        ...
        assert e.hasInto()
        if not e.hasInto():
            self.notify.warning(f"No into found for collision {e}")
