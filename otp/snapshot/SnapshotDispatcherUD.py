...
# TODO find alternative to depercated http module
    ...
    def clearRecentDeleteRecord(self, avatarId):
        self.notify.debug(f"Removing deletion record for {avatarId}.")
    ...
    def requestRender(self, avatarId, replyTo=None):
        ...
        if avatarId in self.recentlyDeletedAvatars:
            self.notify.debug(
                f"Ignoring requestRender for deleted avatar {avatarId}.")
    ...
    def avatarDeleted(self, avatarId):
        ...
        """
        self.notify.debug(f"Creating deletion record for {avatarId}.")
        self.recentlyDeletedAvatars[avatarId] = 1
        uber.taskMgr.doMethodLater(
            1800.0,
            self.clearRecentDeleteRecord,
            f'clearRecentDeleteRecord-{avatarId}',
            [avatarId])
    ...
    def jobTimedOut(self, jobId, task=None):
        ...
        if job is not None:
            ...
            self.notify.warning(
                "Didn't have a job %d listed when we got the timeout." %
                jobId)
        return Task.done
    ...
    def errorFetchingAvatar(self, rendererLoc, jobId):
        ...
        if job is None:
            self.notify.warning(
                "Got back an error for an unrecognized job: %d" %
                jobId)
            return
    ...
    def renderSuccessful(self, rendererLoc, jobId):
        ...
        if job is None:
            self.notify.warning(
                "Got back success for an unrecognized job: %d" %
                jobId)
            return

        self.notify.debug(
            "Job %d: Successfully rendered avatar %d" %
            (jobId, job.avatarId))
        ...
        if job.replyTo is not None:
            job.replyTo.respond(
                f"<html><body>{job.writeToFile}</body></html>")
