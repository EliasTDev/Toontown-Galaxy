...
class LastSeenDB:
    ...
    def __init__(self, log, host, port, user, passwd, dbname):
        ...
        try:
            ...
            self.log.warning(
                "Failed to connect to MySQL at %s:%d.  LastSeenDB is disabled." %
                (host, port))
            self.sqlAvailable = 0
            return

        self.log.info(
            "Connected to lastseen MySQL db at %s:%d." %
            (host, port))
        ...
        try:
            ...
            self.log.debug(
                f"{self.dbname} database not found, lastseen not active.")
    ...
    def getInfo(self, playerId, isRetry=False):
        ...
        try:
            ...
            if isRetry:
                self.log.error(
                    f"Error on getInfo retry, giving up:\n{str(e)}")
                return FriendInfo(playerName="NotFound")
                ...
                self.log.error(
                    f"Unknown error on getInfo, retrying:\n{str(e)}")
                self.reconnect()
                return self.getInfo(playerId, True)
        except Exception as e:
            self.log.error(f"Unknown error on getInfo, giving up:\n{str(e)}")
            return FriendInfo(playerName="NotFound")
    ...
    def setInfo(self, playerId, info, isRetry=False):
        ...
        try:
            ...
            if isRetry:
                ...
                self.log.error(
                    f"Unknown error on setInfo, retrying:\n{str(e)}")
                self.reconnect()
                self.setInfo(playerId, info, True)
                return
        except Exception as e:
            self.log.error(f"Unknown error on setInfo, giving up:\n{str(e)}")
            return
    ...
    def getTableStatus(self, isRetry=False):
        ...
        try:
            ...
            if isRetry:
                self.log.error(
                    "Error on getTableStatus retry, giving up:\n" %
                    str(e))
                return None
            elif e[0] == SERVER_GONE_ERROR or e[0] == SERVER_LOST:
                self.reconnect()
                ...
                self.log.error(
                    f"Unknown error on getTableStatus, retrying:\n{str(e)}")
                self.reconnect()
                return self.getTableStatus(True)
        except Exception as e:
            self.log.error(
                f"Unknown error on getTableStatus, giving up:\n{str(e)}")
            return None
