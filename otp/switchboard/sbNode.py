...
class sbNode(Pyro5.core.SynchronizedObjBase):  # ,ChannelListener):
    ...
    def __init__(self,
        ...
        self.log = sbLog(f":sb.node.{nodeName}", clHost, clPort)
        ...
        self.log.info(f"-- sb.node.{self.nodeName} is ready. --")

    def joinChannels(self):
        self.log.debug(f"Joining channels: {str(self.channelList)}")
    ...
    def initPyro(self, nsHost, nsPort, listenPort):
        if nsHost is None:
            self.log.warning(
                "No NS host/port specified, trying location via broadcast.")
        ...
        try:
            ...
            pass
        try:
            self.pyroDaemon.connectPersistent(
                self, f":sb.node.{self.nodeName}")
        except Pyro.errors.NamingError:
            self.log.debug(
                ":sb.node.%s was already registered, overwriting old entry." %
                self.nodeName)
            self.nameServer.unregister(f":sb.node.{self.nodeName}")
            self.pyroDaemon.connectPersistent(
                self, f":sb.node.{self.nodeName}")
    ...
    def rcvMessage(self, message):
        ...
        if message.targetChannel == sbConfig.DISL2SBChannel:
            if message.functionCode == sbConfig.FC_DISLSendFriendAdd:
                ...
                self.log.warning(
                    f"Invalid FC received from DISL MD: {message.functionCode}")
                self.log.warning(
                    "Offending message:  %s %s %s %s" %
                    (message.targetChannel,
                     message.responseChannel,
                     message.functionCode,
                     message.otherData))
            ...
            # we should not be here!
            self.log.warning(
                "Got an MD message with a weird target channel:" %
                message.targetChannel)
            self.log.warning(
                "Offending message:  %s %s %s %s" %
                (message.targetChannel,
    ...
    def handleDISLSendFriendsList(self, message):
        ...
        if friendOne in self.localPlayers:
            ...
            # tell the other nodes I'm here (they probably need to know, but
            # might already)
            self.sendEnterRemotePlayer(
                friendOne,
                self.localPlayers[friendOne],
                self.id2Friends[friendOne])
    ...
    def handleDISLSendNewFriendship(self, message):
        ...
        if not len(friends) == 1:
            self.log.error(
                f"len(friends) was {len(friends)} in handleDISLSendNewFriendship!")
            return
    ...
    def handleDISLSendFriendRemove(self, message):
        ...
        self.log.debug(
            "Got a friend remove notice from disl: %d and %d" %
            (friendOne, friendTwo))
    ...
    def parsingError(self, message):
        self.log.error(
            "Couldn't parse MD message: %s %s %s %s" %
            (message.targetChannel,
             message.responseChannel,
             message.functionCode,
             message.otherData))
    ...
    def sendEnterNode(self):
        if self.wedge:
            ...
            try:
                ...
                self.log.info(
                    f"Couldn't contact sb.wedge.{self.nodeName}, wedge is None.")
                self.wedge = None

        for node in self.nodeList:
            self.log.debug(f"poking node {node}")
            try:
                proxy = self.nodeProxy[node]
                proxy.recvEnterNode(self.nodeName)
            except Exception as e:
                self.log.info(
                    f"Couldn't contact sb.node.{node}, removing from nodelist.")
    ...
    def recvEnterNode(self, nodeName):
        self.log.debug(f"Received enterNode({nodeName})")
        # clear all players present on the remote node
        self.updateNodes()

    def recvExitNode(self, nodeName):
        self.log.debug(f"Received exitNode({nodeName})")
        # clear all players present on the remote node
        self.updateNodes()

    def recvEnterWedge(self, wedgeName):
        self.log.debug(f"Received enterWedge({wedgeName}).")
        self.wedgeName = wedgeName
        # clear all local players?
        self.updateWedge()

    def recvExitWedge(self, wedgeName):
        self.log.debug(f"Received exitWedge({wedgeName}).")
        # clear all local players?
        self.updateWedge()

    def updateWedge(self):
        try:
            self.wedge = Pyro.core.getProxyForURI(
                self.nameServer.resolve(
                    f":sb.wedge.{self.wedgeName}"))
            ...
            self.log.info(f"-- Connected to sb.wedge.{self.wedgeName}. --")
        except Pyro.errors.NamingError:
            self.wedge = None
            self.log.info(
                f"sb.wedge.{self.wedgeName} not found on name server, wedge is None.")
    ...
    def updateNodes(self):
        ...
        for node in nodelist:
            if node[0] != self.nodeName:
                self.log.debug(f"Getting proxy for node {node[0]}")
                self.nodeList.append(node[0])
                self.nodeProxy[node[0]] = Pyro.core.getProxyForURI(
                    self.nameServer.resolve(f":sb.node.{node[0]}"))
        ...
        self.log.debug(f"Updated node list: {str(self.nodeList)}")
    ...
    def recvEnterLocalPlayer(self, playerId, playerInfo):
        if playerId in self.localPlayers:
            self.log.warning(
                "Warning: enterPlayer(%d) called, but I already have this player." %
                (playerId))
        ...
        try:
            self.log.debug(
                "Sending msg to DISL: %s %s %s %s" %
                (sbConfig.SB2DISLChannel,
                 sbConfig.DISL2SBChannel +
                 self.nodeName,
    ...
    def recvExitLocalPlayer(self, playerId, playerInfo=None):
        if playerId not in self.localPlayers:
            self.log.warning(
                "Warning: exitPlayer(%d) called, but I don't have this one." %
                playerId)
    ...
    def sendEnterRemotePlayer(self, playerId, playerInfo, friendsList):
        try:
            for node in self.nodeList:
                self.nodeProxy[node].recvEnterRemotePlayer(
                    playerId, self.nodeName, playerInfo, friendsList)
        except ConnectionClosedError as e:
            self.log.error(
                "ConnectionClosedError in sendEnterRemotePlayer.  Refreshing all nodes.")
            self.updateNodes()
        except ProtocolError as e:
            self.log.error(
                "ProtocolError (%s) in sendEnterRemotePlayer.  Refreshing all nodes." %
                str(e))
            self.updateNodes()
        except Exception as e:
            self.log.error(
                "Error sending enterRemotePlayer to sb.node.%s: %s" %
                (node, ''.join(
                    Pyro.util.getPyroTraceback(e))))
    ...
    def sendExitRemotePlayer(self, playerId, friendsList):
        try:
            for node in self.nodeList:
                self.nodeProxy[node].recvExitRemotePlayer(
                    playerId, self.nodeName, friendsList)
        except ConnectionClosedError as e:
            self.log.error(
                "ConnectionClosedError in sendExitRemotePlayer.  Refreshing all nodes.")
            self.updateNodes()
        except ProtocolError as e:
            self.log.error(
                "ProtocolError (%s) in sendExitRemotePlayer.  Refreshing all nodes." %
                str(e))
            self.updateNodes()
        except Exception as e:
            self.log.error(
                "Error sending exitRemotePlayer to sb.node.%s: %s" %
                (node, ''.join(
                    Pyro.util.getPyroTraceback(e))))

    # node->node

    def recvEnterRemotePlayer(
            self,
            playerId,
            nodeName,
            playerInfo,
        ...
        if playerId in self.remotePlayerLoc:
            self.log.warning(
                "Warning: enterRemotePlayer(%d) called, but I already see this player." %
                (playerId))

        self.log.debug(
            "Saw player %d enter at :sb.node.%s." %
            (playerId, nodeName))
        ...
        if self.wedge is not None:
            try:
                self.wedge.recvEnterRemotePlayer(
                    playerId, playerInfo, friendsList)
            except ConnectionClosedError as e:
                self.log.error(
                    "ConnectionClosedError in recvEnterRemotePlayer.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError in recvEnterRemotePlayer.  Reconnecting to wedge.")
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't send player enter notice to my wedge, had an error: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
                self.log.error(
                    f"I sent: {str(playerId)}, {str(playerInfo)}, {str(friendsList)}")
        else:
    ...
    def recvExitRemotePlayer(self, playerId, nodeName, friendsList):
        if playerId not in self.remotePlayerLoc:
            self.log.warning(
                "Warning: exitRemotePlayer(%d) called, but I don't see this player.  Ignoring." %
                (playerId))
            return
        ...
        if self.wedge is not None:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in recvExitRemotePlayer.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError in recvExitRemotePlayer.  Reconnecting to wedge.")
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't send player exit notice to my wedge, had an error: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
        else:
    ...
    def addFriendship(self, playerId1, playerId2):
        ...
        try:
            ...
            try:
                ...
                self.log.error(
                    f"Unknown exception in removeFriendship: {str(e)}")
        if err is not None:
            try:
                self.wedge.recvAddFriendshipError(playerId, err)
            except ConnectionClosedError as e:
                self.log.error(
                    "ConnectionClosedError in recvSecretRequestError.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError in recvSecretRequestError.  Reconnecting to wedge.")
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't send recvAddFriendshipError to my wedge")
    ...
    def recvSecretRequest(self, playerId, parentUsername, parentPassword):
        ...
        try:
            ...
            try:
                ...
                self.log.error(
                    f"Unknown exception in generateToken: {str(e)}")
        ...
        if err is None:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in recvSecretGenerated.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError in recvSecretGenerated.  Reconnecting to wedge.")
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't send recvSecretGenerated to my wedge, had an error: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
        else:
            try:
                ...
                self.log.error(
                    "Couldn't send recvSecretRequestError to my wedge, had an error: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))

    def recvSecretRedeem(
            self,
            playerId,
            secret,
            parentUsername,
        ...
        try:
            ...
            try:
                ...
                self.log.error(f"Unknown exception in redeemToken: {str(e)}")
        ...
        if err is not None:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in recvSecretRedeemError.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError in recvSecretRedeemError.  Reconnecting to wedge.")
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't send recvSecretRedeemError to my wedge, had an error: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
    ...
    def sendLocalFriendsUpdate(self, friendOne, friends):
        ...
        if self.wedge is not None:
            ...
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in sendLocalFriendsUpdate.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError (%s) in sendLocalFriendsUpdate.  Reconnecting to wedge." %
                    str(e))
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't sendLocalFriendsUpdate to my wedge, had an error: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
    ...
    def sendLocalFriendshipRemoved(self, friendOne, friendTwo):
        self.log.debug(
            "sendLocalFriendshipRemoved on %d,%d" %
            (friendOne, friendTwo))
        if self.wedge is not None:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in sendFriendshipRemoved.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError in sendLocalFriendshipRemoved.  Reconnecting to wedge.")
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't sendFriendshipRemoved to my wedge, had an error: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
    ...
    def sendWhisper(self, recipientId, senderId, msgText):
        ...
        if recipientId not in self.remotePlayerLoc:
            self.log.warning(
                "I don't see %d anywhere!  Whisper not delivered." %
                recipientId)
            return
        ...
        self.log.debug(f"Delivering whisper to :sb.node.{loc}.")
        ...
        try:
            ...
            self.log.error(
                "ConnectionClosedError in sendWhisper.  Reconnecting to nodes.")
            self.updateNodes()
        except ProtocolError as e:
            self.log.error(
                "ProtocolError in sendWhisper.  Reconnecting to nodes.")
            self.updateNodes()
        except Exception as e:
            self.log.error(
                "Couldn't send whisper to sb.node.%s, had an error: %s" %
                (loc, ''.join(
                    Pyro.util.getPyroTraceback(e))))
    ...
    def recvWhisper(self, recipientId, senderId, msgText):
        ...
        if self.wedge is not None:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in recvWhisper.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError in recvWhisper.  Reconnecting to wedge.")
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't send whisper to my wedge, had an error: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
                self.log.error(
                    "I sent: %d, %d, %s" %
                    (recipientId, senderId, msgText))
        else:
            self.log.warning("recvWhisper: No wedge connected!")

    def sendSCWhisper(self, recipientId, senderId, msgText):
        self.log.debug(
            "sbNode.sendSCWHISPER %d to %d: %s" %
            (senderId, recipientId, msgText))
        # if self.localPlayers.has_key(recipientId):
        #    self.log("Warning: I own %d.  Ignoring whisper." % (recipientId))
        #    self.wedge.recvWhisperFailed(recipientId,senderId,msgText)
        ...
        # CHECK FRIENDSHIP, permissions, ignore list, etc?
        loc = self.remotePlayerLoc[recipientId]
        self.log.debug(f"Delivering SCwhisper to :sb.node.{loc}.")
        # self.nodeProxy[loc]._setOneway(["recvWhisper"])
        try:
            ...
            self.log.error(
                "ConnectionClosedError in sendSCWhisper.  Reconnecting to nodes.")
            self.updateNodes()
        except ProtocolError as e:
            self.log.error(
                "ProtocolError in sendSCWhisper.  Reconnecting to nodes.")
            self.updateNodes()
        except Exception as e:
            self.log.error(
                "Couldn't send SCwhisper to sb.node.%s, had an error: %s" %
                (loc, ''.join(
                    Pyro.util.getPyroTraceback(e))))

    def recvSCWhisper(self, recipientId, senderId, msgText):
        self.log.debug(
            "sbNode.recvSCWHISPER %d to %d: %s" %
            (senderId, recipientId, msgText))
        if self.wedge is not None:
            try:
                self.wedge.recvWhisper(recipientId, senderId, msgText)
                ...
                self.log.error(
                    "ConnectionClosedError in recvSCWhisper.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError in recvSCWhisper.  Reconnecting to wedge.")
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't send whisper to my wedge, had an error: %s" %
    ...
    def recvMailUpdate(self, recipientId, senderId, msgText):
        if self.wedge is not None:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in recvMailUpdate.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError in recvMailUpdate.  Reconnecting to wedge.")
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't send mail update to my wedge, had an error: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
                self.log.error(
                    "I sent: %d, %d, %s" %
            ...
            self.log.debug(
                "Tried to receive mail update, but no wedge connected!")
    ...
    def getMail(self, recipientId):
        ...
        if self.wedge is not None:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in recvMail.  Reconnecting to wedge.")
                self.updateWedge()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError in recvMail.  Reconnecting to wedge.")
                self.updateWedge()
            except Exception as e:
                self.log.error(
                    "Couldn't send mail to my wedge, had an error: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
