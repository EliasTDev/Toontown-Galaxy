...
class sbWedge(Pyro.core.SynchronizedObjBase):
    def __init__(self, wedgeName,
        ...
        self.log = sbLog(f":sb.wedge.{wedgeName}", clHost, clPort)

        badwordpy.init(bwDictPath, "")

        if not badwordpy.test("fuck") and not allowUnfilteredChat:
            self.log.error(
                "Dirty word filter not working, refusing to start.  Devs should add .prc setting 'allow-unfiltered-chat 1' to bypass.")
            raise Exception(
                "No dirty word filter, aborting SB Wedge startup.  Devs should add .prc setting 'allow-unfiltered-chat 1' to bypass.")
        ...
        try:
            ...
            self.log.warning(
                "Failed to connect to Switchboard.  Player friends are being faked.")
        ...
        self.log.info(f"-- sb.wedge.{self.wedgeName} is ready. --")
    ...
    def initPyro(self, nsHost, nsPort, listenPort, bcLocate):
        ...
        if nsHost is None:
            if bcLocate is True:
                ...
                self.log.warning(
                    "No NS host/port specified and no broadcast enabled.  sbWedge not connecting to Switchboard.")
                return
        else:
        ...
        try:
            self.pyroDaemon.connectPersistent(
                self, f":sb.wedge.{self.wedgeName}")
        except Pyro.errors.NamingError:
            self.log.debug(
                ":sb.wedge.%s was already registered, overwriting old entry." %
                self.wedgeName)
            self.nameServer.unregister(f":sb.wedge.{self.wedgeName}")
            self.pyroDaemon.connectPersistent(
                self, f":sb.wedge.{self.wedgeName}")
    ...
    def refreshNS(self):
        self.nameServer = Pyro.naming.NameServerLocator().getNS(
            host=self.nsHost, port=self.nsPort)
        self.pyroDaemon.useNameServer(self.nameServer)
    ...
    def sendEnterWedge(self):
        if self.node:
            try:
                ...
                self.log.warning(
                    f"Error contacting my node ({str(e)}), node is None.")
                self.node = None
    ...
    def updateNode(self):
        ...
        try:
            self.refreshNS()
            self.node = Pyro.core.getProxyForURI(
                self.nameServer.resolve(
                    f":sb.node.{self.nodeName}"))
            ...
            self.node._setOneway(["recvEnterWedge",
                                  ...
                                  "recvOpenInvite",
                                  "recvDeclineInvite"])
            self.log.info(f"-- Connected to sb.node.{self.nodeName}. --")
            self.sbConnected = True
        except Exception as e:
            self.node = None
            self.sbConnected = False
            self.log.debug(
                f"Failed to locate sb.node.{self.nodeName}, node is None.")
    ...
    def enterPlayer(self, playerId, playerInfo):
        ...
        if self.node:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in enterPlayer.  Refreshing node.")
                self.updateNode()
            except ProtocolError as e:
                self.log.error(
                    f"ProtocolError ({str(e)}) in enterPlayer.  Refreshing node.")
                self.updateNode()
            except Exception as e:
                self.log.error(
                    "Unknown error sending enterLocalPlayer to my node: %s" %
                    ''.join(
    ...
    def exitPlayer(self, playerId):
        ...
        if self.node:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in exitPlayer.  Refreshing node.")
                self.updateNode()
            except ProtocolError as e:
                self.log.error(
                    f"ProtocolError ({str(e)}) in exitPlayer.  Refreshing node.")
                self.updateNode()
            except Exception as e:
                self.log.error(
                    "Error sending exitLocalPlayer to my node: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
    ...
    def exitAvatar(self, avatarId):
        ...
        if self.node:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in exitAvatar.  Refreshing node.")
                self.updateNode()
            except ProtocolError as e:
                self.log.error(
                    f"ProtocolError ({str(e)}) in exitAvatar.  Refreshing node.")
                self.updateNode()
            except Exception as e:
                self.log.error(
                    "Error sending exitLocalAvatar to my node: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
    ...
    def sendSecretRequest(
        ...
        if self.node:
            try:
                self.node.recvSecretRequest(
                    playerId, parentUsername, parentPassword)
            except ConnectionClosedError as e:
                self.log.error(
                    "ConnectionClosedError in sendSecretRequest.  Refreshing node.")
                self.updateNode()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError (%s) in sendSecretRequest.  Refreshing node.")
                self.updateNode()
            except Exception as e:
                self.log.error(
                    "Error sending secretRequest to my node: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))

    def sendSecretRedeem(
            self,
            playerId,
            secret,
            parentUsername=None,
        ...
        if self.node:
            try:
                self.node.recvSecretRedeem(
                    playerId, secret, parentUsername, parentPassword)
            except ConnectionClosedError as e:
                self.log.error(
                    "ConnectionClosedError in sendSecretRedeem.  Refreshing node.")
                self.updateNode()
            except ProtocolError as e:
                self.log.error(
                    "ProtocolError (%s) in sendSecretRedeem.  Refreshing node.")
                self.updateNode()
            except Exception as e:
                self.log.error(
                    "Error sending secretRedeem to my node: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
    ...
    def sendWhisper(self, recipientId, senderId, msgText):
        ...
        if self.node:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in sendWhisper.  Refreshing node.")
                self.updateNode()
            except ProtocolError as e:
                self.log.error(
                    f"ProtocolError ({str(e)}) in sendWhisper.  Refreshing node.")
                self.updateNode()
            except Exception as e:
                self.log.error(
                    "Error sending whisper to my node: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
    ...
    def sendWLWhisper(self, recipientId, senderId, msgText):
        ...
        if self.node:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in sendWLWhisper.  Refreshing node.")
                self.updateNode()
            except ProtocolError as e:
                self.log.error(
                    f"ProtocolError ({str(e)}) in sendWLWhisper.  Refreshing node.")
                self.updateNode()
            except Exception as e:
                self.log.error(
                    "Error sending WLwhisper to my node: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
    ...
    def sendSCWhisper(self, recipientId, senderId, msgText):
        ...
        if self.node:
            try:
                ...
                self.log.error(
                    "ConnectionClosedError in sendSCWhisper.  Refreshing node.")
                self.updateNode()
            except ProtocolError as e:
                self.log.error(
                    f"ProtocolError ({str(e)}) in sendSCWhisper.  Refreshing node.")
                self.updateNode()
            except Exception as e:
                self.log.error(
                    "Error sending SCwhisper to my node: %s" %
                    ''.join(
                        Pyro.util.getPyroTraceback(e)))
    ...
    def recvWLWhisper(self, recipientId, senderId, msgText):
        self.log.debug(
            "sbWedge.recvWLWHISPER %d to %d: %s" %
            (senderId, recipientId, msgText))
        pass
    ...
    def recvSCWhisper(self, recipientId, senderId, msgText):
        self.log.debug(
            "sbWedge.recvSCWHISPER %d to %d: %s" %
            (senderId, recipientId, msgText))
        pass
    ...
    def recvWhisperFailed(self, recipientId, senderId, msgText):
        self.log.debug(
            "Received whisper fail notice for %d to %d: %s" %
            (senderId, recipientId, msgText))
        pass
    ...
    def recvSCWhisperFailed(self, recipientId, senderId, msgText):
        self.log.debug(
            "Received SCwhisper fail notice for %d to %d: %s" %
            (senderId, recipientId, msgText))
        pass
    ...
    def _validateChatMessage(self, recipientId, senderId, msg):
        if len(msg) < 1 or len(msg) > sbConfig.chatMessageMaxChars:
            self.log.security(
                "Invalid chat message length.  %d->%d: %s" %
                (recipientId, senderId, msg))
            return False
    ...
    def recvMailUpdate(self, recipientId, senderId, msgText):
        self.log.debug(
            "Received mail update for %d to %d: %s" %
            (senderId, recipientId, msgText))
        pass
