...
uber.wantSwitchboard = 0  # off for now till we get switchboard fixed
if uber.wantSwitchboard:
    ...
    class udWedge(sbWedge):
        ...
        def recvWhisper(self, recipientId, senderId, msgText):
            self.log(
                "**Whisper from %d received for %d: %s" %
                (senderId, recipientId, msgText))
            if uber.GEMdemoWhisperRecipientDoid == 0:
                self.log.error(
                    "SB demo enabled but no recipient DOID specified!")
            self.dcmUD.sendWhisperFrom(
                uber.GEMdemoWhisperRecipientDoid,
                uber.GEMdemoWhisperRecipientDoid,
                msgText)
...
class DistributedChatManagerUD(DistributedObjectGlobalUD):
    ...
    def accountOnline(self, accountId):
        ...
        if accountId in self.isAccountOnline:
            assert self.notify.debug(
                f"\n\nWe got a duplicate account online notice {accountId}")
    ...
    def checkAvatarId(self, avatarId):
        if not avatarId:
            # SUSPICIOUS
            self.notify.warning(f"Bogus avatarId ({avatarId})")
        # elif not self.isAvatarOnline.has_key(avatarId):
            ...
            # attacked.
            self.notify.warning(
                f"Too many requests at once from avatar {avatarId}")
        else:
            return True
        return False
    ...
    def setAvatarLocation(self, avatarId, parentId, zoneId):
        ...
        assert self.notify.debugCall(f"sender:{sender}")
    ...
    def chatTo(self, message, chatFlags):
        ...
        if not accountId:
            assert self.notify.debugCall(f"accountId:{accountId} (zero!)")
            return
        if accountId not in self.isAccountOnline:
            assert self.notify.debugCall(
                f"accountId:{accountId} (not online!)")
            return
        assert self.notify.debugCall(f"accountId:{accountId}")
        ...
        if uber.wantSwitchboardHacks:
            print(f"DistributedChatManagerUD->Switchboard: {message}")
    ...
    def speedChatTo(self, msgIndex):
        ...
        if self.checkAvatarId(avatarId):
            ...
            if location is not None:
                ...
                assert self.notify.debug(
                    f"Unknown location for {accountId}")
    ...
    def whisperTo(self, toId, message):
        ...
        if self.checkAvatarId(avatarId):
            ...
            if chatPermissions is not None:
                ...
                assert self.notify.debug(
                    f"Unknown permissions for {accountId}")
    ...
    def crewChatTo(self, message):
        ...
        if crewChannel is not None:
            ...
            assert self.notify.debug(f"Unknown crew for {accountId}")
    ...
    def guildChatTo(self, message):
        ...
        if guildChannel is not None:
            ...
            assert self.notify.debug(f"Unknown guild for {accountId}")
