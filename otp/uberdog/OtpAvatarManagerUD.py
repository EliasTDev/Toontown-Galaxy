...
class AsyncRequestRemove(ManagedAsyncRequest):
    ...
    def removeAvatar(self):
        try:
            ...
            self.distObj.db.removeAvatarFromSubscription(
                self.avatarId, self.subId)
            uber.air.writeServerEvent(
                'removeAvatar', self.avatarId, f'{self.accountId}|{self.subId}')
            ...
            self.distObj.sendUpdateToAccountId(
                self.accountId, "removeAvatarResponse", [
                    self.avatarId, self.subId])
        except ValueError:
...
class OtpAvatarManagerUD(DistributedObjectGlobalUD):
    ...
    def __init__(self, air):
        ...
        self.DBhost = uber.config.GetString(
            "accountavatars-db-host", "localhost")
        self.DBport = uber.config.GetInt("accountavatars-db-port", 3306)
        self.DBname = uber.config.GetString(
            "accountavatars-db-name", "avatars")
        ...
        if priorAvatar:
            if config.GetBool('want-orphanedavatar-report', 0):
                assert self.notify.warning(
                    f'found orphaned avatar: ({accountId},{avatarId})')
    ...
    def clearPendingAvatarCreates(self, accountId):
        ...
        if subId:
            ...
            if numPending == 1:
                ...
                self.notify.warning(
                    "clearPendingAvatarCreates: accountId: %s, subId: %s, numPending: %s" %
                    (accountId, subId, numPending))
                return 0
        else:
    ...
    def requestAvatarList(self, senderId):
        ...
        if not self.air.checkAccountId(accountId):
            ...
            self.sendUpdateToAccountId(
                accountId, "rejectAvatarList", [
                    RejectCode.INVALID_ACCOUNT])
            return reasonCode
        ...
        assert self.notify.debug(
            f"accountId: {accountId} avatarRecords: {avatarRecords}")
        self.AsyncRequestAvatarList(
            self, replyToChannel, accountId, avatarRecords)
        return 1

    def sendAvIdList(self, accountId):
        # Tell the game server the list of avatars this account is allowed to
        # play
        avatarIds = []
        # Get the avatar records for all family member's subscriptions
        accountDetails = self.air.getAccountDetails(accountId)
        ...
        if accountDetails is None:
            self.notify.warning(
                'tried to send avId list for account %s that has since logged out' %
                (accountId))
            return
    ...
    def requestRemoveAvatar(self, senderId, avatarId, subId, confirmPassword):
        ...
        if not self.air.checkAccountId(accountId):
            reasonCode = RejectCode.INVALID_ACCOUNT
            self.sendUpdateToAccountId(
                accountId, "rejectRemoveAvatar", [reasonCode])
            return reasonCode
        ...
        if avatarId not in avatarIds:
            self.notify.warning(
                "accountId: %s tried to delete avatarId: %s not on subId: %s" %
        ...
        if self.air.isAvatarOnline(avatarId):
            self.notify.warning(
                "accountId: %s tried to delete avatarId: %s currently in use" %
                (accountId, avatarId))
        ...
        # Rule: When using linked accounts, you can only delete avatars you
        # created
        creatorId = creatorIds[avatarIds.index(avatarId)]
        if simbase.config.GetBool('allow-linked-accounts', 0) and \
           accountId != creatorId:
            self.notify.warning(
                "accountId: %s tried to delete avatarId: %s from another creatorId: %s" %
        ...
        self.air.writeServerEvent(
            'requestRemoveAvatar', avatarId, f'{accountId}|{subId}')

        self.AsyncRequestRemove(
            self,
            replyToChannel,
            subId,
            accountId,
    ...
    def requestShareAvatar(self, senderId, avatarId, subId, shared):
        ...
        if not self.air.checkAccountId(accountId):
            reasonCode = RejectCode.INVALID_ACCOUNT
            self.sendUpdateToAccountId(
                accountId, "rejectShareAvatar", [reasonCode])
            return reasonCode
        ...
        if avatarId not in avatarIds:
            self.notify.warning(
                "accountId: %s tried to share avatarId: %s he did not create" %
                (accountId, avatarId))
            reasonCode = RejectCode.NOT_YOUR_AVATAR
            self.sendUpdateToAccountId(
                accountId, "rejectShareAvatar", [reasonCode])
        ...
        if accountId != creatorId:
            self.notify.warning(
                "accountId: %s tried to share avatarId: %s from another creatorId: %s" %
                (accountId, avatarId, creatorId))
            reasonCode = RejectCode.NOT_YOUR_AVATAR
            self.sendUpdateToAccountId(
                accountId, "rejectShareAvatar", [reasonCode])
        ...
        self.air.writeServerEvent('requestShareAvatar', avatarId,
                                  f'{accountId}|{subId}|{shared}')

        self.db.setSharedFlag(avatarId, subId, shared)
        self.sendUpdateToAccountId(
            accountId, "shareAvatarResponse", [
                avatarId, subId, shared])
        return 1
    ...
    def requestAvatarSlot(self, senderId, subId, slot):
        ...
        if not self.air.checkAccountId(accountId):
            reasonCode = RejectCode.INVALID_ACCOUNT
            self.sendUpdateToAccountId(
                accountId, "rejectAvatarSlot", [
                    reasonCode, subId, slot])
            return reasonCode
        ...
        if oldSubId:
            ...
            self.notify.warning(
                "accountId: %s already had a slot reserved for subId: %s" %
                (accountId, oldSubId))
            # Let's clear it and move forward
            del self.__pendingCreatesForAccount[accountId]
            oldNumPending = self.__pendingCreatesForSubscription.get(
                oldSubId, 0)
            if oldNumPending > 0:
    ...
    def requestPlayAvatar(self, senderId, avatarId, subId):
        ...
        if not self.air.checkAccountId(accountId):
            reasonCode = RejectCode.INVALID_ACCOUNT
            self.sendUpdateToAccountId(
                accountId, "rejectPlayAvatar", [
                    RejectCode.INVALID_ACCOUNT, avatarId])
            return reasonCode

        playingAccountId = self.air.getAvatarAccountOnline(avatarId)
        if playingAccountId:
            if playingAccountId == accountId:
                # We already think this requesting avatarId is online playing
                # this avatar.
                self.notify.warning(
                    "avatar is already online: %s played by same accountId: %s" %
                ...
                self.notify.warning(
                    "avatar is already online: %s played by another accountId: %s" %
                    (avatarId, playingAccountId))
                reasonCode = RejectCode.AVATAR_ONLINE
                self.sendUpdateToAccountId(
                    accountId, "rejectPlayAvatar", [
        ...
        # Let's go ahead and mark this avatar as online since we are expecting
        # him soon.
        self.sendUpdateToAccountId(
            accountId, "playAvatarResponse", [
