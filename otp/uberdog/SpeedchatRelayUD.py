...
class SpeedchatRelayUD(DistributedObjectGlobalUD):
    ...
    def forwardSpeedchat(
        ...
        message = self.translateMessage(
            messageType, indexArray, senderDISLName)
        if not message:
            return
        ...
        dg = self.dclass.aiFormatUpdate(
            fieldName, pfmID, pfmID, pfmID, parameters)
    ...
    def translateMessage(self, messageType, indexArray, senderDISLName):
        if messageType == SpeedchatRelayGlobals.NORMAL:
            ...
            return SCDecoders.decodeSCEmoteWhisperMsg(
                indexArray[0], senderDISLName)
        else:
            return None
