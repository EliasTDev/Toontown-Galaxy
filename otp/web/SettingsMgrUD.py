...
class SettingsMgrUD(DistributedObjectGlobalUD, SettingsMgrBase):
    ...
    def announceGenerate(self):
        ...
        self._newSessionId()
        # clear any changed state from a previous session across all AIs and
        # clients
    ...
    def handleHTTPSettings(self, **kw):
        assert self.notify.debugCall()
        if (f'restart{self._sessionId}' in kw) and ('sessionId' not in kw):
        ...
        if not staleSession:
            ...
            for settingName, valueStr in kw.items():
                try:
                    ...
                    self.notify.warning(f'unknown setting "{settingName}"')
                    continue
                ...
                while True:
                    ...
                    if i <= lastCharIndex - 2:
                        try:
                            ...
                            self.notify.warning(
                                f'error un-escaping string: {valueStr}')
                            break
                        else:
                            valueStr = valueStr[:i] + \
                                chr(num) + valueStr[i + 3:]
        ...
        settingNames = sorted(self._settings.keys())
        page += f"<table><caption>{self.getPageTitle()}</caption><thead><tr><th scope=col>Setting</th><th scope=col>Value</th></tr></thead>\n"
        rowNum = -1
        for name in settingNames:
            ...
            if valueRepr != origValueRepr:
                inputColorCode = f' STYLE="background-color: #{SettingsMgrUD.ModifiedColor}"'
            if rowNum % 2 == 1:
                page += "<tr class=\"odd\">"
            else:
                page += "<tr>"
            ...
            if valueRepr != origValueRepr:
                page += f""" <b>modified, original value</b>={origValueRepr}"""
            page += """</td></tr>\n"""
    ...
    def requestAllChangedSettings(self):
        ...
        self.notify.debug(
            f'got requestAllChangedSettings from {returnChannel}')
        for settingName in self._iterSettingNames():
            if self._isSettingModified(settingName):
                curRepr = self._getCurrentValueRepr(settingName)
                self.sendUpdateToChannel(
                    returnChannel, 'settingChange', [
                        settingName, curRepr])
