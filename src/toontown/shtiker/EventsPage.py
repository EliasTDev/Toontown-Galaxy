...
class EventsPage(ShtikerPage.ShtikerPage):
    ...
    DefaultNewsUrl = '/news/news_urls.txt'#to edit here too
    ...
    def getRssFeed(self):
        if self.gotRssFeed:
            return
        self.rssFeed = feedparser.parse('http://www.wdwinfo.com/news/rss.xml') #change to our rss feed 
        feedText = []
    ...
    def getNewsUrl(self):
        ...
        if self.NewsUrl == self.DefaultNewsUrl:
            ...
            if 'test.toontown' in serverAddress.getServer():
                result = 'http://www.ourwebsitehere.com'#to change
