...
class NewsPageButtonManager(FSM.FSM):
    ...
    def load(self):
        ...
        self.newIssueButton = DirectButton(relief=None, sortOrder=DGG.BACKGROUND_SORT_INDEX - 1,
                                           ...
                                           text_font=ToontownGlobals.getInterfaceFont(), parent=base.a2dTopRight,
        ...
        self.gotoPrevPageButton = DirectButton(relief=None,
                                               ...
                                               text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_scale=textScale,
                                               text_font=ToontownGlobals.getInterfaceFont(), pos=shtickerBookPos,
                                               scale=shtickerBookScale, command=self.__handleGotoPrevPageButton)
        self.goto3dWorldButton = DirectButton(relief=None,
                                              image=(self.closeNewsUp, self.closeNewsHover, self.closeNewsHover),
                                              text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_scale=textScale,
                                              text_font=ToontownGlobals.getInterfaceFont(), pos=shtickerBookPos,
                                              scale=shtickerBookScale, command=self.__handleGoto3dWorldButton)
        ...
        self.__bookCheck = Sequence(Func(self.__checkButton), Wait(0.00001), Func(self.__checkButton), Wait(0.00001),
                                    Func(self.__checkButton), Wait(0.00001), Func(self.__checkButton), Wait(0.0001))
        self.__bookCheck.loop()
        self.__bookCheck.pause()
        self.__blinkIval = Sequence(Func(self.__showOpenEyes), Wait(2), Func(self.__showClosedEyes), Wait(0.1),
                                    Func(self.__showOpenEyes), Wait(0.1), Func(self.__showClosedEyes), Wait(0.1))
