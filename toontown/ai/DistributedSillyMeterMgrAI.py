...
class DistributedSillyMeterMgrAI(
        DistributedPhaseEventMgrAI.DistributedPhaseEventMgrAI):
    ...
    def __init__(self, air, startAndEndTimes, phaseDates):
        ...
        DistributedPhaseEventMgrAI.DistributedPhaseEventMgrAI.__init__(
            self, air, startAndEndTimes, phaseDates)
        air.SillyMeterMgr = self

    def setCurPhase(self, newPhase):
        DistributedPhaseEventMgrAI.DistributedPhaseEventMgrAI.setCurPhase(
            self, newPhase)
        messenger.send('SillyMeterPhase', [newPhase])

    def calcCurPhase(self):
        DistributedPhaseEventMgrAI.DistributedPhaseEventMgrAI.calcCurPhase(
            self)
