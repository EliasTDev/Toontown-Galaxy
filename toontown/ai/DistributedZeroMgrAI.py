...
class DistributedPhaseEventMgrAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def setupNextPhase(self):
        ...
        if curTime < endTime:
            ...
            self.notify.debug(f"startingNextPhase in {waitTime}")
            self.startSwitchPhaseTask(waitTime)
        else:
            self.notify.warning(
                "at phase %s, endTime is in the past %s, not starting task to switch" %
                (self.curPhase, endTime))
        pass
    ...
    def doSwitchPhase(self, task):
        ...
        if self.curPhase < 0:
            self.notify.warning(
                f"doSwitchPhase doing nothing as curPhase={self.curPhase}")
    ...
    def getPhaseEndTime(self, phase):
        ...
        if (0 <= phase) and (phase < (self.getNumPhases() - 1)):
            ...
            self.notify.warning(
                f"getPhaseEndTime got invalid phase {phase} returning now")
    ...
    def forcePhase(self, newPhase):
        ...
        if newPhase >= self.getNumPhases():
            self.notify.warning(f"ignoring newPhase {newPhase}")
            return
