...
# we are creating this system so it's easier to start holidays on the test
# server ahead of schedule
TestServerHolidayDaysAhead = simbase.config.GetInt(
...
OriginalHolidays = {
    ...
    {'startAndEndPairs': [datetime.datetime(2010, Month.MAY, 5, 8, 0),  # firstMoveArmUp1
     ...
     'phaseDates': [datetime.datetime(2010, Month.MAY, 9, 11, 0o5),  # firstMoveStruggle
                    datetime.datetime(
         2010, Month.MAY, 13, 11, 0o5),  # firstMoveArmUp2
        # firstMoveJump hydrants around hydrant zero animate
        datetime.datetime(2010, Month.MAY, 18, 11, 0o5),
        datetime.datetime(
         2010,
         Month.MAY,
         21,
         16,
         0o5),
        # firstMoveJumpBalance
        # firstMoveArmUp3 Hydrant Zero and his hydrant pals get more elaborate
        # animations
        datetime.datetime(2010, Month.MAY, 22, 15, 30),
    ...
    {'startAndEndPairs': [datetime.datetime(2010, Month.MAY, 8, 12, 15),  # firstMoveLidFLip1
     ...
     'phaseDates': [datetime.datetime(2010, Month.MAY, 11, 11, 0o5),  # firstMoveStruggle
                    datetime.datetime(
         2010, Month.MAY, 15, 11, 0o5),  # firstMoveLidFlip2
        # firstMoveJump trashcans around trashcan zero animate
        datetime.datetime(2010, Month.MAY, 20, 11, 0o5),
        datetime.datetime(2010, Month.MAY, 23, 11, 0o5),  # firstMoveLidFlip3
        # firstMoveJumpHit Trashcan Zero and his trashcan pals get more
        # elaborate animations
        datetime.datetime(2010, Month.MAY, 29, 14, 10),
    ...
    {'startAndEndPairs': [datetime.datetime(2010, Month.MAY, 9, 12, 00),  # firstMoveFlagSpin1
     ...
     'phaseDates': [datetime.datetime(2010, Month.MAY, 16, 16, 55),  # firstMoveStruggle & Jump
                    datetime.datetime(
         2010, Month.MAY, 21, 16, 55),  # firstMoveFlagSpin2
        # firstMoveFlagSpin3 mailboxs around mailbox zero animate
        datetime.datetime(2010, Month.MAY, 23, 17, 0o5),
        # firstMoveJumpSummersault
        datetime.datetime(2010, Month.JUNE, 1, 11, 0o5),
        # firstMoveJumpFall Mailbox Zero and his mailbox pals get more
        # elaborate animations
        datetime.datetime(2010, Month.JUNE, 5, 12, 0o1),
        # firstMoveJump3Summersaults
        datetime.datetime(2010, Month.JUNE, 8, 11, 45),
    ],
    },
    ...
    {'startAndEndPairs': [datetime.datetime(2010, Month.MAY, 14, 0, 0o1),          # Stage 1 animates
     ...
     'phaseDates': [datetime.datetime(2010, Month.MAY, 17, 16, 0o1),    # Stage 1 animates, stage 2 built
                    datetime.datetime(
         2010, Month.MAY, 19, 00, 0o1),           # Stage 1 loc 2
        datetime.datetime(
         2010,
         Month.MAY,
         22,
        ...
        # Stage 3 is added and animates
        datetime.datetime(2010, Month.JUNE, 2, 0, 0o1),
        datetime.datetime(
         2010,
         Month.JUNE,
         5,
        ...
        # Stage 5 silly meter plummets
        datetime.datetime(2010, Month.JUNE, 13, 13, 30),

        # Scientist chatter change
        datetime.datetime(2010, Month.JUNE, 14, 0, 0o1),

        # Silly meter shuts down
        datetime.datetime(2010, Month.JUNE, 28, 0, 0o1),
    ],
    },
    ...
    {'startAndEndPairs': [datetime.datetime(2010, Month.JUNE, 12, 12, 0o1),  # they just animate but don't help
     ...
     # they're actually helping now
     'phaseDates': [datetime.datetime(2010, Month.JUNE, 14, 3, 0), ],
     },
    ...
    {'startAndEndPairs': [datetime.datetime(2010, Month.JUNE, 12, 12, 0o1),  # they just animate but don't help
     ...
     # they're actually helping now
     'phaseDates': [datetime.datetime(2010, Month.JUNE, 18, 00, 0o1), ],
     },
...
print(f"AdjustedHolidays = {AdjustedHolidays}")
...
class HolidayManagerAI:
        ...
        ToontownGlobals.HALLOWEEN: HolidayInfo_Yearly(
            ...
            [(Month.OCTOBER, 30, 17, 0, 0),        # 10am-3pm PST, 1pm-6pm EST
             (Month.OCTOBER, 30, 22, 0, 0),
                ...
                # 6pm-11pm PST, 9pm-2am EST
                (Month.OCTOBER, 31, 1, 0, 0),
    ...
    holidaysGerman = {
        ToontownGlobals.JULY4_FIREWORKS: HolidayInfo_Yearly(
            ...
            [(Month.OCTOBER, 3, 0, 0, 0),
             # Stop them in the middle of the final hour so we do not interrupt
             # a show in the middle
    ...
    if language not in ['japanese', 'german', 'portuguese', 'french']:
    ...
    def createHolidays(self):
        ...
        for holidayId, holidayInfo in list(self.holidays.items()):
            ...
            self.notify.debug(f"holidayId = {holidayId}")
            self.notify.debug(f"startTime = {startTime}")
            self.notify.debug(f"endTime = {endTime}")
            ...
            try:
                ...
                if endTime < startTime:
                    ...
                    if end[2] == start[2]:
                        raise ValueError(
                            "createEvents: Invalid Start/End Tuple combination in holiday %s" %
                            (holidayId))

                    newDate = holidayInfo.adjustDate(date)
                    endTime = holidayInfo.getEndTime(newDate)
                    self.notify.debug(f"wrapped: endTime = {endTime}")
                ...
                # wait for next years holiday
                    ...
                    self.notify.debug(f"next: sTime = {sTime}")
                    ...
                    if sTime is not None:
                        ...
                        self.notify.info(
                            f"One time holiday {holidayId} has passed")
    ...
    def waitForHolidayStart(self, holidayId, startTime):
        ...
        task = taskMgr.doMethodLater(
            waitTime, self.startHolidayDoLater, taskName)
        task.holidayId = holidayId
        self.notify.info(
            "Waiting until %s (- %s = %s) for holiday %s start" %
    ...
    def waitForHolidayEnd(self, holidayId, endTime):
        ...
        task = taskMgr.doMethodLater(
            waitTime, self.endHolidayDoLater, taskName)
        task.holidayId = holidayId
    ...
    def startHoliday(self, holidayId, testMode=0):
        self.notify.info(f"startHoliday: {holidayId}")
        ...
        if holidayClass:
            if holidayInfo.hasPhaseDates():
                ...
                holidayObj = holidayClass(
                    self.air, holidayId, holidayInfo.tupleList, testHolidays)
            else:
                holidayObj = holidayClass(self.air, holidayId)
            try:
                # Start the holiday
    ...
    def forcePhase(self, holidayId, newPhase):
        ...
        if holidayObj:
            if hasattr(holidayObj, 'forcePhase'):
                ...
                self.notify.warning(f"{holidayObj} does not have forcePhase")
        return result
    ...
    def endHoliday(self, holidayId, stopForever=False):
        self.notify.info(f"endHoliday: {holidayId}")
        ...
        self.notify.debug(f"currentTime = {currentTime}")
        self.notify.debug(f"startTime = {startTime}")
        ...
        if isinstance(holidayInfo, HolidayInfo_Daily):
            ...
            if (startTimeForToday < currentTime) and (
                    currentTime < endTimeForToday):
                # the task manager can sometimes end the task some seconds before we expected it
                # avoid the case where we start it again for a few seconds then
                # end it
                oldStartTime = startTime
                startTime = holidayInfo.getNextHolidayTime(endTimeForToday)
                self.notify.debug(
                    "oldStart = %s newStart=%s" %
                    (time.ctime(oldStartTime), time.ctime(startTime)))

        # if we are stopping the holiday prematurely, kill the task that waits
        # for it to end
        taskName = "waitHoliday-end-" + str(holidayId)
        ...
        if startTime is not None:
            # Handle the case that the start and end times straddle the new
            # year
    ...
    def delayedEnd(self, holidayId, stopForever=False):
        self.notify.info(f"delayedEnd: {holidayId}")
        holidayInfo = self.holidays[holidayId]
        ...
        for key in self.holidays:
            ...
            if holidayInfo.displayOnCalendar:
                if isinstance(holidayInfo, HolidayInfo_Weekly):
                    self.air.newsManager.addWeeklyCalendarHoliday(
                        key, holidayInfo.tupleList[0][0][0])
                elif isinstance(holidayInfo, HolidayInfo_Yearly):
                    # we can have multiple start times and end times, just pick
                    # the bookends
                    firstStartTime = holidayInfo.tupleList[0][0]
                    lastEndTime = holidayInfo.tupleList[-1][-1]
                    self.air.newsManager.addYearlyCalendarHoliday(
                        key, firstStartTime, lastEndTime)
                    ...
                    if key in OncelyMultipleStartHolidays:
                        ...
                        self.air.newsManager.addMultipleStartHoliday(
                            key, startAndEndList)
                    else:
                        # we can have multiple start times and end times, just
                        # pick the bookends
                        firstStartTime = holidayInfo.tupleList[0][0]
                        lastEndTime = holidayInfo.tupleList[-1][-1]
                        self.air.newsManager.addOncelyCalendarHoliday(
                            key, firstStartTime, lastEndTime)
                    ...
                    firstStartTime = holidayInfo.tupleList[0][0]
                    lastEndTime = holidayInfo.tupleList[-1][-1]
                    self.air.newsManager.addRelativelyCalendarHoliday(
                        key, firstStartTime, lastEndTime)
