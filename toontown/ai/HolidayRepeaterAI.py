...
class HolidayRepeaterAI(HolidayBaseAI.HolidayBaseAI):
    ...
    def start(self):
        ...
        for holiday in list(self.testHolidays.keys()):
            ...
            taskMgr.doMethodLater(
                nextStepIn * scaleFactor,
                self.handleNewHolidayState,
                "testHoliday_" + str(holiday),
                extraArgs=[holiday])
    ...
    def handleNewHolidayState(self, holiday):
        ...
        if curState == 0:
            if self.air.holidayManager.isHolidayRunning(holiday):
                self.air.holidayManager.endHoliday(holiday, True)
            self.notify.debug(f"Starting holiday: {holiday}")
            simbase.air.newsManager.sendSystemMessage(
                "Holiday " + str(holiday) + " started")
            nextStepIn = self.testHolidays[holiday][(
                curState + 1)] - self.testHolidays[holiday][curState]
            self.air.holidayManager.startHoliday(holiday, testMode=1)
        elif len(self.testHolidays[holiday]) == (curState + 1):
            self.notify.debug(f"Ending holiday: {holiday}")
            simbase.air.newsManager.sendSystemMessage(
                "Holiday " + str(holiday) + " ended")
            self.air.holidayManager.endHoliday(holiday, True)
            self.testHolidayStates[holiday] = -1
        else:
            self.notify.debug(
                f"Forcing holiday: {holiday} to phase: {curState}")
            simbase.air.newsManager.sendSystemMessage(
                "Holiday " + str(holiday) + " is in phase " + str(curState))
            nextStepIn = self.testHolidays[holiday][(
                curState + 1)] - self.testHolidays[holiday][curState]
        ...
        if nextStepIn == -1:
            ...
            if count == len(self.testHolidays):
                ...
                simbase.air.newsManager.sendSystemMessage(
                    "Holiday cycle complete: " + str(self.holidayId))
                taskMgr.doMethodLater(
                    restartWaitTime, self.startLoop, "RepeatHolidays")
        else:
            taskMgr.doMethodLater(
                nextStepIn * scaleFactor,
                self.handleNewHolidayState,
