...
class NewsManager(DistributedObject.DistributedObject):
    ...
    def setInvasionStatus(self, msgType, cogType, numRemaining, skeleton):
        ...
        if msgType == ToontownGlobals.SuitInvasionBegin:
            ...
            self.notify.warning(
                f"setInvasionStatus: invalid msgType: {msgType}")
            return
    ...
    def startHoliday(self, holidayId):
        if holidayId not in self.holidayIdList:
            self.notify.info(f"setHolidayId: Starting Holiday {holidayId}")
    ...
    def endHoliday(self, holidayId):
        if holidayId in self.holidayIdList:
            self.notify.info(f"setHolidayId: Ending Holiday {holidayId}")
    ...
    def holidayNotify(self):
        # this function is used to notify players just logging in of which
        # Silly Saturday event is going
    ...
    def getMultipleStartHolidaysForDate(self, theDate):
        ...
        for theHoliday in self.multipleStartHolidays:
            times = theHoliday[1:]
            # weird why it's we'd have to do this, investigate dc definition
            # when we have time
            tempTimes = times[0]
            for startAndStopTimes in tempTimes:
                ...
                if startTime[0] == theDate.year and startTime[1] == theDate.month and \
                   startTime[2] == theDate.day:
                    # holiday starts on this date
                    # We fake the calendarGuiDay and say this is a oncely
                    # holiday
        ...
        for holidayItem in self.relativelyCalendarHolidays:
            ...
            while(i < len(item)):
                ...
                while(1):
                    ...
                    # Handles the case when the end weekday is less than the
                    # start
                    if(self.weekDaysInMonth[eWeekday][1] > eRepNum):
                        ...
                        item[i + 1][0] += 1
                        item[i + 1][1] = 1
                newItem.append([item[i][0], sDay, item[i]
                               [3], item[i][4], item[i][5]])
                newItem.append([item[i + 1][0], nDay, item[i + 1]
                               [3], item[i + 1][4], item[i + 1][5]])
                i += 2
