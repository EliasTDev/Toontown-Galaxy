...
class WinterCarolingMgrAI(ScavengerHuntMgrAI.ScavengerHuntMgrAI):
    ...
    def createListeners(self):
        ...
        for id in list(self.goals.keys()):
            mgrAI = DistributedWinterCarolingTargetAI.DistributedWinterCarolingTargetAI(
    ...
    def huntCompletedReward(self, avId, goal, firstTime=False):
        ...
        if firstTime:
            self.air.writeServerEvent(
                'pumpkinHeadEarned',
                avId,
                'WinterCaroling scavenger hunt complete.')
        ...
        newDate = HolidayManagerAI.HolidayManagerAI.holidays[self.holidayId].adjustDate(
            date)
        endTime = HolidayManagerAI.HolidayManagerAI.holidays[self.holidayId].getEndTime(
            newDate)
    ...
    def huntGoalFound(self, avId, goal):
        ...
        av.sendUpdate('winterCarolingTargetMet',
