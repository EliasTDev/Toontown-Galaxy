...
class DistributedBattleBase(DistributedNode.DistributedNode, BattleBase):
    ...
    backOfCogsPos = None
    backOfCogsHpr = None
    sideOfBattlePos = None
    sideOfBattleHpr = None
    ...
    def cleanupBattle(self):
        ...
        if self.__battleCleanedUp:
            return
        self.notify.debug(f"cleanupBattle({self.doId})")
    ...
    def loadTrap(self, suit, trapid):
        ...
        assert(suit.battleTrapProp is None)
        ...
        self.notify.debug(f'suit.battleTrapProp = trap {trap}')
        if trap.getName() == 'traintrack':
            # the train track's parent is battle
            assert self.notify.debug(
                'Not doing wrtReparentTo(suit) for traintrack')
            pass
        else:
    ...
    def unlureSuit(self, suit):
        self.notify.debug(f'movie unluring suit {suit.doId}')
        if (self.luredSuits.count(suit) != 0):
    ...
    def lureSuit(self, suit):
        self.notify.debug(f'movie luring suit {suit.doId}')
        if (self.luredSuits.count(suit) == 0):
    ...
    def setInitialSuitPos(self, x, y, z):
        ...
        assert(
            self.notify.debug(
                "setInitialSuitPos: suit pos=%s, battle heading=%s" %
                (self.initialSuitPos, self.getH())))
    ...
    def setState(self, state, timestamp):
        ...
        self.notify.debug(f'setState({state})')
    ...
    def setMembers(self, suits, suitsJoining, suitsPending, suitsActive,
            ...
            if suit is not None:
                if ((trapid == NO_TRAP or
                     trapid != suit.battleTrap) and
                        suit.battleTrapProp is not None):
                    self.notify.debug(
        ...
        for t in toonsJoining:
            if int(t) < len(self.toons):
                ...
                self.notify.warning(
                    f'setMembers toonsJoining t={t} not in self.toons {self.toons}')
        ...
        for t in toonsPending:
            if int(t) < len(self.toons):
                ...
                self.notify.warning(
                    f'setMembers toonsPending t={t} not in self.toons {self.toons}')
    ...
    def __createJoinInterval(self, av, destPos, destHpr, name, ts, callback,
        ...
        if (len(plist) == 0):
            ...
            if (totalTime > MAX_JOIN_T):
                self.notify.warning(f'__createJoinInterval() - time: {totalTime:f}')
        ...
        if (av == base.localAvatar):
         ...
         #   camTrack.append(Func(camera.setHpr, self.camJoinHpr))
            camInterval = camera.posHprInterval(
                0.6, self.camJoinPos, self.camJoinHpr)
    ...
    def __makeToonRun(self, toon, ts):
        ...
        self.notify.debug(f'duration: {runMTrack.getDuration():f}')
        runMTrack.start(ts)
    ...
    def d_requestPetProxy(self, toonId, av):
        assert(toonId == base.localAvatar.doId)
        self.notify.debug(f'network:requestPetProxy({av})')
    ...
    def __handleLocalToonBattleEvent(self, response):
        ...
        if (mode == 'Attack'):
            ...
            if (len(self.luredSuits) > 0):
                if (track == TRAP or (track == LURE and (
                        not levelAffectsGroup(LURE, level)))):
                    if target != -1:
    ...
    def __adjust(self, ts, callback):
        self.notify.debug(f'__adjust({ts:f})')
