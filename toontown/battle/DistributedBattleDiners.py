...
class DistributedBattleDiners(DistributedBattleFinal.DistributedBattleFinal):
    ...
    def showSuitsFalling(self, suits, ts, name, callback):
        ...
        for suit in suits:
            ...
            self.notify.debug(f'startPos for {suit} = {startPos}')
            suit.reparentTo(self)
        ...
        if (self.hasLocalToon()):
            ...
            # Choose a back angle to see the diners flying to their battle
            # position
            self.notify.debug(f'self.battleSide ={self.battleSide}')
            camHeading = -20
            camX = -4
