...
def doFanfare(delay, toon, panel):
    ...
    openBall = Parallel(LerpHprInterval(partyBallLeft, duration=.2, startHpr=Vec3(90, 0, 0), hpr=Vec3(
        90, 30, 0)), LerpHprInterval(partyBallRight, duration=.2, startHpr=Vec3(-90, 0, 0), hpr=Vec3(-90, 30, 0)))
    ...
    ballAppear = Parallel(
        LerpColorScaleInterval(
            partyBallLeft, .3, startColorScale=Vec4(
                1, 0, 0, 0), colorScale=Vec4(
                1, 0, 0, 1)), LerpColorScaleInterval(
                    partyBallRight, .3, startColorScale=Vec4(
    ...
    play = Parallel(
        SoundInterval(
            crabHorn, startTime=1.5, duration=4.0, node=toon), Sequence(
            Wait(.25), longshake(
                [
                    trumpet1, trumpet2], 3, .2), Wait(.5), longshake(
                        [
                            trumpet1, trumpet2], 3, .2), Wait(.5), longshake(
                                [
                                    trumpet1, trumpet2], 9, .1), longshake(
                                        [
                                            trumpet1, trumpet2], 3, .2)))
    ...
    # if we need to show a panel, we return the panel we created, otherwise we
    # return the None
