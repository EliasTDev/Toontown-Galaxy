...
class FireCogPanel(StateData.StateData):
    ...
    def enter(
        ...
        if not self.toon:
            if (len(luredIndices) > 0):
                # You can't place a trap in front of a suit that is already
                # lured
    ...
    def __placeButtons(self, numAvatars, invalidTargets, localNum, fireCosts):
        ...
        if numAvatars == 1:
            ...
            self.notify.error(f"Invalid number of avatars: {numAvatars}")
