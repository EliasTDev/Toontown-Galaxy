        ...
        if (kbbonus == 1 or hp > 0):
            ...
            if (suitName in MovieUtil.largeSuits):
                ...
                reachTrack = ActorInterval(
                    suit, 'reach', duration=reachAnimDuration)  # Reach for the dollar
...
def __lureSmallMagnet(lure):
    ...
    return __createMagnetMultiTrack(
        lure, magnet, pos, hpr, scale, isSmallMagnet=1)
...
def __lureLargeMagnet(lure):
    ...
    return __createMagnetMultiTrack(
        lure, magnet, pos, hpr, scale, isSmallMagnet=0)
...
def __createSuitDamageTrack(battle, suit, hp, lure, trapProp):
    ...
    # Now we learn which level trap we're using to use the appropriate suit
    # reaction
    ...
    # Now decide which trap this is and use the different suit animation
    # reactions
    if (trapName == 'banana'):
        ...
        # Also need to make the rake pop up when stepped on, then fall back
        # down
        ...
        # Where suit drops from after the quicksand
        ...
        # We grab the name tag so we can hide it while the suit is in the
        # quicksand
        ...
        # Where suit drops from after the trap door
        dropPos = trapProp.getPos(battle)
        landPos = trapProp.getPos(battle)  # Where the suit lands from the drop
        ...
        notify.warning(
            f'unknown trapName: {trapName} detected on suit: {suit}')
