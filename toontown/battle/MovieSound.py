...
# set this to zero if we don't want to move back duriing sound gags
...
def __doSoundsLevel(sounds, delay, hitCount, npcs):
    ...
    # that suits only flinch once for the group of simultaneous sounds that
    # hit.
    ...
    for sound in sounds:
        ...
        # get suit track no longer creates the deathTrack append it after
        # creating all attack tracks
        for target in targets:
            battle = sound['battle']
            suit = target['suit']
            died = target['died']
...
def __createToonInterval(sound, delay, toon, operaInstrument=None):
    ...
    if operaInstrument:
        ...
        retval.append(
            __getPartTrack(
                sprayEffect, 0.0, 6.0, [
                    sprayEffect, toon, 0], softStop=-3.5))
