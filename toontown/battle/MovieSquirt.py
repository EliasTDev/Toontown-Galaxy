...
hitSoundFiles = ('AA_squirt_flowersquirt.ogg',
                 'AA_squirt_glasswater.ogg',
                 'AA_squirt_neonwatergun.ogg',
                 'AA_squirt_seltzer.ogg',
                 'firehose_spray.ogg',
                 'AA_throw_stormcloud.ogg',
                 'AA_squirt_Geyser.ogg')  # UBER

missSoundFiles = ('AA_squirt_flowersquirt_miss.ogg',
...
def doSquirts(squirts):
    ...
    for squirt in squirts:
        ...
        if skip:
            ...
            if isinstance(squirt['target'], type([])):  # if target is a list
...
def __doSuitSquirts(squirts):
    ...
    if isinstance(squirts[0]['target'], type([])):  # is target is a list?
...
def __doSquirt(squirt, delay, fShowStun, uberClone=0):
    ...
    if isinstance(squirt['target'], type([])):  # is target is a list?
        for target in squirt['target']:
            ...
            if (suitType == 'a'):
                ...
                animTrack.append(
                    ActorInterval(
                        ...
                        'slip-forward',
                        startTime=2.58))
            # Be sure to unlure the suit so it doesn't walk back (already
            # knocked back)
...
def __doFlower(squirt, delay, fShowStun):
    ...
    def getSprayStartPos(flower=flower):
        ...
        # force update of LOD 0 to current animation frame (dont know if it is
    ...
    # dont bother with LOD 2 intervals for now since viewer will be very far
    # away flower will be too small to see
    ...
    flowerTrack = Sequence(  # this series of intervals will take tTotalFlowerToonAnimationTime
        ...
        # parent the flower to the toon joint LOD 0 only (this is used by
        # getSprayStartPos)
    ...
    # eventually would like to just create these once and parent to joint at
    # load time
...
def __doWaterGlass(squirt, delay, fShowStun):
    ...
    # all LOD hand joints to the 1 glass.  would like to do this once at init
    # time eventually
    ...
    glassTrack = Sequence(
        ...
        # must get rid of stuff created by attachNewNode and instanceTo
        # eventually would like to just create these once and parent to joint
        # at load time
    ...
    def getSprayStartPos(toon=toon):
        ...
        n.reparentTo(toon)
        # TODO: adjust this based on snout length
...
def __doWaterGun(squirt, delay, fShowStun):
    ...
    def getSprayStartPos(pistol=pistol, toon=toon):
        # note:  dont have to call toon.pose() since getSprayStartPos isnt called
        # until the exact frame we need the posn, so the animation has already
        # been updated
        # force update of LOD 0 to current animation frame (dont know if it is
        # being displayed or not)
        toon.update(0)
