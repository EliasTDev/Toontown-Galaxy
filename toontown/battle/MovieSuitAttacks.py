...
def __doDamage(toon, dmg, died):
    assert(
        notify.debug(
            '__doDamage(toon: %s  damage: %d died: %d' %
            (toon.getName(), dmg, died)))
...
def doSuitAttack(attack):
    """ doSuitAttack(attack)
    """
    notify.debug(f"building suit attack in doSuitAttack: {attack['name']}")
    name = attack['id']
    if (name == AUDIT):
...
def getResetTrack(suit, battle):
    ...
    moveDuration = 0.5
# moveDuration = (moveDist / BattleBase.suitSpeed) # Too short a time
# interval to see
    walkTrack = Sequence(
...
def doDefault(attack):
    ...
    if (suitName == 'f'):
        ...
        self.notify.error(f'doDefault() - unsupported suit type: {suitName}')
    return None
...
def getSuitTrack(attack, delay=0.000001, splicedAnims=None):
    ...
    # Some attacks used spliced animations instead of just the default
    # animation
    if splicedAnims:
        ...
        track.append(
            ActorInterval(
                suit,
                attack['animName']))  # Attack animation
...
def getToonTracks(
    ...
    for i in range(len(targets)):
        ...
        # print "ZZZZ processing attack ", i,"  damage=", tgt['hp'],"
        # toondied=", tgt['died']," suitlevel=", slevel
        toonTracks.append(
            getToonTrack(
                attack,
                damageDelay,
...
def getToonDodgeTrack(target, dodgeDelay, dodgeAnimNames,
    ...
    if dodgeAnimNames:  # If exists, use these animations
        for d in dodgeAnimNames:  # Use each dodge animation in order
            # If the dodge is a sidestep, other toons may need to get out of
            # the way
            if (d == 'sidestep'):
...
def getThrowTrack(
    ...
    def calcOriginAndVelocity(
        ...
        # Calculate the initial velocity required for the object to land at the
        # target
...
def getToonTakeDamageTrack(toon, died, dmg, delay, damageAnimNames=None,
    ...
    assert(
        notify.debug(
            'getToonTakeDamageTrack(toon: %s damage: %d died: %d' %
            (toon.getName(), dmg, died)))
    ...
    for nextAnim in anims:
        ...
        if (len(nextAnim) >= 2):  # If using a delay before the animation
            if (nextAnim[1] > 0):  # Make sure that the given delay is not zero
                # And if not, go ahead and use provided value
                delay = nextAnim[1]
...
def getSplicedLerpAnims(
    ...
    # The animInterval is how much the animation progresses forward each
    # interval
    ...
    for i in range(0, int(numAnims)):
        # Constructing the animation list for later use with
...
def doClipOnTie(attack):  # top tm: fixed
    ...
    if (dmg > 0):  # If damage is greater than zero
        # Now throw the tie, which travels much faster if thrown "well" (hits
        # toon)
...
def doPoundKey(attack):  # top f: fixed
    ...
    receiverPosPoints = [Point3(0.23, 0.17, -0.11),
                         VBase3(5.939, 2.763, -177.591)]
...
def doShred(attack):  # top f: fixed
    ...
    paperPropTrack = getPropTrack(
        ...
        animDuration=1.5,
        animStartTime=2.8)
    shredderPosPoints = [Point3(0, -0.12, -0.34),
                         VBase3(-90.000, -53.770, -0.000)]
...
def doFillWithLead(attack):  # top p: particle w/ 2 props; cringe; sidestep
    ...
    # calculate the height of the body part to correctly place the particle
    # effects
    ...
    # Same for each effect, adjusted slightly for each
    ...
    if (dmg > 0):
        ...
        # Wait before next part color change
...
def doRubOut(attack):  # top p: fixed
    ...
    suitTrack = getSuitTrack(attack)
    padPosPoints = [Point3(-0.66, 0.81, -0.06),
                    VBase3(14.930, -2.290, 180.000)]
    padPropTrack = getPropTrack(
        pad, suit.getLeftHand(), padPosPoints, 0.5, 2.57)
    pencilPosPoints = [Point3(0.04, -0.38, -0.10),
    ...
    # Same for each effect, will be adjusted slightly for each
...
def doWriteOff(attack):  # top bc: fixed
    ...
    # Calculate missPoint of the check mark with lambda function for combined
    # checkmark, pencil Track
    def missPoint(
        checkmark=checkmark,
        toon=toon): return __toonMissPoint(
        checkmark,
    ...
    # Begin extraArgs for __showProp function
    extraArgsForShowProp = [pencil, suit.getRightHand()]
    # Add in pos points for extraArgs
    ...
    pencilPropTrack.append(
        LerpScaleInterval(
            pencil,
            0.5,
            MovieUtil.PNT3_NEARZERO))  # Scale down the pencil
...
def doRubberStamp(attack):  # top ym: fixed
    ...
    propTrack.append(
        LerpScaleInterval(
            stamp,
            0.5,
            MovieUtil.PNT3_NEARZERO))  # Scale down the stamp
...
def doRazzleDazzle(attack):  # top ym:fixed
    ...
    signPosPoints = [Point3(0.00, -0.42, -0.04),
...
def doSynergy(attack):  # top ym: fixed
    ...
    # Now get animation interval with time inserted to slow down the jump at
    # its peak
    ...
    synergySoundTrack = Sequence(Wait(0.9), SoundInterval(
        globalBattleSoundCache.getSound('SA_synergy.ogg'), node=suit))
...
def doBrainStorm(attack):  # top mm(c), m/s(b): fixed
    ...
    # Initial height of cloud set to 3 meters above suit
    ...
    # Now calculate the targetPoint for the cloud to move to (over the target
    # toon)
    targetPoint = __toonFacePoint(toon)
    # Set end height of cloud to 3 meters above toon head
    ...
    # a separate track and get removed before the effect is parented, it will
    # crash
    ...
    cloudPropTrack.append(LerpScaleInterval(cloud, 0.5,
                                            MovieUtil.PNT3_NEARZERO))
    # The particle effect has already been cleaned up, it's safe to remove the
    # cloud
...
def doBuzzWord(attack):  # top mm(c), dt(a): fixed
    ...
    # Some suits need to move the position and orientation of the particle
    # effects
...
def doDemotion(attack):  # top mm: fixed
    ...
    # Now set the freezeEffect and unFreezeEffect height to the head of the
    # toon
    ...
    # Now gradually continue to cringe but become slower until freezing
    # completely
...
def doCanned(attack):  # ds(b), cr(c) throw/particles?; struggle, slip-backward; sidestep
    ...
    # At this point, the can either cans the toon or bounces because the toon
    # dodged
    ...
    damageAnims = [['struggle', propDelay + suitDelay +
                    throwDuration, 0.01, 0.7], ['slip-backward', 0.01, 0.45], ]
    toonTrack = getToonTrack(
        attack,
        splicedDamageAnims=damageAnims,
        dodgeDelay=dodgeDelay,
    ...
    if (dmg > 0):  # Then manipulate the head and chest
        ...
        for partNum in range(0, headParts.getNumPaths()):
            ...
            headTracks.append(Sequence(
                ...
                LerpPosInterval(
                    part, 0.25, Point3(
                        x, y, z + 2.2)),  # pop off head
                LerpHprInterval(
                    part, 0.4, VBase3(
                        360, 0, 180)),  # spin and flip it
                LerpPosInterval(
                    part, 0.3, Point3(
                        x, y, z + 3.1)),  # raise it a bit
                LerpPosInterval(
                    part, 0.15, Point3(
                        x, y, z + 0.3)),  # put it back on
                Wait(0.15),
                ...
                LerpPosInterval(
                    part, 0.15, Point3(
                        x, y, z + 1)),  # pull it up again
                LerpHprInterval(
                    part, 0.3, VBase3(
                        h, p, r)),  # flip it back over
                Wait(0.2),
                # put it back down finally
        ...
        # interval for movement involving the chest performed upon the body
        # part parameter.
...
def doSacked(attack):  # ds(b) throw; struggle, jump; sidestep
    ...
    # At this point, the sack either sack the toon or bounces because the toon
    # dodged
...
def doGlowerPower(attack):  # tw: fixed
    ...
    for i in range(0, 3):
        ...
        # Put delay between each knife throw
...
def doHeadShrink(attack):  # hh(a) special, shrink head; cringe; jump
    ...
    # The cloud will "circle" around the toon a bit as it sprinkles down
    # shrinking dust
    ...
    if (dmg > 0):  # then use the shrinkTrack
        shrinkSound = globalBattleSoundCache.getSound(
            'SA_head_shrink_only.ogg')
        growSound = globalBattleSoundCache.getSound(
            'SA_head_grow_back_only.ogg')
        soundTrack = Sequence(
            Wait(2.1),
...
#    particleEffect = BattleParticles.createParticleEffect(file='rollodexVortex')
    ...
    suitType = getSuitBodyType(attack['suitName'])
    if (suitType == 'a'):
        propPosPoints = [Point3(-0.51, -0.03, -0.10),
...
def doEvilEye(attack):  # cr(c) throw; cringe, slip-backward; duck
    ...
    if (suitName == 'cr'):
        posPoints = [Point3(-0.46, 4.85, 5.28),
                     VBase3(-155.000, -20.000, 0.000)]
    elif (suitName == 'tf'):
        posPoints = [Point3(-0.4, 3.65, 5.01),
                     VBase3(-155.000, -20.000, 0.000)]
    elif (suitName == 'le'):
        posPoints = [Point3(-0.64, 4.45, 5.91),
                     VBase3(-155.000, -20.000, 0.000)]
    else:
        posPoints = [Point3(-0.4, 3.65, 5.01),
                     VBase3(-155.000, -20.000, 0.000)]
    ...
    if (dmg > 0):
        ...
        lerpInterval = LerpPosInterval(eye, moveDuration, Point3(
            toonFace.getX(), toonFace.getY() - 5, toonFace.getZ() - 2))
...
def doFreezeAssets(attack):  # top cc(c), pp(a): fixed
    ...
    suitTrack = getSuitTrack(attack, delay=0.9)
    # Initial cloud height set to 3 meters above suit
    initialCloudHeight = suit.height + 3
    ...
    cloudPropTrack.append(Func(cloud.wrtReparentTo, render))
    # Now calculate the targetPoint for the cloud to move to (right over the
    # target toon)
    targetPoint = __toonFacePoint(toon)
    targetPoint.setZ(targetPoint[2] + 3)
    # Push the cloud over to the target point (whether it hits or misses)
...
def doHotAir(attack):  # top cc(c): fixed
    ...
    # Damage animation is toon cringing as gets hit then, then falls to the
    # floor
    toonTrack = getToonTrack(
        attack,
        damageDelay=damageDelay,
...
def doPickPocket(attack):  # top tm: fixed
    ...
    target = attack['target']
    dmg = target['hp']
    # Get the dollar bill from prop pool
    ...
    if (dmg > 0):  # if toon takes damage (bill was stolen)
        ...
        # show the bill in the suit's hand
...
def doSchmooze(attack):  # top gh(c), mingler(a): fixed
    ...
    for i in range(0, 4):  # Add each particle track for all four phrases
        ...
        # Extra delay to add the lower particle wave to blend the two together
        # in one wave
    ...
    # Take damage fourth time and finish
    ...
    # Finish off the duck animation (stand up)
...
#    soundTrack = getSoundTrack('SA_schmooze.ogg', delay=2.1, node=suit))
...
def doHangUp(attack):  # top ac: fixed
    ...
    propTrack = Sequence(  # Single propTrack combines both phone and receiver
        ...
        # Scale down phone & child receiver
...
def doRedTape(attack):  # top ac: fixed
    ...
    for i in range(0, 3):
        tubes.append(globalPropPool.getProp(
            'redtape-tube'))  # Get prop from pool
...
def doParadigmShift(attack):  # m(a) group2 particle w/ 0 props; shift; sidestep
    ...
    for t in targets:
        ...
        if (dmg > 0):
            ...
            # We'll manipulate a fake shadow and hide the normal one so they
            # don't conflict
...
def doBounceCheck(attack):  # top pp: fixed
    ...
    def bounce1Point(suit=suit, toon=toon, battle=battle): return \
        getThrowEndPoint(suit, toon, battle, 'one')

    def bounce2Point(suit=suit, toon=toon, battle=battle): return \
        getThrowEndPoint(suit, toon, battle, 'two')

    def hit3Point(suit=suit, toon=toon, battle=battle): return \
        getThrowEndPoint(suit, toon, battle, 'threeHit')

    def miss3Point(suit=suit, toon=toon, battle=battle): return \
        getThrowEndPoint(suit, toon, battle, 'threeMiss')
    def bounce4Point(suit=suit, toon=toon, battle=battle): return \
    ...
    # Wait amount of time before release
...
def doWatercooler(attack):  # top sc: fixed
    ...
    def missPoint(prop=watercooler, toon=toon): return \
