...
def doThrows(throws):
    ...
    notify.debug(f'singleHitDict = {singleHitDict}')
    notify.debug(f'groupHitDict = {groupHitDict}')
    notify.debug(f'totalHitDict = {totalHitDict}')
...
def __animProp(props, propName, propType):
    if 'actor' == propType:
        ...
        notify.error(f"No such propType as: {propType}")
...
def __piePreMissGroup(missDict, pies, suitPoint, other=render):
    ...
    notify.debug(f"startPos={missDict['startPos']}")
    notify.debug(f'v={v}')
    notify.debug(f"endPos={missDict['endPos']}")
...
def __createWeddingCakeFlight(throw, groupHitDict, pie, pies):
    ...
    # so loop through our targets, and throw 1 layer of the wedding cake at
    # each one
    splatName = 'splat-' + pieName
    if pieName == 'wedding-cake':
        splatName = 'splat-birthday-cake'
    ...
    for part in cakePartStrs:
        cakeParts.append(pie.find(f'**/{part}'))
    ...
    cakePartDivisions[4] = [[cakeParts[0], ], [
        cakeParts[1], ], [cakeParts[2], ], [cakeParts[3], ]]
...
def reparentCakePart(pie, cakeParts):
    ...
    notify.debug(f'pieParent = {pieParent}')
