            ...
            if trap['level'] == UBER_GAG_LEVEL_INDEX:
                ...
                for oneTarget in trap['target']:
                    ...
                    if suit.battleTrap != NO_TRAP:
                        # we can get here in a 3 suit battle, by placing
                        # quicksand then doing train
                        hasUberTrapConflict = True
...
def __createThrownTrapMultiTrack(trap, propList, propName, propPos=None,
    ...
    animDelay = 3.1
    # All the traps use the default trap distance, except the rake (gets
    # stepped into)
    ...
    # Update on the toon to be sure that all LOD's see the same thrown trap
    # right
    ...
    def placeTrap(trapProp, suit, battle=battle, trapName=trapName):
        ...
        if (trapName == 'rake'):
            ...
            # which has a different animation on it for when the rake is
            # stepped on
...
def __createPlacedGroupTrapTrack(
    ...
    if True:
        ...
        if (explode == 1):  # If the trap prop collides with another trap, destroy it
            ...
            # check if we have a case of a train trap appearing under a lured
            # suit
