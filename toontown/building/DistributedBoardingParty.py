...
class DistributedBoardingParty(
    ...
    def announceGenerate(self):
        ...
        self.notify.debug(f'canonicalZoneId = {canonicalZoneId}')
    ...
    def postGroupInfo(self, leaderId, memberList, inviteeList, kickedList):
        ...
        if (newGroupEntry[0]) == [0] or (
                not newGroupEntry[0]):  # if the new memberList is empty
        ...
        if isMyGroup:
            ...
            for removedMemberId in removedMemberIdList:
                ...
                # group using the leave button. It won't work if the
                # removedMember Alt+F4ed out.
                if not removedMember:
                    # Try second time, just in case removedMemberId is a
                    # friend.
                    removedMember = base.cr.identifyFriend(removedMemberId)
                if removedMember:
                    removedMemberName = removedMember.name
                    # Message to the members in the group that the
                    # removedMember has left the Boarding Group.
    ...
    def postKick(self, leaderId):
        self.notify.debug(
            f"{localAvatar.doId} was kicked out of the Boarding Group by {leaderId}")
    ...
    def postSizeReject(self, leaderId, inviterId, inviteeId):
        self.notify.debug(
            f"{inviteeId} was not invited because the group is full")
    ...
    def postInviteDelcined(self, inviteeId):
        self.notify.debug(
            f"{inviteeId} delinced {localAvatar.doId}'s Boarding Group invitation.")
    ...
    def postInviteAccepted(self, inviteeId):
        self.notify.debug(
            f"{inviteeId} accepted {localAvatar.doId}'s Boarding Group invitation.")
    ...
    def postInviteNotQualify(self, avId, reason, elevatorId):
        ...
        if (avId == localAvatar.doId):
            if (reason == BoardingPartyBase.BOARDCODE_MINLAFF):
                rejectText = TTLocalizer.BoardingInviteMinLaffInviter % (
                    minLaff)
            ...
            if (reason == BoardingPartyBase.BOARDCODE_PROMOTION):
                rejectText = TTLocalizer.BoardingInvitePromotionInvitee % (
                    avatarNameText)
            if (reason == BoardingPartyBase.BOARDCODE_BATTLE):
                rejectText = TTLocalizer.TeleportPanelNotAvailable % (
                    avatarNameText)
            if (reason == BoardingPartyBase.BOARDCODE_NOT_PAID):
                rejectText = TTLocalizer.BoardingInviteNotPaidInvitee % (
                    avatarNameText)
            if (reason == BoardingPartyBase.BOARDCODE_DIFF_GROUP):
                rejectText = TTLocalizer.BoardingInviteeInDiffGroup % (
                    avatarNameText)
            if (reason == BoardingPartyBase.BOARDCODE_PENDING_INVITE):
                rejectText = TTLocalizer.BoardingInviteePendingIvite % (
                    avatarNameText)
            if (reason == BoardingPartyBase.BOARDCODE_IN_ELEVATOR):
                rejectText = TTLocalizer.BoardingInviteeInElevator % (
                    avatarNameText)
    ...
    def postMessageInvited(self, inviteeId, inviterId):
        ...
        # Message to the members in the group that the inviter has invited the
        # invitee.
    ...
    def postMessageInvitationFailed(self, inviterId):
        ...
        if inviter:
            inviterName = inviter.name
        # Don't generate the message more than once a minute, so that the
        # inviter can't spam the invitee.
        if self.invitationFailedMessageOk(inviterId):
            # Message to the invitee that the inviter had tried to invite the
            # invitee.
            messageText = TTLocalizer.BoardingMessageInvitationFailed % (
                inviterName)
            localAvatar.setSystemMessage(
                0, messageText, WhisperPopup.WTToontownBoardingGroup)
    ...
    def postMessageAcceptanceFailed(self, inviteeId, reason):
        ...
        # Message to the inviter that the invitee failed to accept the
        # invitation.
        localAvatar.setSystemMessage(
            0, messageText, WhisperPopup.WTToontownBoardingGroup)
    ...
    def showRejectMessage(
        ...
        if (reason == BoardingPartyBase.BOARDCODE_MINLAFF):
            ...
            if (leaderId in avatarsFailingRequirements):
                rejectText = TTLocalizer.BoardcodeMinLaffLeader % (
                    minLaffPoints)
            else:
            ...
            if (leaderId in avatarsFailingRequirements):
                ...
                avatarNameText = getAvatarText(avatarsFailingRequirements)
                if (len(avatarsFailingRequirements) == 1):
                    rejectText = TTLocalizer.BoardcodePromotionNonLeaderSingular % (
                        avatarNameText)
                else:
                    rejectText = TTLocalizer.BoardcodePromotionNonLeaderPlural % (
                        avatarNameText)

        elif (reason == BoardingPartyBase.BOARDCODE_BATTLE):
            self.notify.debug(
                f"{leaderId} 's group cannot board because it is in a battle")
            if (leaderId in avatarsInBattle):
                ...
                if (len(avatarsInBattle) == 1):
                    rejectText = TTLocalizer.BoardcodeBattleNonLeaderSingular % (
                        avatarNameText)
                else:
                    rejectText = TTLocalizer.BoardcodeBattleNonLeaderPlural % (
                        avatarNameText)

        elif (reason == BoardingPartyBase.BOARDCODE_SPACE):
            self.notify.debug(
                f"{leaderId} 's group cannot board there was not enough room")
            rejectText = TTLocalizer.BoardcodeSpace

        elif (reason == BoardingPartyBase.BOARDCODE_MISSING):
            self.notify.debug(
                f"{leaderId} 's group cannot board because something was missing")
    ...
    def postGroupDissolve(self, quitterId, leaderId, memberList, kick):
        self.notify.debug(f"{leaderId} group has dissolved")
            ...
            if groupFormed:
                ...
                if (leaderId == quitterId):
                    ...
                    if not (localAvatar.doId == leaderId):
                        # Message to the members in the group that the group
                ...
                # If the non-leader is the quitter.
                    ...
                    # kick = 1 means leader kicked out the quitter. Don't show
                    # any message to the leader.
                    if not kick:
                        # Don't message the quitter because he already knows.
                        if not (localAvatar.doId == quitterId):
                            quitter = base.cr.doId2do.get(quitterId)
                            if quitter:
                                # If we can find the quitter, message saying
                                # quitter has left the group.
                                quitterName = quitter.name
                                # Message to the members in the group that the
                                # quitter has left the Boarding Group.
                                messageText = TTLocalizer.BoardingMessageLeftGroup % (
                                    quitterName)
                                localAvatar.setSystemMessage(
                                    0, messageText, WhisperPopup.WTToontownBoardingGroup)
                            else:
                                # If we can't find the quitter write a generic
                                # message saying your group was disbanded.
                                messageText = TTLocalizer.BoardingMessageGroupDisbandedGeneric
                                localAvatar.setSystemMessage(
                                    0, messageText, WhisperPopup.WTToontownBoardingGroup)

    def requestInvite(self, inviteeId):
        self.notify.debug(f"requestInvite {inviteeId}")
        ...
        if elevator:
            if elevator.allowedToEnter():
                ...
                if self.inviterPanels.isInvitingPanelUp():
                    ...
                    # The Boarding Group is full, so you can't send any more
                    # invites.
    ...
    def requestAcceptInvite(self, leaderId, inviterId):
        self.notify.debug(f"requestAcceptInvite {leaderId} {inviterId}")
    ...
    def handleEnterElevator(self, elevator):
        ...
        if self.getGroupLeader(localAvatar.doId) == localAvatar.doId:
            if base.localAvatar.hp > 0:
                ...
                # Disable the Group Panel GO Button and Destination Scrolled
                # List here.
    ...
    def requestGoToFirstTime(self, elevatorId):
        ...
        # Start the Go To pre show immediately, we'll stop it if we hear a
        # reject from the AI.
    ...
    def cancelGoToElvatorDest(self):
        ...
        """
        self.notify.debug(f'{localAvatar.doId} cancelled the GoTo Button.')
        self.firstRequestAccepted = False
        self.waitingForFirstResponse = False
        self.finishGoToPreShowTrack()
        place = base.cr.playGame.getPlace()
