...
class DistributedBoardingPartyAI(
    ...
    def __init__(self, air, elevatorList, maxSize=4):
        ...
        # the elevators this boardingParty can send groups to
    ...
    def addWacthAvStatus(self, avId):
        ...
        messageToonReleased = f"Battle releasing toon {avId}"
        self.accept(messageToonReleased, self.handleToonLeftBattle)

    def handleToonJoinedBattle(self, avId):
        self.notify.debug(f"handleToonJoinedBattle {avId}")

    def handleToonLeftBattle(self, avId):
        self.notify.debug(f"handleToonLeftBattle {avId}")
    ...
    def requestInvite(self, inviteeId):
        ...
        if invitee and (invitee.battleId != 0):
            ...
            self.sendUpdateToAvatarId(
                inviterId, "postInviteNotQualify", [
                    inviteeId, reason, 0])
            # Send a message to the invitee saying that inviter had tried to
            # invite him.
            self.sendUpdateToAvatarId(
                inviteeId, "postMessageInvitationFailed", [inviterId])
            return

        # Send a reject to the inviter if the invitee is already in a boarding
        # group.
        ...
        # Send a reject to the inviter if the invitee has already boarded an
        # elevator.
        ...
        if len(self.elevatorIdList) == 1:
            if inviteeOkay:
                ...
                # Send a reject to the inviter if the invitee does not qualify
                # for the only elevator.
                self.sendUpdateToAvatarId(
                    inviterId, "postInviteNotQualify", [
                ...
                if inviterOkay:
                    if (inviterOkay == REJECT_MINLAFF):
                        ...
                        reason = BoardingPartyBase.BOARDCODE_PROMOTION
                    # Send a reject to the inviter if the inviter does not
                    # qualify for the only elevator.
                    self.sendUpdateToAvatarId(
                        inviterId, "postInviteNotQualify", [
        ...
        if inviterId in self.avIdDict:  # existing group
            ...
            if groupList:
                ...
                if (len(self.getGroupMemberList(leaderId))) >= self.maxSize:
                    ...
                    self.sendUpdateToAvatarId(
                        inviteeId, "postInvite", [
                            leaderId, inviterId])
                    # Send a message to all members in the group except the inviter that an invitation was sent out.
                    # Inviter doesn't need to get this message, because he
                    # knows that he sent out the invitation.
    ...
    def requestAcceptInvite(self, leaderId, inviterId):
        ...
        if inviteeId in self.avIdDict:
            # Send reject to the invitee if the invitee is already in a
            # boarding group
            if self.hasActiveGroup(inviteeId):
            ...
            if self.avIdDict[inviteeId]:
                # Don't do anything if the invitee already exists in the
                ...
                if self.avIdDict[inviteeId] == leaderId:
                    if inviteeId in memberList:
                        ...
                        return
                # If the invitee's leader is different, remove him from his old
                # leader's group.
                else:
                    self.air.writeServerEvent(
                        'suspicious: ',
            ...
            self.sendUpdateToAvatarId(
                inviterId, "postInviteAccepted", [inviteeId])
...
##            self.notify.warning('boarding_requestLeave: StackTrace: %s' %stack.compact())
                    ...
                    if elevator:
                        ...
                        if boardOkay == BoardingPartyBase.BOARDCODE_OKAY:
                            ...
                            if not (leaderId in elevator.seats):
                                ...
                                self.notify.warning(
                                    f'avId: {leaderId} has hacked his/her client.')
                                self.air.writeServerEvent(
    ...
    def requestGoToFirstTime(self, elevatorId):
        ...
        if self.testGoButtonRequirements(callerId, elevatorId):
            self.sendUpdateToAvatarId(
                callerId, "acceptGoToFirstTime", [elevatorId])
    ...
    def requestGoToSecondTime(self, elevatorId):
        ...
        if self.testGoButtonRequirements(callerId, elevatorId):
            for avId in avList:
                self.sendUpdateToAvatarId(
                    avId, "acceptGoToSecondTime", [elevatorId])
            THREE_SECONDS = 3.0
    ...
    def sendAvatarsToDestinationTask(self, elevatorId, avList, task):
        ...
        if len(avList):
            if elevatorId in self.elevatorIdList:
                elevator = simbase.air.doId2do.get(elevatorId)
                if elevator:
                    self.notify.warning(f'Sending avatars {avList}')
                    boardOkay, avatarsFailingRequirements, avatarsInBattle = self.testBoard(
    ...
    def handleAvatarDisco(self, avId):
        self.notify.debug(f"handleAvatarDisco {avId}")
    ...
    def handleAvatarZoneChange(self, avId, zoneNew, zoneOld):
        self.notify.debug(
            f"handleAvatarZoneChange {avId} new{zoneNew} old{zoneOld} bp{self.zoneId}")
...
##                self.sendUpdate("postGroupInfo", [leaderId, group[0], group[1], group[2]])
    ...
    def removeFromGroup(self, leaderId, memberId, kick=0, post=1):
        self.notify.debug("")
        self.notify.debug(
            f"removeFromGroup leaderId {leaderId} memberId {memberId}")
        self.notify.debug(f"Groups {self.groupListDict}")
        self.notify.debug(f"avDict {self.avIdDict}")
        ...
        # Either the leader wants to leave or the group has only 1 member, so
        # dissolve the group.
        if (memberId == leaderId) or (len(group[0]) < 2):
            if leaderId in self.avIdDict:
                ...
                for inviteeId in group[1]:
                    if inviteeId in self.avIdDict:
                        ...
                        # Send a message to the invitee to remove the invitee
                        # message.
                        self.sendUpdateToAvatarId(
            ...
            # This is done so that even the removed member is removed from the
            # avIdDicts in all the clients.
    ...
    def informDestinationInfo(self, offset):
        ...
        if (offset > len(self.elevatorIdList)):
            self.air.writeServerEvent(
                'suspicious: ',
                leaderId,
                f'has requested to go to {offset} elevator which does not exist')
            return
        memberList = self.getGroupMemberList(leaderId)
        ...
        for avId in memberList:
            ...
            if (avId != leaderId):
                self.sendUpdateToAvatarId(
                    avId, "postDestinationInfo", [offset])
