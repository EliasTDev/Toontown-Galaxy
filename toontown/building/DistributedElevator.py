...
class DistributedElevator(DistributedObject.DistributedObject):
    ...
    # There is a difference in jump offsets in cases like the cog kart
    # elevators.
    ...
    def gotToon(self, index, avId, toonList):
        ...
        if request:
            ...
            self.notify.error(
                f"gotToon: already had got toon in slot {index}.")
    ...
    def fillSlot(self, index, avId, wantBoardingShow=0):
        self.notify.debug(
            f"{self.doId}.fillSlot({index}, {avId}, ...)")
        request = self.toonRequests.get(index)
        if request:
            self.cr.relatedObjectMgr.abortRequest(request)
            del self.toonRequests[index]
        ...
        if avId == 0:
            ...
            if avId == base.localAvatar.getDoId():
                ...
                # Cleanup any leftover elevator messages before boarding the
                # elevator.
                if hasattr(base.localAvatar, "elevatorNotifier"):
                    base.localAvatar.elevatorNotifier.cleanup()
    ...
    def emptySlot(self, index, avId, bailFlag, timestamp, timeSent=0):
        ...
        if avId == 0:
            ...
            if avId in self.cr.doId2do:
                ...
                if self.canHideBoardingQuitBtn(avId):
                    # Enable the Boarding Group Panel Quit Button here if it is
                    # relevant.
                    track.append(
                        Func(
                            localAvatar.boardingParty.groupPanel.enableQuitButton))
                    # Enable the Boarding Group GO Button here if it is
                    # relevant.
                    track.append(
                        Func(
    ...
    def rejectBoard(self, avId, reason=0):
        ...
        # reason 0: unknown, 1: shuffle, 2: too low laff, 3: no seat, 4: need
        # promotion
        print(f"rejectBoard {reason}")
        if hasattr(base.localAvatar, "elevatorNotifier"):
