...
class DistributedToonHallInterior(DistributedToonInterior):
    ...
    def selectPhase(self, newPhase):
        try:
            ...
            if self.sillyFSM.hasStateNamed(gotoPhase) and (
                    not self.sillyFSM.getCurrentState() == self.sillyFSM.getStateNamed(gotoPhase)):
                self.sillyFSM.request(gotoPhase)
        except BaseException:
            self.notify.warning(
                f'Illegal phase transition requested {newPhase}')
    ...
    def startIfNeeded(self):
        ...
        try:
            ...
            if self.curPhase >= 0:
                ...
                # if the holiday is not running, display our flat "under
                # repair"
        ...
        if base.cr.newsManager.isHolidayRunning(
            ...
            if hasattr(
                ...
                if hasattr(base.cr, "SillyMeterMgr"):
                    self.notify.debug(
                        f"isDisabled = {base.cr.SillyMeterMgr.isDisabled()}")
                else:
    ...
    def calculatePhaseDuration(self):
        ...
        if base.cr.newsManager.isHolidayRunning(
            ...
            self.notify.debug("holiday is not running")
        self.notify.debug(f"valid = {valid}")
        if valid and \
           base.cr.SillyMeterMgr.getIsRunning():
