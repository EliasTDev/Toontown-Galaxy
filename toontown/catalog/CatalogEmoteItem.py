...
class CatalogEmoteItem(CatalogItem.CatalogItem):
    ...
    def recordPurchase(self, avatar, optional):
        if self.emoteIndex < 0 or self.emoteIndex > len(avatar.emoteAccess):
            self.notify.warning(
                f"Invalid emote access: {self.emoteIndex} for avatar {avatar.doId}")
    ...
    def getPicture(self, avatar):
        ...
        if duration is None:
            duration = 0
        name = f"emote-item-{self.sequenceNumber}"
    ...
    def output(self, store=~0):
        return f"CatalogEmoteItem({self.emoteIndex}{self.formatOptionalData(store)})"
