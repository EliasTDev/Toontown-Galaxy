...
FLBillboard = 128
FLTrunk = 256
# this is essentially the same as HouseGlobals.houseColors2 with the
# addition of alpha = 1
furnitureColors = [
...
# Also see TTLocalizer.FurnitureNames and
# TTLocalizer.AwardManagerFurnitureNames
FurnitureTypes = {
...
# If you add any new Animated furniture, update
# CatalogAnimatedFurniture.AnimatedFurnitureItemKeys
...
class CatalogFurnitureItem(CatalogAtticItem.CatalogAtticItem):
    ...
    def needsCustomize(self):
        ...
        return self.colorOption is None and \
            FurnitureTypes[self.furnitureType][FTColorOptions] is not None
...
def nextAvailableBank(avatar, duplicateItems):
    ...
    if bankId is None or bankId == MaxBankId:
...
def nextAvailableCloset(avatar, duplicateItems):
    ...
    # we can't guarantee order on these dict values, so we'll sort them as a
    # list
