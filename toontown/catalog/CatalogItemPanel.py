...
class CatalogItemPanel(DirectFrame):
    ...
    def __init__(self, parent=aspect2d, parentCatalogScreen=None, **kw):
        ...
        assert parentCatalogScreen is not None, "parentCatalogScreen: is none"
    ...
    def load(self):
        ...
        priceStr = str(self['item'].getPrice(self['type'])
                       ) + " " + TTLocalizer.CatalogCurrency
        priceScale = 0.07
    ...
    def getTeaserPanel(self):
        # display the appropriate page of the feature browser for this catalog
        # item type
        typeName = self['item'].getTypeName()
        if (typeName == TTLocalizer.EmoteTypeName) or (
                typeName == TTLocalizer.ChatTypeName):
            page = 'emotions'
    ...
    def updateGiftButton(self, giftUpdate=0):
        ...
        if numFriends > 0:
            ...
            if (self['item'].isGift() <= 0):
                ...
                # print "not a gift"
                # self.giftButton.hide()
            # otherwise if you have a valid avater for your friend attempt to
            # activate the button
    ...
    def handleSoundOnButton(self):
        ...
        if hasattr(item, 'changeIval'):
            if self.ival:
                self.ival.finish()
                self.ival = None
            self.ival = item.changeIval(
                volume=base.settings.getFloat(
                    'game', 'sfxVolume', 1.0))
            self.ival.loop()
