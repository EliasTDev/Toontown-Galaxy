...
class MailboxScreen(DirectObject.DirectObject):
    ...
    def load(self):
        ...
        self.accept(
            f"setMailboxContents-{base.localAvatar.doId}",
            self.__refreshItems)
        self.accept(
            f"setAwardMailboxContents-{base.localAvatar.doId}",
            self.__refreshItems)
        # load the buttons
    ...
    def __makeDiscardInterface(self):
        ...
        if self.itemIndex >= 0 and self.itemIndex < len(
                self.items):  # check to see if index is within range, should help with button mashing
            item = self.items[self.itemIndex]
            if isinstance(item, InviteInfoBase):
                itemText = (
                    TTLocalizer.MailBoxRejectVerify %
                    (self.getItemName(item)))
                yesText = TTLocalizer.MailboxReject
            else:
                itemText = (
                    TTLocalizer.MailBoxDiscardVerify %
                    (self.getItemName(item)))
    ...
    def __acceptItemCallback(self, retcode, item, index):
        ...
        if retcode < 0:
            ...
            # appropriate dialog.
            self.notify.info(
                f'Could not take item {item}: retcode {retcode}')
            if retcode == ToontownGlobals.P_NoTrunk:
                ...
                self.dialogBox = TTDialog.TTDialog(
                    style=TTDialog.TwoChoiceCustom,  # TwoChoice YesNo
                    # what happens when you try to take chat from the mailbox
    ...
    def __showCurrentItem(self):
        ...
        if self.itemIndex == self.acceptingIndex:
            ...
            self.gettingText['text'] = TTLocalizer.MailboxGettingItem % (
                self.getItemName(item))
        ...
        assert(self.itemPanel is None and self.ival is None)
        self.currentItem = item
        # giftTag is the senders do Id
        # giftCode is the type of tag to use (added after giftTag or I would
        # have named them differently)
        if isinstance(item, CatalogItem.CatalogItem):
    ...
    def getNumberOfAwardItems(self):
        ...
        # since award items go first, the index being sent to ai needs to be
        # adjusted
        result = 0
        for item in self.items:
            if isinstance(
