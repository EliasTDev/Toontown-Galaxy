...
class TTChatInputWhiteList(ChatInputWhiteListFrame):
    ...
    def shiftPressed(self):
        ...
        assert self.notify.debug(f'shiftPressed {self.desc}')
    ...
    def handleTypeGrab(self):
        assert self.notify.debug(f"handleTypeGrab {self.desc}")
    ...
    def sendChatByData(self, text):
        assert self.notify.debug(
            f'sendChatByData desc={self.desc} tfChat={self.trueFriendChat}')
...
##               online = 1
    ...
    def applyFilter(self, keyArgs, strict=False):
        ...
        if len(text) > 0 and text[0] in ['~', '>']:
            ...
            words = text.split(" ")
            newwords = []
            assert self.notify.debug(f"{words}")
            ...
            if not strict:
                ...
                if lastword == "" or self.whiteList.isPrefix(
