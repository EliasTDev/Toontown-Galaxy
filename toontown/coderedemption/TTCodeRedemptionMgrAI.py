...
class TTCRMAIRetryMgr(DirectObject):
    ...
    def addRedemption(self, avId, context, code):
        ...
        self._redemptions[serial] = ScratchPad(
            avId=avId, context=context, code=code, attemptNum=0)
    ...
    def resolveRedemption(self, serial, context, avId, result, awardMgrResult):
        ...
        if serial not in self._redemptions:
            self.notify.warning(
                'unexpected redemption resolution: %s, %s, %s, %s, %s' %
        ...
        if hasattr(self, '_stressTestInfo'):
            if (avId, context) in self._stressTestInfo.redemptions:
                ...
                if result:
                    ...
                    self.notify.debug(
                        f'stress test redemption succeeded for {avId} ({code})')
                ...
                if (now - self._stressTestInfo.lastLogT) > 10:
                    ...
                    self.notify.info(
                        ...
                        (self._stressTestInfo.numCodesResolved,
                         (self._stressTestInfo.numCodesResolved /
                          duration)))
                self._checkCleanupStressTest()
        self._codeRedemptionMgr.sendUpdateToAvatarId(
            avId, 'redeemCodeResult', [
    ...
    def _doRedemption(self, serial, directCall, task=None):
        ...
        if info.attemptNum > 1:
            self._retryPeriod = max(
                self.MinRetryPeriod,
                self._retryPeriod *
                self.RetryGrowMult)
            self.notify.info(
                'code redemption retry #%s for %s: %s' %
                ((info.attemptNum - 1), info.avId, info.code))
        self.air.sendUpdateToDoId('TTCodeRedemptionMgr',
                                  'redeemCodeAiToUd',
                                  OtpDoGlobals.OTP_DO_ID_TOONTOWN_CODE_REDEMPTION_MANAGER,
                                  [serial,
                                   self._codeRedemptionMgr.doId,
                                   info.context,
                                   info.code,
                                   info.avId])
        info.doLater = self.doMethodLater(self._retryPeriod, Functor(
            self._doRedemption, serial), uniqueName('CodeRedemptionRetry'))
        return Task.done
...
class TTCodeRedemptionMgrAI(DistributedObjectAI):
    ...
    RandomizeStressTestCode = config.GetBool(
        'randomize-code-redemption-stress-test-code', 0)
    ...
    def redeemCodeResultUdToAi(
        ...
        self._retryMgr.resolveRedemption(
            serial, context, avId, result, awardMgrResult)

    def _doStressTest(self, task):
        self._stressTestCode = 'stresstest'
        fPath = PM.Filename.expandFrom(
            '$TOONTOWN/%scoderedemption/StressTestAvIds.txt' %
            (choice(
                __dev__, 'src/', ''))).toOsSpecific()
    ...
    def _stressTest(self, task):
        ...
        while numCodes:
            ...
            if done:
                rate = self._stressTestAvCount / \
                    (globalClock.getRealTime() - self._stressTestStartTime)
                self.notify.info(
