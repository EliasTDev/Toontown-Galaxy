...
class CogdoFlyingLocalPlayer(CogdoFlyingPlayer):
    ...
    def initLocalPlayerIntervals(self):
                    ...
                    Func(
                        ...
                        self.toon, 1.0, Vec3(
                            720, 0, 0)), LerpFunctionInterval(
                                self.toon.setScale, fromData=1.0, toData=0.1, duration=1.0), self.toon.posInterval(
                                    0.5, Vec3(
        ...
        self.waitingForWinInterval = Sequence(
            ...
            Func(
                ...
                '...'),
            Wait(1.5),
            name=f'{self.__class__.__name__}.waitingForWinInterval')
        self.waitingForWinSeq = Sequence(
            ...
            name=f'{self.__class__.__name__}.waitingForWinSeq')
        self.winInterval = Sequence(
            ...
            Func(
                ...
                0),
            name=f'{self.__class__.__name__}.winInterval')
        self.goSadSequence = Sequence(
            Wait(2.5),
            Func(
                base.transitions.irisOut,
                1.5),
            name=f'{self.__class__.__name__}.goSadSequence')
        self.introGuiSeq = Sequence(
            ...
            name=f'{self.__class__.__name__}.introGuiSeq')
        return
    ...
    def initCollisions(self):
        ...
        self.accept('Flyer.cHeadCollSphere-enter-%s' %
                    ceilingColl, self.__handleHeadCollisionIntoCeiling)
        self.accept('Flyer.cHeadCollSphere-exit-%s' %
                    ceilingColl, self.__handleHeadCollisionExitCeiling)
        self.accept('Flyer.cFloorEventSphere-exit-%s' %
                    floorColl, self.__handleEventCollisionExitFloor)
        self.accept(
            f'Flyer.cRayNode-enter-{floorColl}',
            self.__handleRayCollisionEnterFloor)
        self.accept(
            f'Flyer.cRayNode-again-{floorColl}',
            self.__handleRayCollisionAgainFloor)
    ...
    def __handleEventCollisionExitFloor(self, collEntry):
        ...
        if self.isToonOnFloor:
            self.notify.debug(f'~~~Exit Floor:{intoName} -> {fromName}')
            self.isToonOnFloor = False
    ...
    def updateControlVelocity(self, dt):
        ...
        if not leftPressed and not rightPressed or self.controlVelocity[
        ...
        if not upPressed and not downPressed or self.controlVelocity[
    ...
    def printFanInfo(self, string):
        if len(self.fanIndex2ToonVelocity) > 0:
            self.notify.info(f'==AFTER {string}==')
            self.notify.info(f'Fan velocity:{self.fanVelocity}')
        if len(self.activeFans) > 0:
            self.notify.info(f'{self.activeFans}')
        if len(self.fanIndex2ToonVelocity) > 0:
            self.notify.info(f'{self.fanIndex2ToonVelocity}')
        if len(self.fansStillHavingEffect) > 0:
            self.notify.info(f'{self.fansStillHavingEffect}')
    ...
    def enterInactive(self):
        CogdoFlyingLocalPlayer.notify.debug(
            f"enter{self.newState}: '{self.oldState}' -> '{self.newState}'")
    ...
    def exitInactive(self):
        CogdoFlyingLocalPlayer.notify.debug(
            f"exit{self.oldState}: '{self.oldState}' -> '{self.newState}'")
