...
class DistCogdoMazeGameAI(DistCogdoGameAI, DistCogdoMazeGameBase):
    ...
    def generate(self):
        ...
        self._speedMonitor = SpeedMonitor(f'cogdoMazeGame-{self.doId}')
            ...
            if elapsed > 10:
                ...
                if self.requestCount >= 3 and secondsPerGrab <= 0.4:
                    ...
                    if simbase.config.GetBool(
                        ...
                        toon.ban(
                            f'suitHit {self.requestCount} suits in {elapsed} seconds')
    ...
    def requestAction(self, action, data):
        ...
        if action == Globals.GameActions.EnterDoor:
            ...
            self.logSuspiciousEvent(
                senderId,
                f'CogdoMazeGameAI.requestAction: invalid action {action}')
    ...
    def requestSuitHitByGag(self, suitType, suitNum):
        ...
        if suitType not in Globals.SuitTypes:
            self.logSuspiciousEvent(
                senderId,
                f'CogdoMazeGameAI.requestSuitHitByGag: invalid suit type {suitType}')
            return False

        if suitNum not in list(self.suits.keys()):
            self.logSuspiciousEvent(
                senderId,
                f'CogdoMazeGameAI.requestSuitHitByGag: invalid suit num {suitNum}')
            return False
    ...
    def requestHitBySuit(self, suitType, suitNum, networkTime):
        ...
        if suitType not in Globals.SuitTypes:
            self.logSuspiciousEvent(
                senderId,
                f'CogdoMazeGameAI.requestHitBySuit: invalid suit type {suitType}')
            return False

        if suitNum not in list(self.suits.keys()):
            self.logSuspiciousEvent(
                senderId,
                f'CogdoMazeGameAI.requestHitBySuit: invalid suit num {suitNum}')
            return False
    ...
    def requestPickUp(self, pickupNum):
        ...
        if pickupNum not in self.pickups:
            self.logSuspiciousEvent(
                senderId,
                f'CogdoMazeGameAI.requestPickUp: invalid pickupNum {pickupNum}')
            return False
    ...
    def _bootPlayerForHacking(self, toonId, reason, wantBan):
        ...
        if not toon:
            simbase.air.writeServerEvent(
                'suspicious',
                toonId,
                f'CogdoMazeGame._bootPlayerForHacking({reason}): toon not present')
            return
