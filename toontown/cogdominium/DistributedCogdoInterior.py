...
class DistributedCogdoInterior(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            'DistributedCogdoInterior', [
                State.State(
                    'WaitForAllToonsInside', self.enterWaitForAllToonsInside, self.exitWaitForAllToonsInside, ['Elevator']), State.State(
                    'Elevator', self.enterElevator, self.exitElevator, ['Game']), State.State(
                    'Game', self.enterGame, self.exitGame, [
                        'Resting', 'Failed', 'BattleIntro']), State.State(
                            'BarrelRoomIntro', self.enterBarrelRoomIntro, self.exitBarrelRoomIntro, [
                                'CollectBarrels', 'Off']), State.State(
                                    'CollectBarrels', self.enterCollectBarrels, self.exitCollectBarrels, [
                                        'BarrelRoomReward', 'Off']), State.State(
                                            'BarrelRoomReward', self.enterBarrelRoomReward, self.exitBarrelRoomReward, [
                                                'Battle', 'ReservesJoining', 'BattleIntro', 'Off']), State.State(
                                                    'BattleIntro', self.enterBattleIntro, self.exitBattleIntro, [
                                                        'Battle', 'ReservesJoining', 'Off']), State.State(
                                                            'Battle', self.enterBattle, self.exitBattle, [
                                                                'Resting', 'Reward', 'ReservesJoining']), State.State(
                                                                    'ReservesJoining', self.enterReservesJoining, self.exitReservesJoining, ['Battle']), State.State(
                                                                        'Resting', self.enterResting, self.exitResting, [
                                                                            'Elevator', 'BarrelRoomIntro']), State.State(
                                                                                'Reward', self.enterReward, self.exitReward, ['Off']), State.State(
                                                                                    'Failed', self.enterFailed, self.exitFailed, ['Off']), State.State(
                                                                                        'Off', self.enterOff, self.exitOff, [
                                                                                            'Elevator', 'WaitForAllToonsInside', 'Battle'])], 'Off', 'Off')
    ...
    def startAlertElevatorLightIval(self, elevatorModel):
        light = elevatorModel.find(
            f'**/floor_light_{self.currentFloor + 1}')
        track = Sequence(
            Func(
                light.setColor, Vec4(
                    1.0, 0.6, 0.6, 1.0)), Wait(0.9), Func(
                light.setColor, LIGHT_ON_COLOR), Wait(0.9))
        ...
        if self.isBossFloor(self.currentFloor):
            ...
            if self.FOType:
                ...
                self.floorModel = loader.loadModel(
                    f'phase_5/models/cogdominium/{suiteName}')
            ...
            if self.FOType:
                ...
                for i in range(4):
                    paintingModel = loader.loadModel(
                        f'phase_5/models/cogdominium/{paintingModelName}')
