...
class DistributedCrate(DistributedCrushableEntity.DistributedCrushableEntity):
    ...
    def loadModel(self):
        ...
        # Flatten out any scale on the crate so the moving platform doesn't get
        # cranky
    ...
    def handleCollision(self, collEntry=None):
        ...
        self.notify.debug(
            f"offsetDot = {offsetDot}, world = {worldVec}, rel = {offsetVec}")
    ...
    def __listenForCancelEvents(self, on):
        self.notify.debug(f"{self.doId}, __listenForCancelEvents({on})")
        if on:
        ...
        self.moveTrack = Parallel(
            Sequence(
                ...
                self.creakSound, node=self), SoundInterval(
                    self.pushSound, node=self, duration=T_PUSH, volume=.2), )
    ...
    def crushTask(self, crusherId, axis):
        ...
        if crusher:
            ...
            self.notify.debug(f"cHeight= {crusherHeight}")
    ...
    def originalTry(self, axis):
        ...
        self.crushTrack = Sequence(
            LerpScaleInterval(
                self, tSquash, VBase3(
                    1.2, 1.2, .25), blendType='easeInOut'), LerpColorScaleInterval(
                self, 2.0, VBase4(
