...
class DistributedDoorEntity(
    ...
    def announceGenerate(self):
        # ("announceGenerate()"))
        self.doorNode = hidden.attachNewNode(f'door-{self.entId}')
    ...
    def acceptAvatar(self):
        self.accept(f"enter{self.getName()}", self.enterTrigger)
    ...
    def getName(self):
        return f"switch-{str(self.entId)}"
    ...
    def setupDoor(self):
        ...
        if model:
            ...
            if 1:
                # Flatten for speed and to avoid scale changes when
                # reparenting:
    ...
    def openInnerDoors(self):
        ...
        if (not self.level.complexVis()) or (
                self.isOuterDoorOpen and (not self.isVisBlocker or self.isVisReady)):
            ...
            # Each door is 7.5 high, we move them a bit more and then hide
            # them:
            moveDistance = 8.0
