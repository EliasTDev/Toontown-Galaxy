...
class DistributedGolfGreenGame(BattleBlocker.BattleBlocker):
    ...
    def lerpSpritePos(self):
        ...
        self.rollTrack = Sequence(
            LerpPosInterval(
                self.spriteNode, 0.5, Point3(
                    x, y, -self.spriteNotchPos * self.cellSizeZ)), )
        if self.controlSprite:
            if not self.controlSprite.isActive:
                pass
                #self.controlSprite.setZ(self.newBallZ + (self.spriteNotchPos * self.cellSizeZ))
    ...
    def printGrid(self):
        ...
        for columnIndex in range(self.gridDimX - 1, -1, -1):
            if columnIndex < 10:
                printout += f"{columnIndex}  "
            else:
                printout += f"{columnIndex} "
        print(printout)
        for rowIndex in range(self.gridDimZ - 1, -1, -1):
            if rowIndex < 10:
                printout = f"row  {rowIndex} "
            else:
                printout = f"row {rowIndex} "
            for columnIndex in range(self.gridDimX - 1, -1, -1):
                hasSprite = "_"
    ...
    def doCountDown(self, task):
        ...
        if (currentTime - self.countTimeOld) < 1.0:
            ...
            self.timerLabel['text'] = f"{self.countDown}"
    ...
    def fillMatchList(self, cellX, cellZ):
        ...
        if colorType == 4:
            return
        if cellZ % 2 == 0:
            if (self.getColorType(cellX - 1, cellZ) ==
                    colorType) or (self.matchWild(cellX - 1, cellZ, colorType)):
                self.fillMatchList(cellX - 1, cellZ)
            if (self.getColorType(cellX + 1, cellZ) ==
                    colorType) or (self.matchWild(cellX + 1, cellZ, colorType)):
                self.fillMatchList(cellX + 1, cellZ)

            if (self.getColorType(cellX, cellZ + 1) ==
                    colorType) or (self.matchWild(cellX, cellZ + 1, colorType)):
                self.fillMatchList(cellX, cellZ + 1)
            if (self.getColorType(cellX + 1, cellZ + 1) ==
                    colorType) or (self.matchWild(cellX + 1, cellZ + 1, colorType)):
                ...
                if colorType == self.bombIndex:
                    ...
                    for entry in self.matchList:
                        ...
                        for neighbor in neighbors:
                            if (neighbor not in allNeighbors) and (
                                    neighbor not in self.matchList):
    ...
    def __run(self, cont=1):
        ...
        while self.controlSprite is None and self.attackPattern:
            ...
            print(("Pattern %s Place %s Type %s" % (self.attackPattern,
                  self.attackCounter, self.attackPattern[self.attackCounter])))
            if self.standbySprite.holdType is not None:
                color = self.standbySprite.holdType
                sprite = self.addControlSprite(
                    self.newBallX, (self.newBallZ + (self.spriteNotchPos * self.cellSizeZ)), color)
            self.ballLoaded = 1
            self.updateSpritePos()
            ...
            for testSpriteIndex in range(movingSpriteIndex, len(self.sprites)):
                ...
                if testSprite and movingSprite:
                    if movingSpriteIndex != testSpriteIndex and (
                        ...
                        # if
                        # movingSprite.nodeObj.getDistance(testSprite.nodeObj)
                        # < (movingSprite.size + testSprite.size):
                        # should use 2.0 but 1.75 give s little
                        # leyway#(movingSprite.size + testSprite.size):
                        if self.testDistance(
                                movingSprite.spriteBase,
                                testSprite.spriteBase) < (
                                self.radiusBall * 1.65):
                            if not (
                                    movingSprite.isActive and testSprite.isActive):
    ...
    def initCollisionGeom(self):
        ...
        self.actSphereNode = CollisionNode(
            f"gridgame-{self.level.getLevelId()}-{self.entId}")
    ...
    def acceptJoin(self, time, timeStamp, avIds):
        ...
        self.timeTotal = time
        if (localAvatar.doId in avIds) and (
                localAvatar.doId not in self.joinedToons):
            self.__startGame()
            ...
            if avId and (not (avId in self.joinedToons)):
                ...
                if (index > 3):
                    ...
                    print(("ERROR! green game has had more than 4 players, we are about to crash\n %s" % (
                        self.everJoinedToons)))
                    print(f"Joining Toon is {avId} index is {index}")
                toon = base.cr.doId2do.get(avId)
    ...
    def useTime(self, time=None):
        ...
        self.scoreLabel['text'] = (
            TTLocalizer.GolfGreenGameScoreString %
            (self.boardsLeft, int(
                self.timeLeft)))
    ...
    def scoreData(self, total=2, closed=1, scoreList="hello world"):
        ...
        for panelIndex in self.toonPanels:
            ...
            panel.extraData["text"] = (
                TTLocalizer.GolfGreenGamePlayerScore % (0))
        ...
        for entryIndex in range(len(scoreList)):
            ...
            if entry[0] in self.toonPanels:
                ...
                panel.extraData["text"] = (
                    TTLocalizer.GolfGreenGamePlayerScore %
                    (entry[1]))
    ...
    def informGag(self, track, level):
        ...
        icon = self.invModel.find(f'**/{iconName}')
