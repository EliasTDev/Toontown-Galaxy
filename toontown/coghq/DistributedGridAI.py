...
class DistributedGridAI(DistributedEntityAI.DistributedEntityAI):
    ...
    def getObjPos(self, objId):
        ...
        if objPos:
            ...
            if row >= 0 and row < self.numRow and col >= 0 and col < self.numCol:
                ...
                self.notify.debug(f"row/col out of range {row}/{col}")
        else:
    ...
    def addObjectByPos(self, objId, pos, width=1):
        ...
        while (col >= 0 and col < self.numCol):
            ...
            self.notify.debug(
                f"requestObjPos: row/col out of range {row}/{col}")
    ...
    def addObjectByRowCol(self, objId, row, col):
        ...
        if (row >= 0 and row < self.numRow - 1 and
            ...
            self.notify.debug(
                f"adding obj {objId} to grid cell {row},{col}")
            # add the obj
            ...
            self.__setChangedActiveCells(onList=[[row, col], [
                                         row + 1, col], [row, col + 1], [row + 1, col + 1]], objId=objId)
            # self.printGrid()
            return 1
        self.notify.debug(f"couldn't obj to grid cell {row},{col}")
        return 0
    ...
    def removeObject(self, objId):
        ...
        row, col = objPos
        # remove the obj
        self.notify.debug(f"removing obj {objId} from {row}, {col}")
    ...
    def checkMove(self, objId, dRow, dCol):
        ...
        validMove = 1
        if dRow < 0:
            validMove = validMove & self.__isEmpty(
                row - 1,
                col) & self.__isEmpty(
                row - 1,
                col + 1)
        ...
        if dCol < 0:
            validMove = validMove & self.__isEmpty(
                row, col - 1) & self.__isEmpty(row + 1, col - 1)
        elif dCol > 0:
            validMove = validMove & self.__isEmpty(
                row, col + 2) & self.__isEmpty(row + 1, col + 2)
    ...
    def __setChangedActiveCells(self, onList=[], offList=[], objId=None):
        ...
        assert((onList == [] or (onList != [] and objId is not None)))

        for cell in self.activeCellList:
            self.notify.debug(
                f"onList = {onList}, offList = {offList}, cell = {cell.getRowCol()}")
