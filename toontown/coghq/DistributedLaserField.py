...
class DistributedLaserField(BattleBlocker.BattleBlocker):
    ...
    def createGrid(self):
            ...
            for row in range(0, self.gridNumY):
                ...
                if self.gridData[column][row] and self.gridData[column][row] < len(
                    ...
                    for iVertex in range(sizeSymbol):
                        ...
                        self.gridVertexWriter.addData3f(
                            columnLeft + (vertex[0] * gridScaleX), rowBottom + (vertex[1] * gridScaleY), self.zFloat)
        ...
        if self.isToonIn:
            ...
            for iVertex in range(sizeSymbol):
                vertex = gridSymbol[iVertex]
                self.gridVertexWriter.addData3f((self.toonX *
                                                 gridScaleX) +
                                                (vertex[0] *
                                                 gridScaleX), (self.toonY *
                                                               gridScaleY) +
                                                (vertex[1] *
                                                 gridScaleY), self.zFloat)
                self.gridColorWriter.addData4f(
                    gridColor[0] * red,
                    gridColor[1] * green,
                    gridColor[2] * blue,
                ...
                self.beamVertexWriter.addData3f((self.toonX *
                                                 gridScaleX) +
                                                (vertex[0] *
                                                 gridScaleX), (self.toonY *
                                                               gridScaleY) +
