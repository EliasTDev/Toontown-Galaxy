...
class DistributedLawbotCannon (DistributedObject.DistributedObject):
    ...
    def setPosHpr(self, x, y, z, h, p, r):
        assert self.notify.debug(
            f'setPosHpr x={x:f} y={y:f} z={z:f} h={h:f}')
    ...
    def setMovie(self, mode, avId, extraInfo):
        assert(
            self.notify.debug(
                f"{self.doId} setMovie({mode}, {avId})"))
    ...
    def __firePressed(self):
        ...
        if not self.boss.state == 'BattleTwo':
            self.notify.debug(
                f'boss is in state={self.boss.state}, not firing')
            return
    ...
    def __createToonModels(self):
        ...
        self.jurorToon.setPosHpr(
            0, 0, -(self.jurorToon.getHeight() / 2.), 0, -90, 0)
    ...
    def __resetToon(self, avatar, pos=None):
        assert(self.notify.debug(f"{self.doId} __resetToon"))
        if avatar:
    ...
    def d_setLanded(self):
        assert self.notify.debug(
            f"d_setLanded ocalToonshooting = {self.localToonShooting}")
    ...
    def __fireCannonTask(self, task):
        ...
        self.jurorToon.setPosHpr(
            0, 0, -(self.jurorToon.getHeight() / 2.), 0, 0, 0)
    ...
    def __calcFlightResults(self, avId, launchTime):
        ...
        """
        assert self.notify.debug(
            '__calcFlightResults avId=%d launchtime=%f' %
            (avId, launchTime))
        head = self.toonHead
    ...
    def __handleCannonHit(self, collisionEntry):
        ...
        assert self.notify.debug(
            f'hitNode={hitNode} interPt = {interPt}')
        ...
        for nodeName in ignoredHits:
            if hitNode == nodeName:  # intoNodePath.find('**/' + nodeName):
                assert self.notify.debug(f'ignoring hit to {nodeName}')
                return
        ...
        for index in range(len(ToontownGlobals.LawbotBossChairPosHprs)):
            chairlist.append(f'Chair-{index}')
