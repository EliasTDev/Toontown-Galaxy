...
class DistributedLawbotCannonAI (DistributedObjectAI.DistributedObjectAI):
    ...
    def __doExit(self):
        ...
        # too long without firing. Or he could have just pressed the leave
        # cannon button
    ...
    def setCannonLit(self, zRot, angle):
        ...
        if not self.boss.state == 'BattleTwo':
            self.notify.debug(
                f'ignoring setCannonList since boss in state {self.boss.state}')
            return
        ...
        if self.boss.getCannonBallsLeft(avId) == 0:
            self.notify.debug(
                f'ignoring setCannonList since no balls left for {avId}')
            return
