...
class DistributedLawbotChair(DistributedObject.DistributedObject, FSM.FSM):
    ...
    def loadModel(self, modelPath, modelFindString=None):
        ...
        # hide the shadow, it looks wrong, bring it back if there's something
        # solid under the chair
    ...
    def attachColSphere(self):
        ...
        collNode.setName(f'Chair-{self.index}')
    ...
    def __touchedChair(self, entry):
        ...
        self.notify.debug(f"self={self} entry={entry}")
    ...
    def exitStopCogs(self):
        assert self.notify.debug(f'exitStopCogs, oldState={self.oldState}')
        pass
    ...
    def enterOn(self):
        ...
        seqName = f"LawbotBossChair-{self.doId}"
    ...
    def computePos(self):
        ...
        newPos += startPt
        self.notify.debug(f'newPos before offset  = {newPos}')
