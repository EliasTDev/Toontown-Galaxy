...
class DistributedMoleField(
    ...
    def handleEnterMole(self, colEntry):
        ...
        self.notify.debug(f'surfaceNormal={surfaceNormal}')
    ...
    def __showToonHitByBomb(self, avId, moleIndex, timestamp=0):
        ...
        endWorldCoords = (self.getX(render) +
                          (endTile[0] *
                           self.spacingX), self.getY(render) +
                          (endTile[1] *
                           self.spacingY))
        ...
        def flyFunc(t, trajectory, startPos=startPos, endPos=endPos,
            ...
            if flyNode and not flyNode.isEmpty():
                # flyNode could be empty if we teleported out and another toon
                # was flying
