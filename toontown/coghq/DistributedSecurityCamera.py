...
class DistributedSecurityCamera(BasicEntities.DistributedNodePathEntity):
    ...
    def __init__(self, cr):
        ...
        self.attackTrack = Parallel(
            SoundInterval(
                self.attackSound, node=self, volume=.8), SoundInterval(
                self.onSound, node=self, volume=.8))
    ...
    def loadModel(self):
        ...
        self.taskName = f'securityCameraupdate {self.doId}'
    ...
    def __updateTrack(self, task):
        ...
        # I know this is ignoring trig but it'll work out, trust me
    ...
    def genTrack(self):
        ...
        self.trackShape = []
        # + abs((self.trackX + self.projector[0]) / self.projector[2])
        wideX = 1
        # + abs((self.trackY + self.projector[1]) / self.projector[2])
        wideY = 1
        ...
        self.trackFloorVertexWriter.addData3f(
            self.trackX, self.trackY, self.zFloat)  # center
