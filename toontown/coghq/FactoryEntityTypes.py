"""FactoryEntityTypes module: contains factory entity type information"""
...
class BarrelBase(Nodepath):
    ...
    attribs = (
        ('rewardPerGrab', 5, 'int'),
        # if higher, defines random range
        ('rewardPerGrabMax', 0, 'int'),
    )
...
class GagBarrel(BarrelBase):
    ...
    attribs = (
        ...
        ('gagTrack', 0, 'choice', {'choiceSet': ('heal', 'trap', 'lure', 'sound',
                                                 'throw', 'squirt', 'drop',
                                                 'random',),
                                   'valueDict': {'heal': 0,
                                                 'trap': 1,
...
class Switch(Nodepath):
    ...
    output = 'bool'
    attribs = (
        ('isOnEvent', 0, 'entId', {'output': 'bool'}),  # AI Only
        ('isOn', 0, 'bool'),  # AI Only
        ('secondsOn', 1, 'float'),  # AI Only
    )
...
class Button(Switch):
    type = 'button'
    attribs = (
        ('scale', Vec3(3), 'scale'),  # Client Only
...
class Trigger(Switch):
    type = 'trigger'
    attribs = (
        ('scale', Vec3(10), 'scale'),  # Client Only
        ('triggerName', ''),
    )
...
class Door(Nodepath):
    ...
    attribs = (
        ('color', Vec4(1, 1, 1, 1), 'color'),  # Client Only
        ('isVisBlocker', 1, 'bool'),  # Client Only
        ...
        ('isOpenEvent', 0, 'entId', {'output': 'bool'}),  # AI Only
        ('isLock0Unlocked', 1, 'bool'),  # AI Only
        ('isLock1Unlocked', 1, 'bool'),  # AI Only
        ('isLock2Unlocked', 1, 'bool'),  # AI Only
        ('isLock3Unlocked', 1, 'bool'),  # AI Only
...
class LaserField(Nodepath):
    ...
    attribs = (
        ...
        ('modelPath', 0, 'choice', {'choiceSet': ['square'], 'valueDict': {'square': 0}}),
        ('pos', Point3(0, 0, 0), 'pos'),
        ('hpr', Vec3(0, 0, 0), 'hpr'),
        ('cellId', 0, 'int'),
        ('switchId', 0, 'entId', {'type': 'button'}),
...
class Lift(Nodepath):
    ...
    attribs = (
        ...
        ('endGuardName', '', 'string'),
        # TODO: integrate new editor widget
...
class Platform(Nodepath):
    ...
    attribs = (
        ...
        ('period', 2, 'float'),
        ('waitPercent', .1, 'float', {'min': 0, 'max': 1}),
...
class Stomper(Crusher):
    ...
    attribs = (
        ('damage', 3, 'int'),
        ('style', 'vertical', 'choice',
         {'choiceSet': ['horizontal', 'vertical']}),
        ('period', 2., 'float'),
        ('phaseShift', 0., 'float', {'min': 0, 'max': 1}),
        ('range', 6, 'float'),
        ('motion', 3, 'choice',
         {'choiceSet': ['linear', 'sinus', 'half sinus', 'slow fast', 'crush', 'switched'],
          'valueDict': {'linear': 0,
                        'sinus': 1,
                        'half sinus': 2,
                        'slow fast': 3,
...
class Rendering(Nodepath):
    ...
    attribs = (
        ...
        ('colorB', 1.0, 'float'),
        ('colorA', 1.0, 'float'),
        ('blending', 'Normal', 'choice',
            {'choiceSet': ['Normal', 'Additive', 'Alpha']}),
        ('fogOn', 0, 'bool'),  # AI Only
        ('renderBin', 'default', 'choice',
