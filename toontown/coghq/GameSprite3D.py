...
class GameSprite:
    ...
    def setBallType(self, type, solidOverride=0):
        ...
        if (not solid) or (colorType > 9):
            ...
            if colorType == 0:
                ...
                #myColor = GameSprite.colorBlue
                myModel = 'phase_12/models/bossbotHQ/bust_a_cog_ball_blue'
            elif colorType == 2:
                #myColor = GameSprite.colorGreen
    ...
    def warningBump(self):
        ...
        track = Sequence(
            Wait(num1 * .1),
            LerpPosInterval(self.nodeObj, num2 * 0.1,
                            Point3(0.0, 0.0, 0.5)),
            LerpPosInterval(self.nodeObj, num3 * 0.1,
                            Point3(0.0, 0.0, 0.0)),
            LerpPosInterval(self.nodeObj, num2 * 0.1,
                            Point3(0.0, 0.0, 0.5)),
            LerpPosInterval(self.nodeObj, num1 * 0.1,
                            Point3(0.0, 0.0, 0.0)),
        )
        track.start()
    ...
    def deathEffect(self):
        ...
        self.delayRemove = 1
        self.canCollide = 0
        #self.deathObj = self.nodeObj
        track = Sequence(
            # LerpPosInterval(self.spriteBase, 0.5 + (num2 * 1.0),
            #        Point3((num1 * curr[0]) + 0.0,  curr[1] + (num3 * 20.0), curr[2] - 10.0)),#, blendType = "easeIn"),
            # LerpPosInterval(self.spriteBase, 0.5 + (num2 * 1.0),
            #        Point3((num1 * curr[0]) + 0.0,  0.0, curr[2] - 20.0)),#, blendType = "easeIn"),
            Parallel(
                ProjectileInterval(self.spriteBase,
                                   #endPos = Point3((num1 * curr[0]) + 0.0, curr[2] - 20.0,  curr[1] + (num3 * 20.0)),
                                   startVel=Vec3(-20.0 + (notNum3 * 40.0), -20.0 + (num3 * 40.0), 30),
                                   duration=0.5 + (num2 * 1.0),
                                   gravityMult=2.00),
                LerpColorScaleInterval(self.spriteBase, duration=0.5 + (num2 * 1.0),
