...
class HoodMgr(DirectObject.DirectObject):
    ...
    def getIdFromName(self, hoodName):
        ...
        if id:
            ...
            self.notify.error(f"No such hood name as: {hoodName}")
            return None
    ...
    def getNameFromId(self, hoodId):
        ...
        if name:
            ...
            self.notify.error(f"No such hood id as: {hoodId}")
            return None
    ...
    def addLinkTunnelHooks(self, hoodPart, nodeList, currentZoneId):
        ...
        for i in nodeList:
            ...
            for p in range(linkTunnelNPC.getNumPaths()):
                ...
                # Name parts will be something like ['linktunnel', 'mm',
                # '3456', 'DNARoot']
                hoodStr = nameParts[1]
                zoneStr = nameParts[2]
                ...
                # enters the tunnel, we are informed which hood it goes to and
                # which zone
    ...
    def makeLinkTunnelName(self, hoodId, currentZone):
        assert(
            self.notify.debug(
                "makeLinkTunnelName() %s" %
                ("**/toph_" +
                 self.getNameFromId(hoodId) +
                 "_" +
                 str(currentZone))))
