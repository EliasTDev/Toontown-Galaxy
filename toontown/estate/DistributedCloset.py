...
class DistributedCloset(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def announceGenerate(self):
        ...
        # sphere for the closet.  SDN: remove that code when we have the closet
        # model
        self.load()
    ...
    def load(self):
        ...
        # determine scale of closet so we can make the appropriate size trigger
        # sphere
    ...
    def handleEnterSphere(self, collEntry):
        ...
        self.notify.debug(
            f"Entering Closet Sphere....{self.closetSphereEnterEvent}")
        ...
        if mode == ClosetGlobals.CLOSED:
            ...
            if self.av:
                if (base.localAvatar.getDoId() == self.customerId):
                    ...
                    # print out our clothes and closet information before we
                    # start
                    print("-----------Starting closet interaction-----------")
                    print(
                        f"customerId: {self.av.doId}, ownerId: {ownerId}")
                    print(
                    ...
                    print(f"topsList = {self.av.getClothesTopsList()}")
                    print(f"bottomsList = {self.av.getClothesBottomsList()}")
                    print("-------------------------------------------------")
    ...
    def __openDoors(self):
        ...
        if hasattr(self, 'rightDoor'):
            if self.rightDoor:
                self.closetTrack.append(
                    self.rightDoor.hprInterval(.5, rightHpr))
