...
class DistributedEstate(DistributedObject.DistributedObject):
    ...
    def setServerTime(self, ts):
        ...
        # now that we know the deltaTime we can start the sun/moon and airplane
        # tasks
    ...
    def __dayTimeTask(self, task):
        ...
        track = Sequence(Parallel(LerpColorScaleInterval(base.cr.playGame.hood.loader.geom,
                         ...
                         Func(
            base.cr.playGame.hood.loader.geom.clearColorScale),
    ...
    def __sunTask(self, task):
        ...
        if hasattr(task, 'ts'):
            ...
            if (ts > HouseGlobals.HALF_DAY_PERIOD and ts < (
                    HouseGlobals.DAY_NIGHT_PERIOD - HouseGlobals.HALF_DAY_PERIOD)):
                self.__stopBirds()
