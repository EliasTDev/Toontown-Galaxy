...
class DistributedEstateAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __init__(self, air):
        ...
        self.goonEnabled = simbase.config.GetBool(
            'estate-goon', 0)  # what is this
        self.goons = None
        ...
        # self.timePerEpoch = 30000 #5000 minutes #NO LONGER A VALID CONCEPT AS
        # EPOCHS HAPPEN ONCE A DAY
        self.gardenTable = []
    ...
    def generate(self):
        DistributedEstateAI.notify.debug(f"DistEstate generate: {self.doId}")
    ...
    def generateWithRequiredAndId(self, doId, air, zoneId):
        ...
        DistributedObjectAI.DistributedObjectAI.generateWithRequiredAndId(
            self, doId, air, zoneId)
...
#        DistributedObjectAI.DistributedObjectAI.requestDelete(self)
    ...
    def delete(self):
        DistributedEstateAI.notify.debug(f"DistEstate delete: {self.doId}")
        self.ignoreAll()
        taskMgr.remove(self.uniqueName("GardenEpoch"))
        taskMgr.remove(self.uniqueName("endCannons"))
        taskMgr.remove(self.uniqueName("endCannonsNotify"))
        ...
        try:
            ...
            self.Estate_deleted
            DistributedEstateAI.notify.debug(
                f"estate already deleted: {self.Estate_deleted}")
        except BaseException:
            #print("deleting Estate")
            DistributedEstateAI.notify.debug(
                f"completing estate delete: {self.__dict__.get('zoneId')}")
    ...
    def initEstateData(
        ...
        self.estateTreasurePlanner = ETreasurePlannerAI.ETreasurePlannerAI(
            self.zoneId)
       ...
       #             ButterflyGlobals.NUM_BUTTERFLY_AREAS[ButterflyGlobals.ESTATE]):
        ...
        dnaData = simbase.air.loadDNAFileAI(
            dnaStore, simbase.air.lookupDNAFileName('estate_1.dna'))
        self.fishingSpots = []
        self.fishingPonds = []
        if (isinstance(dnaData, DNAData)):
            fishingPonds, fishingPondGroups = self.air.findFishingPonds(
                dnaData, self.zoneId, MyEstate, overrideDNAZone=1)
            self.fishingPonds += fishingPonds
            for dnaGroup, distPond in zip(fishingPondGroups, fishingPonds):
                self.fishingSpots += self.air.findFishingSpots(
                    dnaGroup, distPond)
        else:
        ...
        if self.fireworksEnabled:
            ...
            self.estateFireworks = DistributedFireworksCannonAI.DistributedFireworksCannonAI(
                self.air, *pos)
        ...
        if self.goonEnabled:
            ...
            self.goons = [None] * 3
            for i in range(3):
                self.goons[i] = DistributedGoonAI.DistributedGoonAI(
                    self.air, None, i)
            ...
            for i in range(4):
                self.gagBarrels[i] = DistributedGagBarrelAI.DistributedGagBarrelAI(
                    self.air, None, -100 - 10 * i, 30 - 10 * i, 0.2, i, i)
                self.gagBarrels[i].generateWithRequired(self.zoneId)
            from toontown.coghq import DistributedBeanBarrelAI
            jelly = DistributedBeanBarrelAI.DistributedBeanBarrelAI(
                self.air, None, -150, -20, 0.2)
            jelly.generateWithRequired(self.zoneId)
            self.gagBarrels.append(jelly)

            from toontown.coghq import DistributedCrateAI
            self.crate = DistributedCrateAI.DistributedCrateAI(
                self.air, -142, 0, 0.0)
    ...
    def startCannons(self, fool=0):
        ...
        if not self.estateFlyingTreasurePlanner:
            self.estateFlyingTreasurePlanner = EFlyingTreasurePlannerAI.EFlyingTreasurePlannerAI(
                self, self.zoneId)
        ...
        if not hasattr(self, "target") or not self.target:
            self.target = DistributedTargetAI.DistributedTargetAI(
                self.air, -5.6, -24, 50)
    ...
    def bootStrapEpochs(self):
        ...
        print(f"last time {self.lastEpochTimeStamp}")
        currentTime = time.time()
    ...
    def scheduleNextEpoch(self):
        ...
        taskMgr.doMethodLater(
            (secondsNextEpoch),
            self.doEpochNow,
            self.uniqueName("GardenEpoch"))

        tupleNextEpoch = time.localtime(whenNextEpoch)

        self.notify.info(
            "Next epoch to happen at %s %s %s %s %s %s %s %s %s" %
            (tupleNextEpoch))
    ...
    def gardenInit(self, avIdList):
        ...
        for index in range(len(avIdList)):
            if self.getToonId(index) != avIdList[index]:
                self.notify.debug(
                    "Mismatching Estate Tag index %s id %s list %s" %
                    (index, self.getToonId(index), avIdList[index]))
            if self.getItems(index) is None:
                self.notify.debug(
                    f"Items is None index {index} items {self.getItems(index)}")
            if self.getToonId(index) != avIdList[index] or self.getItems(
                    index) is None:
                self.notify.debug(f"Resetting items index {index}")
                self.b_setToonId(index, avIdList[index])
                # resetting the item database
                #self.b_setItems(index, [])
                # empty garden tag
                self.b_setItems(index, [(255, 0, -1, -1, 0)])
    ...
    def doEpochNow(self, taskFooler=0, onlyForThisToonIndex=None, numEpoch=1):
        taskMgr.remove(self.uniqueName("GardenEpoch"))
        # used when a zone is active. So the garden data will be cleared and
        # reinited
        currentTime = time.time()
        timeDiff = currentTime - self.lastEpochTimeStamp
        self.notify.info(f"Epoch Now! {timeDiff}")
        # self.scheduleNextEpoch()
        self.doEpochData(
            self.dayInSeconds,
            numEpoch,
            onlyForThisToonIndex=onlyForThisToonIndex)  # make sure the epoch always happens
    ...
    def doEpochData(self, time, numEpochs=0, onlyForThisToonIndex=None):
        # this function just updates the data buit doesn't effect anything
        # within the zone
        self.saveTime()
        ...
        for index in range(len(self.gardenTable)):
            ...
            if numHealthy or numWilted:
                # log how many healthy and wilted plants he has after these
                # epochs have been run
                self.air.writeServerEvent(
                    'garden_epoch_run', self.getToonId(index), '%d|%d|%d' %
                    (numEpochs, numHealthy, numWilted))
            ...
            if self.getToonId(index) == avId:
                ...
                if specificHardPoint == -1:
                    for plot in range(len(self.gardenTable[index])):
                        ...
                        if someLawnDecor and hasattr(
                                someLawnDecor, 'b_setWaterLevel'):
            ...
            if self.getToonId(index) == avId:
                ...
                if specificHardPoint == -1:
                    for plot in range(len(self.gardenTable[index])):
                        someLawnDecor = self.gardenTable[index][plot]
                        if someLawnDecor and hasattr(
                                someLawnDecor, 'b_setGrowthLevel'):
    ...
    def plantFlower(self, avId, type, plot, water, growth, optional=0):
        for index in range(self.toonsPerAccount):
            if self.getToonId(index) == avId:
                ...
                itemId = self.addLawnDecorItem(
                    index, type, plot, water, growth, optional)
                itemList = self.getItems(index)
    ...
    def plantTree(self, avId, type, plot, water, growth, optional=0):
        for index in range(self.toonsPerAccount):
            if self.getToonId(index) == avId:
                self.removePlant(index, plot)
                itemDoId = self.addLawnDecorItem(
                    index, type, plot, water, growth, optional)
                itemList = self.getItems(index)
                itemList.append((type, plot, water, growth, optional))
                self.b_setItems(index, itemList)
    ...
    def addLawnDecorItem(
        ...
        if plantType == GardenGlobals.GAG_TREE_TYPE:
            ...
            testPlant = DistributedGagTreeAI.DistributedGagTreeAI(
                type, waterLevel, growthLevel, optional, slot, hardPoint)
            ...
            # print("FLOWER")
            plantClass = DistributedFlowerAI.DistributedFlowerAI
            testPlant = DistributedFlowerAI.DistributedFlowerAI(
                type, waterLevel, growthLevel, optional, slot, hardPoint)
            ...
            if type in GardenGlobals.ToonStatuaryTypeIndices:
                ...
                # optional = 2325 #Pig = 3861 #Bear = 3349 #Monkey = 2837 #Duck
                # = 2325 #Rabbit = 1813 #Mouse = 1557 #Horse = 1045 #Cat = 533
                # #Dog = 21
                testPlant = DistributedToonStatuaryAI.DistributedToonStatuaryAI(
                    type, waterLevel, growthLevel, optional, slot, hardPoint)
            elif type in GardenGlobals.ChangingStatuaryTypeIndices:
                testPlant = DistributedChangingStatuaryAI.DistributedChangingStatuaryAI(
                    type, waterLevel, growthLevel, optional, slot, hardPoint)
            else:
                testPlant = DistributedStatuaryAI.DistributedStatuaryAI(
                    type, waterLevel, growthLevel, optional, slot, hardPoint)
            postion = GardenGlobals.estatePlots[slot][hardPoint]
            testPlant.setPosition(postion[0], postion[1], 0)
            testPlant.setH(postion[2])
        ...
        self.notify.debug(f'testPlant.doId : {testPlant.doId}')
        ...
        for plotPointIndex in (list(range(len(plotList)))):
            ...
            if item:
                ...
                if type not in list(GardenGlobals.PlantAttributes.keys()):
                    self.notify.warning(
                        'type %d not found in PlantAttributes, forcing it to 48' %
                        type)
                    type = 48
                hardPoint = item[1]
                waterLevel = item[2]
                growthLevel = item[3]
                # all fields are 8bit except optional which is 16bits
                optional = item[4]
                itemdoId = self.addLawnDecorItem(
                    toonIndex, type, hardPoint, waterLevel, growthLevel, optional)
            else:
                # pass
    ...
    def addGardenPlot(self, slot, hardPoint):
        ...
        testPlant = DistributedGardenPlotAI.DistributedGardenPlotAI(
            plot[2], slot, hardPoint)
    ...
    def destroyEstateData(self):
        if hasattr(self, "Estate_deleted"):
            DistributedEstateAI.notify.debug(
                f"destroyEstateData: estate already deleted: {self.Estate_deleted}")
            return

        DistributedEstateAI.notify.debug(
            f"destroyEstateData: {self.__dict__.get('zoneId')}")
...
# lots of get and set functions, not really the prettiest way to do this
# but it works
    ...
    def setOneItem(
        ...
        if itemsIndex != -1 and waterLevel is None:
    ...
    def checkItems(self, items, slot=0):
        ...
        for item in items:
            if (item[0] < 0) or (item[1] < 0) or (item[4] < 0):
                self.air.writeServerEvent(
                    "Removing_Invalid_Garden_Item_on_Toon ",
                    toonId,
                    f" item {str(item)}")
                items.remove(item)
        return items
    ...
    def creditFlowerBasket(self, av):
        ...
        assert(
            self.notify.debug(
                f"creditFlowerBasket av: {av.getDoId()} is selling all flower"))
        oldBonus = int(len(av.flowerCollection) /
        ...
        self.air.writeServerEvent(
            "garden_flower_sell", av.getDoId(), '%d|%d' %
            (value, len(
                av.flowerBasket.flowerList)))
        ...
        for flower in av.flowerBasket.flowerList:
            ...
            if retval == GardenGlobals.COLLECT_NEW_ENTRY:
                ...
                self.air.writeServerEvent(
                    "garden_collection_add", av.getDoId(), '%d|%d' %
                    (flower.species, flower.variety))
        ...
        newBonus = int(len(av.flowerCollection) /
                       GardenGlobals.FLOWERS_PER_BONUS)
        if newBonus > oldBonus:
            self.notify.info(
                f"avatar {av.doId} gets a bonus: old: {oldBonus}, new: {newBonus}")
            oldMaxHp = av.getMaxHp()
            newMaxHp = min(
                ToontownGlobals.MaxHpLimit,
                oldMaxHp + newBonus - oldBonus)
            ...
            # log garden trophy
            self.air.writeServerEvent(
                "garden_trophy", av.doId, f"{trophyId}")
            ...
            assert(
                self.notify.debug(
                    f"avatar {av.doId} no bonus: old: {oldBonus}, new: {newBonus}"))
            return 0
    ...
    def makeCannonsUntil(self, endTime):
        ...
        taskMgr.doMethodLater(
            secondsUntil,
            self.endCannons,
            self.uniqueName("endCannons"))
        taskMgr.remove(self.uniqueName("endCannonsNotify"))
        taskMgr.doMethodLater(
            secondsUntil,
            self.notifyCannonsEnd,
            self.uniqueName("endCannonsNotify"))
    ...
    def startGameTable(self, avatar=0):
        ...
        if not self.picnicTable:
            ...
            hpr = Point3(0, 0, 0)
            self.picnicTable = DistributedPicnicTableAI.DistributedPicnicTableAI(
                self.air, self.zoneId, 'gameTable', pos[0], pos[1], pos[2], hpr[0], hpr[1], hpr[2])
    ...
    def printPlanterPos(self, slot, index):
        box = self.gardenBoxLispdb
        t[slot][index]
        print(f"X {box.getX()} Y{box.getY} Heading {box.getH()}")
