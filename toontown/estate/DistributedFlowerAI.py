...
class DistributedFlowerAI(DistributedPlantBaseAI.DistributedPlantBaseAI):
    ...
    def handleSkillUp(self, toon):
        ...
        recipeKey = GardenGlobals.PlantAttributes[self.typeIndex]['varieties'][self.optional][0]
        ...
        self.air.writeServerEvent(
            "garden_flower_pick", toon.doId, '%d|%d|%d' %
            (giveSkillUp, toon.getShovel(), toon.getShovelSkill()))
    ...
    def removeItem(self):
        ...
        if not senderId == estateOwnerDoId:
            self.notify.warning(
                "how did this happen, picking a flower you don't own")
    ...
    def movieDone(self):
        if self.lastMovie == GardenGlobals.MOVIE_REMOVE:
            ...
            estate = simbase.air.estateMgr.estate.get(estateOwnerDoId)
            if estate:
                itemId = estate.removePlantAndPlaceGardenPlot(
            ...
            if estate:
                ...
                # results dialog doesn't come up again when he exits from his
                # house
                item = simbase.air.doId2do.get(itemId)
