...
class DistributedFurnitureManagerAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def suggestDirector(self, avId):
        ...
        if (avId != 0) and (not avatar):
            ...
            self.air.writeServerEvent(
                'suspicious', avId, 'FurnitureManager.suggestDirector invalid avId')
            return
    ...
    def avatarEnter(self):
        ...
        self.notify.debug(f"avatarEnter: {self.doId}, {avId}")
        avatar = simbase.air.doId2do.get(avId)
        if avatar:
            avatar.b_setGhostMode(1)
        else:
            self.notify.warning(
                f"{self.doId} got avatarEnter for unknown avatar {avId}")

    def avatarExit(self):
        # Sent from the client when he enters furniture moving mode.
        avId = self.air.getAvatarIdFromSender()
        self.notify.debug(f"avatarExit: {self.doId}, {avId}")
        avatar = simbase.air.doId2do.get(avId)
        if avatar:
    ...
    def isHouseFull(self):
        numAtticItems = len(self.house.atticItems) + \
            len(self.house.atticWallpaper) + len(self.house.atticWindows)
    ...
    def __doMoveItemToAttic(self, avId, doId):
        ...
        if avId != self.director:
            self.notify.warning(
                f"Ignoring request from non-director {avId}.")
            return ToontownGlobals.FM_NotDirector

        dfitem = simbase.air.doId2do.get(doId)
        if dfitem is None or dfitem not in self.dfitems:
            self.notify.warning(
                f"Ignoring request for unknown dfitem {doId}.")
    ...
    def moveItemFromAtticMessage(self, index, x, y, z, h, p, r, context):
        ...
        avId = self.air.getAvatarIdFromSender()
        retcode, objectId = self.__doMoveItemFromAttic(
            avId, index, (x, y, z, h, p, r))
    ...
    def __doMoveItemFromAttic(self, avId, index, posHpr):
        ...
        if index < 0 or index >= len(self.house.atticItems):
            self.notify.warning(
                f"Ignoring request for invalid index {index}.")
    ...
    def __doDeleteItemFromAttic(self, avId, item, index):
        ...
        if avId != self.house.ownerId:
            self.notify.warning(
                f"Ignoring request to delete from non-owner {avId}.")
        ...
        if self.house.atticItems[index] != item:
            self.notify.warning(
                f"Request to delete item {index} does not match {item}.")
            return ToontownGlobals.FM_InvalidIndex

        if not item.isDeletable():
            self.notify.warning(
                f"Ignoring request to delete special item {item}.")
    ...
    def __doDeleteItemFromRoom(self, avId, item, doId):
        ...
        if dfitem.item != item:
            self.notify.warning(
                f"Request to delete item {doId} does not match {item}.")
            return ToontownGlobals.FM_InvalidIndex
    ...
    def __doMoveWallpaperFromAttic(self, avId, index, room):
        ...
        if slot < 0 or slot >= len(self.house.interiorWallpaper):
            self.notify.warning(
                "Ignoring request for invalid wallpaper room %s, surface %s." %
                (room, surface))
            return ToontownGlobals.FM_InvalidIndex
    ...
    def __doDeleteWallpaperFromAttic(self, avId, item, index):
        ...
        if self.house.atticWallpaper[index] != item:
            self.notify.warning(
                f"Request to delete wallpaper {index} does not match {item}.")
            return ToontownGlobals.FM_InvalidIndex
    ...
    def moveWindowToAtticMessage(self, slot, context):
        ...
        self.sendUpdateToAvatarId(
            avId, "moveWindowToAtticResponse", [
                retcode, context])
    ...
    def __doMoveWindowToAttic(self, avId, slot):
        ...
        if index is None:
            self.notify.warning(
                f"Ignoring request for invalid placement slot {slot}.")
            return ToontownGlobals.FM_InvalidIndex
    ...
    def __doMoveWindow(self, avId, fromSlot, toSlot):
        ...
        if fromIndex is None:
            self.notify.warning(
                f"Ignoring request for invalid placement slot {fromSlot}.")
            return ToontownGlobals.FM_InvalidIndex
    ...
    def __doDeleteWindowFromAttic(self, avId, item, index):
        ...
        if self.house.atticWindows[index] != item:
            self.notify.warning(
                f"Request to delete window {index} does not match {item}.")
            return ToontownGlobals.FM_InvalidIndex
    ...
    def __doRecoverDeletedItem(self, avId, item, index):
        ...
        if avId != self.house.ownerId:
            self.notify.warning(
                f"Ignoring request to recover from non-owner {avId}.")
            return ToontownGlobals.FM_NotOwner
        ...
        if self.house.deletedItems[index] != item:
            self.notify.warning(
                f"Request to recover item {index} does not match {item}.")
            return ToontownGlobals.FM_InvalidIndex
        ...
        if isinstance(item, CatalogFurnitureItem.CatalogFurnitureItem):
            ...
            self.notify.warning(
                "Ignoring request to recover invalid item type %s." %
    ...
    def requestControl(self, dfitem, directorAvId):
        ...
        if self.dfitems is None or dfitem not in self.dfitems:
            return 0
    ...
    def manifestInteriorItem(self, item):
        ...
        if not hasattr(item, "getFlags"):
            self.notify.warning(
                f"Ignoring attempt to manifest {item} as a furniture item.")
            return None
