...
class DistributedGagTreeAI(DistributedPlantBaseAI.DistributedPlantBaseAI):
    ...
    def __init__(
            ...
            ownerIndex=0,
            plot=0):
        DistributedPlantBaseAI.DistributedPlantBaseAI.__init__(
            self, typeIndex, waterLevel, growthLevel, optional, ownerIndex, plot)
    ...
    def doEpoch(self, numEpochs):
        for i in range(numEpochs):
            ...
            if estate:
                lowestPlotIndex = estate.findLowestGagTreePlot(
                    self.ownerIndex, self.gagTrack, self.gagLevel)
    ...
    def requestHarvest(self):
        ...
        if not senderId == estateOwnerDoId:
            self.notify.warning(
                "how did this happen, harvesting a flower you don't own")
            self.sendInteractionDenied(senderId)
            return

        if not self.isFruiting() or self.isWilted():
            self.notify.warning(
                "how did this happen, this tree isn't fruiting")
            self.sendInteractionDenied(senderId)
            return
        ...
        self.air.writeServerEvent(
            "garden_gag_harvest", senderId, '%d|%d|%d' %
            (self.doId, self.gagTrack, self.gagLevel))
    ...
    def hasGagBonus(self):
        ...
        """
        retval = (
            self.growthLevel >= self.growthThresholds[2] -
            1) and not self.isWilted()
        return retval
