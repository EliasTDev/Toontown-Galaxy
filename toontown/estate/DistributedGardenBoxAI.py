...
class DistributedGardenBoxAI(DistributedLawnDecorAI.DistributedLawnDecorAI):
    ...
    def setupPetCollision(self):
        # adding petcollision after setting the estateId since we need access
        # to the EstateGeom
        if simbase.wantPets:

            if not DistributedGardenBoxAI.GardenBoxModelA:
                DistributedGardenBoxAI.GardenBoxModelA = loader.loadModel(
                    "phase_5.5/models/estate/planterA.bam")
            if not DistributedGardenBoxAI.GardenBoxModelC:
                DistributedGardenBoxAI.GardenBoxModelC = loader.loadModel(
                    "phase_5.5/models/estate/planterC.bam")
            if not DistributedGardenBoxAI.GardenBoxModelD:
                DistributedGardenBoxAI.GardenBoxModelD = loader.loadModel(
                    "phase_5.5/models/estate/planterD.bam")

            if self.typeIndex == GardenGlobals.BOX_THREE:
                coll = DistributedGardenBoxAI.GardenBoxModelA.find(
                    '**/collision')
            elif self.typeIndex == GardenGlobals.BOX_TWO:
                coll = DistributedGardenBoxAI.GardenBoxModelC.find(
                    '**/collision')
            else:
                coll = DistributedGardenBoxAI.GardenBoxModelD.find(
                    '**/collision2')
