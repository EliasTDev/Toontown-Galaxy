...
class DistributedGardenPlotAI(DistributedLawnDecorAI.DistributedLawnDecorAI):
    ...
    def plantNothing(self, burntBeans):
        ...
        print(f"burning money{burntBeans}")
    ...
    def plantFlower(self, species, variety):
        ...
        if species not in GardenGlobals.PlantAttributes:
            self.air.writeServerEvent(
                'suspicious',
                senderId,
                f'Planting a species {species} that does not exist.')
            return
        ...
        if estateOwnerDoId:
            ...
            if estate:
                ...
                if plantType == GardenGlobals.STATUARY_TYPE:
                    # Statues have only one type. Added this so that I could
                    # use the variety to contain the optional field for the
                    # ToonStatuary
                ...
                if allOk:
                    self.setMovie(GardenGlobals.MOVIE_PLANT,
                                  senderId, (species, variety))

    def burnSpecial(self, species):
        variety = 0

        if species not in GardenGlobals.PlantAttributes or variety not in GardenGlobals.PlantAttributes[
                species]['varieties']:
            self.notify.warning(
                "Suspicious: Calling on a species does not exist.")
            return
    ...
    def plantGagTree(self, gagTrack, gagLevel):
        self.notify.info(f"Planting GagTree: {gagTrack} {gagLevel}")
        senderId = self.air.getAvatarIdFromSender()
        ...
        if not senderId == estateOwnerDoId:
            self.notify.warning(
                "how did this happen, planting in a plot you don't own")
            return
        ...
        if estateOwnerDoId:
            ...
            if estate:
                ...
                if toon.inventory.numItem(gagTrack, gagLevel) <= 0:
                    self.notify.warning(
                        "Toon tried to plant a tree without having the gag! %s" %
                        toon.doId)
                    allOk = False
                ...
                if gagLevel > 0:
                    if not estate.hasGagTree(
                            self.ownerIndex, gagTrack, gagLevel - 1):
                        allOk = False

                # if all goes well plant the flower
                if allOk:
                    self.setMovie(GardenGlobals.MOVIE_PLANT,
                                  senderId, (gagTrack, gagLevel))
                else:
                    self.setMovie(
                        GardenGlobals.MOVIE_PLANT_REJECTED, senderId, (gagTrack, gagLevel))
        ...
        if self.lastMovie == GardenGlobals.MOVIE_PLANT:
            ...
            if type == GardenGlobals.GAG_TREE_TYPE:
                ...
                if estate:
                    ...
                    # tell the new tree to tell the avatar to finish up the
                    # planting movie
                ...
                if estate:
                   ...
                   # since plant statuary calls plant flower, lets
                   # differentiate between the two
                    if type == GardenGlobals.FLOWER_TYPE:
                        ...
                        self.notify.warning(f'unhandled case {type}')
