...
class DistributedLawnDecor(
    ...
    def makeMovieNode(self):
        # create a movieNode... this is where the toon will stand while playing
        # a movie
    ...
    def stick2Ground(self, taskfooler=0):
        ...
        if queue.getNumEntries() > 0:
            ...
            for index in range(queue.getNumEntries()):
                ...
                # if
                # entry.getIntoNode().getParent(0).getParent(0).getParent(0).getName()
                # == 'terrain_DNARoot':
                if recurseParent(entry.getIntoNode(), 'terrain_DNARoot'):
                    # if str(entry.getIntoNodePath()) ==
                    # "render/Estate/30000:estate/terrain_DNARoot/-PandaNode/group3/collision1":
                    self.setZ(entry.getSurfacePoint(
                        render)[2] + self.stickUp + 0.1)
    ...
    def canBePicked(self):
        ...
        """
        retval = True
        # if there is no longer a localToon or it is not the owner, return
        # false

        self.notify.debug(f'base.localAvatar.doId : {base.localAvatar.doId}')
        self.notify.debug(f'self.getOwnerId : {self.getOwnerId()} ')
        self.notify.debug(f"statue's DoId : {self.doId} ")
            ...
            Sequence(ActorInterval(toon, "start-dig"),
                     Parallel(ActorInterval(toon, "loop-dig", loop=1, duration=5.13),
                              Sequence(Wait(0.25), SoundInterval(sound, node=toon, duration=0.55),
                                       Wait(0.80), SoundInterval(
                                           sound, node=toon, duration=0.55),
                                       Wait(1.35), SoundInterval(
                                           sound, node=toon, duration=0.55),
                                       ),
                              ),
