...
class DistributedLawnDecorAI(DistributedNodeAI.DistributedNodeAI):
    ...
    def destroy(self):
        self.notify.info(f'destroy entity(elevatorMaker) {self.entId}')
    ...
    def movieDone(self):
        self.clearInteractingToon()
        self.notify.debug(
            '---- got movieDone, self.lastMovie=%d lastMovieAvId=%d---')
    ...
    def requestInteractingToon(self, toonId):
        ...
        if self.interactingToonId == 0:
            ...
            self.notify.debug(
                'denying interaction by %d since %s is using it' %
                (toonId, self.getInteractingToon()))
