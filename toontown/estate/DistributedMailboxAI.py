...
class DistributedMailboxAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def b_setFullIndicator(self, full):
        DistributedMailboxAI.notify.debug(
            f"b_setFullIndicator( full={full} )")
        self.setFullIndicator(full)
        self.d_setFullIndicator(full)

    def d_setFullIndicator(self, full):
        DistributedMailboxAI.notify.debug(
            f"d_setFullIndicator( full={full} )")
        self.sendUpdate("setFullIndicator", [full])

    def setFullIndicator(self, full):
        DistributedMailboxAI.notify.debug(f"setFullIndicator( full={full} )")
    ...
    def raiseFlagLater(self, duration):
        ...
        DistributedMailboxAI.notify.debug(
            f"raiseFlagLater( duration={duration:.2f} )")
        if self.raiseFlagTask:
            taskMgr.remove(self.raiseFlagTask)
    ...
    def avatarEnter(self):
        ...
        if not av:
            self.air.writeServerEvent(
                'suspicious', avId, 'DistributedMailboxAI.avatarEnter unknown')
            self.notify.warning(
                f"av {avId} not in doId2do tried to pick up mailbox")
            return
        ...
        if self.house.ownerId == avId:
            ...
            if len(av.mailboxContents) != 0:
                ...
                self.sendUpdate(
                    "setMovie", [
                        MailboxGlobals.MAILBOX_MOVIE_WAITING, avId])
                self.sendClearMovie()
            else:
                ...
                self.sendUpdate(
                    "setMovie", [
                        MailboxGlobals.MAILBOX_MOVIE_EMPTY, avId])
                self.sendClearMovie()
        else:
            # Wrong mailbox.  Go away.
            # The client should filter this already.
            self.air.writeServerEvent(
                'suspicious', avId, 'DistributedMailboxAI.avatarEnter not owner')
            self.sendUpdate(
                "setMovie", [
                    MailboxGlobals.MAILBOX_MOVIE_NOT_OWNER, avId])
            self.sendClearMovie()
    ...
    def isItemIndexValid(self, item, itemIndex):
        ...
        if itemIndex < 0 or itemIndex >= (
                len(self.av.mailboxContents) + len(self.av.awardMailboxContents)):
            result = False
        if result:
            if not item.isAward():
    ...
    def discardItemMessage(self, context, blob, itemIndex, optional):
        ...
        if self.busy != avId:
            ...
            self.air.writeServerEvent(
                'suspicious',
                avId,
                f'DistributedMailboxAI.discardItem invalid index {itemIndex}')
            retcode = ToontownGlobals.P_InvalidIndex
        elif not self.isItemIndexValid(item, itemIndex):
            self.air.writeServerEvent(
                'suspicious',
            ...
            if item.isAward():
                del self.av.awardMailboxContents[itemIndex]
                self.air.writeServerEvent(
                    "Discarding Item award",
                    avId,
                    f"discarded item {item.getName()}")
    ...
    def acceptInviteMessage(self, context, inviteKey):
        ...
        if self.busy != avId:
            ...
            self.air.writeServerEvent(
                'suspicious',
                avId,
                f'DistributedMailboxAI.acceptInvite busy with {self.busy}')
            self.notify.warning(
                "Got unexpected accept invite request from %s while busy with %s." %
                (avId, self.busy))
            retcode = ToontownGlobals.P_NotAtMailbox
            ...
            self.air.writeServerEvent(
                'suspicious',
                avId,
                f'DistributedMailboxAI.acceptInvite invalid inviteKey {inviteKey}')
            retcode = ToontownGlobals.P_InvalidIndex
        else:
            # hand it off to party manager
            # it's possible the party was cancelled right before he accepted
            self.air.partyManager.respondToInviteFromMailbox(
                context, inviteKey, InviteStatus.Accepted, self.doId)
    ...
    def respondToAcceptInviteCallback(self, context, inviteKey, retcode):
        ...
        if self.av:
            self.sendUpdateToAvatarId(
                self.av.doId, "acceptItemResponse", [
                    context, retcode])
        pass
    ...
    def rejectInviteMessage(self, context, inviteKey):
        ...
        if self.busy != avId:
            ...
            self.air.writeServerEvent(
                'suspicious',
                avId,
                f'DistributedMailboxAI.rejectInvite busy with {self.busy}')
            DistributedMailboxAI.notify.warning(
                "Got unexpected reject invite request from %s while busy with %s." %
                (avId, self.busy))
            retcode = ToontownGlobals.P_NotAtMailbox
            ...
            self.air.writeServerEvent(
                'suspicious',
                avId,
                f'DistributedMailboxAI.rejectInvite invalid inviteKey {inviteKey}')
            retcode = ToontownGlobals.P_InvalidIndex
        else:
            # hand it off to party manager
            # it's possible the party was cancelled right before he rejected
            self.air.partyManager.respondToInviteFromMailbox(
                context, inviteKey, InviteStatus.Rejected, self.doId)
    ...
    def respondToRejectInviteCallback(self, context, inviteKey, retcode):
        ...
        if self.av:
            self.sendUpdateToAvatarId(
                self.av.doId, "discardItemResponse", [
                    context, retcode])
        pass
