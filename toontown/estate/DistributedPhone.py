...
class DistributedPhone(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def setupCamera(self, mode):
        ...
        if (mode == PhoneGlobals.PHONE_MOVIE_PICKUP):
            quat = Quat()
            quat.setHpr((35, -8, 0))
            LerpPosQuatInterval(camera, 1, (4, -4, base.localAvatar.getHeight() - 0.5),
                                quat, blendType="easeOut", other=base.localAvatar).start()
    ...
    def setLimits(self, numHouseItems):
        assert(self.notify.debug(f"setLimits({numHouseItems})"))
    ...
    def setMovie(self, mode, avId, timestamp):
        ...
        self.notify.debug(f"setMovie: {mode} {avId} {isLocalToon}")
    ...
    def takePhoneInterval(self, toon):
        ...
        phoneOutAnim = f'{torso}{legs}_phoneOut'
        takePhoneAnim = f'{torso}{legs}_takePhone'
        phoneNeutralAnim = f'{torso}{legs}_phoneNeutral'
    ...
    def replacePhoneInterval(self, toon):
        ...
        phoneBackAnim = f'{torso}{legs}_phoneBack'
