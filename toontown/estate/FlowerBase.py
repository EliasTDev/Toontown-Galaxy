...
class FlowerBase:
    ...
    def __init__(self, species, variety):
        ...
        assert self.variety < len(
            GardenGlobals.PlantAttributes[species]['varieties'])
    ...
    def setVariety(self, variety):
        self.variety = variety
        assert self.variety < len(
            GardenGlobals.PlantAttributes[self.species]['varieties'])
    ...
    def getVarietyName(self):
        # TTLocalizer.FishSpeciesNames[self.genus][self.species]
    ...
    def getFullNameWithRecipe(self):
        ...
        name += f" ({GardenGlobals.Recipes[recipeKey]['beans']})"
        return name

    def __str__(self):
        return (f"{self.getFullName()}, value: {self.getValue()}")
