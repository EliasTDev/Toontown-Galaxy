...
class FlowerPhoto(NodePath):
    ...
    def makeFlowerFrame(self, actor):
        ...
        if 'photoPos' in attrib:
            self.notify.debug(f'oldPos = {actor.getPos()}')
            photoPos = attrib['photoPos']
            self.notify.debug(f'newPos = {str(photoPos)}')
    ...
    def show(self, showBackground=0):
        ...
        self.actor = self.loadModel(self.species,
                                    self.variety)  # loader.loadModel(modelName)
