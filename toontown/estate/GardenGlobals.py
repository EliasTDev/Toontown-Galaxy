...
PlantAttributes = {
    ...
    230: {'name': TTLocalizer.StatuaryMeltingSnowman,
          ...
          'varieties': ((1030, 1, 0),),
          'pinballScore': (500, 1),
          # different models at growth level 0, 1, then 2 and up
    ...
    for key in list(Recipes.keys()):
        ...
        for letter in beans:
            if key not in (2001, 2002):
                assert letter in BeanColorLetters, 'Bad bean %c -- key=%d, beans=%s' % (
                    letter, key, beans)
        testTuple = (beans, recipe['special'])
        assert testTuple not in uniqueRecipes, 'Duplicate recipe %s key=%d' % (
            testTuple, key)
        ...
        if special != -1:
            assert special in list(
                Specials.keys()), 'No special %d in Specials dict' % (special)
            assert special not in uniqueSpecials, 'Duplicate special %s key=%d' % (
                testTuple, key)
...
def validatePlantAttributes(notify):
    ...
    for key in list(PlantAttributes.keys()):
        ...
        notify.debug(f"now validating {plant['name']}")
        # check growth thresholds
        if plant['plantType'] in (GAG_TREE_TYPE, FLOWER_TYPE):
            growthThresholds = plant['growthThresholds']
            assert len(
                growthThresholds) == 3, 'Need 3 numbers in growthThresholdskey=%d' % key
            lastValue = 0
            for testValue in growthThresholds:
                assert lastValue <= testValue, 'prevValue <= nextValue growthThreholds=%s' % str(
                    growthThresholds)
                lastValue = testValue
        ...
        if plant['plantType'] in (STATUARY_TYPE, FLOWER_TYPE):
            ...
            for variety in varieties:
                ...
                assert recipeNum in list(Recipes.keys(
                )), 'Invalid recipeNum %d, key=%d, variety=%s' % (recipeNum, key, str(variety))
                assert recipeNum not in uniqueRecipes, 'duplicate recipe %d, key=%d, variety=%s' % (
                    recipeNum, key, str(variety))
    ...
    for numBeans in range(len(flowerRecipeDistribution)):
        ...
        for flower in flowerRecipeDistribution[numBeans]:
            notify.debug(
                f'    {flower[0]},  beans = {flower[1]}, color={flower[2]}')
