...
class FishManagerAI:
    ...
    def __chooseItem(self, av, zoneId):
        ...
        for cutoff in FishGlobals.SortedProbabilityCutoffs:
            if rand <= cutoff:
                ...
                self.notify.debug(f"__chooseItem: {itemType}")
                return itemType
        self.notify.warning(
            "Somehow we did not choose an item, returning boot")
    ...
    def recordCatch(self, avId, zoneId, pondZoneId):
        ...
        if av.bingoCheat:
            # Fished up a boot
            self.notify.debug(f"av: {avId} caught the boot")
            rodId = av.getFishingRod()
            self.air.writeServerEvent(
                "fishedBoot", avId, f"{rodId}|{zoneId}")
        ...
        if item:
            self.notify.debug(f"av: {avId} caught quest item: {item}")
            # Write to server logs
            rodId = av.getFishingRod()
            self.air.writeServerEvent(
                "fishedQuestItem", avId, f"{rodId}|{zoneId}|{item}")
        ...
        if itemType == FishGlobals.FishItem:
            ...
            if success:
                ...
                if inTank:
                    # TODO: This is a bit wasteful to loop through the fish
                    # tank twice
                    hasBiggerAlready = av.fishTank.hasBiggerFish(
                ...
                if added:
                    ...
                    if collectResult == FishGlobals.COLLECT_NO_UPDATE:
                        ...
                        # If it is not in our tank also, it really is a new
                        # entry
                        if not inTank:
                        ...
                        self.notify.error(
                            f"unrecognized collectResult: {collectResult}")
                else:
                    self.notify.debug(f"av: {avId} is over the tank limit")
                    return (FishGlobals.OverTankLimit, None)
            else:
                # If you did not choose a fish, you get the boot
                self.notify.debug(
                    f"av: {avId} tried to catch fish, but got the boot")
                rodId = av.getFishingRod()
                self.air.writeServerEvent(
                    "fishedBoot", avId, f"{rodId}|{zoneId}")
                self.air.handleAvCatch(avId, pondZoneId, FishGlobals.BingoBoot)
            ...
            self.notify.debug(
                f"av: {avId} caught {jellybeanAmount} jellybeans")
            self.air.writeServerEvent(
                "fishedJellybeans", avId, f"{rodId}|{zoneId}|{jellybeanAmount}")
    ...
    def creditFishTank(self, av):
        ...
        assert(
            self.notify.debug(
                f"creditFishTank av: {av.getDoId()} is selling all fish"))
        ...
        if newBonus > oldBonus:
            ...
            self.air.writeServerEvent("fishTrophy", av.doId, f"{trophyId}")
            return 1
        else:
            assert(
                self.notify.debug(
