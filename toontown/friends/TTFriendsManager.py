...
class TTFriendsManager(DistributedObjectGlobal):
    ...
    def petInfo(
        ...
        fields.extend(zip(("setBoredom",
                           ...
                           "setSurprise"),
                          moods))
        fields.extend(zip(("setForgetfulness",
                           "setBoredomThreshold",
    ...
    def teleportQuery(self, id):
        ...
        if not hasattr(
            base.localAvatar,
            'ghostMode' or hasattr(
                base.localAvatar,
                'getTeleportAvailable')):
            self.sendUpdate('teleportResponse', [id, 0, 0, 0, 0])
            return
        ...
        if base.localAvatar.ghostMode or not base.localAvatar.getTeleportAvailable():
            if hasattr(avatar, 'getName'):
                base.localAvatar.setSystemMessage(
                    id, OTPLocalizer.WhisperFailedVisit %
                    avatar.getName())
            self.sendUpdate('teleportResponse', [id, 0, 0, 0, 0])
            return
        ...
        if hasattr(avatar, 'getName'):
            base.localAvatar.setSystemMessage(
                id, OTPLocalizer.WhisperComingToVisit %
                avatar.getName())
    ...
    def setTeleportResponse(self, id, available, district, hoodId, zoneId):
        base.localAvatar.teleportResponse(
            id, available, district, hoodId, zoneId)
