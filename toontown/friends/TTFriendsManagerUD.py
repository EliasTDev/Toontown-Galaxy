...
class OperationFSM(FSM):
    ...
    def __init__(self, mgr, air, senderId, targetId=None, callback=None):
        FSM.__init__(self, f'OperationFSM-{senderId}')
        self.mgr = mgr
        self.air = air
    ...
    def enterError(self, message=None):
        self.mgr.notify.warning(
            f"An error has occurred in a '{type(self).__name__}'. Message: {message}")
...
class FriendsListOperation(OperationFSM):
    ...
    def addFriend(self, dclass, fields):
        ...
        self.actualFriendsList.append([friendId,
                                       fields['setName'][0],
        ...
        self.air.dbInterface.queryObject(
            self.air.dbId, self.friendsList[self.friendIndex], self.addFriend)
...
class RemoveFriendOperation(OperationFSM):
    ...
    def handleRetrieve(self, dclass, fields):
        ...
        self.demand(
            'Retrieved',
            fields['setFriendsList'][0],
            fields['setTrueFriends'][0])
    ...
    def enterRetrieved(self, friends, trueFriends):
        ...
        self.air.dbInterface.updateObject(
            self.air.dbId, self.sender, self.air.dclassesByName['DistributedToonUD'], {
    ...
    def enterRetrieved(self, friends):
        for friend in friends:
            newOperation = RemoveFriendOperation(
                self.mgr, self.air, friend, targetId=self.sender, alert=True)
...
class TTFriendsManagerUD(DistributedObjectGlobalUD):
    ...
    def getPetInfo(self, petId):
        ...
        def handlePet(dclass, fields):
            ...
            self.sendUpdateToAvatarId(
                senderId, 'petInfo', [
                    petId, fields.get(
                        "setOwnerId", [0])[0], fields.get(
                        "setPetName", ["???"])[0], fields.get(
                        "setTraitSeed", [0])[0], fields.get(
                        "setSafeZone", [0])[0], traits, moods, dna, fields.get(
                            "setLastSeenTimestamp", [0])[0]])
    ...
    def friendOnline(self, toonId, friendsList):
        ...
        dgcleanup = self.dclass.aiFormatUpdate(
            'goingOffline', self.doId, self.doId, self.air.ourChannel, [toonId])
        dg = PyDatagram()
        dg.addServerHeader(
            channel,
            self.air.ourChannel,
        ...
        def handleToon(dclass, fields):
            ...
            for friend in friendsList:
                if friend in self.onlineFriends:
                    self.sendUpdateToAvatarId(
                        friend, 'friendIsOffline', [friendId])
            if friendId in self.onlineFriends:
    ...
    def giveUpTeleportQuery(self, senderId, toId):
        if senderId in self.teleportRequests:
            ...
            self.sendUpdateToAvatarId(
                senderId, 'setTeleportResponse', [
                    toId, 0, 0, 0, 0])
            self.notify.warning(
                'Teleport request that was sent by %d to %d timed out.' %
                (senderId, toId))
    ...
    def teleportResponse(self, id, available, shardId, hoodId, zoneId):
        ...
        if self.teleportRequests.get(id) != senderId:
            self.air.writeServerEvent(
                'suspicious',
                senderId,
                "toon tried to send a teleportResponse for a query that isn't theirs!")
            return
        self.sendUpdateToAvatarId(
            id, 'setTeleportResponse', [
                senderId, available, shardId, hoodId, zoneId])
    ...
    def whisperSCToontaskTo(self, id, taskId, toNpcId, toonProgress, msgIndex):
        requester = self.air.getAvatarIdFromSender()
        self.sendUpdateToAvatarId(
            id, 'setWhisperSCToontaskFrom', [
                requester, taskId, toNpcId, toonProgress, msgIndex])
