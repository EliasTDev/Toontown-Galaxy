...
class DistributedGolfHole(
    ...
    def __init__(self, cr):
        ...
        FSM.__init__(self, f"Golf_{self.id}_FSM")

        #base.g = self
        self.currentGolfer = 0
        # {avId : (ballNodePath, actorNode, ballActorNodePath)}
        self.ballDict = {}
        self.ballShadowDict = {}
        self.holeNodes = []
        ...
        # last ball position where we played a bump sfx
    ...
    def loadLevel(self):
        ...
        if self.teeNodePath.isEmpty():
            ...
            self.notify.debug(
                f'teeNodePath heading = {self.teeNodePath.getH()}')
        ...
        while not teeNode.isEmpty():
            ...
            self.notify.debug(f'teeNodeP heading = {teeNode.getH()}')
            teeIndex += 1
            teeNode = self.terrainModel.find('**/tee%d' % teeIndex)
    ...
    def loadSounds(self):
        ...
        self.holeInOneSfx = loader.loadSfx(
            "phase_6/audio/sfx/Golf_Hole_In_One.ogg")
        ...
        self.bumpHardSfx = loader.loadSfx(
            "phase_6/audio/sfx/Golf_Hit_Barrier_3.ogg")
    ...
    def setup(self):
        ...
        self.arrowKeys.setReleaseHandlers(
            [None, None, None, None, self.__endTossGolf])
    ...
    def __aimTask(self, task):
        ...
        # Gah always point the club at the ball, avoid weird club penetrating
        # ball problems
    ...
    def sendSwingInfo(self):
        ...
        # this is a good point to check for the warning if he hasn't pressed
        # control
        ...
        self.notify.debug(f'curAimTime = {curAimTime:f}')
    ...
    def enterAim(self):
        ...
        self.notify.debug(f'currentGolfer = {self.currentGolfer}')
    ...
    def exitWatchAim(self):
        ...
        if av:
            ...
            av.setPos(toonPos)
            self.notify.debug(
                f'av {av.getName()} now at position {av.getPos()}')
        else:
    ...
    def enterPlayback(self):
        ...
        # we have to correct for the time the toon swings before he hits the
        # ball
        ...
        if self.ballWillGoInHole:
            ...
            if adjustedBallFirstTouchedHoleTime < timeToPlayBallRest:
                ...
                # do we have more than enough time to play the full rattling
                # sound?
        ...
        if self.ballWillGoInHole:
            ...
            self.notify.debug(f'buildupLength={buildupLength}')
            ...
            buildupLength = crowdBuildupSfx.length()
            self.notify.debug(
                f'touched but not going in buildupLength={buildupLength}')
            diffTime = adjustedBallFirstTouchedHoleTime - buildupLength
            if diffTime > 0:
                self.notify.debug(
                    f'waiting {diffTime:.2f} to play crowd buildup')
                crowdIval.append(Wait(diffTime))
                crowdIval.append(SoundInterval(crowdBuildupSfx))
    ...
    def exitPlayback(self):
        ...
        if av:
            if self.ballWillGoInHole:
                # we need this so we don't get a noticeable pause on the
                # spectator
    ...
    def isCurBallInHole(self):
        ...
        for holePos in self.holePositions:
            ...
            self.notify.debug(f'hole {holePos} length={length}')
    ...
    def updateTranslucentObjects(self):
        ...
        for transNpCurFrame in self.translucentCurFrame:
            if transNpCurFrame not in self.translucentLastFrame:
                ...
                self.notify.debug(f'making translucent {transNpCurFrame}')
    ...
    def playBackFrame(self):
        ...
        if self.playbackFrameNum >= self.destFrame[0]:
            ...
            if self.destFrameNum < (len(self.recording)):
                ...
                self.notify.debug(f"recording length {len(self.recording)}")
    ...
    def ballMovie2Client(
        ...
        self.notify.debug(
            f'playback movie duration={self.playbackMovieDuration}')
    ...
    def golfersTurn(self, avId):
        ...
        assert self.notify.debug(f"golfers Turn {avId}")
    ...
    def createBall(self, avId, index=None):
        ...
        golfBallGeom, golfBall, odeGeom = self.createSphere(
            self.world, self.space, GolfGlobals.GOLF_BALL_DENSITY, GolfGlobals.GOLF_BALL_RADIUS, index)
        ...
        golfBallGeom.setName(f'golfBallGeom{avId}')
    ...
    def golferChooseTee(self, avId):
        ...
        assert self.notify.debug(f"golferChoseTee  {avId}")
        self.readyCurrentGolfer(avId)
    ...
    def showOnlyCurGolfer(self):
        ...
        assert self.notify.debugStateCall(self)
        self.notify.debug(f'curGolfer = {self.currentGolfer}')
    ...
    def doFlyOverMovie(self, avId):
        ...
        title = GolfGlobals.getCourseName(self.golfCourse.courseId) + ' :\n ' + GolfGlobals.getHoleName(
            self.holeId) + '\n' + TTLocalizer.GolfPar + ' : ' + f"{self.holeInfo['par']}"
        ...
        try:
            ...
            self.flyOverActor.setBlend(base.wantSmoothAnimations)
        except Exception:
            # this hole doesn't have the flyover animation, just return
            self.notify.debug(f"Couldn't find flyover {camModelFullPath}")
            return False
    ...
    def safeRequestToState(self, newState):
        ...
        if not doingRequest:
            self.notify.warning(
                f'ignoring transition from {self.state} to {newState}')
