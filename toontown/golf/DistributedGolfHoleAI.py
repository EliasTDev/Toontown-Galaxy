...
class DistributedGolfHoleAI(
    ...
    def delete(self):
        ...
        self.notify.debug(f'self.barrierPlayback is {self.barrierPlayback}')
    ...
    def selectNextGolfer(self):
        ...
        while (
            safety < 50) and (
            not self.golfCourse.checkGolferPlaying(
                self.golfCourse.getGolferIds()[
                self.activeGolferIndex])):
            self.activeGolferIndex += 1
            self.notify.debug(f"Index {self.activeGolferIndex}")
            if self.activeGolferIndex >= len(self.golfCourse.getGolferIds()):
                self.activeGolferIndex = 0
            self.activeGolferId = self.golfCourse.getGolferIds()[
                self.activeGolferIndex]
    ...
    def ballMovie2AI(
        ...
        if sentFromId == self.trustedPlayerId:
            ...
            if self.state == 'WaitPlayback' or self.state == 'WaitTee':
                self.notify.warning(
                    f'ballMovie2AI requesting from {self.state} to WaitPlayback')
    ...
    def performReadyAction(self):
        ...
        if self.state == 'WaitPlayback' or self.state == 'WaitTee':
            self.notify.warning(
                f'performReadyAction requesting from {self.state} to WaitPlayback')
        self.request("WaitPlayback")
