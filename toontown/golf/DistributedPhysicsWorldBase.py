...
class DistributedPhysicsWorldBase:
    ...
    def __init__(self, canRender=1):
        ...
        if self.canRender:
            ...
            self.worldAttach = render.attachNewNode(
                "physics geom attach point")
        else:
    ...
    def setupSimulation(self):
        ...
        self.world.setSurfaceEntry(
            0, 0, 150, 0.15, 0.1, 0.9, 0.00001, 0.0, 0.40)
        self.world.setSurfaceEntry(
            1, 1, 5000, 0.15, 0.1, 0.9, 0.00001, 0.0, 1.00)
        self.world.setSurfaceEntry(
            2, 2, 150, 0.15, 0.1, 0.9, 0.00001, 0.0, 0.40)
        self.world.setSurfaceEntry(
            0, 2, 150, 0.15, 0.1, 0.9, 0.00001, 0.0, 0.40)
    ...
    def createCommonObject(
        ...
        if type == 0:  # create Box
            ...
            self.commonObjectDict[commonId] = (
                commonId, type, box, motor, timeData, forceData)
            ...
            model, cross = self.createPinWheel(
                self.world, self.space, 10.0, 0.8, 10.0, 0.6, 0.35, 2, millFan, (0, 0, 90), (-4.8, 0, -2.25))
            motor = OdeHingeJoint(self.world)
            ...
            model, box = self.createBox(
                self.world, self.space, 10.0, sizeX, sizeY, 1.0, 2)
            box.setPosition(vPos)
            box.setQuaternion(self.placerNode.getQuat())
            motor = OdeSliderJoint(self.world)
            motor.attach(box, 0)
    ...
    def createSphere(self, world, space, density, radius, ballIndex=None):
        ...
        # GolfGlobals.BALL_COLLIDE_ID
        print((("collide ID is %s") % (self.space.setCollideId(geom, 42))))
