...
class GolfHoleBase:
    ...
    def loadLevel(self):
        ...
        self.notify.debug(f'hardSurface = {hardSurface}')
    ...
    def trackRecordBodyFlight(
        ...
        self.notify.debug(f"start position {startPos}")
        ...
        for frame in range(startFrame):
            ...
            self.setTimeIntoCycle(
                self.swingTime + (float(frameCount) * self.DTAStep))
            frameCount += 1
        ...
        self.notify.debug(f"self.comObjNeedPass {self.comObjNeedPass}")
        firstDisabled = -1
        reEnabled = 0
        lastFrameEnabled = 0
        checkFrames = self.FPS * (self.timingCycleLength + 1.0)
        hasPrinted = 0
        while (ball.isEnabled() and len(self.recording) <
               2100) or self.comObjNeedPass or (len(self.recording) < 10):
            ran = 1
            if len(self.recording) > 2100 and not hasPrinted:
                self.notify.debug(f"recording too long {len(self.recording)}")
                hasPrinted = 1
                ball.disable()
            self.preStep()
            self.simulate()
            ...
            if self.comObjNeedPass:
                if firstDisabled == -1 and (not ball.isEnabled()):
                    ...
                    self.notify.debug(f"firstDisabled {firstDisabled}")
        ...
        self.notify.debug(f"lastFrameEnabled {lastFrameEnabled}")
        if lastFrameEnabled < 3:
            lastFrameEnabled = 3

        self.record(ball)
        self.notify.debug(f"Frames {self.frame}")
    ...
    def record(self, ball):
        ...
        if (self.frame) > 50 and not (self.frame % 13):
            ...
            # if (abs(currPosA - past5PosA) < 0.05) and (abs(currPosA -
            # past10PosA) < 0.05):
    ...
    def postStep(self):
        ...
        for entry in self.colEntries:
            ...
            if c0 == GolfGlobals.GRASS_COLLIDE_ID and \
                ...
                if self.curGolfBall().getPosition()[2] < (
                        z + 0.2) and rayCount == 0:
                    if self.skyContact in [1, 2]:
                        ...
                        skyRayHitPos = Vec3(
                            x, y, z)  # self.curGolfBall().getPosition()
            ...
            if self.doingRecording:
                if ((c0 == GolfGlobals.OOB_RAY_COLLIDE_ID) or
                    ...
                    # we need to zero out ball In hole frame since the ball
                    # bounced out
                ...
                if self.canRender:
                    ...
                    if GolfGlobals.CAMERA_RAY_COLLIDE_ID in [c0, c1] and \
                       GolfGlobals.WINDMILL_BASE_COLLIDE_ID in [c0, c1]:
                        ...
                        self.translucentCurFrame.append(
                            self.windmillFanNodePath)
                        self.translucentCurFrame.append(
                            self.windmillBaseNodePath)
        ...
        if rayCount == 0:
            ...
            if ballRayHitPos:
                ...
                if self.doingRecording:
                    ...
                    self.notify.debug(f"{self.curGolfBall().getLinearVel()}")
        ...
        if self.grayCount > self.backAmount and self.doingRecording:
            ...
            self.curGolfBall().setPosition(
                self.recording[self.greenIn][1], self.recording[self.greenIn][2], self.recording[self.greenIn][3] + 0.27)
            ...
            #    self.curGolfBall().setLinearVel(-0.6 * self.llv[0], -0.6 * self.llv[1], abs(0.3*self.llv[2]))
                ...
                self.notify.debug(f"back disable {self.frame}")
    ...
    def predict(self, frame, sourceFrame, destFrame, errorMult=1.0):
        ...
        # tests whether the frame could be predicted by the source and
        # destframe within the tolerences
    ...
    def simulate(self):
        ...
        if self.maxColCount < self.colCount:
            ...
            self.notify.debug(f'New Max Collision Count {self.maxColCount}')
