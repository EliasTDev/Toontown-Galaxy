...
class GolfScoreBoard:
    ...
    def setup(self):
        ...
        for parHoleIndex in range(self.golfCourse.numHoles):
            parLabel = DirectLabel(
                ...
                relief=None,
                pos=(self.lineVStart + 0.055 +
                     (horzOffset * parHoleIndex), 0, holeTop - 0.1),
                #color = (0.4, 0.4, 0.4, 1),
                text_align=TextNode.ACenter,
                text=(
                    f"{GolfGlobals.HoleInfo[self.golfCourse.holeIds[parHoleIndex]]['par']}"),
                text_scale=0.05,
                text_wordwrap=10
            )
            totPar = totPar + \
            ...
            if(isinstance(textN, OnscreenText)):
                try:
                    if(textN.textNode.getWordwrappedWtext() != name):
                        ...
                        vert = self.playaTop - \
                            (self.vertOffset * playaIndex) - self.vCenter
                except BaseException:
                    # handle potential unicode ascii encode error
                    vert = self.playaTop - (self.vertOffset * playaIndex)

            self.playaTags[playaIndex].setPos(self.lineVStart - 0.23, 0, vert)

            self.notify.debug(
                f'self.text height = {self.playaTags[playaIndex].getHeight():f}')

            holeIndex = 0
            for holeIndex in range(self.golfCourse.numHoles):
                holeLabel = DirectLabel(parent=self.scoreboard,
                                        relief=None,
                                        pos=(self.lineVStart + 0.055 + (horzOffset * holeIndex),
                                             0,
        ...
        self.lines.drawTo(self.lineVStart +
                          (11 * self.lineVertOffset), 0, self.lineHStart)
        ...
        self.lines.moveTo(
            ...
            self.lineHStart +
            0.19)
        self.lines.drawTo(self.lineVStart + 11 *
                          self.lineVertOffset, 0, self.lineHStart - (4 * 0.13))
    ...
    def getScoreLabel(self, avIdorIndex, holeNum):
        ...
        # return self.scoreTags[(holeNum * self.numPlayas) + index]
    ...
    def showBoard(self, task=None):
        ...
        for playaIndex in range(self.numPlayas):
            if self.golfCourse.isGameDone():
                ...
                if(currentGolfer == self.golfCourse.avIdList[playaIndex]):
                    ...
                    self.highlightCur.setPos(-0.003, 0, 0.038 - \
                                             (playaIndex * (self.lineVertOffset + 0.005)))
                    self.highlightCur.show()
        ...
        for avId in self.avIdList:
            ...
            if not playerExited:
                ...
                self.totalTags[x]['text'] = f"{totScore}"
    ...
    def exitCourse(self):
        course = self.golfCourse  # dance! around deallocation
        self.delete()
        course.exitEarly()
