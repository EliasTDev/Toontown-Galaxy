...
class GSHoodDataAI(HoodDataAI.HoodDataAI):
    ...
    def startup(self):
        ...
        # sBlockAI.get( racePadId )  )

        #    for viewPadId in viewPadAI:
        # print "viewPad %s has starting blocks %s\n" %( viewPadId,
        # vBlockAI.get( viewPadId )  )

        #    pdb.set_trace()

    def shutdown(self):
        self.notify.debug(f"shutting down GSHoodDataAI: {self.zoneId}")
        messenger.send("GSHoodDestroyed", [self])
        HoodDataAI.HoodDataAI.shutdown(self)

    def cleanup(self):
        # put away those billboards
        self.notify.debug(f"cleaning up GSHoodDataAI: {self.zoneId}")
