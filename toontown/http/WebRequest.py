...
class WebRequestDispatcher(object):
    ...
    def listenOnPort(self, listenPort):
        ...
        if self.listenPort is None:
            ...
            self.notify.warning(
                "Already listening on port %d.  Ignoring request to listen on port %d." %
                (self.listenPort, listenPort))

    def invalidURI(self, replyTo, **kw):
        resp = "<html><body>Error 404</body></html>\r\n"
        replyTo.respondHTTP("404 Not Found", resp)
        self.notify.warning(f"{replyTo.getSourceAddress()} - {replyTo.getURI()} - 404")
    ...
    def handleGET(self, req):
        ...
        callable, returnsResponse, autoSkin = self.uriToHandler.get(
            uri, [self.invalidURI, False, False])

        if callable != self.invalidURI:
            self.notify.info(
                f"{req.getSourceAddress()} - {uri} - {args} - 200")
        ...
        if returnsResponse:
            ...
            if autoSkin:
                ...
                self._bodyTag = ET.Element('body')
                req.respond(
                    self.landingPage.skin(
                        result,
                        uri,
                        headTag=self._headTag,
                        bodyTag=self._bodyTag))
                del self._bodyTag
                del self._headTag
            else:
                req.respond(result)
    ...
    def poll(self):
        ...
        while request is not None:
            ...
            if wreq.getRequestType() == "GET":
                ...
                self.notify.warning(
                    "Ignoring a non-GET request from %s: %s" %
                    (request.GetSourceAddress(), request.GetRawRequest()))
    ...
    def registerGETHandler(
        ...
        if self.uriToHandler.get(uri, None) is None:
            self.notify.info(
                f"Registered handler {handler} for URI {uri}.")
            self.uriToHandler[uri] = [handler, returnsResponse, autoSkin]
        else:
            self.notify.warning(
                "Attempting to register a duplicate handler for URI %s.  Ignoring." %
                uri)
        ...
        if enable:
            if "landingPage" not in self.__dict__:
                ...
                self.registerGETHandler(
                    "/", self._main, returnsResponse=True, autoSkin=True)
                self.registerGETHandler(
                    "/services",
                    self._services,
