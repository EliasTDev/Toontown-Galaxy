...
class QuickLauncher(LauncherBase):
    ...
    launcherFileDbFilename = f'patcher.ver?{random.randint(1, 1000000000)}'
    CompressionExt = 'bz2'
    PatchExt = 'pch'
    ...
    def __init__(self):
        ...
        print(f"useTTSpecificLogin={self.useTTSpecificLogin}")
        self.contentDir = '/'
    ...
    def addDownloadVersion(self, serverFilePath):
        ...
        if origPath[-1] == '/':
            url.setPath(f'{origPath}{serverFilePath}')
        else:
            url.setPath(f'{origPath}/{serverFilePath}')
        return url
    ...
    def downloadLauncherFileDbDone(self):
        ...
        if sys.platform == 'win32':
            ...
            self.notify.warning(f'Unknown sys.platform: {sys.platform}')
            fileList = settings['REQUIRED_INSTALL_FILES']
        ...
        for fileDesc in fileList.split():
            ...
            # Really, we should store this flag and use it to unpack after
            # decompression
        ...
        self.notify.info(
            f'requiredInstallFiles: {self.requiredInstallFiles}')

        self.mfDetails = {}
        for mfName in self.requiredInstallFiles:
            currentVer = settings[f'FILE_{mfName}.current']
            details = settings[f'FILE_{mfName}.{currentVer}']
            size, hash = details.split()
            self.mfDetails[mfName] = (currentVer, int(size), hash)
            self.notify.info(
                f'mfDetails[{mfName}] = {self.mfDetails[mfName]}')
    ...
    def resumeMultifileDownload(self):
        ...
        localFilename = Filename(
            self.topDir, Filename(
                f'_{self.currentMfname}.{curVer}.{self.CompressionExt}'))
    ...
    def resumeInstall(self):
        for self.currentPhaseIndex in range(len(self.LauncherPhases)):
            ...
            self.currentMfname = f'phase_{self.currentPhase}.mf'
            if sys.platform == 'darwin' and (
                    self.currentMfname == 'phase_1.mf' or self.currentMfname == 'phase_2.mf'):
                self.currentMfname = f'phase_{self.currentPhase}OSX.mf'
            ...
            self.curPhaseFile = Filename(
                self.topDir, Filename(
                    self.currentMfname))
            self.notify.info(f'working on: {self.curPhaseFile}')
            if self.curPhaseFile.exists():
                ...
                self.notify.info(
                    f'clientMd5: {clientMd5} expectedMd5: {expectedMd5}')
                self.notify.info(
                    f'clientSize: {fileSize} expectedSize: {expectedSize}')
                if (fileSize == expectedSize) and (
        ...
        raise Exception(
            f'Some phases not listed in LauncherPhases: {self.requiredInstallFiles}')
    ...
    def getDecompressMultifile(self, mfname):
        if not self.DecompressMultifiles:
            ...
            if 1:
                ...
                localFilename = Filename(
                    self.topDir, Filename(
                        f'_{mfname}.{curVer}.{self.CompressionExt}'))
                self.decompressMultifile(
                    mfname, localFilename, self.decompressMultifileDone)
            else:
                self.notify.info(
                    f'decompressMultifile: Multifile already decompressed: {mfname}')
                self.decompressMultifileDone()
    ...
    def decompressMultifileTask(self, task):
        ...
        if data:
            ...
            if not unlinked:
                self.notify.warning(
                    f'unlink failed on file: {task.localFilename.cStr()}')

            # Move the temp file over top the real one to finalize the
            # extraction.
            realMf = Filename(self.topDir, Filename(self.currentMfname))
            renamed = task.mfFilename.renameTo(realMf)
            if not renamed:
                self.notify.warning(
                    f'rename failed on file: {task.mfFilename.cStr()}')
    ...
    def parseWebAcctParams(self):
        ...
        self.notify.info(f"webAcctParams = {s}")
    ...
    def getRegistry(self, name, missingValue=None):
        ...
        self.notify.info(f"self.VISTA = {self.VISTA}")
    ...
    def getDownloadPath(self, origPath, serverFilePath):
        return f'{origPath}/{serverFilePath}'
