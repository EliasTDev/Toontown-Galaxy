...
class ToonSignTextCtrl(wx.TextCtrl):
    def __init__(
        ...
        wx.TextCtrl.__init__(
            ...
            pos=pos,
            value=value,
            validator=ToonSignTextCtrlValidator())
...
class SignEditFrame(wx.MiniFrame):
    def __init__(self, parent, editor, baselineDNA, objNP, hasGraphics=False):
        wx.MiniFrame.__init__(self, parent, -1, 'Sign Text', size=(400, 500),
        ...
        self.signTxtStatic = wx.StaticText(
            self.panel, -1, "Caption", pos=(15, 15))
        self.signTxt = wx.TextCtrl(
            self.panel, -1, "", pos=(60, 15), size=(270, 20))

        fontChoices = self.editor.styleManager.getCatalogCodes('font')
        self.fontStatic = wx.StaticText(self.panel, -1, "Font", pos=(15, 45))
        self.fontChoice = wx.ComboBox(
            self.panel, -1, "", pos=(
                60, 45), size=(
                100, 20), choices=fontChoices, style=wx.CB_READONLY)

        self.CapFirstLetterCheck = wx.CheckBox(
            self.panel, -1, "Capitalize First Letter", pos=(215, 45))
        self.AllCapsCheck = wx.CheckBox(
            self.panel, -1, "Make All Caps", pos=(215, 65))
        self.DropShadowCheck = wx.CheckBox(
            self.panel, -1, "Drop Shadow", pos=(215, 85))

        self.topOffset = 90

        self.kernStatic = wx.StaticText(
            self.panel, -1, "Kern", pos=(15, 15 + self.topOffset))
        self.kernValue = ToonSignTextCtrl(
            self, -1, "0", pos=(60, 15 + self.topOffset))

        self.wiggleStatic = wx.StaticText(
            self.panel, -1, "Wiggle", pos=(15, 45 + self.topOffset))
        self.wiggleValue = ToonSignTextCtrl(
            self, -1, "0", pos=(60, 45 + self.topOffset))

        self.stumbleStatic = wx.StaticText(
            self.panel, -1, "Stumble", pos=(15, 75 + self.topOffset))
        self.stumbleValue = ToonSignTextCtrl(
            self, -1, "0", pos=(60, 75 + self.topOffset))

        self.stompStatic = wx.StaticText(
            self.panel, -1, "Stopm", pos=(15, 105 + self.topOffset))
        self.stompValue = ToonSignTextCtrl(
            self, -1, "0", pos=(60, 105 + self.topOffset))

        self.curveStatic = wx.StaticText(
            self.panel, -1, "Curve", pos=(15, 135 + self.topOffset))
        self.curveValue = ToonSignTextCtrl(
            self, -1, "0", pos=(60, 135 + self.topOffset))

        self.xStatic = wx.StaticText(
            self.panel, -1, "X", pos=(15, 165 + self.topOffset))
        self.xValue = ToonSignTextCtrl(
            self, -1, "0", pos=(60, 165 + self.topOffset))

        self.zStatic = wx.StaticText(
            self.panel, -1, "Z", pos=(15, 195 + self.topOffset))
        self.zValue = ToonSignTextCtrl(
            self, -1, "0", pos=(60, 195 + self.topOffset))

        self.xScaleStatic = wx.StaticText(
            self.panel, -1, "Scale X", pos=(15, 225 + self.topOffset))
        self.xScaleValue = ToonSignTextCtrl(
            self, -1, "0", pos=(60, 225 + self.topOffset))

        self.zScaleStatic = wx.StaticText(
            self.panel, -1, "Scale Z", pos=(15, 255 + self.topOffset))
        self.zScaleValue = ToonSignTextCtrl(
            self, -1, "0", pos=(60, 255 + self.topOffset))

        self.rollStatic = wx.StaticText(
            self.panel, -1, "Roll", pos=(15, 285 + self.topOffset))
        self.rollValue = ToonSignTextCtrl(
            self, -1, "0", pos=(60, 285 + self.topOffset))

        self.revertAllButton = wx.Button(
            self.panel, -1, "Revert All", pos=(60, 315 + self.topOffset), size=(100, 20))
        ...
        self.Bind(
            wx.EVT_CHECKBOX,
            self.OnCapFirstLetterCheck,
            self.CapFirstLetterCheck)
        self.Bind(wx.EVT_CHECKBOX, self.OnAllCapsCheck, self.AllCapsCheck)
        self.Bind(
            wx.EVT_CHECKBOX,
            self.OnDropShadowCheck,
            self.DropShadowCheck)
#          self.Bind(wx.EVT_SET_FOCUS, self.OnSetFocus)
