...
class ClothesGUI(StateData.StateData):
    ...
    def enter(self, toon):
        ...
        # The tailor shop doesn't add the toon's current clothes in the list,
        # so don't do this.
...
##            self.bottomRButton['text'] = TTLocalizer.ClothesShopBottoms
    ...
    def swapTop(self, offset):
        ...
        assert(self.notify.debug(f"topChoice: {self.topChoice}"))
        assert(
            self.notify.debug(
                'shirt: %d color: %d sleeve color: %d' %
                (self.toon.style.topTex,
                 self.toon.style.topTexColor,
                 self.toon.style.sleeveTexColor)))
    ...
    def swapBottom(self, offset):
        ...
        assert(
            self.notify.debug(
                f"bottoms: choice = {self.bottomChoice}, length = {length}"))
