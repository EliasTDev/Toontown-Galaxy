...
class CogThief(DirectObject):
    ...
    def __init__(self, cogIndex, suitType, game, cogSpeed):
        ...
        self.notify.debug(f'cogSpeed={self.cogSpeed}')
    ...
    def updateGoal(
        ...
        assert self.notify.debugStateCall(self)
        self.notify.debug(
            f'self.netTimeSentToStartByHit ={self.netTimeSentToStartByHit}')
        if not self.game:
        ...
        if goalType == CTGG.NoGoal:
            self.notify.debug(f'updateGoal setting position to {pos}')
        ...
        if inResponseClientStamp < self.netTimeSentToStartByHit and \
           ...
           goalType == CTGG.RunAwayGoal:
            ...
            self.doneAdjust = False
        self.signalledAtReturnPos = False
        # TODO move the suit to where we expect him to be given the time
        # difference

    def startWalkAnim(self):
        if self.suit:
            self.suit.loop('walk')
            # float(MazeData.CELL_WIDTH) / self.cellWalkDuration
            speed = self.cogSpeed
    ...
    def thinkAboutCatchingToon(self):
        ...
        if av:
            ...
            if not self.doneAdjust:
                ...
                self.notify.debug(
                    f'thinkAboutCatchingToon not doneAdjust setting pos {myPos}')
    ...
    def thinkAboutGettingBarrel(self):
        ...
        if not self.doneAdjust:
            myPos = self.lastPosFromAI
            self.notify.debug(
                f'thinkAboutGettingBarrel not doneAdjust setting position to {myPos}')
            self.suit.setPos(myPos)
            """
    ...
    def commonMove(self):
        ...
        # This isn't strictly necessary, but does lead to more realistic
        # looking flight.

        # Step 11:  Constrain our speed.
        # If we're moving faster than we're allowed to move, constrain our
        # velocity.
    ...
    def sameGoal(self, cogIndex):
        ...
        cogThief = self.game.getCogThief(cogIndex)
        result = (
            cogThief.goalId == self.goalId) and (
            cogThief.goal == self.goal)
        return result
    ...
    def keepDistance(self):
        ...
        if self.distToNearestFlockmate < self.SeparationDistance:
            ...
            # in the UNLIKELY event we're exactly the right distance away, do
            # nothing
            change = Vec3(0, 0, 0)
