...
class DistributedCogThiefGame(DistributedMinigame):
    ...
    # ControlKeyLimitTime = 0.0 # how many seconds before player can press
    # control again
    ...
    def onstage(self):
        ...
        # disable the local toon's 'input drive' so that the player can't run
        # yet
        self.stopGameWalk()
    ...
    def showToonHitBySuit(self, avId, timestamp):
        ...
        slipBack = Parallel(
            Sequence(
                ...
                # we can control how long he stays down
        ...
        self.notify.debug(f'hitTrack duration = {hitTrack.getDuration()}')
            ...
            if cog.barrel == CTGG.NoBarrelCarried and \
               barrelIndex not in self.stolenBarrels:
                ...
                if cog.suit:
                    ...
                    self.sendUpdate(
                        "cogHitBarrel", [
                            timestamp, cogIndex, barrelIndex, cogPos[0], cogPos[1], cogPos[2]])
    ...
    def makeCogCarryBarrel(
        ...
        self.notify.debug(f'barrelPos= {barrel.getPos()}')
        cog = self.cogInfo[cogIndex]['suit']
        ...
        if base.config.GetBool('cog-thief-check-barrels', 1):
            if not base.config.GetBool('cog-thief-endless', 0):
                if len(self.stolenBarrels) == len(self.barrels):
                    ...
                    self.notify.debug(f'clockStopTime = {gameTime}')
