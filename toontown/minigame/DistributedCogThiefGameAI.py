...
class DistributedCogThiefGameAI(DistributedMinigameAI.DistributedMinigameAI):
    ...
    def initCogInfo(self):
        ...
        for cogIndex in range(self.getNumCogs()):
            self.cogInfo[cogIndex] = {
                ...
                'goal': CTGG.NoGoal,
                # avatar id if he's chasing a toon, barrel index otherwise
    ...
    def chaseToon(self, suitNum, avId):
        ...
        timestamp = globalClockDelta.localToNetworkTime(
            globalClock.getFrameTime(), bits=32)
        self.notify.debug(
            f'chaseToon time={timestamp} suitNum={suitNum}, avId={avId}')
        gameTime = self.getCurrentGameTime()
        self.sendUpdate(
            'updateSuitGoal', [
    ...
    def hitBySuit(self, avId, timestamp, suitNum, x, y, z):
        ...
        if suitNum >= self.getNumCogs():
            self.notify.warning(f'hitBySuit, possible hacker avId={avId}')
            return
    ...
    def cogHitBarrel(self, clientStamp, cogIndex, barrelIndex, x, y, z):
        ...
        if cogIndex >= self.getNumCogs():
            self.notify.warning(
                f'cogHitBarrel, possible hacker cogIndex={cogIndex}')
            return
        if barrelIndex >= CTGG.NumBarrels:
            self.notify.warning(
                f'cogHitBarrel, possible hacker barrelIndex={barrelIndex}')
            return
    ...
    def chooseReturnPos(self, cogIndex, cogPos):
        ...
        self.notify.debug(f'chooseReturnpos returning {shortestReturnIndex}')
    ...
    def cogAtReturnPos(self, clientstamp, cogIndex, barrelIndex):
        ...
        if cogIndex not in self.cogInfo or \
           barrelIndex not in self.barrelInfo:
    ...
    def checkForGameOver(self):
        ...
        for key in self.barrelInfo:
            if self.barrelInfo[key]['stolen']:
                numStolen += 1
        self.notify.debug(f'numStolen = {numStolen}')
