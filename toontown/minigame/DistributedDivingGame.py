...
class DistributedDivingGame(DistributedMinigame):
    ...
    def load(self):
        ...
        self.addSound(
            'dropGold',
            "diving_treasure_drop_off.ogg",
            "phase_4/audio/sfx/")
        self.addSound(
            'getGold',
            "diving_treasure_pick_up.ogg",
            "phase_4/audio/sfx/")
    ...
    def addSound(self, name, soundName, path=None):
        ...
        if path:
            self.soundPath = path
        soundSource = f"{self.soundPath}{soundName}"
        # print(soundSource)
    ...
    def fishSpawn(self, timestamp, fishcode, spawnerId, offset):
        ...
        if fish.name == 'clown':
            ...
            # NOTE: the nurse shark would be very cool to have a behavior where
            # it's Lerp causes it to cross the toon with the treasure
            fish.moveLerp = Sequence(
                LerpPosInterval(
                    fish,
                    duration=20 *
    ...
    # movie goes from where the toons are near the boat, pans to the bottom,
    # then pans up to the toons
    ...
    def setGameReady(self):
        ...
        for i in range(self.NUMCRABS):
            self.crabs.append(Actor.Actor(loadBase + 'kingCrab-zero.bam', {
                              'anim': loadBase + 'kingCrab-swimLOOP.bam'}).setBlend(base.wantSmoothAnimations))
        ...
        # collision setup
        # the first traverser is for the handler event, the second is for the
        # pusher
        ...
        spawnY = 0.6
        # create the DivingFishSpawns, they are placed around the sides of the
        # map
        ...
        cSphereNode = CollisionNode(f'{self.localAvId}')
        cSphereNode.addSolid(cSphere)
        ...
        for avId in self.remoteAvIdList:
            ...
            if toon:
                ...
                cSphere = CollisionSphere(0.0, 0.0, 0.0, 1.4)
                cSphereNode = CollisionNode(f'{avId}')
                cSphereNode.addSolid(cSphere)
    ...
    def enterSwim(self):
        ...
        for avId in self.remoteAvIdList:
            ...
            # So instead let's try this get function which will tell us without
            # crashing
    ...
    def incrementScore(self, avId, newSpot, timestamp):
        ...
        if avId == self.localAvId:
            self.reachedFlag = 0
        # we do a quick check to make sure the toon really does have a treasure
        # (and all the assets are still here)
            ...
            for i in range(self.NUMTREASURES):
                ...
                if self.treasures[i].grabbedId == avId:
                    ...
                    shrinkIcon = LerpScaleInterval(
                        self.chestIcons[i], duration=1.0, startScale=self.chestIcons[i].getScale(), scale=Vec3(
                            .001, .001, .001), blendType='easeIn')
                    ...
                    # respawn the treasure in a new spot at the bottom, along
                    # with updating minimap
                    ...
                    place = Parallel(Func(self.treasures[i].treasureNode.setPos, Vec3(
                        newSpot, .25, -36)), Func(self.treasures[i].treasureNode.setHpr, Vec3(0, 0, 0)))
    ...
    def performFishCollision(self, avId, spawnId, spawnerId, timestamp):
        ...
        if spawnId in self.spawners[spawnerId].fishArray:
            ...
            fishSoundPath = f"phase_4/audio/sfx/{fishSoundName}"
            ...
            # bears and nurses don't care about you and will go about their
            # merry way
            ...
            if fish.name == 'clown' or fish.name == 'piano':
                ...
                fish.moveLerp = Sequence(
                    LerpHprInterval(
                        fish, duration=.4, startHpr=fish.getHpr(), hpr=endHpr), LerpFunc(
                ...
                # setH here to make sure the fish won't intersect the
                # environment
                fish.setH(180)
    ...
    def setTreasureGrabbed(self, avId, chestId):
        if not self.hasLocalToon:
            return
        # we seem to be getting a number of client crashes here, let's use
        # get()
    ...
    def __crabTask(self, task):
        ...
        # if crab is not moving, tell the AI to get them moving with the new
        # calc
        for crab in self.crabs:
            if not (crab.moveLerp.isPlaying()):
                # the looped Wait makes sure we don't send an update again next
                # frame
