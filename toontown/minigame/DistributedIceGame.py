...
class DistributedIceGame(DistributedMinigame.DistributedMinigame,
    ...
    def load(self):
        ...
        # lets use the upright version of the tnt, but scale it down to match
        # barrel
...
# self.ignoreAll()
    ...
    def enterInputChoice(self):
        ...
        if self.curRound == 0:
            ...
            self.notify.debug(f'self.curRound = {self.curRound}')
        ...
        self.notify.debug(
            f'self.localForceArrow() heading = {self.localForceArrow().getH()}')
    ...
    def enterMoveTires(self):
        ...
        for key in self.tireDict:
            ...
            # make sure it's not spinning when it starts
            body.setAngularVel(0, 0, 0)
            # make sure it's not moving  when it starts
    ...
    def enterScoring(self):
        ...
        self.notify.debug(f'newScores = {self.newScores}')
        circleStartTime = 0
        for index in range(len(sortedByDistance)):
            ...
            # scaleXY /= 2.0 # the real scoring circle has a radius of 1, not
            # 0.5
            self.notify.debug(f'circleStartTime = {circleStartTime}')
    ...
    def updateForceArrow(self, avId, curHeading, curForce):
        ...
        self.notify.debug(f'curHeading = {curHeading}')
    ...
    def setNewState(self, state):
        ...
        if not self.hasLocalToon:
            return
        self.notify.debug(f'setNewState gameFSM={self.gameFSM} newState={state}')
    ...
    def putAllTiresInStartingPositions(self):
        ...
        for index in range(len(self.avIdList)):
            ...
            self.notify.debug(f'avId={avId} newPos={np.getPos}')
            np.setHpr(0, 0, 0)
            quat = np.getQuat()
    ...
    def setPenaltyGrabbed(self, avId, penaltyNum):
        ...
        if not self.hasLocalToon:
            return
        self.notify.debug(f"penalty {penaltyNum} grabbed by {avId}")
