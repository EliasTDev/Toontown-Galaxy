...
class DistributedIceGameAI(DistributedMinigameAI.DistributedMinigameAI):
    ...
    def __init__(self, air, minigameId):
        try:
            ...
            # for each avatar, keep track of force and which direction he wants
            # to go
            self.avatarChoices = {}
            ...
            self.finalEndingPositions = [
                Point3(
                    IceGameGlobals.StartingPositions[0]), Point3(
                    IceGameGlobals.StartingPositions[1]), Point3(
                    IceGameGlobals.StartingPositions[2]), Point3(
                    IceGameGlobals.StartingPositions[3]), ]
...
# pass
    ...
    def enterProcessChoices(self):
        ...
        self.notify.debug(f'tireInputs = {forceAndHeading}')
    ...
    def endingPositions(self, positions):
        ...
        if self.gameFSM.getCurrentState().getName() != 'waitEndingPositions':
            return
        self.notify.debug(f'got endingPositions from client {positions}')
    ...
    def reportScoringMovieDone(self):
        ...
        # or been caught
        self.notify.debug(f'reportScoringMovieDone: avatar {avId} is done')
