...
class DistributedTargetGame(DistributedMinigame):
        ...
        for typeIndex in range(len(self.targetList)):
            for targetIndex in range(self.targetList[typeIndex]):
                ...
                while not goodPlacement:
                    ...
                    placeY = ((random.random() * 0.60) + (0.00 + (0.40 *
                ...
                while subIndex:
                    ...
                    targetNodePathGeom.setBin(
                        'ground', (combinedIndex * 2) + 1)
    ...
    def onstage(self):
        ...
        base.camera.setPosHpr(
            toonPos[0], toonPos[1] - 18, toonPos[2] + 10, 0, -15, 0)
    ...
    def reset(self):
        ...
        toonPos = base.localAvatar.getPos()
        base.camera.setPosHpr(
            toonPos[0], toonPos[1] - 26, toonPos[2] + 10, 0, -15, 0)
    ...
    def setGameReady(self):
        ...
        if not self.isSinglePlayer():
            ...
            cSphereNode = CollisionNode(f'RingGameSphere-{self.localAvId}')
            ...
            for avId in self.remoteAvIdList:
                ...
                if toon:
                    ...
                    cSphereNode = CollisionNode(f'RingGameSphere-{avId}')
                    cSphereNode.addSolid(cSphere)
    ...
    def doCountDown(self, task=None):
        if self.countDown < 0:
            ...
            self.powerBar['text'] = (
                TTLocalizer.TargetGameCountdown %
                (self.countDown))
            self.countDown -= 1
    ...
    def __updatePowerBarTask(self, task):
        ...
        if self.lastPressTime:
            ...
            if self.ticker >= 0.1:
                self.ticker = 0.0
                powerDiv = 0.05
                self.power -= (1.0 + (0.20 * ((self.power *
                               powerDiv) * (self.power * powerDiv))))
    ...
    def __updateLocalToonTask(self, task):
        ...
        if stateName == 'fall':
            ...
            self.laterial = self.laterial - \
                (self.laterial * self.umbrellaResistance * dt)
            if toonPos[2] > 350.0:
            ...
            self.speedForward = self.speedForward - \
                (self.speedForward * self.airResistance * dt)
            self.laterial = self.laterial - \
                (self.laterial * self.airResistance * dt)
        t = globalClock.getFrameTime() - self.__timeBase
        self.distance = self.distance + (dt * self.speedForward)
        ...
        onScreenDebug.add(f"{self.getDisplayPrefix()} Vel", self.zVel)
        onScreenDebug.add(f"{self.getDisplayPrefix()} Lift", lift)
        onScreenDebug.add(
            f"{self.getDisplayPrefix()} Gravity",
            self.gravity)
        onScreenDebug.add(f"{self.getDisplayPrefix()} Pos", pos[2])
        onScreenDebug.add(f"{self.getDisplayPrefix()} Drifty", self.driftY)
        ...
        # self.zVel -= (0.1 * (self.speedStall - self.speedForward)) + 1.0
        # #Gravity
        ...
        fogRange = (minTrans * self.FOGDISTGROUND) + \
            (fogVis * self.FOGDISTCLOUD)
    ...
    def setSingleScore(self, score, avId):
        ...
        for existIndex in range(len(self.avIdList)):
            if self.avIdList[existIndex] == avId:
                self.scoreLabels[existIndex]['text'] = f"{score}"
