...
class DistributedTravelGameAI(DistributedMinigameAI):
    ...
    def __init__(self, air, minigameId):
        try:
            ...
            # for each avatar, keep track how many votes, and which direction
            # he wants to go
            self.avatarChoices = {}
    ...
    def enterProcessChoices(self):
        ...
        """
        # all this hoopla so we can support a switch going to 3 or more
        # directions
        self.directionVotes = []
        ...
        self.notify.debug(f'self.directionVotes = {self.directionVotes}')
        ...
        if len(self.winningDirections) > 1:
            self.notify.debug(
                f'multiple winningDirections={self.winningDirections}')
    ...
    def setAvatarChoice(self, votes, direction):
        ...
        self.notify.debug(f'currentVotes = {self.currentVotes}')
        self.notify.debug(f'avatarChoices = {self.avatarChoices}')
    ...
    def handleExitedAvatar(self, avId):
        ...
        for avId in self.avIdList:
            if avId in list(
                    self.stateDict.keys()) and self.stateDict[avId] != EXITED:
                allExited = False
                break
