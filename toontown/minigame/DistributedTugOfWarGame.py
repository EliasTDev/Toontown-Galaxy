...
class DistributedTugOfWarGame(DistributedMinigame):
    ...
    def enterTug(self):
        ...
        # start the timeout in case we don't get sent the stop signal from the
        # AI
    ...
    def sendGoSignal(self, index):
        ...
        def enableKeys(self=self):
            ...
            # Change the order of the press handlers because we are only using
            # 2 keys
    ...
    def sendStopSignal(self, winners, losers, tieers):
        ...
        if self.suit:
            if self.suitId in winners:
                ...
                # oops, the suit fell in while celebrating(only happens
                # sometimes)
                    ...
                    oopsTrack = Sequence(
                        Parallel(
                            Func(
                                ...
                                self.suit, .5, destHpr, startHpr=startHpr)), Parallel(
                            Func(
            ...
            if avId not in self.fallenList:
                ...
                if toon:
                    ...
                    if (curPos[0] < 0 and curPos[0] > -
                        ...
                        # report end of contest by telling server which side
                        # lost
                        self.sendUpdate("reportEndOfContest", [losingSide])
    ...
    def computeForce(self, keyRate):
        ...
        # otherwise, make force proportional to how close you are to key rate
            ...
            F = self.advantage * (self.rateMatchAward + 4 + .4 * self.idealRate) * math.pow(
