...
##            self.accept('y', self.localToonVictory)
    ...
    def load(self):
        ...
        # Get the start and end X values of the game. Required for the progress
        # line.
        self.gameStartX = -40.
        endSection = self.sectionMgr.sections[-1]
        # This is just to confirm that the last section is actually the
        # endSection
...
##        lt.setBin('fixed', 10)
    ...
    def setGameReady(self):
        ...
        if DistributedMinigame.setGameReady(self):
            return
        # all of the remote toons have joined the game; it's safe to show them
        # now.
...
##                toon.setBin('fixed', drawNum)
                ...
                toonSD.enter()
                toonSD.fsm.request('normal')
                # The toon with the higher drawNum is rendered closer to the
                # camera
    ...
    def setGameStart(self, timestamp):
        ...
        # all players have finished reading the rules, and are ready to start
        # playing.
...
##        self.twoDDrive = TwoDDrive(self, self.TOON_SPEED, maxFrameMove=self.MAX_FRAME_MOVE)
    ...
    def exitPlay(self):
        ...
        # Stop listening to 'jumpStart' because we don't need to play jump
        # sound
    ...
    def __updateLocalToonTask(self, task):
        ...
        if self.gameFSM.getCurrentState().getName() == 'play':
            # Only exception is when the local toon has reached the victory
            # state.
    ...
    def setupHeadCollision(self):
        ...
        # a jump onto a 2nd platform without banging it's head on the 3rd
        # platform.
...
# self.notify.debug('startJump')
    ...
    def checkValidity(self, avId):
        ...
        if self.gameFSM.getCurrentState().getName() != 'play':
            self.notify.warning(
                f'ignoring msg: av {avId} performing some action.')
            return False
    ...
    def showShootGun(self, avId, timestamp):
        ...
        if self.checkValidity(avId):
            self.notify.debug(f"avatar {avId} is shooting water gun")
    ...
    def toonFellDown(self, avId, timestamp):
        ...
        if self.checkValidity(avId):
            self.notify.debug(f"avatar {avId} fell down.")
            if avId != self.localAvId:
    ...
    def toonSquished(self, avId, timestamp):
        ...
        if self.checkValidity(avId):
            self.notify.debug(f"avatar {avId} is squished.")
            if (avId != self.localAvId):
                # Update remote toon's score
                self.updateScore(
    ...
    def localToonVictory(self):
        ...
        if not ToonBlitzGlobals.EndlessGame:
            self.ignoreInputs()
        if not (
                self.toonSDs[self.localAvId].fsm.getCurrentState().getName() == 'victory'):
    ...
    def toonVictory(self, avId, timestamp):
        ...
        if self.checkValidity(avId):
            self.notify.debug(f"avatar {avId} achieves victory")
            if avId != self.localAvId:
    ...
    def __treasureGrabbed(self, sectionIndex, treasureIndex):
        ...
        """
        self.notify.debug(
            f'treasure {sectionIndex}-{treasureIndex} grabbed')
    ...
    def setTreasureGrabbed(self, avId, sectionIndex, treasureIndex):
        ...
        if (self.gameFSM.getCurrentState().getName() == 'play'):
            self.notify.debug(
                f"treasure {sectionIndex}-{treasureIndex} grabbed by {avId}")
...
##        self.notify.debug('enemy %s-%s shot' %(sectionIndex, enemyIndex))
    ...
    def setEnemyShot(self, avId, sectionIndex, enemyIndex, enemyHealth):
        ...
        if (self.gameFSM.getCurrentState().getName() == 'play'):
            self.notify.debug(
                f'enemy {enemyIndex} is shot by {avId}. Health left {enemyHealth}')
            if (enemyHealth > 0):
                # Enemy is still alive.
                # Don't play the shotTrack for the local client, because we
                # already played it.
