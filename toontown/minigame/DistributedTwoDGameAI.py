...
##            simbase.mgAI = self
    ...
    def setTrolleyZone(self, trolleyZone):
        # We need the trolley zone before we can setup the sections, so do it
        # here
    ...
    def setGameReady(self):
        ...
        for avId in list(self.scoreDict.keys()):
            ...
            self.treasuresCollectedDict[avId] = [
                0, 0, 0, 0]  # [value1, value2, value3, value4]

        # Maintaining a table for enemy health and another table for treasure
        # taken
...
# self.gameOver()
    ...
    def claimTreasure(self, sectionIndex, treasureIndex):
        ...
        self.notify.debug(
            f'treasure {sectionIndex}-{treasureIndex} claimed by {avId}')
        ...
        if (treasureIndex < len(initialTreasureList)):
            # treasureValue can be found from what the AI had initially
            # determined it.
    ...
    def claimEnemyShot(self, sectionIndex, enemyIndex):
        avId = self.air.getAvatarIdFromSender()
        self.notify.debug(
            f'enemy {sectionIndex}-{enemyIndex} shot claimed by {avId}')
...
##                self.treasureTakenTable[sectionIndex] += [0]
            self.sendUpdate(
                'setEnemyShot', [
                    avId, sectionIndex, enemyIndex, self.enemyHealthTable[sectionIndex][enemyIndex]])
    ...
    def toonVictory(self, avId, timestamp):
        ...
        if avId not in list(self.scoreDict.keys()):
            self.notify.warning(f'Avatar {avId} not in list.')
            self.air.writeServerEvent(
        ...
        self.notify.debug(f'curTime ={curTime} timeLeft = {timeLeft}')
    ...
    def setupSections(self):
        ...
        # Sort the difficultyList so that the more difficult sections appear at
        # the end of the game
        difficultyList.sort()
        ...
        # Do not repeat. If we are out of sections with that difficulty take
        # one from the next difficulty level
        for targetDifficulty in difficultyList:
            whileCount = 0
            difficulty = targetDifficulty
            # If there is no section left to pick from that difficulty level
            # pick one from the next difficulty level
            while not (len(sectionsPoolByDifficulty[difficulty]) > 0):
                ...
                # Removing this sectionIndex from the list of
                # sectionsPoolByDifficulty
        ...
        # Fill up sectionIndicesSelected from sectionsSelectedByDifficulty to
        # maintain 1 comprehensive list
        ...
        for i in range(len(sectionIndicesSelected)):
            ...
            # Select a random list of numEnemies enemyIndices from the
            # enemyIndicesPool
            enemyIndicesPool = []
            ...
            if (enemiesPool is not None):
                ...
                # Sort the indices in enemyIndicesSelected so that they appear
                # in the right order of location in a section
                enemyIndicesSelected.sort()

            # Select a random list of numTreasures treasureIndices from the
            # treasureIndicesPool
            treasureIndicesPool = []
            # 1 value treasures have a 40% chance of getting picked and the 4
            # value treasures have only a 10% chance of getting picked.
            treasureValuePool = []
            for value in range(1, 5):
                treasureValuePool += [value] * \
            ...
            if (treasuresPool is not None):
                ...
                # Sort the indices in treasureIndicesSelected so that they
                # appear in the right order of location in a section
                treasureIndicesSelected.sort()

            # Select a random list of numSpawnPoints spawnPointIndices from the
            ...
            if (spawnPointsPool is not None):
                ...
                # Sort the spawnPoints in a section so that they appear in the
                # right order in the section
                spawnPointIndicesSelected.sort()

            # Select a random list of numStompers stomperIndices from the
            # stomperIndicesPool
            stomperIndicesPool = []
            ...
            if (stompersPool is not None):
                ...
                # Sort the indices in stomperIndicesSelected so that they
                # appear in the right order of location in a section
            ...
            # The clients can take the length and the blockList from
            # ToonBlitzGlobals.SectionTypes
            sctionTuple = (
                sectionIndex,
                enemyIndicesSelected,
                treasureIndicesSelected,
        ...
        # self.sectionsSelected is the finalised list of sections along with
        # enemyIndices, treasureIndices, spawnPointIndices of each section.
