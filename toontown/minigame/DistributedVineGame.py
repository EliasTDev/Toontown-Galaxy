...
class DistributedVineGame(DistributedMinigame):
    ...
    # name of the update task called each frame
    ...
    def onstage(self):
        ...
        gravityForce = LinearVectorForce(
            0, 0, -self.Gravity)  # gravity acceleration
    ...
    def updateToonInfo(
        ...
        if newVelZ is not None and ((newVelZ < -1000) or (newVelZ > 1000)):
            self.notify.warning(
                ...
                (newVelZ, avId))
            newVelZ = 0
        if (newFallingInfo < self.FallingNot) or (
                newFallingInfo > self.FallingBat):
            self.notify.warning(
    ...
    def setGameReady(self):
        ...
        for index in range(self.numPlayers):
            ...
            if toon:
                ...
                # make sure they show up in the radar, if we don't use head
                # frames
                toon.useLOD(1000)
    ...
    def enterPlay(self):
        ...
        for i in range(self.numPlayers):
            ...
            scorePanel.setPos(.75 - spacing *
                              ((self.numPlayers - 1) - i), 0.0, .85)
    ...
    def handleLocalToonFellDown(self):
        ...
        if self.toonInfo[self.localAvId][8] == self.FallingSpider:
            # if the vine we will connect to has a spider, move it 1 to the
            # left
    ...
    def setupCollisions(self, anp):
        ...
        # fromObject.show()
        self.notify.debug(f'fromCollideMask = {fromCollideMask}')
        ...
        eventName = f'{fromObject.getName()}-into'
        self.accept(eventName, self.swingVineEnter)
        ...
        eventName = f'{self.treasureCollNodePath.getName()}-intoTreasure'
        self.notify.debug(f'eventName = {eventName}')
    ...
    def getFocusCameraPos(self, vineIndex, facingRight):
        ...
        self.notify.debug(f'getFocusCameraPos returning {retPos}')
        return retPos
    ...
    def attachLocalToonToVine(
            ...
            playSfx=False):
        self.notify.debug(
            f'focusCameraImmediately = {focusCameraImmediately}')
        if playSfx:
            ...
            if swingSeq:
                ...
                for i in range(len(swingSeq)):
                    ...
                    if not actorIval.isStopped():
                        ...
                        self.notify.debug(
                            f'actor ival is not stopped, testFrame={testFrame:f}')
                        if testFrame:
                            curFrame = testFrame
                            break
                if curFrame > -1:
                    ...
                    if curFrame > self.ToonMaxRightFrame:
                        ...
                        # stop the toon from swinging all the way to the right
                        # then a bit left
    ...
    def detachLocalToonFromVine(self, vineIndex, facingRight):
        ...
        # we must do this before we detach the toon
    ...
    def treasureEnter(self, entry):
        ...
        self.notify.debug(f'{entry}')
        name = entry.getIntoNodePath().getName()
        parts = name.split('-')
        if len(parts) < 3:
            self.notify.debug(f'collided with {name}, but returning')
            return
        if not int(parts[1]) == self.doId:
            self.notify.debug(
                f"collided with {name}, but doId doesn't match")
            return
    ...
    def makeOtherToonJump(self, avId, posX, posZ, velX, velZ):
        ...
        if avId not in self.otherToonPhysics:
            an = ActorNode(f'other-physics{avId}')
    ...
    def makeLocalToonJump(self, vineIndex, t, pos, normal):
        ...
        self.notify.debug(f'jumping from vine with velocity of {velocity}')
    ...
    def setTreasureGrabbed(self, avId, treasureNum):
        ...
        if not self.hasLocalToon:
            return
        self.notify.debug(f"treasure {treasureNum} grabbed by {avId}")
    ...
    def setupChangeFacingInterval(self, vineIndex, newFacingRight):
        ...
        if not (
            (vineIndex == 0) or (
                vineIndex == VineGameGlobals.NumVines -
                1)):
    ...
    def localToonHitSpider(self, colEntry):
        ...
        if self.toonInfo[self.localAvId][0] == -1:
            ...
            fallingInfo = self.toonInfo[self.localAvId][8]
            if not (fallingInfo == self.FallingBat or fallingInfo ==
                    self.FallingSpider):
    ...
    def getIntroTrack(self):
        ...
        for index in range(len(self.avIdList)):
            ...
            if av:
                ...
                endPos.setZ(endPos.getZ() -
                            toonOffset.getZ() -
                            (VineGameGlobals.VineStartingT *
                             self.vines[0].cableLength))
                # - (index * 2) # uncomment if we are showing multiple toons
                ...
                self.notify.debug(
                    f'leftPos = {leftPos} platformPos={platformPos}')
                ...
                oneSeq.append(Parallel(
                    ...
                    # SoundInterval(exclamationSfx, duration = 3), # exclaim
                    # sound
                ))
    ...
    def doEndingTrackTask(self, avId):
        ...
        taskName = f'VineGameEnding-{avId}'
        if avId not in self.endingTracks:
    ...
    def debugCameraPos(self):
        ...
        self.notify.debug(f'cameraPos = {base.camera.getPos()}')
    ...
    def setupEndingTrack(self, avId):
        ...
        self.notify.debug(
            f'avPos={avPos} cameraPos={base.camera.getPos()}')
        if av:
