...
class MazeMapGui(DirectFrame):
    ...
    def _createMapTextureCard(self):
       ...
       #blockFiles = []
        # iterate through the map data and place a block in the map image where
        # appropriate
    ...
    def revealCell(self, x, y, playerIndex=None):
        ...
        if playerIndex is not None:
            ...
            self._players[playerIndex].setPos(
                ax / self._maskResolution * 2.0 - 0.97,
                0,
    ...
    def updateToon(self, toon, tX, tY):
        ...
        if tY < 0 or tY >= len(self._revealedCells):
            ...
            self.notify.warning(f'(tX, tY): ({tX}, {tY})')
            self.notify.warning(f'len(_revealedCells): {len(self._revealedCells)}')
            if len(self._revealedCells) > 0:
                self.notify.warning(
                    f'len(_revealedCells[0]): {len(self._revealedCells[0])}')
            return
        if tX < 0 or tX >= len(self._revealedCells[tY]):
            ...
            if tY < len(self._revealedCells):
                self.notify.warning(
                    f'len(_revealedCells[tY]): {len(self._revealedCells[tY])}')
