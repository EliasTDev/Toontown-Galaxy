...
class Purchase(PurchaseBase):
    ...
    def __init__(
        ...
        assert self.notify.debug(f'self.votesArray = {self.votesArray}')
    ...
    def load(self):
        ...
        self.bg = loader.loadModel(
            f"phase_{interiorPhase}/models/modules/toon_interior")
        self.bg.setPos(0., 5., -1.)

        self.wt = self.bg.find("**/random_tc1_TI_wallpaper")
        wallTex = loader.loadTexture(
            f"phase_{interiorPhase}/maps/wall_paper_a5.jpg")
        ...
        wainTex = loader.loadTexture(
            f"phase_{interiorPhase}/maps/wall_paper_b4.jpg")
    ...
    def enterReward(self):
        ...
        # the bean counters take up a lot of real estate: clamp the whisper
        # bubbles
        ...
        if self.metagameRound == TravelGameGlobals.FinalMetagameRoundIndex:
            ...
            countVotesUpTime = (
                self.maxVotes * COUNT_UP_RATE) + DELAY_AFTER_COUNT_UP
    ...
    def doMetagamePlayAgain(self):
        ...
        if hasattr(self, 'metagamePlayAgainResult'):
            assert self.notify.debug(
                f'doMetagamePlayAgain 1 returning {self.metagamePlayAgainResult}')
        ...
        for avId in self.ids:
            if avId in base.cr.doId2do and \
               avId not in self.unexpectedExits:
                numToons += 1
                assert self.notify.debug(
                    f'found avId={avId} numToons={numToons}')
        ...
        assert self.notify.debug(
            f'doMetagamePlayAgain 2 returning {self.metagamePlayAgainResult}')
        return self.metagamePlayAgainResult
...
class PurchaseHeadFrame(DirectFrame):
    ...
    def setAvatarState(self, state):
        ...
        if state == PURCHASE_DISCONNECTED_STATE:
            ...
            Purchase.notify.warning(f'unknown avatar state: {state}')
    ...
    def removeAvKeep(self):
        ...
        if hasattr(self, 'avKeep'):
            self.notify.debug(f'destroying avKeep {self.avKeep}')
            self.avKeep.destroy()
