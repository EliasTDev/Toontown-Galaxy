...
class SwingVine(NodePath):
    ...
    def setupCable(self):
        ...
        if self.vineIndex == VineGameGlobals.NumVines - 1:
            ...
            # self.setColorScale(1.0, 0.843, 0, 1.0) # set the end vine to a
            # diff color
    ...
    def setupTubes(self):
        ...
        for tubeIndex in range(self.numTubes):
            ...
            # let's try using a collisionPoly to stop a fast moving toon moving
            # through a vine
            quad = CollisionPolygon(Point3(0.25, radius + 1, bz),
                                    Point3(0.25, radius + 1, az),
                                    Point3(0.25, -radius - 1, az),
            ...
            colNode.addSolid(quad)
            colNode.setCollideMask(ToontownGlobals.PieBitmask)
            # we must increase the bounding volume to make sure he doesn't pass
            # through
            colNode.setBounds(BoundingSphere(Point3(0, 0, 0), 10))
            ...
            # RAU I tried parenting the tube to self, but it made the rope
            # disappear
    ...
    def getAttachNode(self, toonId):
        ...
        if toonId in self.attachedToons:
            ...
            retval = render.attachNewNode(f'vineAttachNode-{self.vineIndex}-{toonId}')
        return retval
    ...
    def calcOffset(self, toonId):
        ...
        if toon:
            ...
            if not leftHand.isEmpty():
                ...
                self.notify.debug(f'offset = {offset}')
            else:
    ...
    def doubleCheckOffset(self, toonId):
        ...
        if toonId in self.attachedToons:
            ...
            if curOffset == Point3.zero():
                ...
                self.notify.info(
                    f'correcting wrong offset {curOffset} and changing to {newOffset}')
                if av:
                    av.setPos(-newOffset)
    ...
    def detachToon(self, toonId):
        ...
        if toonId in self.attachedToons:
            ...
            if swingIval:
                self.notify.debug(f'deleting swing ival {swingIval}')
                # swingIval.pause()
                swingIval.finish()
    ...
    def calcTFromTubeHit(self, colEntry):
        ...
        if parts[0] == 'StaticVine':
            ...
            self.notify.debug(f'projection = {projection}')
        ...
        self.notify.debug(f'retval = {retval}')
        return retval
        ...
        if (self.swingingForward and facingRight) or \
           (not self.swingingForward and not facingRight):
            ...
            if 0:  # toonT < 0.5:
                ...
                numLeftFramesChoppedOff = (
                    maxLeftFrame - midFrame) * (1 - toonT)
                numRightFramesChoppedOff = (
                    midFrame - maxRightFrame) * (1 - toonT)
