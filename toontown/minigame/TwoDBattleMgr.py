...
class TwoDBattleMgr(DirectObject):
    """
    This class controls all that happens when the shoot button is pressed.
    """
...
##        self.WATER_SPRAY_COLOR = Point4(0.75, 0.75, 1.0, 0.8)
    ...
    def destroy(self):
        if (self.toon == base.localAvatar):
            if self.waterBulletIval:
                ...
                self.waterBulletIval = None
            # Stop listening for water bullet collision
    ...
    def createShootCollision(self):
        ...
        """Make a sphere, give it a unique name, attach it to the local Avatar
           and lerp it from startPos to endPos"""
        ...
        # Move the water bullet to its hide position after it finishes its
        # LerpPosInterval
        self.waterBulletIval.append(
            Func(
    ...
    def getToonShootTrack(self):
        ...
        # Playing only segments of the water-gun in an attempt to speed up the
        # whole animation
        toonTrack = Sequence(
            ActorInterval(
                self.toon,
    ...
    def calcSprayStartPos(self):
        # Note: We dont have to call toon.pose() since getSprayStartPos isnt called
        ...
        if self.toon:
            # force update of LOD 0 to current animation frame (dont know if it
    ...
    def calcSprayEndPos(self):
        ...
        # If toon does't exist assume that the toon was facing right. Cheap
        # HACK!!
        else:
            xDirection = -math.sin(-90)
        endPos = Point3(self.origin.getX() +
                        (self.SHOOT_DISTANCE *
                         xDirection), self.origin.getY(), self.origin.getZ())
        self.target = endPos
