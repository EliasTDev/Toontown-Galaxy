...
class TwoDCamera(DistributedObject.DistributedObject):
    ...
    def __init__(self, camera):
        self.notify.debug(f'Constructing TwoDCamera with {camera}')
        self.camera = camera
    ...
    def setupChangeFacingInterval(self, newHeading):
        ...
        self.changeFacingInterval = LerpFunc(
            self.myLerpPos, duration=5.0)  # , blendType = 'easeOut')
    ...
    def myLerpPos(self, t):
        ...
        if (math.fabs(self.camera.getX(render) - finalCamX) <
                0.01):  # In order to avoid floating point error
