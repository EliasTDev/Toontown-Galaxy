...
class TwoDEnemy(DirectObject):
    ...
    def __init__(self, enemyMgr, index, suitAttribs):
        ...
        # The index is a compound number string Eg: 1-2. The 1st no. is the
        # section no. and the 2nd no. is the enemy no.
        self.index = index
    ...
    def setupEnemy(self, suitAttribs):
        ...
        self.suitName = f'Enemy-{self.index}'
        ...
        if (len(suitPosAttribs) == 3):
            ...
            # Setup the interval that makes the block move back and forth
            # between initPos and finalPos
            self.clearMoveIval()
            ...
            def getForwardIval(blendTypeStr, self=self):
                forwardIval = LerpPosInterval(
                    ...
                    pos=finalPos,
                    startPos=initPos,
                    name=f'{self.suitName}-moveFront',
...
##        point = Point3(0, 0, self.suit.height / 2. + 1.)
    ...
    def doDeathTrack(self):
        ...
        self.suit.hide()
        # We don't need a collision for the suit, so we reparent it to the
        # deathSuit
        ...
        def removeParticle(particle):
            # Adding a wrapper because I think a particle is trying to get
            # cleaned up twice
