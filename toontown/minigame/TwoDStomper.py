...
# self.smoke.reparentTo(self.model)
    ...
    def getMotionIval(self):
        ...
        def motionFunc(t, self=self):
            ...
            if t < stickTime:
                ...
                # t: turnaround..1
                self.head.setFluidZ(
                    (-self.range + (t - turnaround) * self.range / (1 - turnaround)) + self.headEndZ)
                if (self.stomperState != GOING_DOWN):
                    self.stomperState = GOING_DOWN
                    # Check for squashed toon only if the stomper is going
                    # down.
    ...
    def adjustCollSolidHeight(self, t):
        ...
        # The last one is reparented to the head. Move all the collision solids
        # in the middle.
    ...
    def checkSquashedToon(self):
        # Check if the local toon is within the head size of the centre of the
        # stomper.
        ...
        # For z check - check if toon is between current head position and the
        # max head position.
        if (toonXDiff > -1.) and (toonXDiff < 1.) and (toonZ >
                                                       headEndZAbs) and (toonZ < self.head.getZ(render)):
