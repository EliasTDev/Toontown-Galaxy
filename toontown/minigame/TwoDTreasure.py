...
class TwoDTreasure(DirectObject):
    ...
    def __init__(
        ...
        self.modelScalIval = Sequence(
            LerpScaleInterval(
                self.model, 0.4, scale=1., startScale=1.03), LerpScaleInterval(
                self.model, 0.4, scale=1.03, startScale=1.))
        self.modelScalIval.loop()

        # Make a sphere, name it uniquely, and child it
        # to the nodepath.
        self.sphereName = f"treasureSphere-{self.game.doId}-{self.index}"
