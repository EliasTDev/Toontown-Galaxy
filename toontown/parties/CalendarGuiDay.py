...
class CalendarGuiDay(DirectFrame):
    ...
    def createGuiObjects(self):
        ...
        self.scrollList = DirectScrolledList(
            ...
            decButton_scale=(self.arrowButtonXScale,
                             1, -self.arrowButtonZScale),

            decButton_pos=decButtonPos,
            # Make the disabled button fade out
            decButton_image3_color=Vec4(1, 1, 1, 0.2),

            # itemFrame is a DirectFrame, shift it down a bit to match top left
            # scroll
    ...
    def destroy(self):
        ...
        self.notify.debug(f'desroying {self.myDate}')
        try:
            for item in self.scrollList['items']:
                if hasattr(
                        ...
                        'destroy'):
                    self.notify.debug(
                        f'desroying description of item {item}')
                    item.unbind(DGG.ENTER)
                    item.unbind(DGG.EXIT)
                    item.description.destroy()
        except e:
            self.notify.debug(f'pass {self.myDate}')
            ...
            for holiday in yearlyHolidays:
                ...
                if holidayStart[0] == self.myDate.month and \
                   holidayStart[1] == self.myDate.day:
                    ...
                    self.notify.error(f'holiday is not today {holiday}')
    ...
    def addOncelyMultipleStartHolidayToScrollList(self, holiday):
        ...
        # I can't think of anything different we'd do from addOncely, so just
        # call that
    ...
    def __clickedOnDay(self):
        ...
        self.notify.debug(f'we got clicked on {self.myDate.date()}')
...
class MiniInviteVisual(DirectFrame):
    ...
    def __init__(self, parent, partyInfo):
        ...
        self.background = DirectFrame(
            parent=self,
            relief=None,
            geom=self.inviteBackgrounds.find(f"**/{backgrounds[self.partyInfo.inviteTheme]}"),
            scale=(0.7, 1.0, 0.23),
            pos=(0.0, 0.0, -0.1),
        )
