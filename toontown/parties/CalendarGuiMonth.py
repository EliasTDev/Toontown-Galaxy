...
class CalendarGuiMonth(DirectFrame):
    ...
    def load(self):
        ...
        for weekday in ('sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'):
            weekDayLoc = self.find(f'**/loc_{weekday}')
        ...
        for row in range(6):
            oneWeek = []
            for col in range(7):
                newDayLoc = self.find(f'**/loc_box_{row}_{col}')
                oneWeek.append(newDayLoc)
            self.dayLocators.append(oneWeek)
    ...
    def changeMonth(self, monthChange):
        ...
        if monthChange != 0:
            # if it's March 1, make sure we dont skip February going back 31
            # days
        ...
        self.notify.debug(f'changeDate took {endTime - startTime:f} seconds')
        # if we have a selected date, it probably changed box
        self.updateSelectedDate()
        if self.onlyFutureDaysClickable and (
                newMonth == self.startDate.month and newYear == self.startDate.year):
            self.monthLeftArrow.hide()
