...
class DistributedPartyCannon(DistributedObject, Cannon):
    ...
    def setMovie(self, mode, avId):
        ...
        """
        self.notify.debug(f"{self.doId} setMovie({avId}, {mode})")
...
# ===============================================================================
    ...
    def removeAvFromCannon(self, avId):
        ...
        self.notify.debug(f"{self.doId} removeAvFromCannon")
        if (av is not None):
            self.notify.debug(
    ...
    def __localCannonMoveTask(self, task):
        ...
        if self.hasMoved():
            ...
            if task.time - task.lastPositionBroadcastTime > CANNON_MOVE_UPDATE_FREQ:
                self.notify.debug(
                    f"Broadcast local cannon {self.doId} position")
            ...
            self.__broadcastLocalCannonPosition()
            self.notify.debug(f"Cannon Rot = {self._rotation}, Angle = {self._angle}")
...
# ===============================================================================
    ...
    def ignoreDisableForAvId(self, avId):
        ...
        toon = base.cr.doId2do.get(avId)
        if toon:
            self.notify.debug(f"ignoring {toon.uniqueName('disable')}")
            self.ignore(toon.uniqueName("disable"))
        else:
            self.notify.debug(f"ignoring disable-{self.toonInsideAvId}")
            self.ignore(f"disable-{self.toonInsideAvId}")
