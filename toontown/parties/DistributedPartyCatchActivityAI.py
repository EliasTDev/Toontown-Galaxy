...
class DistributedPartyCatchActivityAI(
        ...
        for toonId, reward in list(self.toonIdsToScores.items()):
            ...
            self.sendUpdateToAvatarId(
                toonId, "showJellybeanReward", [
                    reward, self.air.doId2do[toonId].getMoney(), TTLocalizer.PartyCatchRewardMessage %
    ...
    def _setUpNextGenScheduleTask(self, startT, delay=None):
        ...
        task = taskMgr.doMethodLater(
            ...
            f'schedNextGen-{self.doId}-{nextGen}')
    ...
    def _scheduleNextGeneration(self, startT, task=None):
        ...
        tCutoff -= 60.
        genIndices = sorted(self._id2gen.keys())
        ...
        for genIndex in genIndices:
            ...
            generations.append(
                [genIndex, globalClockDelta.localToNetworkTime(timestamp, bits=32), numPlayers])
...
##             self.sendUpdateToAvatarId(senderId, "startRequestResponse", [int(False)])
        ...
        if self.activityFSM.state == "Idle":
            if senderId != self.party.partyInfo.hostId:
                self.air.writeServerEvent(
                    'suspicious', senderId, 'non-host trying to start party catch')
                return
    ...
    def toonExitDemand(self):
        ...
        self._setUpNextGenScheduleTask(
            globalClock.getRealTime() -
            self.activityStartTime)

    def sendToonJoinResponse(self, toonId, joined):
        # since toons can join mid-activity, make sure to add to scores
    ...
    def claimCatch(self, generation, objNum, DropObjTypeId):
        ...
        if not gen.caughtList[objNum]:
            ...
            DistributedPartyCatchActivityAI.notify.debug(
                f'avatar {avId} caught object {objNum}: {objName}')
        ...
        self.sendUpdate(
            'setStartTimestamp', [
                globalClockDelta.localToNetworkTime(
                    self.activityStartTime, bits=32), ])
        self.sendUpdate(
            "setState",
            [
                "Active",  # new state
