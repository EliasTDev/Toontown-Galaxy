...
# ===============================================================================
    ...
    def pieHitsCog(
            ...
            direction,
            hitHead):
        # Count it only if it's the active state. It may be a very delayed hit,
        # sadly.
        ...
        if hitHead:
            if z < self.cogHeadStartZ:
                ...
                self.notify.debug(
                    "pieHitsCog suspicious behavior for toon: %s, z: %d < %d" %
...
# ===============================================================================
    ...
    def _removeToonFromTeam(self, toonId, team):
        if DistributedPartyTeamActivityAI._removeToonFromTeam(
                self, toonId, team) and toonId in self.toonScore:
...
# ===============================================================================
    ...
    def computeMatchResults(self):
        ...
        for distance in self.cogDistances:
            if distance < 0.0:
                self.score[PartyGlobals.TeamActivityTeams.LeftTeam] += abs(
                    PartyCogUtils.getCogDistanceUnitsFromCenter(distance))
            elif distance > 0.0:
                self.score[PartyGlobals.TeamActivityTeams.RightTeam] += abs(
                    PartyCogUtils.getCogDistanceUnitsFromCenter(distance))
    ...
    def resultIsPerfect(self):
        ...
        return self.score[self.winningTeam] >= 3.0 * \
            PartyGlobals.CogActivityArenaLength / 2.0
