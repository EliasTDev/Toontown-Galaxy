...
class DistributedPartyDanceActivityBase(DistributedPartyActivity):
    ...
    def load(self):
        ...
        # Reparent to render so that when the fireworks are on, it "glows" in
        # the dark
...
# ===============================================================================
    ...
    def __initOrthoWalk(self):
        ...
        # run speed = run frames (15) / fps (24fps) * avg. run speed (14.667
        # ft./s)
...
# ===============================================================================
    ...
    def setToonsPlaying(self, toonIds, toonHeadings):
        ...
        self.notify.debug(f"\ttoonIds: {toonIds}")
        self.notify.debug(f"\ttoonHeadings: {toonHeadings}")

        (exitedToons, joinedToons) = self.getToonsPlayingChanges(
            self.toonIds, toonIds)
        self.notify.debug(f"\texitedToons: {exitedToons}")
        self.notify.debug(f"\tjoinedToons: {joinedToons}")
    ...
    def __handleExitDanceFloor(self, collEntry):
        ...
        """
        if self.localToonDanceSequence is not None:
            self.notify.debug(f"finishing {self.localToonDanceSequence}")
            self.localToonDanceSequence.finish()
            self.localToonDanceSequence = None
        self.finishRules()
    ...
    def handleToonExited(self, toonId):
        ...
        """
        self.notify.debug(f"exitDanceFloor {toonId}")
        if toonId == base.localAvatar.doId:
            self.__localStopDancing()
...
# ===============================================================================
    ...
    def __doDanceMove(self, pattern):
        ...
        """
        self.notify.debug(f"Dance move! {pattern}")
...
# ===============================================================================
    ...
    def _requestToonState(self, toonId, state, anim):
        if toonId in self.dancingToonFSMs:
            ...
            assert(
                self.notify.debug(
                    f"requestToonState toonId={toonId}, state={state}, anim={anim}"))
            try:
                self.dancingToonFSMs[toonId].request(state, anim)
            except FSM.RequestDenied:
                self.notify.warning(
                    f"could not go from state={curState} to state {state}")
            if state == ToonDancingStates.getString(ToonDancingStates.Cleanup):
                self.notify.debug(
                    f"deleting this fsm {self.dancingToonFSMs[toonId]}")
                del self.dancingToonFSMs[toonId]
