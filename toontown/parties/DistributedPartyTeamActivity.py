...
# ===============================================================================
    ...
    def setToonsPlaying(self, leftTeamToonIds, rightTeamToonIds):
        ...
        assert(self.notify.debug(f"\tshifters: {shifters}"))
        assert(self.notify.debug(f"\tswitchers: {switchers}"))

        if base.localAvatar.doId in exitedToons:
            # make sure localToon's exit animation plays when the server clears
            # out all players
            self.localToonExiting()
    ...
    def setState(self, newState, timestamp, data):
        ...
        assert(self.notify.debug(f"setState( newState={newState}, ... )"))
...
# ===============================================================================
    ...
    def isState(self, state):
        return hasattr(
            self,
            "activityFSM") and self.activityFSM.getCurrentOrNextState() == state
