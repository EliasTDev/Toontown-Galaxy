...
class DistributedPartyTeamActivityAI(DistributedPartyActivityAI):
    ...
    def __init__(
        ...
        # How long should it wait for more players before going to the active
        # state
        self._startDelay = simbase.config.GetFloat(
            "party-team-activity-start-delay", startDelay)

        # Should it calculate whether both team have roughly an equal amount of
        # players?
        self._shouldBalanceTeams = balanceTeams

        # Should it calculate the advantage of a smaller team over a larger
        # team?
        ...
        # Keep track of which side each avatar is on for quick look up during
        # game
        self.toonIdsToTeams = {}
...
# ===============================================================================
    ...
    def _addToonToTeam(self, toonId, team):
        ...
        # check that toonId isn't already in that team's list
            ...
            self.notify.debug(
                f"Added toonId={toonId} to {self.getTeamName(team)}.")
            self.notify.debug(
                f"toonIds:\n\t\t{self.toonIds[0]}\n\t\t{self.toonIds[1]}.")
    ...
    def _removeToonFromTeam(self, toonId, team):
        ...
        if not isinstance(team, type(1)):
            self.notify.warning(
                "Tried to remove toonId=%s from %s, but team is not an integer" %
                (toonId, str(team)))
        elif toonId not in self.toonIds[team]:
            self.notify.warning(
            ...
            self.notify.debug(
                f"Removed toonId={toonId} from {self.getTeamName(team)}.")
            self.notify.debug(
                f"toonIds:\n\t\t{self.toonIds[0]}\n\t\t{self.toonIds[1]}.")
    ...
    def _removeAllToons(self):
        ...
        for team in (PartyGlobals.TeamActivityTeams.LeftTeam,
                     PartyGlobals.TeamActivityTeams.RightTeam, ):
            toonIds = list(self.toonIds[team])[:]
            self.notify.debug(
                f"initial toonIds for team {team} = {toonIds}")

            for toonId in toonIds:
                self.notify.debug(f"removing toon {toonId}")
                self._removeToonFromTeam(toonId, team)

                self.notify.debug(
                    f"self.toonIds[team] is now {self.toonIds[team]}")
    ...
    def balanceTeams(self):
        ...
        """
        # We assume that the left team is smaller, but later make the
        # correction
        ...
        if abs(len(self.toonIds[smallerTeam]) -
            ...
            while abs(len(self.toonIds[largerTeam]) -
    ...
    def calcAdvantage(self):
        ...
        if len(
                self.toonIds[smallerTeam]) == 0 or len(
        ...
        if len(self.toonIds[smallerTeam]) > len(self.toonIds[largerTeam]):
            ...
            self.advantage[smallerTeam] = len(
                self.toonIds[largerTeam]) / float(len(self.toonIds[smallerTeam]))
            self.notify.debug(f"Advantage set to {self.advantage}")
...
# ===============================================================================
    ...
    def _handleUnexpectedToonExit(self, toonId):
        ...
        self.notify.debug(f"currentState = {state}")
...
# ===============================================================================
    ...
    def toonJoinRequest(self, team):
        ...
        self.notify.debug(
            f"toonJoinRequest( toon = {toonId} team={self.getTeamName(team)} )")
        ...
        if self.isTeamFull(team):
            # decline the toon
            self.sendToonJoinResponse(
                toonId, False, team, PartyGlobals.DenialReasons.Full)
    ...
    def sendToonJoinResponse(
        ...
        if joined:
            ...
            self.sendUpdateToAvatarId(
                toonId, "joinRequestDenied", [denialReason])
    ...
    def toonExitRequest(self, team):
        ...
        # waiting to start
        self.notify.debug(
            f"toonExitRequest( team={self.getTeamName(team)} )")
