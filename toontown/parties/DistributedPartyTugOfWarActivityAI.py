...
class DistributedPartyTugOfWarActivityAI(DistributedPartyTeamActivityAI):
    ...
    def __init__(self, air, partyDoId, x, y, h):
        ...
        if self._allowSinglePlayer:
            self.notify.warning(
                "This team activity doesn't work party-team-activity-single-player config." +
    ...
    def reportKeyRateForce(self, keyRate, force):
        ...
        # sometimes the game has cleaned up and we get an old update from a
        # client
        if toonId in self.toonIdsToTeams:
            ...
            # send the current position to the clients if we have gotten all
            # the clients forces
    ...
    def reportFallIn(self, losingTeam):
        self.notify.debug("reportFallIn( losingTeam=%s )" %
                          PartyGlobals.TeamActivityTeams.getString(losingTeam))

        if losingTeam not in PartyGlobals.TeamActivityTeams:
            self.notify.warning(
                "Got an invalid losingTeam value %d" %
                losingTeam)
            return

        # if the losing team has already been reported and this report doesn't
        # match
        if (self.losingTeam != PartyGlobals.TeamActivityNeitherTeam) and (
                losingTeam != self.losingTeam):
            self.notify.warning(
                "Report of %s as the losingTeam doesn't match previously reported value. Ignoring." %
                PartyGlobals.TeamActivityTeams.getString(losingTeam))
            return
        ...
        if curState != "Active":
            ...
            self.notify.warning(
                f"got reportFallIn but state is {curState}, ignoring")
            return
    ...
    def computeMatchResults(self):
        ...
        # This seems like the easiest way to get this bean multiplier in for
        # all these different jelly bean cases
        beanMultiplier = 1
        ...
        if self.endedWithFall:
            # take advantage of enum nature of PartyGlobals.TeamActivityTeams
            winningTeam = 1 - self.losingTeam
            ...
            if abs(self.globalOffset) < PartyGlobals.TugOfWarTieThreshold:
                ...
                # take advantage of enum nature of
                # PartyGlobals.TeamActivityTeams
                winningTeam = 1 - self.losingTeam
                for toonId in self.toonIds[winningTeam]:
