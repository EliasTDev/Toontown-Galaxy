...
class JukeboxGui(DirectObject):
    ...
    def __createButton(self, guiNode, imagePrefix, parent=hidden, **kwargs):
        ...
        return DirectButton(
            ...
            relief=None,
            image=(
                guiNode.find(f"**/{imagePrefix}_up"),
                guiNode.find(f"**/{imagePrefix}_down"),
                guiNode.find(f"**/{imagePrefix}_rollover"),
            ),
            **kwargs
        )
    ...
    def __createLabel(self, guiNode, locatorPrefix, parent=hidden, **kwargs):
        ...
        return DirectLabel(
            parent=parent,
            relief=None,
            pos=guiNode.find(f"**/{locatorPrefix}_text_locator").getPos(),
            **kwargs
        )
    ...
    def __createLabeledScrolledList(
        ...
        return (DirectScrolledList(
            ...
            relief=None,
            incButton_image=(
                guiNode.find(f"**/{namePrefix}ButtonDown_up"),
                guiNode.find(f"**/{namePrefix}ButtonDown_down"),
                guiNode.find(f"**/{namePrefix}ButtonDown_rollover"),
            ),
            ...
            decButton_image=(
                guiNode.find(f"**/{namePrefix}ButtonUp_up"),
                guiNode.find(f"**/{namePrefix}ButtonUp_down"),
                guiNode.find(f"**/{namePrefix}ButtonUp_rollover"),
            ),
            decButton_relief=None,
            # Make the disabled button darker
            decButton_image3_color=Vec4(0.6, 0.6, 0.6, 0.6),
            image=guiNode.find(f"**/{namePrefix}_background"),
            itemFrame_relief=None,
            itemFrame_pos=guiNode.find(
                f"**/{namePrefix}List_locator").getPos(),
