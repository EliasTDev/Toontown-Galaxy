...
class PartyCogActivity(DirectObject):
    ...
    def load(self):
        ...
        self.winStatus = self.createText(
            2, Point3(0.0, 0.0, -0.8), self.teamColors[0])
    ...
    def _playArenaDoorIval(self, team, opening=True):
        ...
        if not opening:
            ...
            pos = LVecBase3f(
                tuple(self._doorStartPos[team])) + LVecBase3f(Point3(0, 0, -7.0))
    ...
    def handleToonJoined(self, toon, team, lateEntry=False):
        ...
        if toon == base.localAvatar:
            ...
            # This only happens if the toon is joining after the activity has
            # started
            if lateEntry:
    ...
    def handleToonSwitchedTeams(self, toon):
        ...
        if player is None:
            self.notify.warning(
                f"handleToonSwitchedTeams: toonId {toonId} not found")
            return
    ...
    def getTossPieInterval(
        ...
        assert 0 <= power <= 100, f"invalid pie throw power {power}"

        t = power / 100.0

        # Distance ranges from CogActivityPieMinDist to CogActivityPieMaxDist
        # ft, time ranges from 1 to 1.5 s.
            ...
            (16. / 24.,
             Sequence(Func(flyPie.reparentTo, render),
                      ...
                      ProjectileInterval(flyPie, startVel=getVelocity,
                                         duration=6),
                      # LerpPosInterval(flyPie, duration = 3,
                      # Point3(0.52,50,2.24)),
    ...
    def showResults(self, resultsText, winner, totals):
        ...
        # is lagging; the rewards screen tears down the GUI, which this ival
        # uses.
        self.accept(
