...
# so if this was 5 and current server time was 8:02 am, choosing asap will
# give 8:05 am start time
PartyPlannerAsapMinuteRounding = 5
# planner code assumes this is true
assert(60 % PartyPlannerAsapMinuteRounding) == 0
# In minutes, how often we check for parties that can start
UberdogCheckPartyStartFrequency = 5.0
# In minutes, how often we check for started but orphaned parties
...
# Note : If this enum changes, TTLocalizer PartyActivityNameDict must
# change too
...
# mudulo divide by this before sending over the wire
...
# This is primarily used by the AI to know where the head of the cog
# starts for score rewarding
CogPinataHeadZ = 4.7
...
# Bean split for ties. Like tug of war, the reward resembles a loss for
# both teams (not total loss).
CogActivityTieBeans = int(round(CogActivityBeansToAward * 0.4))

# Bean split for winning team vs. losing team if the winners got all the
# cogs to the very end.
...
# If the player has this many consecutive short throws, then we consider
# them spamming and will warn.
...
# HACK: this value is derived from the CameraLookAtHeightOffest. If you
# change that, update this
...
# from 1 shot, whats the maximum beans they'll ever get, no matter how
# many clouds hit
CannonMaxTotalReward = 100
