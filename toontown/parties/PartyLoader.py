...
class PartyLoader(SafeZoneLoader.SafeZoneLoader):
    ...
    def enterParty(self, requestStatus):
        self.notify.debug(f"enterParty: requestStatus = {requestStatus}")
        ownerId = requestStatus.get("ownerId")
        if ownerId:
            self.partyOwnerId = ownerId
        zoneId = requestStatus["zoneId"]
        self.notify.debug(
            f"enterParty, ownerId = {self.partyOwnerId}, zoneId = {zoneId}")
    ...
    def handlePartyDone(self, doneStatus=None):
        PartyLoader.notify.debug(
            f"handlePartyDone doneStatus = {doneStatus}")
        if not doneStatus:
    ...
    def loadSkyCollision(self):
        ...
        plane.setTangible(0)
        # Previously part of the cloudSpere.
