...
def getPartyActivityIcon(activityIconsModel, activityName):
    ...
    """
    icon = activityIconsModel.find(f"**/{activityName}Icon")
    if icon.isEmpty():
...
def arcPosInterval(duration, object, pos, arcHeight, other):
    ...
    def setArcPos(t):
        newX = startX + dx * t
        newY = startY + dy * t
        newZ = startZ + dz * t + arcHeight * \
            (-(2.0 * t - 1.0)**2 + 1.0)  # parabola
...
def truncateTextOfLabelBasedOnWidth(directGuiObject, textToTruncate, maxWidth):
    ...
    if directGuiObject["text"] != textToTruncate:
        directGuiObject["text"] = f"{directGuiObject['text']}..."


def truncateTextOfLabelBasedOnMaxLetters(
        directGuiObject, textToTruncate, maxLetters):
    # the frameSize for the host name is way bigger now, it's faster just to
    # base it on num letters
...
# 60 seconds per minute, 60 minutes per hour, 24 hours
...
def getTimeDeltaInSeconds(td):
    ...
    result = (td.days * SecondsInOneDay) + \
        td.seconds + (td.microseconds / 1000000.0)
    return result
