...
class ToontownTimeManager(DistributedObject.DistributedObject):
    ...
    def __init__(self, serverTimeUponLogin=0, clientTimeUponLogin=0,
        ...
        # TODO: Perhaps the AI and UD should have their own version of this
        # class? SG-SLWP
        try:
    ...
    def getCurServerDateTimeForComparison(self):
        ...
        # PartyInfo.startTime is in PST, this force the return value to always
        # be in PST
        secondsPassed = globalClock.getRealTime() - self.globalClockRealTimeUponLogin + \
            self.debugSecondsAdded
    ...
    def debugTest(self):
        ...
        self.notify.info(f'startTime = {startTime}')
        serverTime = self.getCurServerDateTime()
        self.notify.info(f"serverTime = {serverTime}")
        result = startTime <= serverTime
        self.notify.info(f"start < serverTime {result}")
        startTime1MinAgo = startTime + timedelta(minutes=-1)
        self.notify.info(f'startTime1MinAgo = {startTime1MinAgo}')
        result2 = startTime1MinAgo <= serverTime
        self.notify.info(f"startTime1MinAgo < serverTime {result2}")
        serverTimeForComparison = self.getCurServerDateTimeForComparison()
        self.notify.info(
            f"serverTimeForComparison = {serverTimeForComparison}")
        result3 = startTime1MinAgo <= serverTimeForComparison
        self.notify.info(
            f"startTime1MinAgo < serverTimeForComparison {result3}")
    ...
    def convertStrToToontownTime(self, dateStr):
        ...
        try:
            ...
            self.notify.warning(f"error parsing date string={dateStr}")
            pass
        result = curDateTime
        return result
    ...
    def convertUtcStrToToontownTime(self, dateStr):
        ...
        curDateTime = self.getCurServerDateTime()
        try:
            # we changed implementation since time.mktime is giving a incorrect
            # result in published build
