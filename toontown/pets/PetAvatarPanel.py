...
class PetAvatarPanel(AvatarPanel.AvatarPanel):
    ...
    def __init__(self, avatar):
        """__init__(self)"""
        self.notify.debug(f"Init(): doId={avatar.doId}")
    ...
    def __checkPetProximity(self, task=None):
        if self.petIsLocal:
            if base.localAvatar.isInWater():
                ...
                self.notify.debug(f"pet distance is {distance}")
    ...
    def cleanup(self):
        self.notify.debug(f"cleanup(): doId={self.avatar.doId}")
        """cleanup(self)"""
    ...
    def __handleToOwner(self):
        self.notify.debug(f"__handleToOwner(): doId={self.avatar.doId}")
    ...
    def __ownerDetailsLoaded(self, avatar):
        self.notify.debug(f"__ownerDetailsLoaded(): doId={self.avatar.doId}")
        self.cleanup()
        if avatar is not None:
            messenger.send("clickedNametag", [avatar])

    def __handleCall(self):
        self.notify.debug(f"__handleCall(): doId={self.avatar.doId}")
    ...
    def __handleFeed(self):
        self.notify.debug(f"__handleFeed(): doId={self.avatar.doId}")
    ...
    def __handleScratch(self):
        self.notify.debug(f"__handleScratch(): doId={self.avatar.doId}")
    ...
    def __handleDisableAvatar(self):
        self.notify.debug(
            f"__handleDisableAvatar(): doId={self.avatar.doId}")
    ...
    def __handleClose(self):
        self.notify.debug(f"__handleClose(): doId={self.avatar.doId}")
        self.cleanup()
        AvatarPanel.currentAvatarPanel = None
    ...
    def __fillPetInfo(self, avatar):
        self.notify.debug(f"__fillPetInfo(): doId={avatar.doId}")
    ...
    def __refreshPetInfo(self, avatar):
        self.notify.debug(f"__refreshPetInfo(): doId={avatar.doId}")
