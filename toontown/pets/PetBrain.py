...
class PetBrain(DirectObject.DirectObject, CPetBrain):
    ...
    def getThinkTaskName(self):
        return f'petThink-{self.pet.doId}'

    def getTeleportTaskName(self):
        return f'petTeleport-{self.pet.doId}'
    ...
    def stop(self):
        PetBrain.notify.debug(f'stop: {self.pet.doId}')
        if not self.started:
            return
        self.started = 0
        del self.lookers
    ...
    def _handleLookingAtOtherStart(self, avId):
        ...
        if avId in self.lookees:
            PetBrain.notify.warning(
                f'{self.pet.doId}: already looking at av {avId}')
            return
    ...
    def _handleLookingAtOtherStop(self, avId):
        ...
        if avId not in self.lookees:
            PetBrain.notify.warning(
                f'{self.pet.doId}: not looking at av {avId}')
            return
    ...
    def _handleLookedAtByOtherStart(self, avId):
        ...
        if avId in self.lookers:
            PetBrain.notify.warning(
                f'{self.pet.doId}: av {avId} already looking at me')
            return
    ...
    def _handleLookedAtByOtherStop(self, avId):
        ...
        if avId not in self.lookers:
            PetBrain.notify.warning(
                f'{self.pet.doId}: av {avId} not looking at me')
            return
    ...
    def setFocus(self, object):
        ...
        PetBrain.notify.debug(f'setFocus: {object}')
    ...
    def _handleActionObserve(self, observe):
        ...
        if action == OA.ATTENDED_START:
            dbg(f'avatar {avId} is looking at me')
            self.pet.lerpMoods({
                'boredom': -.1,
                'excitement': .05,
                'loneliness': -.05,
            ...
            if avId != self.pet.doId:
                ...
                PetBrain.notify.debug(
                    f'{self.pet.doId}.CHANGE_ZONE: {avId}, {oldZoneId}->{newZoneId}')
            ...
            dbg(f'avatar {avId} is feeding me')
            self.pet.lerpMoods({
                'affection': .35,
                'anger': -.07,
                'boredom': -.5,
            ...
            dbg(f'avatar {avId} is scratching me')
            self.pet.lerpMoods({
                'affection': .45,
                'anger': -.1,
                'boredom': -.8,
            ...
            dbg(f'avatar {avId} is gardening')
            avatar = simbase.air.doId2do.get(avId)
            if avatar is not None:
    ...
    def _addGoalsReAvatar(self, avId):
        ...
        if av is None:
            PetBrain.notify.warning(
                f'{self.pet.doId}._addGoalsReAvatar: {avId} not in doId2do')
            return
    ...
    def _removeGoalsReAvatar(self, avId):
        ...
        if avId not in self.doId2goals:
            PetBrain.notify.warning(
                f'no goals re av {avId} to remove')
            return
        for goal in self.doId2goals[avId]:
            self.goalMgr.removeGoal(goal)
            goal.destroy()
    ...
    def _considerBecomeAwareOf(self, avId):
        ...
        if av is None:
            PetBrain.notify.warning(
                f'_considerBecomeAwareOf: av {avId} does not exist')
            return
    ...
    def _handleAvatarArrive(self, avId):
        ...
        PetBrain.notify.debug(f'{self.pet.doId}._handleAvatarArrive: {avId}')
        """assert avId not in self.nearbyAvs"""
        if avId in self.nearbyAvs:
            PetBrain.notify.warning(
                f'{avId} already in self.nearbyAvs')
            return
    ...
    def _handleAvatarLeave(self, avId):
        PetBrain.notify.debug(f'{self.pet.doId}._handleAvatarLeave: {avId}')
        ...
        if avId not in self.nearbyAvs:
            PetBrain.notify.warning(f'av {avId} not in self.nearbyAvs')
            return
