...
class QuestMap(DirectFrame):
    ...
    def updateCogInfo(self):
        ...
        if currPercentage is None:
            return
        self.cInfo['text'] = f'{currPercentage[0]}%'
        self.lInfo['text'] = f'{currPercentage[1]}%'
        self.mInfo['text'] = f'{currPercentage[2]}%'
        self.sInfo['text'] = f'{currPercentage[3]}%'
        return
        ...
        if self.av:
            try:
                ...
                mapsGeom = loader.loadModel(
                    f'phase_4/models/questmap/{ToontownGlobals.dnaMap[hoodId]}_maps')
                mapImage = mapsGeom.find(
                    f'**/{ToontownGlobals.dnaMap[hoodId]}_{zoneId}_english')
    ...
    def initMarker(self, task):
        if self.av:
            if not hasattr(
                    ...
                    'isInterior') or not base.cr.playGame.getPlace().isInterior:
                relX, relY = self.transformAvPos(self.av.getPos())
                self.marker.setPos(relX, 0, relY)
                self.marker.setHpr(0, 0, -180 - self.av.getH())
            self.marker['geom_scale'] = 1.4 * task.time % 0.5 * 10 + 1
            self.marker['geom_color'] = (
                1, 1, 1, 0.8 - 1.4 * task.time %
                0.5 * 2 / 0.8 + 0.2)
        if task.time < 1:
            return Task.cont
        else:
