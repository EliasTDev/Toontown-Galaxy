...
def simulateRecoveryVar(numNeeded, baseChance, list=0, cap=1):
    ...
    if list:
        print(f"failures for each succes {attemptList}")
...
class Quest:
    ...
    def check(self, cond, msg):
        assert cond, f"quest {self.id}: {msg}"
    ...
    def checkRecoveryItemHolderAndType(self, holder, holderType='type'):
        ...
        self.check(holderType in holderTypes,
                   f"invalid recovery item holderType: {holderType}")
        if holderType == 'type':
            holders = [
                Any,
                AnyFish,
    ...
    def checkTrackChoice(self, option):
        self.check(option >= ToontownBattleGlobals.MIN_TRACK_INDEX and
                   ...
                   f"invalid track option: {option}")

    def checkNumFriends(self, num):
        self.check(1,
                   f"invalid number of friends: {num}")

    def checkNumMinigames(self, num):
        self.check(1,
                   f"invalid number of minigames: {num}")
    ...
    def filterFunc(avatar):
        ...
        # 1 means give the quest, 0 means do not. By default, you can give all
        # quests
        return 1
...
class LocationBasedQuest(Quest):
    ...
    def isLocationMatch(self, zoneId):
        ...
        # If a branch is specified, check the branch zone
            ...
            if loc == ZoneUtil.getCanonicalBranchZone(zoneId):
                return 1
            # Is this the *actual* zone? Why would you do that? I'll support it
            # just in case.
            elif loc == zoneId:
                return 1
            else:
    ...
    def getChooseString(self):
        # How the quest appears in the QuestChoiceGui panel
        return (
            TTLocalizer.QuestsLocationString % {
    ...
    def getDefaultQuestDialog(self):
        ...
        return ((TTLocalizer.QuestsLocationString + TTLocalizer.Period) %
                {"string": self.getString(), "location": self.getLocationName()})
    ...
    def getObjectiveStrings(self):
        ...
        if numCogs == 1:
            ...
            text = TTLocalizer.QuestsCogQuestDefeatDesc % {
                "numCogs": numCogs, "cogName": cogName}
        return (text,)
    ...
    def getObjectiveStrings(self):
        ...
        if numCogs == 1:
            ...
            text = TTLocalizer.QuestsCogTrackDefeatDesc % {
                "numCogs": numCogs, "trackName": self.trackNamesP[track]}
        return (text,)

    def getString(self):
        # Return the full string
        return (
            TTLocalizer.QuestsCogTrackQuestDefeat %
            self.getObjectiveStrings()[0])
...
class BuildingQuest(CogQuest):
    ...
    def getString(self):
        # Return the full string
        return (
            TTLocalizer.QuestsBuildingQuestString %
            self.getObjectiveStrings()[0])
...
class FactoryQuest(LocationBasedQuest):
    ...
    def getString(self):
        # Return the full string
        return (
            TTLocalizer.QuestsFactoryQuestString %
            self.getObjectiveStrings()[0])
    ...
    def getObjectiveStrings(self):
        ...
        if num == 1:
            ...
            text = TTLocalizer.QuestsItemNameAndNum % {
                "num": TTLocalizer.getLocalNum(num), "name": gagName}
        return (text,)

    def getString(self):
        # Return the full string
        return (
            TTLocalizer.QuestsDeliverGagQuestString %
            self.getObjectiveStrings()[0])

    def getRewardString(self, progress):
        return (
            TTLocalizer.QuestsDeliverGagQuestStringLong %
            self.getObjectiveStrings()[0])

    def getDefaultQuestDialog(self):
        return ((TTLocalizer.QuestsDeliverGagQuestStringLong %
                 self.getObjectiveStrings()[0]) +
                "\a" +
...
class DeliverItemQuest(Quest):
    ...
    def getString(self):
        # Return the full string
        return (
            TTLocalizer.QuestsDeliverItemQuestString %
            self.getObjectiveStrings()[0])
...
class RecoverItemQuest(LocationBasedQuest):
    ...
    def testDone(self, progress):
        numberDone = progress & (pow(2, 16) - 1)
        print(f"Quest number done {numberDone}")
        if numberDone >= self.getNumItems():
            return 1
        else:
            return 0
    ...
    def getObjectiveStrings(self):
        ...
        if holder == Any:
            ...
            holderName = TTLocalizer.QuestsRecoverItemQuestHolderString % {
                'level': TTLocalizer.Level, 'holder': holder, 'cogs': TTLocalizer.Cogs}
        elif (holderType == 'track'):
            if (holder == 'c'):
        ...
        if num == 1:
            ...
            itemName = TTLocalizer.QuestsItemNameAndNum % {
                "num": TTLocalizer.getLocalNum(num), "name": ItemDict[item][1]}
...
class FriendNewbieQuest(FriendQuest, NewbieQuest):
    ...
    def getString(self):
        # Return the full string
        return (
            TTLocalizer.QuestsFriendNewbieQuestObjective %
            (self.getNumFriends()))
...
Quest2RewardDict = {}
# Tier2Reward2QuestsDict is a two-level dict of {tier : {reward :
# [questId, questId...]}}
Tier2Reward2QuestsDict = {}
# Quest2RemainingStepsDict is a dictionary which records the number of
# steps until quest completion
...
def findFinalRewardId(questId):
    ...
    # Used to initialize Quest2RewardDict, Tier2Reward2QuestsDict,
    # Quest2RemainingStepsDict
    ...
    if finalRewardId:
        ...
        if nextQuestId == NA:
            ...
            # Recurse down chain
            if isinstance(nextQuestId, type(())):
                # Multiple next links, use first one for ultimate answer
                finalRewardId, remainingSteps = findFinalRewardId(
                    nextQuestId[0])
...
def getNextQuest(id, currentNpc, av):
    ...
    if nextQuest == NA:
        return NA, NA
    elif isinstance(nextQuest, type(())):
...
def filterQuests(entireQuestPool, currentNpc, av):
    ...
    """
    if notify.getDebug():
        notify.debug(f'filterQuests: entireQuestPool: {entireQuestPool}')
    ...
    if notify.getDebug():
        notify.debug(f'filterQuests: av quest history: {history}')
    ...
    for questId in entireQuestPool:
        ...
        if questId in history:
            if notify.getDebug():
                notify.debug(
                    f"filterQuests: Removed {questId} because in history")
        ...
        if not getQuestClass(questId).filterFunc(av):
            if notify.getDebug():
                notify.debug(
                    f"filterQuests: Removed {questId} because of filterFunc")
            validQuestPool[questId] = 0
            continue
        ...
        for quest in currentQuests:
            ...
            if ((potentialToNpc == toNpcId) and (toNpcId != ToonHQ)):
                ...
                if notify.getDebug():
                    notify.debug(
                        f"filterQuests: Removed {questId} because npc involved")
                break

    finalQuestPool = [
        key for key in list(
            validQuestPool.keys()) if validQuestPool[key]]
    if notify.getDebug():
...
def chooseTrackChoiceQuest(tier, av, fixed=0):
    ...
    def fixAndCallAgain():
        if ((not fixed) and av.fixTrackAccess()):
            notify.info(f"av {av.getDoId()} trackAccess fixed: {trackAccess}")
    ...
    if (tier == MM_TIER):
        ...
        if (trackAccess[ToontownBattleGlobals.HEAL_TRACK] == 1):
            ...
            notify.warning(f"av {av.getDoId()} has bogus trackAccess: {trackAccess}")
...
def chooseMatchingQuest(tier, validQuestPool, rewardId, npc, av):
    ...
    if ((rewardId == 400) and (
            QuestDict[questsMatchingReward[0]][QuestDictNextQuestIndex] == NA)):
...
def chooseBestQuests(tier, currentNpc, av):
    ...
    for i in range(numChoices):
        ...
        # If that came back None for some reason, just continue to the next
        # choice
...
def chooseQuestDialog(id, status):
    ...
    if isinstance(questDialog, type(())):
...
def getNpcLocationDialog(fromNpcId, toNpcId):
    ...
    if (fromBranchId == toBranchId):
        ...
        if isInPlayground:
            streetDesc = TTLocalizer.QuestsStreetLocationNamedPlayground % (
                toHoodName)
        else:
            streetDesc = TTLocalizer.QuestsStreetLocationNamedStreet % {
                'toStreetName': toStreetName, 'toHoodName': toHoodName}
...
class CheesyEffectReward(Reward):
    ...
    def getString(self):
        ...
        if (hoodId == 0):
            ...
            whileStr = (TTLocalizer.CheesyEffectExceptIn %
                        (TTLocalizer.ToontownCentral[-1]))
        else:
            hoodName = base.cr.hoodMgr.getFullnameFromId(hoodId)
...
def getReward(id):
    ...
    if reward:
        ...
        notify.warning(f'getReward(): id {id} not found.')
        return None
...
def getNextRewards(numChoices, tier, av):
    ...
    # give trialers something to do since we won't let them fininsh their last
    # gag track task
    ...
    if isLoopingFinalTier(tier):
        ...
        if notify.getDebug():
            notify.debug(
                f'getNextRewards: current rewards (history): {rewardHistory}')
    else:
        rewardHistory = av.getRewardHistory()[1]
        if notify.getDebug():
    ...
    for rewardId in getRewardsInTier(tier):
        ...
        # If it is, we do not want to offer a reward for a part the toon
        # already has
        if getRewardClass(rewardId) == CogSuitPartReward:
            ...
            if av.hasCogPart(cogPart, dept):
                notify.debug(
                    f'getNextRewards: already has cog part: {cogPart} dept: {dept}')
                # The avatar already has this cog part - do not offer the
                # reward
    ...
    if notify.getDebug():
        notify.debug(
            f'getNextRewards: final reward pool: {finalRewardPool}')
...
def avatarHasAllRequiredRewards(av, tier):
    ...
    for rewardId in rewardList:
        if (rewardId == 900):
            ...
            # Track training rewards are replaced, so check the replacement
            # values
            ...
            # the reward and if so, count it as done.
                ...
                if av.hasCogPart(cogPart, dept):
                    ...
                    # any single required reward, the answer is 0 - no you are
                    # not done
                    if notify.getDebug():
                        notify.debug(
                ...
                # We can safely bail out here because if you are missing
                # any single required reward, the answer is 0 - no you are not
                # done
                if notify.getDebug():
                    notify.debug(
    ...
    if notify.getDebug():
        ...
        for rewardId in rewardHistory:
            if not isRewardOptional(tier, rewardId):
                notify.warning(
                    f"required reward found, expected only optional: {rewardId}")
...
def checkReward(questId, forked=0):
    ...
    if nextQuests is None:
        ...
        assert reward != NA, f"end-of-chain quest {questId} ends with no reward"
        validRewards = list(RewardDict.keys()) + \
            [Any, AnyCashbotSuitPart, AnyLawbotSuitPart, OBSOLETE]
        assert reward in validRewards, f"quest {questId}: unknown reward {reward}"
        if reward is OBSOLETE:
            # this quest is obsolete; notify
            print(f"warning: quest {questId} is obsolete")
        return reward
    else:
        # internal quests currently cannot give out rewards
        assert reward is NA, f"internal chain quest {questId} has a reward ({reward})"
        ...
        for qId in nextQuests[1:]:
            ...
            assert thisReward == firstReward, "quest %s leads to different rewards through chained quests %s(%s) and %s(%s)" % (
                questId, nextQuests[0], firstReward, qId, thisReward)
        return firstReward
