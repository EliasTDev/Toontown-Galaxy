...
class DistributedLeaderBoardAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __init__(
        ...
        """
        self.notify.debug(
            "__init__: initialization of leaderboard AI: name=%s, Subscriptions=%s, pos=%s, hpr=%s" %
            (pName, pSubscribeList, pos, hpr))
        DistributedObjectAI.DistributedObjectAI.__init__(self, air)
        self.name = pName

        # each subscription icludes: id : track title, period title, list of
        # (time, name)
    ...
    def sendNewDisplay(self):
        ...
        self.sendUpdate("setDisplay", [pickle.dumps(
            self.subscriptionDict[self.subscriptionList[self.curIndex]], 1)])
    ...
    def start(self):
        ...
        '''
        self.notify.debug(
            f"start: leaderboard cycling started on leaderboard {self.name}")
        self.accept("GS_LeaderBoardSwap" +
                    str(self.zoneId), self.__switchDisplayData)

    def stop(self):
        self.notify.debug(
            f"stop: leaderboard cycling stopped on leaderboard {self.name}")
        self.ignore("GS_LeaderBoardSwap" + str(self.zoneId))

    def unsubscribeTo(self, pID):
        self.notify.debug(
            f"unsubscribeTo: removing subscription {pID} for LB {self.name}")
    ...
    def subscribeTo(self, pID):
        ...
        '''
        self.notify.debug(
            f"subscribeTo: adding subscription {pID} for LB {self.name}")
        ...
        self.subscriptionDict[pID] = [
            KartRace_TrackNames[pID[0]], RecordPeriodStrings[pID[1]], []]
    ...
    def delete(self):
        self.notify.debug(
            f"delete: stopping distributedleaderboardAI {self.name}")
        self.ignore("UpdateRaceRecord")
        self.stop()
