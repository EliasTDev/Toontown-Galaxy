...
class DistributedRace(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self.bananaSound = base.loader.loadSfx(
            "phase_6/audio/sfx/KART_tossBanana.ogg")
        self.anvilFall = base.loader.loadSfx(
            "phase_6/audio/sfx/KART_Gag_Hit_Anvil.ogg")
    ...
    def announceGenerate(self):
        self.notify.debug(f'announceGenerate: {self.doId}')
    ...
    def disable(self):
        self.notify.debug(f'disable {self.doId}')
        if self.musicTrack:
    ...
    def racerDisconnected(self, avId):
        self.notify.debug(f"lost racer: {avId}")
    ...
    def setCircuitPlace(
        ...
        avatar = base.cr.doId2do.get(avId, None)
        if(avatar):
            print(f"circuit trophies {trophies}")
    ...
    def goToSpeedway(self, avIds, reason=RaceGlobals.Exit_UserReq):
        self.notify.debug(f"goToSpeedway {avIds} {reason}")
    ...
    def setTrackId(self, id):
        DistributedRace.notify.debug(f'setTrackId: {id}')
        self.trackId = id
    ...
    def setAvatars(self, avIds):
        ...
        for i in avIds:
            ids += str(i) + " "
        DistributedRace.notify.debug(f'setAvatars: {ids}')
        self.avIds = avIds
    ...
    def setEnteredRacers(self, avAndKarts):
        ...
        self.notify.debug(f"setEnteredRacers {avAndKarts}")
        avatarsGone = []
        avatarsLeft = []
    ...
    def raceWatcher(self, task):
        ...
        # if he's so far off that we have to clamp position, bring him back
        # into the track
    ...
    def stopDriving(self):
        ...
        localAvatar.reparentTo(hidden)
        self.localKart.reparentTo(hidden)
        # stop smoothing/broadcast after we take control of the kart - prevents
        # out of range crash
    ...
    def enterLeave(self):
        ...
        if (self.raceType == RaceGlobals.Circuit and
            ...
            taskMgr.doMethodLater(1, self.goToSpeedway, "leaveRace", extraArgs=[
                                  [localAvatar.doId], RaceGlobals.Exit_UserReq])
    ...
    def timerTask(self, task):
        ...
        # make sure we don't display any numbers greater than countdown start
        # (3)
    ...
    def showBuildings(self, t, forceRecompute=False):
        # RAU show buildings expect t to be between 0 and 1, not happening in
        # Urban Track B
        firstTimeCalled = 0
        if (self.curve):
        ...
        for side in ['inner', 'outer']:
            ...
            # based on a value t, (0 < t < 1), determine which group of
            # buildings to show
            ...
            if bldgInd != self.currBldgInd[side]:
                ...
                # TODO: it may be inneficient to hide all of them, since we
                # probably only need to hide one
                if currBldgGroups:
        ...
        if self.trackId in (
                RaceGlobals.RT_Urban_2,
                RaceGlobals.RT_Urban_2_rev):
            # side street computation follows the reverse curve, so reverse t
            # again
            if (self.reversed):
                t = 1.0 - t
            ...
            for side in ['innersidest', 'outersidest']:
                ...
                if segmentInd != self.currBldgInd[side] or forceRecompute:
                    ...
                    # we should have the correct value in currBldgGroups at
                    # this point
                    for i in self.currBldgGroups[side]:
    ...
    def setupGeom(self):
        ...
        self.notify.debug(f"self.startingPos: {self.startingPos}")
        self.wrongWay = False
        self.laps = 0
        ...
        for i in range(4000):
            ...
            self.curve.getPoint(
                (i / 4000.0) * (self.curve.getMaxT() - .00000000001), self.curvePoints[-1])
            self.curveTs.append(
                (i / 4000.0) * (self.curve.getMaxT() - .00000000001))
        ...
        for i in range(len(gagList)):
            self.notify.debug(f"generating gag: {i}")
            self.gags.append(
                RaceGag(
                    self,
                    i,
    ...
    def precomputeSideStreets(self):
        ...
        for i in range(int(self.barricadeSegments)):
            ...
            self.curve.getPoint((i / self.barricadeSegments) *
                                (self.curve.getMaxT() - .00000000001), testPoint)
            for side in ('innersidest', 'outersidest'):
                for bldgGroupIndex in range(len(self.buildingGroups[side])):
                    ...
                    # do the same test
                    # for the firt child
                    # for childIndex in (0, bldgGroup.getNumChildren() / 2,
                    # bldgGroup.getNumChildren()-1):
                    for childIndex in (0,):
        ...
        # we need to call this again, since the first time arround the
        # dictionary was not set up
        if self.isUrbanTrack:
    ...
    def hitBoostArrow(self, cevent):
        ...
        if arrowVec is None:
            print(f"Unknown boost arrow {idStr}")
            return
