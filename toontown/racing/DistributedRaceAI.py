...
class DistributedRaceAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def generate(self):
        ...
        self.notify.debug(f'generate {self.doId}, id={self.trackId}, ')
        ...
        taskMgr.doMethodLater(.5,
                              self.enableEntryBarrier,
                              "enableWaitingBarrier")

    def enableEntryBarrier(self, task):
        self.enterRaceBarrier = self.beginBarrier(
            "waitingForJoin",
            self.avIds,
            TTLocalizer.DRAwaitingForJoin,
            self.b_racersJoined)
    ...
    def removeObject(self, object):
        if(object):
            self.notify.debug(f"deleting object: {object.doId}")
            object.requestDelete()

    def requestDelete(self, lastRace=True):
        self.notify.debug(f'requestDelete: {self.doId}')
        self.ignoreAll()
    ...
    def b_racersJoined(self, avIds):
        assert self.notify.debug(f"b_racersJoined {avIds}")
    ...
    def b_startTutorial(self, avIds):
        ...
        for avId in avIds:
            ...
            if(not av):
                self.notify.warning(
                    f"b_racersJoined: Avatar not found with id {avId}")
                ...
                if(avTickets < raceFee):
                    ...
                    # Okay, toon has enough tickets so now we must subtract
                    # them.
    ...
    def b_startRace(self, avIds):
        ...
        # don't start the race until the message has arrived on the client and
        ...
        timeout = qualTime + TTLocalizer.DRAwaitingForJoin + \
            3     # 3 is for the 'countdown'
    ...
    def kickSlowRacers(self, task):
        ...
        for racer in list(self.racers.values()):
            ...
            if (not racer.finished) and (avId not in self.kickedAvIds):
                self.notify.info(f'Racer {racer.avId} timed out - kicking.')
                self.d_kickRacer(avId, RaceGlobals.Exit_Slow)
                self.ignore(racer.exitEvent)
                racer.exited = True
                racer.finished = True
                taskMgr.doMethodLater(
                    10, self.removeObject, f"removeKart-{racer.kart.doId}", extraArgs=[
                        racer.kart])

                # Make them invincible in the eyes of the anvil dropper
                taskMgr.remove(f"make {avId} invincible")
    ...
    def d_dropAnvil(self, ownerId):
        ...
        if(len(possibleTargets)):
            ...
            if(id != ownerId):
                ...
                taskMgr.doMethodLater(
                    4, setattr, f"make {id} invincible", extraArgs=[
                        self.racers[id], "anvilTarget", False])

                # This only happens when the player tries to drop on themselves
            self.sendUpdate(
                "dropAnvilOn", [
                    ownerId, id, globalClockDelta.getFrameNetworkTime()])

    def d_makeBanana(self, avId, x, y, z):
        gag = DistributedGagAI.DistributedGagAI(
            simbase.air, avId, self, 3, x, y, z, 0)
    ...
    def d_makePie(self, avId, x, y, z):
        ...
        gag = DistributedProjectileAI.DistributedProjectileAI(
            simbase.air, self, avId)
        self.thrownGags.append(gag)
        gag.generateWithRequired(self.zoneId)
    ...
    def racerLeft(self, avIdFromClient):
        ...
        if(avId in self.racers and avId == avIdFromClient):
            ...
            # Make them invincible in the eyes of the anvil dropper
            taskMgr.remove(f"make {id} invincible")
            self.racers[avId].anvilTarget = True
        ...
        if index < 0 or index > (len(self.gagList) - 1):  # check for cheaters
            self.air.writeServerEvent(
                ...
                (slot,
                 ...
                 index))
            self.notify.warning(
                "somebody is trying to check for a non-existant karting gag %s %s %s! avId: %s" %
                (slot, type, index, avId))

        if slot < 0 or slot > (len(self.gagList) -
                               1):  # crash from TTInjector hack
            self.air.writeServerEvent(
                'suspicious',
                avId,
    ...
    def d_setCircuitPlace(
        ...
        self.sendUpdate(
            "setCircuitPlace", [
                avId, place, entryFee, winnings, bonus, trophies])
    ...
    def unexpectedExit(self, avId):
        self.notify.debug(f"racer disconnected: {avId}")
    ...
    def sendToonsToNextCircuitRace(self, raceZone, trackId):
        for avId in self.avIds:
            self.notify.debug(
                f"Handling Circuit Race transisiton for avatar {avId}")
    ...
    def everyoneDone(self):
        ...
        for racer in list(self.racers.values()):
            if (not racer.exited and (racer.avId not in self.playersFinished)
