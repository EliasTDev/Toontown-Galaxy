...
class DistributedRacePadAI(DistributedKartPadAI, FSM):
    ...
    def __init__(self, air, area, tunnelGenre, tunnelId):
        ...
        FSM.__init__(self, f"RacePad_{DistributedRacePadAI.id}_FSM")
    ...
    def cycleTrack(self, task=None):
        self.notify.debug(f"Cycling track - {self.doId}")
        raceInfo = RaceGlobals.getNextRaceInfo(
            self.trackId, self.tunnelGenre, self.tunnelId)
        self.trackId = raceInfo[0]
        self.trackType = raceInfo[1]
    ...
    def addAvBlock(self, avId, block, paid):
        ...
        if(not av):
            self.notify.warning(
                f"addAvBlock: Avatar not found with id {avId}")
    ...
    def __handleSetRaceCountdownTimeout(self, params=[]):
        ...
        for avId in list(self.avId2BlockDict.keys()):
            if(avId):
                self.notify.debug(
                    f"Handling Race Launch Countdown for avatar {avId}")
    ...
    def enterWaitCountdown(self):
        ...
        self.__startCountdown(
            self.uniqueName(
                f'CountdownTimer-{self.doId}'),
    ...
    def exitWaitBoarding(self):
        ...
        """
        self.notify.debug(
            f"exitWaitBoarding: Exiting WaitBoarding State for RacePad {self.id}")
    ...
    def enterAllAboard(self):
        ...
        if kickSoloRacer:
            ...
            self.notify.info(
                f"enterAllAboard: only one toon, kicking him: {players[0]}")
            ...
            self.notify.warning(
                "The RacePad was empty, so no one entered a race. Returning to WaitEmpty State.")
            self.d_setState('WaitEmpty')
    ...
    def exitAllAboard(self):
        ...
        """
        self.notify.debug(
            f"exitAllAboard: Exiting AllAboard State for RacePad {self.id}")
