...
class DistributedStartingBlock(DistributedObject.DistributedObject, FSM):
    ...
    def __init__(self, cr):
        ...
        FSM.__init__(
            self, f"staringBlock_{DistributedStartingBlock.id}_FSM")
    ...
    def __handleEnterSphere(self, collEntry):
        ...
        if((base.localAvatar.hp > 0)):
            def handleEnterRequest(self=self):
                self.ignore("stoppedAsleep")
                if hasattr(
                        self.dialog, 'doneStatus') and (
                        self.dialog.doneStatus == 'ok'):
    ...
    def generateKartDisappearTrack(self):
        ...
        if not self.holeActor:
            ...
            self.holeActor.setBlend(base.wantSmoothAnimations)
