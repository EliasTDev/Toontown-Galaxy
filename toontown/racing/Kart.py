...
class Kart(NodePath, ShadowCaster.ShadowCaster):
    ...
    def __init__(self):
        ...
        self.kartAccessories = {
            KartDNA.ebType: None, KartDNA.spType: None, KartDNA.fwwType: (
                None, None), KartDNA.bwwType: (
                None, None)}
        self.texCount = 1
    ...
    def generateKart(self, forGui=0):
        ...
        assert not hasattr(
            self, 'geom'), "Kart::generateKart - Kart has already been generated."
        assert self.kartDNA != [-1] * (getNumFields(
        )), "Kart::generateKart - attempt to generate Kart before DNA has been set."
        assert checkKartDNAValidity(
            self.kartDNA), f"Kart::generateKart - attempt to generate Kart based on bad DNA {self.kartDNA}"
            ...
            if(field == KartDNA.bodyType):
                ...
                if(self.kartAccessories[KartDNA.ebType] is not None):
                    ...
                    for key in list(self.geom.keys()):
                        ...
                        self.geom[key].find(f"**/{name}").removeNode()
                ...
                if(self.kartAccessories[KartDNA.fwwType] != (None, None)):
                    ...
                    for key in list(self.geom.keys()):
                        # Remove node found in geom lod
                        self.geom[key].find(
                            f"**/{left.getName()}").removeNode()
                        self.geom[key].find(
                            f"**/{right.getName()}").removeNode()
    ...
    def __applyEngineBlock(self):
        ...
        assert attachNode is not None, "Kart::__applyEngineBlock - invalid attach node for accessory %s" % (
            ebType,)
        ...
        for kart in self.geom:
            engineBlockNode = self.geom[kart].find(f"**/{attachNode}")
    ...
    def __applySpoiler(self):
        ...
        assert attachNode is not None, "Kart::__applySpoiler - invalid attach node for accessory %s" % (
            sbType,)
        ...
        for kart in self.geom:
            spoilerNode = self.geom[kart].find(f"**/{attachNode}")
    ...
    def __applyFrontWheelWells(self):
        ...
        assert attachNode is not None, "Kart::__applyFrontWheelWeels - invalid attach node for accessory %s" % (
            fwwType,)
        ...
        for kart in self.geom:
            ...
            leftNode = self.geom[kart].find(f"**/{leftAttachNode}")
            rightNode = self.geom[kart].find(f"**/{rightAttachNode}")
    ...
    def __applyBackWheelWells(self):
        ...
        assert attachNode is not None, "Kart::__applyBackWheelWells - invalid attach node for accessory %s" % (
            bwwType,)
    ...
    def setDNA(self, dna):
        ...
        # valid kart body.
        assert checkKartDNAValidity(
            dna), f"Kart::setDNA - INVALID DNA {dna}"
    ...
    def generateEngineStartTrack(self):
        ...
        track = Parallel(
            SoundInterval(
                self.kartStartSfx), Func(
                self.kartLoopSfx.play), LerpFunctionInterval(
