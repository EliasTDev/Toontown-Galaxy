    ...
    for field in range(len(dna)):
        if(field == KartDNA.bodyType):
            ...
            if(dna[field] not in accList):
                # if( not dna[ field ] in AccessoryTypeDict.get(
                # KartDNA.bodyColor ) ):
                return 0
        else:
...
def getAccessoryItemList(accessoryType):
    ...
    """
    assert accessoryType in AccessoryTypeDict, "KartDNA.py - accessoryType %s is invalid!!!" % (
        accessoryType,)
...
def getAccessoryDictFromOwned(accessoryOwnedList, pType=-1):
    ...
    # remove default rim #this is the simpliest, chepest way to do this, not
    # the most elegant!
...
def getAccessDictByType(accessoryOwnedList):
    ...
    """
    accessDict = {}
    if (isinstance(accessoryOwnedList, list)):
        for accOwnedId in accessoryOwnedList:
        ...
        print(
            "KartDNA: getAccessDictByType: bad accessory list: ",
            accessoryOwnedList)
