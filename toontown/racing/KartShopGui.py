...
# options for "you bought this kart" dialogue
BTK_OPTIONS = PythonUtil.Enum('Ok', -1)
# options for "you bought this accessory" dialogue
...
class KartShopGuiMgr(DirectObject.DirectObject):
    ...
    class MainMenuDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            # to be able to window shop even if they have no room for an
            # accessory
    ...
    class BuyKartDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            self.kartView.load(
                model, "KartViewerFrame", [
                    "rotate_right_up", "rotate_right_down", "rotate_right_roll", "rotate_right_down", (.255, -.054)], [
                    "rotate_left_up", "rotate_left_down", "rotate_left_roll", "rotate_left_down", (-.24, -.054)], (0, -.055))
            ...
            if self.numKarts > 0:
                ...
                if info[KartInfo.cost] > base.localAvatar.getTickets():
                    ...
                    self.buyKartButton.configure(
                        text_scale=KS_TEXT_SIZE_SMALL * .75)
                    self.buyKartButton.configure(
    ...
    class BuyAccessoryDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            self.kartView.load(
                model, "KartViewerFrame", [
                    "rotate_right_up", "rotate_right_down", "rotate_right_roll", "rotate_right_down", (.255, 0)], [
                    "rotate_left_up", "rotate_left_down", "rotate_left_roll", "rotate_left_down", (-.24, 0)], (0, 0))
        ...
        def __handleAccessoryChange(self, nDir):
            ...
            if len(self.unownedAccDict[self.curAccType]
                   ) < 1:  # we own ALL these accesories
        ...
        def showAccessory(self):
            ...
            # else if there's at least one accessory
                ...
                # should arrows be active?
                if self.curAccIndex[self.curAccType] + \
                        1 < len(self.unownedAccDict[self.curAccType]):
                    self.arrowRightButton['state'] = DGG.NORMAL
                ...
                cost = TTLocalizer.KartShop_Cost % (
                    AccessoryDict[curAcc][KartInfo.cost])
            ...
            if len(base.localAvatar.getKartAccessoriesOwned()
                ...
                self.buyAccessoryButton.configure(
                    text_scale=KS_TEXT_SIZE_SMALL * .8)
                self.buyAccessoryButton.configure(
        ...
        def __init__(self, doneEvent, accID):
            ...
            if(accType in [KartDNA.ebType, KartDNA.spType, KartDNA.fwwType, KartDNA.bwwType]):
                ...
                tex = loader.loadTexture(
                    f"phase_6/maps/{texNodePath}.jpg",
                    f"phase_6/maps/{texNodePath}_a.rgb")
...
# =================
    ...
    def __doDialog(self, dialogType):
        ...
        if dialogType == MENUS.ConfirmBuyKart:
            ...
            self.dialog = eventDlg(
                pageName="karting",
                doneFunc=self.__doLastMenu)
        else:
            self.dialog = eventDlg(eventType)

        # Do not reset lastMenu if we are going to the TeaserPanel - it needs
        # to know the last
    ...
    def __handleBuyKartDlg(self, exitType, args=[]):
        ...
        if(exitType == BK_OPTIONS.Cancel):
            # self.__popDialog() #pop the buy kart dialog off the stack,
            # leaving main.
            self.__doDialog(MENUS.MainMenu)
        # exitType is the id for a Kart # (exitType == BK_OPTIONS.BuyKart):
        else:
    ...
    def __handleBuyAccessoryDlg(self, exitType, args=[]):
        ...
        if(exitType == BA_OPTIONS.Cancel):
            ...
            # messenger.send(self.eventDict['guiDone'])
        # exitType is the id for an accessory # (exitType ==
        # BK_OPTIONS.BuyKart):
        else:
            # player is buyin the Kart
            self.accID = exitType
