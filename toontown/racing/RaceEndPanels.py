...
class RaceResultsPanel(DirectFrame):
    ...
    def displayRacer(
        ...
        def flipText(flip,
            ...
            self.entryList[place -
                           1][3].configure(text_fg=(0.95, 0.0, 0.0, 1.0))
        ...
        if not circuitPoints == []:
            ...
            self.entryList[place - 1][5]['text'] = f"{currentPoints}"
            self.entryList[place - 1][6]['text'] = f"+{newPoints}"

            def totalPointTicker(t,
                                 label=self.entryList[place - 1][5],
                                 current=currentPoints,
                                 new=newPoints):
                label['text'] = f"{int(currentPoints + t * new)}"

            def newPointTicker(t,
                               label=self.entryList[place - 1][6],
                               new=newPoints):
                label['text'] = f"+{int(new - t * new)}"
    ...
    def circuitFinished(self, placeFixup):
        ...
        for (oldPlace, newPlace) in placeFixup:
            if oldPlace != newPlace:
                def fixPlaceValue(oldPlace=oldPlace, newPlace=newPlace):
                    self.entryList[oldPlace][0]['text'] = f"{str(newPlace + 1)} -"
        ...
        if (not len(self.race.circuitLoop) == 0):
            ...
            # so we know that the next race button is up, if he doesn't press
            # it in 30 secs, do it for him
            self.notify.debug(
    ...
    def generateDisplaySequences(
        ...
        if ticBonus > 0:
            if (not endOfCircuitRace):
                ...
                bonusType = list(
                    RaceGlobals.PeriodDict.values()).index(ticBonus)
            else:
                # do not generate a bonus ticket sequence since we've done that
                # when the last race ended
                ticBonus = 0
