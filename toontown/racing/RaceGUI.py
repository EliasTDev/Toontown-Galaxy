...
# self.mapCam.node().getDisplayRegion(0).setActive(True)
    ...
    def enableResultMode(self):
        ...
        # make this panel eventually go away eventually if this is the last
        # race
    ...
    def update(self, time):
        ...
        if(self.timerEnabled and self.maxLapHit < self.race.lapCount):
            ...
            self.timeLabels[self.maxLapHit][1]['text'] = '%s%d\'\'' % (
                padding, seconds)
            self.timeLabels[self.maxLapHit][2]['text'] = f'{fraction}'
...
# face.reparentTo(self.raceModeRoot)
        ...
        for key, i in zip(list(self.racerDict.keys()), list(
                range(len(list(self.racerDict.keys()))))):
