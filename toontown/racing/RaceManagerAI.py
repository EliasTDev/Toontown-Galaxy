...
class RaceManagerAI(DirectObject.DirectObject):
    ...
    serverDataFolder = simbase.config.GetString(
        'server-data-folder', "dependencies/backups/racing")
    ...
    def createRace(
        ...
        assert self.notify.debug(f"createRace: players = {players}")
    ...
    def exitedRace(self, race, playerInfo):
        ...
        self.notify.debug(f"exited race: {playerInfo.avId}")
        ...
        self.notify.debug(f"place: {place} of {race.toonCount}")

        self.notify.debug(f"pre-tie totalTime: {totalTime}")
        ...
        self.notify.debug(f"CircuitTimeList {race.circuitTimeList}")
        ...
        if totalTime <= RaceGlobals.getQualifyingTime(race.trackId):
            ...
            self.notify.debug("Racer Qualified time: %s required: %s" % (
            ...
            if totalTime < threshold:
                self.air.writeServerEvent(
                    "kartingSuspiciousRacer", playerInfo.avId, "%s|%s" %
                    (race.trackId, totalTime))
                suspicious = True

            # you can only place if you beat qualifying time and you are NOT
            # practicing
            if race.raceType == RaceGlobals.Practice:
                winnings = RaceGlobals.PracticeWinnings
                self.notify.debug(
                    f"GrandTouring: Checking from branch: practice {playerInfo.avId}")
                ...
                self.air.writeServerEvent(
                    "kartingPlaced", playerInfo.avId, f"{place}|{race.toonCount}")

                if race.raceType != RaceGlobals.Circuit:
                    # calculate their winnings
                    entryFee = RaceGlobals.getEntryFee(
                        race.trackId, race.raceType)
                    ...
                    if newHistory:
                        self.notify.debug(f"history {newHistory}")
            ...
            if av:
                ...
                self.notify.debug(f"old tickets: {oldTickets}")
                newTickets = oldTickets + winnings + entryFee + bonus
                # Write to the server log
                self.air.writeServerEvent(
                    "kartingTicketsWon", playerInfo.avId, f"{newTickets - oldTickets}")
                self.notify.debug(f"entry fee: {entryFee}")
                self.notify.debug(f"place mult: {placeMultiplier}")
                self.notify.debug(f"winnings: {winnings}")
                self.notify.debug(f"bonus: {bonus}")
                self.notify.debug(f"new tickets: {newTickets}")
                self.notify.debug(f"circuit points: {points}")
                self.notify.debug(f"circuit time: {circuitTime}")
                self.notify.debug(
                    f"circuitTotalBonusTickets: {race.circuitTotalBonusTickets}")
                av.b_setTickets(newTickets)

        else:
            race.circuitTimeList[playerInfo.avId][currentTimeIndex][1] = -1
            self.notify.debug(
                f"GrandTouring: Checking from branch: Not Qualified {playerInfo.avId}")
            trophies = self.checkForNonRaceTrophies(playerInfo.avId)
        ...
        if(race in self.races):
            ...
            if race.isCircuit():
                ...
                if race.everyoneDone():
                    ...
                    # it's a problem only when the last player in the 3rd race
                    # breaks a record
                    if race.isLastRace():
                        taskMgr.doMethodLater(
                            10, self.endCircuitRace, "DelayEndCircuitRace=%d" %
                            race.doId, (race, bonus))
                    else:
    ...
    def endCircuitRace(self, race, bonus=0):
        ...
        self.notify.debug(f"Final standings {finalStandings}")
        # check for trophies, apply winnings, etc.
        for avId in finalStandings:
            self.notify.debug(f"avId {avId}")
            ...
            if race.isLastRace():
                ...
                if av and (avId in race.playersFinished):
                    ...
                    self.air.writeServerEvent(
                        "kartingCircuitFinished", avId, "%s|%s|%s|%s" %
                        (place, places, race.circuitTimes[avId], race.trackId))
                    print(
                        "kartingCircuitFinished", avId, "%s|%s|%s|%s" %
                        (place, places, race.circuitTimes[avId], race.trackId))
                    ...
                    oldTickets = av.getTickets()
                    self.notify.debug(
                        f"endCircuitRace: old tickets: {oldTickets}")
                    newTickets = oldTickets + winnings + entryFee
                    # Write to the server log
                    self.air.writeServerEvent(
                        "kartingTicketsWonCircuit", avId, f"{newTickets - oldTickets}")
                    self.notify.debug(f"entry fee: {entryFee}")
                    self.notify.debug(f"place mult: {placeMultiplier}")
                    self.notify.debug(f"winnings: {winnings}")
                    self.notify.debug(f"new tickets: {newTickets}")
                    self.notify.debug(f"trophies: {trophies}")
                    self.notify.debug(f"bonus: {bonus}")
                    av.b_setTickets(newTickets)
    ...
    def updateTrophiesFromList(self, avId, trophies):
        self.air.writeServerEvent(
            "Writing Trophies: Updating trophies from list",
            avId,
            f"{trophies}")
        av = self.air.doId2do.get(avId)
        if not av:
    ...
    def checkForCircuitTrophies(self, race, avId, inHistory=None):
        ...
        newTrophies.extend(
            self.checkHistoryForTrophy(
                ...
                qualIndices))
        self.notify.debug(
            f"GrandTouring: Checking from branch: Circuit {avId}")
    ...
    def checkForRaceTrophies(self, race, avId, inHistory=None):
        ...
        singleGenreNewTrophies = self.checkHistoryForTrophyByValue(
            trophies, history, totalQuals, [
                RaceGlobals.TotalQualifiedRaces], [
                RaceGlobals.TotalQuals])
        newTrophies.extend(singleGenreNewTrophies)
        self.notify.debug(
            f"GrandTouring: Checking from branch: Race {avId} ")
        newTrophies.extend(self.checkForNonRaceTrophies(avId, history))
        newTrophies.sort()
    ...
    def checkForNonRaceTrophies(self, avId, inHistory=None):
        ...
        self.notify.debug(
            f"GrandTouring: Checking for non-race trophies {avId}")

        # check and retrieve the needed data

        av = self.air.doId2do.get(avId)
        if not av:
            self.notify.debug(f"NO AV {avId}")
            self.notify.debug(
                f"GrandTouring: can't convert avId to Av {avId}")
            return []
        ...
        newTrophies = []
        self.notify.debug(f"GrandTouring: history-- {history}")
        self.notify.debug(f"GrandTouring: trophies- {trophies}")
        ...
        if not trophies[RaceGlobals.GrandTouring]:
            ...
            self.notify.debug("GrandTouring: checking for grand touring %s" % (
                trophies[RaceGlobals.GrandTouring]))
            #import pdb; pdb.set_trace()
            best = av.getKartingPersonalBestAll()
            self.notify.debug(f"personal best {best}")
            self.notify.debug(
                f"GrandTouring: checking personal best {best}")
            counter = 0
            for time in best:
                if not time == 0:
                    counter += 1
            self.notify.debug(f"counter {counter} tracks {len(RaceGlobals.TrackDict)}")
            self.notify.debug(
                "GrandTouring: bests comparison counter: %s tracks: %s" %
                (counter, len(
                    RaceGlobals.TrackDict)))
            if counter >= len(RaceGlobals.TrackDict):
                newTrophies.append(RaceGlobals.GrandTouring)
                addTrophyCount += 1
                self.air.writeServerEvent(
                    "kartingTrophy", avId, f"{RaceGlobals.GrandTouring}")
                self.notify.debug(
                    "trophy: " + TTLocalizer.KartTrophyDescriptions[RaceGlobals.GrandTouring])
                self.notify.debug(
                    f"GrandTouring: awarding grand touring new trophies {newTrophies}")
        else:
            self.notify.debug("already has grandtouring")
            self.notify.debug(f"trophies {trophies}")
            self.notify.debug("GrandTouring: already has grand touring %s" % (
                trophies[RaceGlobals.GrandTouring]))

        # check to see if we need a laff point
        for i in range(1.0, RaceGlobals.NumTrophies //
                       RaceGlobals.TrophiesPerCup + 1):
            cupNum = (trophies[:RaceGlobals.NumTrophies].count(
                1) + addTrophyCount) / (i * RaceGlobals.TrophiesPerCup)
            self.notify.debug(f"cupNum: {cupNum}")
            ...
            if cupNum and not trophies[trophyIndex]:
                ...
                self.notify.debug(f"cup awarded! new max laff : {newMaxHp}")
                av.b_setMaxHp(newMaxHp)
                # Also, give them a full heal
                av.toonUp(newMaxHp)
                # Write to the server log
                self.air.writeServerEvent(
                    "kartingTrophy", avId, f"{trophyIndex}")
                self.notify.debug(
                    "trophy: " +
                    TTLocalizer.KartTrophyDescriptions[trophyIndex])

        self.notify.debug(f"NONRACE TROPHIES {newTrophies}")
        return newTrophies
    ...
    def getNewCircuitHistory(self, race, avId, positionFinished):
        ...
        # increment the sweep history if it's not maxed ~ reached the highest
        # requirement
        if swept:
        ...
        self.notify.debug(f"qual times {race.qualTimes}")
        self.notify.debug(f"avatar times {race.circuitTimeList[avId]}")
        # if len(race.qualTimes) <= len(race.circuitTimeList[avId]):
        qualified = 1
        self.notify.debug(
            f"End Race Circuit Time List {race.circuitTimeList}")
        self.notify.debug("check for qualify")
        for qual in race.circuitTimeList[avId]:
            self.notify.debug(f"qual {qual}")
            if qual[1] == -1:
                qualified = 0
                self.notify.debug("not qualified")

        # increment the qualify history if it's not maxed ~ reached the highest
        # requirement
        if qualified:
    ...
    def raceOver(self, race, normalExit=True):
        ...
        if(race in self.races):
            if normalExit and race.isCircuit() and not race.isLastRace():
                ...
                assert self.notify.debug(
                    "race.kickedAvIds=%s continuingAvs = %s" %
                    (race.kickedAvIds, continuingAvs))
                lastRace = False
                if len(continuingAvs) > 0:
                    ...
                    lastRace = True
                    assert self.notify.debug(
                        "no avatars to continue race, not creating a new one")
    ...
    def checkForTrophies(self, place, trackId, raceType, numRacers, avId):
        ...
        if place == 1:
            ...
            trophiesWon += self.checkForTrophy(place,
                                               trackId,
                                               raceType,
                                               numRacers,
                                               avId,
        ...
        # quals
        # circuit quals cannot use checkfortrophy because it assumes you get a
        # qual if you get anything
    ...
    def checkForTrophy(
        ...
        # then add this race to players race history (keeps us from adding up
        # to infinity)
        ...
        for i in range(0, len(trophyReqList)):
            if not trophies[trophyIndices[i]]:
                ...
                if history[historyIndex] == trophyReqList[i]:
                    ...
                    # Write to the server log
                    self.air.writeServerEvent(
                        "kartingTrophy", avId, f"{trophyIndices[i]}")
                    self.notify.debug(
                        "trophy: " + TTLocalizer.KartTrophyDescriptions[trophyIndices[i]])
                    break
        ...
        if not raceType == RaceGlobals.Circuit:
            ...
            if not trophies[totalTrophyIndex]:
                ...
                for i in historyTotalList:
                    total += history[i]
                self.notify.debug(
                    f"checking for total trophy: {total} >= {totalReq}")
                if total >= totalReq:
                    ...
                    # Write to the server log
                    self.air.writeServerEvent(
                        "kartingTrophy", avId, f"{totalTrophyIndex}")
                    self.notify.debug(
                        "trophy: " + TTLocalizer.KartTrophyDescriptions[totalTrophyIndex])

        # check to see if we need a laff point
        for i in range(1, RaceGlobals.NumTrophies /
                       RaceGlobals.TrophiesPerCup + 1):
            cupNum = trophies[:RaceGlobals.NumTrophies].count(
                1) / (i * RaceGlobals.TrophiesPerCup)
            self.notify.debug(f"cupNum: {cupNum}")
            trophyIndex = RaceGlobals.TrophyCups[i - 1]
            if cupNum and not trophies[trophyIndex]:
                # award the cup!
        ...
        self.notify.debug(f"newTrophies: {newTrophies}")
        self.notify.debug(f"trophies: {trophies}")
        self.notify.debug(f"kartingHistory: {history}")
    ...
    def checkPersonalBest(self, trackId, time, raceType, numRacers, avId):
        ...
        if av:
            ...
            self.notify.debug(f"thisTime: {time}, bestTime: {bestTime}")
            if (bestTime == 0.0) or (time < bestTime):
                ...
                self.notify.debug(f"personal best: {bestTimes}")
        ...
        for period in RaceGlobals.PeriodIds:
            for record in range(0, RaceGlobals.NumRecordsPerPeriod):
                ...
                if time < recordTime:
                    ...
                    # new record!
                    self.notify.debug(
                        f"new {TTLocalizer.RecordPeriodStrings[period]} record!")
                    # TODO: alert player
                    # format: time, race type, num racers, racer name
                    av = simbase.air.doId2do.get(avId)
                    # TODO: what do we do if the av goes away?
                    if av:
                        ...
                        self.trackRecords[trackId][period].insert(
                        ...
                        self.air.writeServerEvent(
                            "kartingRecord", avId, f"{period}|{trackId}|{time}")
                    else:
    ...
    def getFilename(self):
        ...
        return f"{self.serverDataFolder}{self.shard}.trackRecords"
    ...
    def resetRecordPeriod(self, period):
        ...
        for trackId in RaceGlobals.TrackIds:
            ...
            for i in range(0, RaceGlobals.NumRecordsPerPeriod):
                self.trackRecords[trackId][period][i] = getDefaultRecord(
                    trackId)
...
class CircuitRaceHolidayMgr(HolidayBaseAI.HolidayBaseAI):
    ...
    def start(self):
        # let the holiday system know we started
        # note: the circuit race holidays can overlap, so we use a counter to
        # control start/stop
        bboard.post(
    ...
    def stop(self):
        ...
        # note: the circuit race holidays can overlap, so we use a counter to
        # control start/stop
        bboard.post(CircuitRaceHolidayMgr.PostName,
                    bboard.get(CircuitRaceHolidayMgr.PostName) - 1)
