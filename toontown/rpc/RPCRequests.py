...
class GetToonIdListRequest(AsyncRequest):
    ...
    def timeout(self, task):
        self.notify.warning(
            "Request timeout in GetToonIdListRequest(%s) from %s." %
            (self.accountName, self.clientIP))
        self.air.writeServerEvent(
...
class GetToonNameRequest(AsyncRequest):
    ...
    def timeout(self, task):
        self.notify.warning(
            "Request timeout in GetToonNameRequest(%s) from %s." %
            (self.toonID, self.clientIP))
        self.air.writeServerEvent(
            "UberRPC-RequestTimeout",
            self.clientIP,
            f"GetToonNameRequest|{self.toonID}")
        self.resultQueue.put(TimeoutException())
        self.delete()
...
class GiveToonBeansCSRequest(AsyncRequest):
    ...
    def submit(self):
        self.air.writeServerEvent(
            "UberRPC-GiveToonBeansCS", self.clientIP, "%u|%u" %
            (self.toonID, self.beanAmount))
...
class GiveToonBeansRATRequest(AsyncRequest):
    ...
    def submit(self):
        self.air.writeServerEvent(
            "UberRPC-GiveToonBeansRAT", self.clientIP, "%u|%u" %
            (self.toonID, self.beanAmount))
        self.air.deliveryManager.giveRecruitAToonPayment(
            self.toonID, self.beanAmount)
        self.finish()
