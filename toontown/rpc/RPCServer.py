    ...
    def run(self):
        while True:
            try:
                ...
                self.SOAPServer.logRequestQueue.put(
                    ("unknown", "Exception", e))
...
class RPCServer(object):
    ...
    def __init__(
        ...
        if keyfile is "" or certfile is "":
            self.notify.warning(
                "No key/cert provided, unable to use SSL.  NO ENCRYPTION ENABLED.")
        else:
            self.notify.info("Initializing SSL using TLSLite.")
            self.notify.info(f"Keyfile: {keyfile}")
            self.notify.info(f"Certificate file: {certfile}")

        if allowed_ipdict == {}:
            self.notify.warning(
                "No allowed IP list provided.  ALLOWING ANYONE TO CONNECT.")
        else:
            self.notify.info(
                f"Allowing requests from: {list(allowed_ipdict.keys())}")

        self.rpcip = rpcip
        self.rpcport = rpcport
        self.serverthread = RPCServerThread(
            rpcip, rpcport, keyfile, certfile, allowed_ipdict)
    ...
    def processLogRequest(self, clientIP, description, info):
        ...
        if description == "Unauthorized IP":
            self.notify.warning(f"HTTP Unauthorized Address {clientIP}")
            self.air.writeServerEvent(
                "UberRPC-SecurityAlert",
                clientIP,
                f"HTTP Unauthorized Address {clientIP}")
        elif description == "TLSError":
            self.notify.warning(
                f"Caught TLSError while serving RAT requests:\n{info}\nContinuing.")
        elif description == "Exception":
            self.notify.warning(
                f"Caught exception while serving RAT requests:\n{info}\nContinuing.")
        else:
            assert(False)
            pass
