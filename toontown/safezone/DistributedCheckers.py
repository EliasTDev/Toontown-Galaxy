...
class DistributedCheckers(DistributedNode.DistributedNode):
    ...
    def enableTurnScreenText(self, player):
        ...
        if player == self.playerNum:
            ...
            if player == 1:
                ...
                color = (0, 0, 0, 1)
        self.turnText = OnscreenText(text=message1 + message2, pos=(-0.80, -0.50),
                                     scale=0.092, fg=color, align=TextNode.ACenter, mayChange=1)
    ...
    def mouseClick(self):
        ...
        if self.isMyTurn and self.inGame:  # cant pick stuff if its not your turn
    ...
    def handleClicked(self, index):
        # You clicked something play the click sound
        ...
        if self.moveList == []:
            ...
            self.blinker.append(
                LerpColorInterval(
                    self.locatorList[index], .7, self.tintConstant, col))
            self.blinker.append(
                LerpColorInterval(
                    self.locatorList[index], .7, col, self.tintConstant))
            self.blinker.loop()
            self.sound.start()
        else:
            if index in self.mySquares or index in self.myKings:  # selected a new peice
                ...
                if type == 3 or type == 4:
                    ...
                    self.moverType = "normal"
            # item is either BLANK or the opposing players (checkJump will
            # validate if it is opposing players)
            else:
                ...
                if self.mustJump:
                    if lastItem.getNum() == index:  # selected the same item twice
                        ...
                        self.moveList = []
                        return
                    # print "CHECK LEGAL JUMP", self.checkLegalJump(lastItem,
                    # thisItem, self.moverType), "TYPE == ", self.moverType
                    if self.checkLegalJump(
                            lastItem, thisItem, self.moverType):  # There is a Legal Jump
                        # ghostConstant here is a small alpha offset to give it a transparent look
                        # tintConstant makes the peice a bit darker (necessary
                        # when ghosting)
                        col = self.locatorList[index].getColor()
                        self.locatorList[index].show()
                        self.sound.start()
                        if self.existsLegalJumpsFrom(
                                index, self.moverType) == False:  # No more Series of jumps => Commit the move
                            self.moveList.append(index)
                            self.blinker.finish()
                            self.d_requestMove(self.moveList)
                            self.moveList = []
                    ...
                    if self.checkLegalMove(
                            lastItem, thisItem, self.moverType):  # there exists a legal move
                        self.moveList.append(index)
                        # ghostConstant here is a small alpha offset to give it a transparent look
                        # tintConstant makes the peice a bit darker (necessary
                        # when ghosting)
    ...
    def existsLegalJumpsFrom(self, index, peice):
        if peice == "king":
            for x in range(4):
                if self.board.squareList[index].getAdjacent(
                )[x] is not None and self.board.squareList[index].getJumps()[x] is not None:  # Off the board
                    ...
                    if adj.getState() == 0:
                        ...
                        if index not in self.moveList and not jump.getNum() in self.moveList:
                            return True
                    else:
                        pass
            return False
    ...
    def checkLegalMove(self, firstSquare, secondSquare, peice):
        if (not firstSquare.getNum() in self.mySquares) and (
                not firstSquare.getNum() in self.myKings):
            return False
        if self.playerNum == 1:
            moveForward = [1, 2]
    ...
    def checkLegalJump(self, firstSquare, secondSquare, peice):
        if (not firstSquare.getNum() in self.mySquares) and (
                not firstSquare.getNum() in self.myKings) and len(self.moveList) == 1:
            return False
        if self.playerNum == 1:
            moveForward = [1, 2]
            opposingPeices = [2, 4]
        ...
        if peice == "king":
            ...
            if secondSquare.getNum() in firstSquare.getJumps():
                ...
                if index in moveForward:
                    if self.board.squareList[firstSquare.getAdjacent()[index]].getState(
                    ) in opposingPeices:  # is the peice jumping over yours or his
                        return True
                    else:
                        return False
                else:
    ...
    def animatePeice(self, tableState, moveList, type, playerColor):
        ...
        for x in range(length - 1):
            checkersPeiceTrack.append(Parallel(SoundInterval(self.moveSound), ProjectileInterval(
                gamePeiceForAnimation, endPos=self.locatorList[moveList[x + 1]].getPos(), duration=.5)))
