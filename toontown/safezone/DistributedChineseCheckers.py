...
class DistributedChineseCheckers(DistributedNode.DistributedNode):
    def __init__(self, cr):
        ...
        self.playerColors = [Vec4(0, .90, 0, 1), Vec4(.9, .9, 0, 1), Vec4(
            .45, 0, .45, 1), Vec4(.2, .4, .8, 1), Vec4(1, .45, 1, 1), Vec4(.8, 0, 0, 1)]
    ...
    def sendTurn(self, playersTurn):
        ...
        if self.fsm.getCurrentState().getName() == 'playing':
            # print "GETTING HERE!", playersTurn - 1, " LENGTH!!!" ,
            # self.playerTagList #self.playerTagList
            ...
            if self.playerSeats is None:
                ...
                self.playersTurnBlinker.append(LerpColorInterval(self.playerTagList[self.playerSeats.index(
                    playersTurn)], .4, origColor - self.tintConstant - self.ghostConstant, origColor))
                self.playersTurnBlinker.append(LerpColorInterval(self.playerTagList[self.playerSeats.index(
                    playersTurn)], .4, origColor, origColor - self.tintConstant - self.ghostConstant))
    ...
    def enableTurnScreenText(self, player):
        ...
        if player == self.playerNum:
            ...
            self.yourTurnBlinker.append(
                LerpScaleInterval(
                    self.turnText, .6, 1.045, 1))
            self.yourTurnBlinker.append(
                LerpScaleInterval(
                    self.turnText, .6, 1, 1.045))
    ...
    def handleClicked(self, index):
        ...
        if self.moveList == []:
            ...
            self.blinker.append(
                LerpColorInterval(
                    self.locatorList[index], .7, self.playerColor - self.tintConstant, self.playerColor))
            self.blinker.append(
                LerpColorInterval(
                    self.locatorList[index], .7, self.playerColor, self.playerColor - self.tintConstant))
            self.blinker.loop()
            self.sound.start()
            ...
            if self.board.squareList[index].getState() == self.playerNum:
                ...
                # Blinker is the color lerp for the peice "flashing" - need to
                # stop the flashing of the old one
                self.blinker.finish()
                self.blinker = Sequence()
                self.blinker.append(
                    LerpColorInterval(
                ...
                if len(
                        self.moveList) == 1 and self.board.squareList[index].getState() == 0:
                    # print "I AM OUTSIDE"
                ...
                if len(self.moveList) >= 1:
                    if index == self.moveList[len(
                            self.moveList) - 1]:  # you clicked the same thing TWICE
                        for x in self.nonOpposingPositions:
                            if index in x:
                    ...
                    # Therefore no jumps possible after it
                        # print "CHECK LEGAL JUMP!",
                        # self.checkLegalMove(self.board.getSquare(self.moveList[len(self.moveList)-1]),
                        ...
                        if index not in self.board.squareList[self.moveList[len(
                            ...
                            for x in self.nonOpposingPositions:
                                # print " LEGAL JUMPS FROM! ",
                                # self.existsLegalJumpsFrom(index)
                            ...
                            # tintConstant makes the peice a bit darker
                            # (necessary when ghosting)
        ...
        if secondSquare.getNum() in firstSquare.getAdjacent():
            ...
            for x in firstSquare.getAdjacent():
                if x is None:
                    ...
                    # print "Xs Adjacent and its Index",
                    # self.board.squareList[x].getAdjacent(), " INDEX == " ,
