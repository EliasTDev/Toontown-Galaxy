...
class DistributedFishingSpotAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def requestEnter(self):
        ...
        if self.avId == 0:
            ...
            self.air.writeServerEvent(
                "fished_enter", self.avId, f"{self.zoneId}")
        else:
            self.sendUpdateToAvatarId(avId, "rejectEnter", [])
    ...
    def doCast(self, power, heading):
        ...
        if not self.validate(
                avId,
                (av),
                "doCast: avId not currently logged in to this AI"):
            return
        ...
        self.air.writeServerEvent(
            "fished_cast", avId, f"{av.getFishingRod()}|{castCost}")
    ...
    def cleanupAvatar(self):
        ...
        self.air.writeServerEvent(
            "fished_exit", self.avId, f"{self.zoneId}")
    ...
    def hitTarget(self, code, item):
        self.notify.debug(f"hitTarget: code: {code} item: {item}")
    ...
    def d_sellFishComplete(self, avId, trophyResult, numFishCaught):
        self.sendUpdateToAvatarId(
            avId, "sellFishComplete", [
                trophyResult, numFishCaught])
    ...
    def sellFish(self):
        ...
        avId = self.air.getAvatarIdFromSender()
        av = self.air.doId2do.get(self.avId)
        self.notify.debug(f"sellFish: avId: {avId}")
        if not self.validate(
                avId,
                (simbase.wantBingo),
                "sellFish: Currently, you can only do this if bingo is turned on"):
            gotTrophy = False
