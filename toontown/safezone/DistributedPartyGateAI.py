...
class DistributedPartyGateAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def getPartyList(self, avId):
        ...
        if avId != senderId:
            self.air.writeServerEvent(
                'suspicious',
                senderId,
                'someone else trying to get a list of public parties for: avId = %d' %
                avId)
            return
        self.sendUpdateToAvatarId(
            avId, "listAllPublicParties", [
                self.air.partyManager.getAllPublicParties()])
    ...
    def partyChoiceRequest(self, avId, shardId, zoneId):
        ...
        # options).
        DistributedPartyGateAI.notify.debug(
            "partyChoiceRequest : avId = %d, shardId = %d, zoneId = %d " %
            (avId, shardId, zoneId))
        senderId = self.air.getAvatarIdFromSender()
        ...
        if avId != senderId:
            self.air.writeServerEvent(
                'suspicious',
                senderId,
                'someone else trying to choose a public party for: avId = %d' %
                avId)
            return
        ...
        if len(allPublicPartyInfo) == 0:
            ...
            DistributedPartyGateAI.notify.debug(
                "partyChoiceRequest denied as no parties exist")
            self.sendUpdateToAvatarId(
                avId, "partyRequestDenied", [
                    PartyGlobals.PartyGateDenialReasons.Unavailable])
        else:
            for partyTuple in allPublicPartyInfo:
                if partyTuple[0] == shardId and partyTuple[1] == zoneId:
                    ...
                    if partyTuple[2] < PartyGlobals.MaxToonsAtAParty:
                        DistributedPartyGateAI.notify.debug(
                            "partyChoiceRequest accepted")
                        self.sendUpdateToAvatarId(
                            avId, "setParty", [partyTuple])
                    else:
                        DistributedPartyGateAI.notify.debug(
                            "partyChoiceRequest denied as number at party is %d and max allowed is %d" %
                            (partyTuple[2], PartyGlobals.MaxToonsAtAParty))
                        self.sendUpdateToAvatarId(
                            avId, "partyRequestDenied", [
                                PartyGlobals.PartyGateDenialReasons.Full])
                    break  # prevent else clause from running
            else:
                # the desired party was not found
                DistributedPartyGateAI.notify.debug(
                    "partyChoiceRequest denied as party could not be found")
                self.sendUpdateToAvatarId(
                    avId, "partyRequestDenied", [
                        PartyGlobals.PartyGateDenialReasons.Unavailable])
