...
class DistributedPicnicTable(DistributedNode.DistributedNode):
    ...
    def showTimer(self):
        # important to stop the timer before you reset it, otherwise it may
        # still be running
        self.clockNode.stop()
    ...
    def announceWinner(self, winString, avId):
        ...
        if avId in self.cr.doId2do:
            ...
            if self.outTrack.isPlaying():  # If toon is jumping out Wait a
                # duration till his anim is over to begin
                self.winTrack.append(Wait(2.0))

            if avId == base.localAvatar.getDoId():
                # stop him from walking locally
                # otherwise he will animate around while moving on other
                # clients
    ...
    def setZone(self, zoneId):
        ...
        # if self.fsm.getCurrentState().getName() == "chooseMode" or
