...
class CogPageManagerAI:
    ...
    def toonEncounteredCogs(self, av, cogList, zoneId):
        ...
        changed = 0
        self.notify.debug(
            "toonEncounteredCogs: avId: %s, avCogs: %s, cogList: %s, zoneId: %s" %
    ...
    def toonKilledCogs(self, av, cogList, zoneId):
        ...
        self.notify.debug(
            "toonKilledCogs: avId: %s, cogCounts: %s, cogList: %s, zoneId: %s" %
        ...
        self.air.writeServerEvent(
            'cogsDefeated', avId, f"{msgText}|{zoneId}")

        # update cog status only if we've had a change of state
        if cogs_changed:
            av.b_setCogStatus(avCogs)

            # check the quotas for each dept and see if cog radar has been
            # achieved
            ...
            for dept in range(0, len(SuitDNA.suitDepts)):
                ...
                if avCogRadar[dept] == 0:
                    if min(avCogs[deptSize * dept:deptSize *
                           (dept + 1)]) == COG_COMPLETE1:
                        avCogRadar[dept] = 1
                        cog_radar_changed = 1
            ...
            # check the quotas for each dept and see if bldg radar has been
            # achieved
            for dept in range(0, len(SuitDNA.suitDepts)):
                # only check if it hasn't been achieved yet!
                if avBuildingRadar[dept] == 0:
                    if min(avCogs[deptSize * dept:deptSize *
                           (dept + 1)]) == COG_COMPLETE2:
