...
class IssueFrame(DirectFrame):
        ...
        for section, ident in enumerate(self.SectionIdents):
            ...
            while not endSearch:
                ...
                if self.strFilenames:
                    # we already have a list of filenames, check if it's in
                    # there
    ...
    def load(self):
        ...
        assert self.notify.debug(
            "before loop, numPagesLoaded=%d total=%d" %
            (numPagesLoaded, totalNumberOfPages))
    ...
    def loadHomePageButtons(self, section, subsection, pageFrame):
        ...
        for section in range(1, len(self.SectionIdents)):
            image = self.gui.find(f'**/{buttonNames[section]}')
            rolloverImage = self.gui.find(
                f'**/{rolloverButtonNames[section]}')
            if image.isEmpty():
                self.notify.error(f'cant find {buttonNames[section]}')
    ...
    def loadNavButtons(self, pageFrame):
        ...
        for section in range(0, len(self.SectionIdents)):
            image = self.guiNav.find(f'**/{buttonNames[section]}')
    ...
    def gotoPage(self, section, subsection):
        ...
        base.cr.centralLogger.writeClientEvent(
            f"news gotoPage {self.dateStr} {section} {subsection}")
    ...
    def loadFlatQuad(self, fullFilename):
        ...
        cm = CardMaker(f'cm-{fullFilename}')
        ...
        if readFile:
            ...
            # if we have an error loading the file, return None to signify an
            # error
            result = None
