            ...
            if section[0] == -1:
                ...
                for phrase in section[1]:
                    if phrase not in OTPLocalizer.SpeedChatStaticText:
                        print(
                            f'warning: tried to link boarding phrase {phrase} which does not seem to exist')
                        break
                    self.append(SCStaticTextTerminal(phrase))
            else:
                # This should be a submenu, get the list of phrases from the
                # corresponding zoneId
                menu = SCMenu()
