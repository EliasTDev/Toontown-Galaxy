...
class ToontownMagicWordManager(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        # The default chat prefix we use to determine if said phrase is a Magic
        # Word
        self.chatPrefix = PREFIX_DEFAULT

        # The default name of the "wizard" that returns responses when
        # executing Magic Words
    ...
    def announceGenerate(self):
        ...
        activatorIndex = base.config.GetBool('magic-word-activator', 0)
    ...
    def checkMagicWord(self, magicWord):
        # Well, this obviously isn't a Magic Word if it doesn't begin with our
        # prefix
        if not magicWord.startswith(self.chatPrefix):
            return

        # We don't even have access to be using Magic Words in the first place
        if base.localAvatar.getStaffAccess() <= AccessLevelName2Int.get('USER'):
    ...
    def generateResponse(
        ...
        base.localAvatar.setSystemMessage(
            0, self.wizardName + ': ' + response, WhisperPopup.WTSystem)
    ...
    def generateMagicWordResponse(
        ...
        # If our Magic Word was a success but has no return value, just send a
        # randomized success phrase
        if responseType == "SuccessNoResp" and magicWord:
            ...
            return response
        # We had a successful Magic Word and also got a return value, so let's
        # just use that
        elif responseType == "Success":
            response += returnValue
            return response

        # Guess it wasn't a success, so let's grab our response via the given
        # code
        response += MAGIC_WORD_RESPONSES.get(responseType,
                                             MAGIC_WORD_NO_RESPONSE)
    ...
    def handleMagicWord(self, magicWord):
        ...
        # A normal affect type-  we aren't trying to target all Toons in the
        # zone, server, or a specific Access Level
        affectType = AFFECT_NORMAL

        # Only used for determining what Access Level will be targeted, if we
        # decide to target a specific one
        affectExtra = -1

        # Used for determining what the affectRange is- it counts the amount of
        # activators uses (ranges 1-3)
        for x in range(3):
            if magicWord.startswith(self.chatPrefix * (3 - x)):
                affectRange = 2 - x
                break

        # If so some reason our affectRange is still none, we can't go any
        # further
        if affectRange == AFFECT_NONE:
            self.generateResponse(responseType="NoEffect")
            return
        # Our affectRange is other, meaning we want to target someone- let's
        # make sure we're allowed to
            ...
            # Like earlier, Magic Words are no good if used while moving
            # between zones
        ...
        # Iterate through the affectType strings and see if any of them were
        # used (e.g. zone, server, or rank)
        ...
        if affectType == AFFECT_RANK:
            # Iterate over all the possible Access Level integers and see if
            # any match with the one provided
            for level in list(OTPGlobals.AccessLevelName2Int.values()):
                ...
                if magicWordNoPrefix.startswith(str(level)):
                    ...
                    # It typically works fine for me but I will keep it here
                    # just in-case
                    try:
            ...
            # The invoker wanted to target an Access Level but provided an
            # invalid integer, so let them know
            if affectExtra == -1:
                self.generateResponse(responseType="BadTarget")
                return
        ...
        if word not in MagicWordIndex:
            # Iterate over all Magic Word names and see if the one provided is
            ...
            for magicWord in list(MagicWordIndex.keys()):
                ...
                if word in magicWord:
                    self.generateResponse(
                        responseType="CloseWord",
                        extraMessageData=magicWord)
                    return
            # Couldn't find any Magic Word close to what was provided, so let
            # them know the word doesn't exist
        ...
        # The location of the Magic Word's execution was not specified, so
        # raise an error
        if magicWordInfo['execLocation'] == EXEC_LOC_INVALID:
            raise ValueError(
                f"execLocation not set for magic word {magicWordInfo['classname']}!")
        # The execLocation is valid, so let's finally send data over to the
        # server to execute our Magic Word
    ...
    def executeMagicWord(
        ...
        # This is alright, but we should only execute it on the target if they
        # are visible on our client
