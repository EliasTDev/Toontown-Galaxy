...
# This is only really useful in Toontown Offline, so other projects
# shouldn't have to really worry about it
...
class ToontownMagicWordManagerAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def requestExecuteMagicWord(
        ...
        if not avId:
            self.notify.warning(
                'requestExecuteMagicWord: Magic Word use requested but invoker avId is non-existent!')
            return

        # We also need the Toon as well, or else this Magic Word isn't going to
        # do much good
        toon = self.air.doId2do.get(avId)
        if not toon:
            self.notify.warning(
                'requestExecuteMagicWord: Magic Word use requested but invoker avatar is non-existent!')
            return
       ...
       #     self.generateResponse(avId=avId, responseType="Tutorial")
        ...
        # In that case, let's try to grab the single target by the
        # lastClickedAvId provided by the invoker
        lastClickedToon = None
        if (affectRange in (AFFECT_OTHER, AFFECT_BOTH)
                ) and affectType == AFFECT_NORMAL:
            if lastClickedAvId:
        ...
        # Gather all of the Toons using whichever method this Magic Word
        # requests
        if affectType in (AFFECT_ZONE, AFFECT_SERVER, AFFECT_RANK):
            ...
            for doId in list(self.air.doId2do.keys())[:]:
                ...
                # We only care if our DistributedObject is a player that is NOT
                # our invoker (we dealt with that earlier)
                if isinstance(
                        do,
                        DistributedPlayerAI) and do.isPlayerControlled() and do != toon:
                    # Only add the Toons that are in the same zone as the
                    # invoker
                    ...
                    # Add every Toon regardless of zone
                        ...
                        toonIds.append(doId)
                    # Only add the Toons that have the Access Level specified
                    # when the Magic Word was used
                    elif affectType == AFFECT_RANK and do.getStaffAccess() == affectExtra:
                        toonIds.append(doId)

            # There were no Toons we could perform this Magic Word on, so let
            # the invoker know that
        ...
        # If, at this point, we still don't have any targets somehow, then let
        # the invoker know that
        if not targetIds:
            self.generateResponse(avId=avId, responseType="NoTarget")
            return
        ...
        for targetId in targetIds:
            ...
            # They should have more perms than USERS, but shouldn't be allowed
            # to target them
            targetAccess = int(round(targetToon.getStaffAccess(), -2))
            # If the Access Level of the target is greater than or equal to
            # than that of the invoker, remove them
            if targetAccess >= invokerAccess:
                targetIds.remove(targetId)
                continue

        # Function that returns a readable name in place of the Toon's Access
        # Level
        def getAccessName(accessLevel):
            return OTPGlobals.AccessLevelDebug2Name.get(
                OTPGlobals.AccessLevelInt2Name.get(accessLevel))

        # If, after the previous check, we don't have any more targets, let's
        # inform the invoker about it
        if len(targetIds) == 0:
            # If affectType is NORMAL, let the invoker know what their Access
            # Level is compared to their target
            if (affectRange in (AFFECT_OTHER, AFFECT_BOTH)
                    ) and affectType == AFFECT_NORMAL:
                ...
                # Create a nice little message to tell the invoker the
                # difference between the Access Levels
                returnValue = MAGIC_WORD_RESPONSES.get("NoAccessSingleTarget")
                returnValue = returnValue.format(
                    lastClickedToon.getName(),
                    parsedTargetAccess,
                    parsedInvokerAccess)
            ...
            # Otherwise, just let the invoker know that everyone who was
            # targeted was not allowed to be
            else:
                self.generateResponse(
                    avId=avId, responseType="NoAccessToTarget")
            return
        ...
        if toon.getStaffAccess() < OTPGlobals.AccessLevelName2Int.get(
                magicWordInfo['access']):
            self.generateResponse(avId=avId, responseType="NoAccess")
            return

        # If a config option disables cheaty Magic Words and ours is deemed
        # cheaty, let the invoker know
        ...
        # up getting used in mass. For example, you don't want to use a word
        # intended to kill a Cog Boss on other Toons
        ...
        # If we have less arguments provided than are required, let the invoker
        # know that
        messageData = "{} argument{}"
        if len(argList) < minArgs:
            messageData = messageData.format(
                minArgs, "s" if minArgs != 1 else '')
            self.generateResponse(
                avId=avId,
                responseType="NotEnoughArgs",
                extraMessageData=messageData)
            return
        # On the other hand, if we have more than what we need, tell them that
        # instead
        ...
        # If we have less arguments provided than the max, use the defaults of
        ...
        if magicWordInfo['execLocation'] == EXEC_LOC_CLIENT:
            # We are only allowed to target ourselves with client-sided Magic
            # Words
    ...
    def sendClientCommand(
        ...
        # Pack up the arg list so it's ready to ship to the client
        parsedArgList = json.dumps(parsedArgList)
        # Execute the Magic Word on the client, because it's a client-sided
        # Magic Word
        self.sendUpdateToAvatarId(avId,
                                  "executeMagicWord",
                                  [word,
