...
class DistributedBossCogAI(DistributedAvatarAI.DistributedAvatarAI):
    ...
    def avatarEnter(self):
        ...
        assert self.notify.debug(f'{self.doId}.avatarEnter({avId})')

        self.addToon(avId)

    def avatarExit(self):
        avId = self.air.getAvatarIdFromSender()
        assert self.notify.debug(f'{self.doId}.avatarExit({avId})')
    ...
    def avatarNearEnter(self):
        avId = self.air.getAvatarIdFromSender()
        assert self.notify.debug(f'{self.doId}.avatarNearEnter({avId})')
    ...
    def avatarNearExit(self):
        avId = self.air.getAvatarIdFromSender()
        assert self.notify.debug(f'{self.doId}.avatarNearExit({avId})')
    ...
    def __handleUnexpectedExit(self, avId):
        assert self.notify.debug(
            f'{self.doId}.handleUnexpectedExit({avId})')
        self.removeToon(avId)

    def addToon(self, avId):
        assert self.notify.debug(f'{self.doId}.addToon({avId})')
    ...
    def removeToon(self, avId):
        assert self.notify.debug(f'{self.doId}.removeToon({avId})')
        resendIds = 0
        try:
    ...
    def damageToon(self, toon, deduction):
        ...
        assert self.notify.debug(
            '%s. toon %s hit for %s to %s/%s' %
            (self.doId, toon.doId, deduction, toon.getHp(), toon.getMaxHp()))
        if toon.getHp() <= 0:
            assert self.notify.debug(
                f'{self.doId}. toon died: {toon.doId}')
    ...
    def healToon(self, toon, increment):
        ...
        assert self.notify.debug(
            '%s. toon %s healed to %s/%s' %
            (self.doId, toon.doId, toon.getHp(), toon.getMaxHp()))

    # setBattleExperience

    def d_setBattleExperience(self):
        assert self.notify.debug(f'{self.doId}.d_setBattleExperience()')
    ...
    def enterWaitForToons(self):
        assert self.notify.debug(f'{self.doId}.enterWaitForToons()')
    ...
    def __doneWaitForToons(self, toons):
        assert self.notify.debug(f'{self.doId}.__doneWaitForToons()')
    ...
    def enterElevator(self):
        assert self.notify.debug(f'{self.doId}.enterElevator()')
    ...
    def __doneElevator(self, avIds):
        assert self.notify.debug(f'{self.doId}.__doneElevator()')
    ...
    def enterIntroduction(self):
        assert self.notify.debug(f'{self.doId}.enterIntroduction()')
    ...
    def enterBattleOne(self):
        assert self.notify.debug(f'{self.doId}.enterBattleOne()')
    ...
    def enterReward(self):
        assert self.notify.debug(f'{self.doId}.enterReward()')
        self.resetBattles()
    ...
    def enterEpilogue(self):
        assert self.notify.debug(f'{self.doId}.enterEpilogue()')
    ...
    def enterFrolic(self):
        assert self.notify.debug(f'{self.doId}.enterFrolic()')
        self.resetBattles()
    ...
    def divideToons(self):
        ...
        # if number of toons is less than or equal to 4 than put them on the
        # same side
        if (numToons <= 4):
            numToonsB = numToons
        else:
    ...
    def handleRoundADone(self, toonIds, totalHp, deadSuits):
        if self.battleA:
            assert self.notify.debug(
                "%s. battle A round done, toonIds = %s, totalHp = %s, deadSuits = %s, suits = %s" %
                (self.doId, toonIds, totalHp, deadSuits, self.battleA.suits,))
    ...
    def handleRoundBDone(self, toonIds, totalHp, deadSuits):
        if self.battleB:
            assert self.notify.debug(
                "%s. battle B round done, toonIds = %s, totalHp = %s, deadSuits = %s, suits = %s" %
                (self.doId, toonIds, totalHp, deadSuits, self.battleB.suits,))
            self.handleRoundDone(
    ...
    def handleBattleADone(self, zoneId, toonIds):
        if self.battleA:
            assert self.notify.debug(
                f"{self.doId}. battle A done, zoneId = {zoneId}, toonIds = {toonIds}")
            self.battleA.requestDelete()
            self.battleA = None
            self.battleAId = 0
            self.sendBattleIds()
    ...
    def handleBattleBDone(self, zoneId, toonIds):
        if self.battleB:
            assert self.notify.debug(
                f"{self.doId}. battle B done, zoneId = {zoneId}, toonIds = {toonIds}")
            self.battleB.requestDelete()
            self.battleB = None
            self.battleBId = 0
            self.sendBattleIds()
    ...
    def handleRoundDone(self, battle, suits, activeSuits,
        ...
        if (len(self.reserveSuits) > 0 and len(activeSuits) < 4):
            ...
            assert(
                self.notify.debug(
                    '%s. totalHp: %d totalMaxHp: %d percent: %f' %
    ...
    def reportToonHealth(self):
        ...
        if self.notify.getDebug():
            ...
            for toonId in self.involvedToons:
                toon = self.air.doId2do.get(toonId)
                if toon:
                    str += f', {toonId} ({toon.getHp()}/{toon.getMaxHp()})'
            self.notify.debug(f'{self.doId}.toons = {str[2:]}')
    ...
    def zapToon(self, x, y, z, h, p, r, bpx, bpy, attackCode, timestamp):
        ...
        if toon:
            ...
            if damage is None:
                self.notify.warning(
                    f"No damage listed for attack code {attackCode}")
                damage = 5
    ...
    def setAttackCode(self, attackCode, avId=0):
        assert self.notify.debug(
            f'{self.doId}.setAttackCode({attackCode}, {avId})')
    ...
    def waitForNextAttack(self, delayTime):
        ...
        if (currState == 'BattleThree'):
            assert self.notify.debug(
                f"{self.doId}.Waiting {delayTime} seconds for next attack.")
            taskName = self.uniqueName('NextAttack')
            taskMgr.remove(taskName)
            taskMgr.doMethodLater(delayTime, self.doNextAttack, taskName)
        else:
            assert self.notify.debug(
                f"{self.doId}.Not doing another attack in state {currState}.")
