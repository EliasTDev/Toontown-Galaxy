...
class DistributedBossbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def announceGenerate(self):
        ...
        # mark the other collision piece set up in the base class with a pie
        # code
        ...
        self.warningSfx = loader.loadSfx(
            'phase_5/audio/sfx/Skel_COG_VO_grunt.ogg')
    ...
    def enterPrepareBattleTwo(self):
        ...
        # remove the disguise, so we can quickly get here riding up the
        # elevator
    ...
    def putFoodOnToon(self, avId, beltIndex, foodNum):
        ...
        if av:
            ...
            # when doing toon standing holding food, and toon moving holding
            # food
            ...
            self.notify.debug(f'curAnim={curAnim}')
            if curAnim in ('walk', 'run'):
                ...
                self.notify.warning(
                    f"don't know what to do with anim={curAnim}")
    ...
    def localToonTouchedChair(self, tableIndex, chairIndex):
        ...
        if (avId in self.toonFoodStatus) and \
           self.toonFoodStatus[avId] is not None:
   ...
   #####  BattleThree state #####
    ...
    def enterPrepareBattleFour(self):
        ...
        assert self.notify.debug(f'{self.doId}.enterPrepareBattleFour()')
        self.controlToons()
    ...
    def enterBattleFour(self):
        ...
        assert self.notify.debug(f'{self.doId}.enterBattleFour()')
        self.releaseToons(finalBattle=1)
        self.setToonsToNeutral(self.involvedToons)
        # remove the disguise, so we can quickly get here riding up the
        # elevator
    ...
    def doDirectedAttack(self, avId, attackCode):
        ...
        if toon:
            ...
            gearTrack = Parallel()
            for i in range(4):
                nodeName = f'{str(i)}-{globalClock.getFrameTime()}'
    ...
    def setAttackCode(self, attackCode, avId=0):
        ...
        assert self.notify.debug(
            f"setAttackCode({attackCode}, {avId}) time={globalClock.getFrameTime():f}")
    ...
    def closeEnter(self, colEntry):
        ...
        if tableStr:
            ...
            # Note we could hit a different table than the one were were going
            # to.
    ...
    def moveBossToPoint(self, fromPos, fromHpr, toPos, toHpr, reverse):
        ...
        self.notify.debug(f"self.distanceToTravel = {self.distanceToTravel}")
    ...
    def zapLocalToon(self, attackCode, origin=None):
        ...
        self.notify.debug(
            f'zapLocalToon frameTime={globalClock.getFrameTime()}')
        messenger.send('interrupt-pie')
    ...
    def isToonRoaming(self, toonId):
        result = not self.isToonOnTable(
            toonId) and not self.isToonOnGolfSpot(toonId)
        return result
    ...
    def doGolfAttack(self, avId, attackCode):
        ...
        if toon:
            distance = toon.getDistance(self)
            self.notify.debug(f'distance = {distance}')
            gearRoot = self.rotateNode.attachNewNode(
                'gearRoot-atk%d' % self.numAttacks)
            gearRoot.setZ(10)
            gearRoot.setTag('attackCode', str(attackCode))
            ...
            # gearRoot.attachNewNode('ballLaunch')
    ...
    def doGolfAreaAttack(self):
        ...
        self.notify.debug(f'toToonH = {toToonH}')
    ...
    def doOvertimeAttack(self, index):
        ...
        self.notify.debug(
            f'attackBelts duration= {attackBelts.getDuration():.2f}')
