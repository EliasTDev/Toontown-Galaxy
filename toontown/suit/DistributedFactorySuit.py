...
class DistributedFactorySuit(DistributedSuitBase.DistributedSuitBase,
    ...
    def setLevelDoId(self, levelDoId):
        self.notify.debug(f"setLevelDoId({levelDoId})")
    ...
    def setPath(self):
        self.notify.debug(f"setPath {self.doId}")
    ...
    def __handleToonAlert(self, collEntry):
        ...
        self.notify.debug(f"{self.doId}: ahah!  i saw you")
    ...
    def startChaseTask(self, delay=0):
        self.notify.debug(
            f'DistributedFactorySuit.startChaseTask delay={delay}')
        taskMgr.remove(self.taskName('chaseTask'))
    ...
    def chaseTask(self, task):
        ...
        if not av:
            self.notify.warning(f"avatar {self.chasing} isn't here to chase")
            return Task.done
