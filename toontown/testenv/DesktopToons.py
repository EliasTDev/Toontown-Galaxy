...
background = loader.loadModel(
    'phase_3/models/gui/loading-background').find("**/bg")
...
for i in range(5):
    ...
    t.loop('neutral')
    t.reparentTo(render)
    t.setPosHpr((random.random() * 20.0) - 10.0, 40, 0,
                150.0 + (random.random() * 60), 0, 0)
    t.useLOD(1000)
    toonList.append(t)

for i in range(len(toonList)):
    t = toonList[i]
    t.setPosHpr((i * 5.0) - 10.0, 32 + random.random() * 5.0, -
                0.25, 120.0 + (random.random() * 90), 0, 0)
