...
class TocTalk:
    ...
    def build_funcs(self):
        ...
        for item in dir(self.__class__):
            if (isinstance(eval(f"self.{item}"), type(self.__init__)) and
                    item[:3] == "on_"):
    ...
    def start_log_in(self):
        ...
        msg = struct.pack(
            "!HHHH", 0, 1, 1, len(
                self._normnick)) + self._normnick
        self.flap_to_toc(1, msg)

        #now, login
        self.flap_to_toc(
            2,
            "toc_signon %s %s %s %s english %s" %
            (TOC_SERV_AUTH[0],
             TOC_SERV_AUTH[1],
             self._normnick,
             ep,
    ...
    def encode(self, data):
        for letter in "\\(){}[]$\"":
            data = data.replace(letter, f"\\{letter}")
        return '"' + data + '"'
    ...
    def flap_to_toc(self, ftype, msg):
        ...
        if len(data) >= 2048:
            raise TOCError(
                "TOC data with protocol overhead cannot exceed 2048 bytes.")

        self.derror(f"SEND : '{data!r}'")
    ...
    def pwdenc(self):
        ...
        for letter in self._passwd:
            ept.append(f"{ord(letter) ^ ord(lookup[x % 7]):02x}")
            x = x + 1
        return "0x" + "".join(ept)
    ...
    def handle_event(self, event):
        ...
        # their imp
        if (f"on_{id}") in self._dir:
            exec(f"self.on_{id}(data)")

        else:
            self.werror(f"INFO : Received unimplemented '{id}' id")
    ...
    def err_disconnect(self):
        ...
        raise TOCDisconnectError(
            "FATAL: We seem to have been disconnected from the TOC server.\n")
    ...
    def c_ERROR(self, id, data):
        ...
        if dt == 980:
            raise TOCError(
                "FATAL: Couldn't sign on; Incorrect nickname/password combination")

        elif dt == 981:
            raise TOCError(
                "FATAL: Couldn't sign on; The AIM service is temporarily unavailable")

        elif dt == 982:
            raise TOCError(
                "FATAL: Couldn't sign on; Your warning level is too high")

        elif dt == 983:
            raise TOCError(
                "FATAL: Couldn't sign on; You have been connecting and disconnecting too frequently")

        elif dt == 989:
            raise TOCError(
                "FATAL: Couldn't sign on; An unknown error occurred")
        ...
        # ... etc etc etc
            ...
            if (f"on_{id}") in self._dir:
                ...
                self.werror(
                    f"ERROR: The TOC server sent an unhandled error string: {data}")

    def c_SIGN_ON(self, type, data):
        self.flap_to_toc(
            2, "toc_add_buddy %s" %
            self.normalize(
                self._nick))  # needs to start up corectly
        self.flap_to_toc(2, f"toc_set_info {self.encode(self._info)}")
    ...
    def do_SEND_IM(self, user, message, autoaway=0):
        sendmessage = f"toc_send_im {self.normalize(user)} {self.encode(message)}"
        if autoaway:
            sendmessage = sendmessage + " auto"
        self.flap_to_toc(2, sendmessage)

    def do_ADD_BUDDY(self, buddies):
        self.flap_to_toc(2, f"toc_add_buddy {self.normbuds(buddies)}")

    def do_ADD_PERMIT(self, buddies):
        self.flap_to_toc(2, f"toc_add_permit {self.normbuds(buddies)}")

    def do_ADD_DENY(self, buddies):
        self.flap_to_toc(2, f"toc_add_deny {self.normbuds(buddies)}")

    def do_REMOVE_BUDDY(self, buddies):
        self.flap_to_toc(2, f"toc_remove_buddy {self.normbuds(buddies)}")
    ...
    def do_SET_AWAY(self, awaymess):
        ...
        self.flap_to_toc(2, f"toc_set_away {self.encode(awaymess)}")

    def do_GET_INFO(self, user):
        self.flap_to_toc(2, f"toc_get_info {self.normalize(user)}")

    def do_SET_INFO(self, info):
        self.flap_to_toc(2, f"toc_set_info {self.encode(info)}")
    ...
    def do_EVIL(self, user, anon=0):
        ...
        self.flap_to_toc(2, f"toc_evil {self.normalize(user)} {acode}")

    # chat
    def do_CHAT_INVITE(self, room, imess, buddies):
        self.flap_to_toc(
            2,
            "toc_chat_invite %s %s %s" %
            (self.normalize(room),
             self.encode(imess),
             self.normbuds(buddies)))

    def do_CHAT_ACCEPT(self, id):
        self.flap_to_toc(2, f"toc_chat_accept {id}")

    def do_CHAT_LEAVE(self, id):
        self.flap_to_toc(2, f"toc_chat_leave {id}")

    def do_CHAT_WHISPER(self, room, user, message):
        self.flap_to_toc(2, "toc_chat_whisper %s %s %s" %
                         (room, self.normalize(user), self.encode(message)))

    def do_CHAT_SEND(self, room, message):
        self.flap_to_toc(2, f"toc_chat_send {room} {self.encode(message)}")

    def do_CHAT_JOIN(self, roomname):
        self.flap_to_toc(2, f"toc_chat_join 4 {roomname}")

    def do_SET_CONFIG(self, configstr):
        self.flap_to_toc(2, f"toc_set_config \"{configstr}\"")

    # error funcs
    def werror(self, errorstr):
        if self._debug:
            self._logfd.write(f"({self._nick}) {errorstr}\n")
