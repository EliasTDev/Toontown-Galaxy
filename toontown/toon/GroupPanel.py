...
class GroupPanel(DirectObject.DirectObject):
    ...
    def cleanup(self):
        ...
        screen, unanswered.
        """
        # Releasing the left cells of the screen so that chat msgs and arrows
        # can appear there again.
    ...
    def __load(self):
        ...
        # Blocking the left cells of the screen so that chat msgs and arrows
        # don't block the gui.
    ...
    def __checkGroupStatus(self):
        ...
        self.notify.debug(
            ...
            (self.boardingParty.getGroupMemberList(
                localAvatar.doId)))
        myMemberList = self.boardingParty.getGroupMemberList(localAvatar.doId)

        # Remove and then add all items from the list because the list might
        # have changed.
    ...
    def __getAvatarButton(self, avId):
        ...
        if toon.battleId:
            ...
            if (avId == self.leaderId):
                # Toon is the group leader - change image to buttonImage to
                # leaderImage
    ...
    def changeCancelToGo(self):
        ...
        if self.goButton and not self.goButton.isEmpty():
            ...
            self.goButton['text'] = (
                '', TTLocalizer.BoardingGo, TTLocalizer.BoardingGo, '')
