...
WantNewsPage = base.config.GetBool('want-news-page', 0)
if WantNewsPage:
...
class LocalToon(DistributedToon.DistributedToon, LocalAvatar.LocalAvatar):
    ...
    def __init__(self, cr):
        ...
        try:
            ...
            self.acceptingNewFriends = base.settings.getBool(
                'game', 'acceptingNewFriends', True) and base.config.GetBool(
    ...
    def initInterface(self):
        ...
        # self.addTIPPage()
        # TODO have a check on the ai server to make sure they aren't hacking
        # this value
        if self.getStaffAccess() >= 200:
            self.magicWordHelpPage = MagicWordsHelpPage()
            self.magicWordHelpPage.load()
            self.book.addPage(
                self.magicWordHelpPage,
                pageName='Magic Words Help Page')
    ...
    def loadDisguisePages(self):
        ...
        if self.disguisePage is not None or self.sosPage is not None:
...
#        LocalAvatar.LocalAvatar.displayWhisperPlayer(self, fromId, chatString, whisperType)
    ...
    def setAsGM(self, state):
        self.notify.debug(f"Setting GM State: {state} in LocalToon")
    ...
    def showClarabelleGui(self, mailboxItems):
        ...
        if not self.mailboxNotify and not self.awardNotify and (
            self.catalogNotify == ToontownGlobals.OldItems) and (
            (self.simpleMailNotify != ToontownGlobals.NoItems) or (
    ...
    def setFurnitureDirector(self, avId, furnitureManager):
        ...
        if avId == 0:
            ...
            if self.furnitureManager is None or \
    ...
    def getAvPosStr(self):
        ...
        if (hasattr(base.cr.playGame.hood, "loader") and
            hasattr(base.cr.playGame.hood.loader, "place") and
                (base.cr.playGame.getPlace() is not None)):
            zoneId = base.cr.playGame.getPlace().getZoneId()
        else:
            zoneId = "?"
        strPosCoordText = f"X: {pos[0]:.3f}" + f", Y: {pos[1]:.3f}" + \
                          f"\nZ: {pos[2]:.3f}" + f", H: {hpr[0]:.3f}" + \
                          f"\nZone: {str(zoneId)}" + f", Ver: {serverVersion}, " + \
...
#        import pdb; pdb.set_trace()
                   ...
                   self.awardNotify == ToontownGlobals.NewItems:
                    ...
                    for quest in self.quests:
                        if quest[0] in Quests.PreClarabelleQuestIds and (
                            (self.mailboxNotify != ToontownGlobals.NewItems) and (
                                self.awardNotify != ToontownGlobals.NewItems)):
                            showClarabelle = 0
                    if showClarabelle:
        ...
        if self.moveFurnitureButtonObscured <= 0:
            if self.furnitureManager is not None and \
    ...
    def newCatalogNotify(self):
        ...
        if ((self.simpleMailNotify == ToontownGlobals.NewItems) or (
                self.inviteMailNotify == ToontownGlobals.NewItems)):
            oldStr = ''
            if message:
                oldStr = message[0] + ' '
    ...
    def loadGardeningGui(self):
        ...
        self.basketText = f"{self.numFlowers} / {self.maxFlowerBasket}"
    ...
    def showShovelButton(self, add=0):
        ...
        self.notify.debug(f"showing shovel {self.shovelButtonActiveCount}")
        self.__gardeningGui.show()
    ...
    def hideShovelButton(self, deduct=0):
        ...
        self.notify.debug(f"hiding shovel {self.shovelButtonActiveCount}")

    # the watering buttons needs to be merged with Pappy's code, potential
    # conflict here
    ...
    def levelWater(self, change=1):
        ...
        if change < 1:
            ...
            changeString = (
                f"+{change} {TTLocalizer.GardeningWaterSkill}")
        # self.holdWateringCanText =
        # TTLocalizer.GardeningWater#self.wateringCanButton["text"]
        self.waterTrack = Sequence(
            ...
            SoundInterval(globalBattleSoundCache.getSound(
                'GUI_balloon_popup.ogg'), node=self),
            Wait(1.0),
    ...
    def levelShovel(self, change=1):
        ...
        if change < 1:
            ...
            changeString = (
                f"+{change} {TTLocalizer.GardeningShovelSkill}")
    ...
    def removeShovelRelatedDoId(self, doId):
        # the if check is important, since we may have gotten the enter before
        # the exit
    ...
    def setPlantToWater(self, plantId):
        ...
        if self.plantToWater is None:
            ...
            self.notify.debug(f"setting plant to water {plantId}")
    ...
    def setGardenStarted(self, bStarted):
        ...
        if self.gardenStarted and (
                not self.gardenPage) and hasattr(
                self,
                "book"):
            self.loadGardenPages()
    ...
    def b_setAnimState(
        ...
        if self.wantStatePrint:
            print(f"Local Toon Anim State {animName}")
    ...
    def setSleepAutoReply(self, fromId):
        ...
        if isinstance(av, DistributedToon.DistributedToon):
            ...
            self.notify.warning(f'setSleepAutoReply from non-toon {fromId}')
