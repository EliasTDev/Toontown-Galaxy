...
class NPCDialogue:
    ...
    def __init__(self, participant, dialogueTopic):
        ...
        if participant and isinstance(
            ...
            self.notify.warning(f"Participant does not exist: {participant}")
            self.participantProgress = 0
    ...
    def calcMaxNumMsgs(self):
        ...
        for participant, spiel in list(self.conversation.items()):
            if len(
                    spiel) > self.maxNumMsgs and participant[1] in self.participants:
                ...
                if partPos[1] == participant.npcId:
                    if not (participant.npcId in self.participants):
                        ...
                        if participant not in self.participants[participant.npcId]:
                            self.participants[participant.npcId].append(
                                participant)
                        else:
                            self.notify.warning(
                                "Participant: %s already in the conversation" %
                                participant)
                    self.calcMaxNumMsgs()
                    return True
            self.notify.warning(
                f"Participant: {participant} should not be in conversation")
        return False
    ...
    def calcNextParticipant(self):
        ...
        if nextPos > len(self.conversation):
            ...
            self.participantProgress = (
                self.participantProgress + 1) % self.maxNumMsgs
        for partPos in list(self.conversation.keys()):
    ...
    def __blather(self, task):
        ...
        for participant in self.participants[self.currentParticipant]:
            ...
            participant.sendUpdate(
                "setChat", [
                    self.topic, convKey[0], convKey[1], self.participantProgress, chatFlags])
