...
class RobotToonManager(DirectObject):
    def __init__(self, toonParent=None):
        ...
        self.namePlusIds = [
            (TTLocalizer.NPCToonNames.get(
                id,
                'Mystery NPC'),
                id) for id in self.toonIds]
...
# action = self.pieMenuCommand)
    ...
    def makeRandomToon(self, toNpcId=None):
        ...
        if self.avatarType == 't':
            ...
            if self.doodleHead is not None:
                desc[0] = self.doodleHead
            if self.doodleEars is not None:
                desc[1] = self.doodleEars
            if self.doodleNose is not None:
                desc[2] = self.doodleNose
            if self.doodleTail is not None:
                desc[3] = self.doodleTail
            if self.doodleBody is not None:
                desc[4] = self.doodleBody
            # not sure why, but None didn't work as a cardinal value in these
            # cases
            ...
            if self.doodleGender is not None:
    ...
    def setStartPos(self, xy=None):
        ...
        # If we got a valid intersection point
        if entry and (self.selectedToon is not None):
    ...
    def parseAvatarProperties(self, line):
        ...
        if type == 't':
            ...
            gender = line[i]
            i += 1
            armColor = string.atoi(line[i])
            i += 1
            gloveColor = string.atoi(line[i])
            i += 1
            legColor = string.atoi(line[i])
            i += 1
            headColor = string.atoi(line[i])
            i += 1
            topTexture = string.atoi(line[i])
            i += 1
            topTextureColor = string.atoi(line[i])
            i += 1
            sleeveTexture = string.atoi(line[i])
            i += 1
            sleeveTextureColor = string.atoi(line[i])
            i += 1
            bottomTexture = string.atoi(line[i])
            i += 1
            bottomTextureColor = string.atoi(line[i])
            i += 1
        ...
        x = string.atof(line[i])
        i += 1
        y = string.atof(line[i])
        i += 1
        z = string.atof(line[i])
        i += 1
        h = string.atof(line[i])
        i += 1
        p = string.atof(line[i])
        i += 1
        r = string.atof(line[i])
        i += 1
        x1 = string.atof(line[i])
        i += 1
        y1 = string.atof(line[i])
        i += 1
        z1 = string.atof(line[i])
        i += 1
        h1 = string.atof(line[i])
        i += 1
        p1 = string.atof(line[i])
        i += 1
        r1 = string.atof(line[i])
        i += 1
        x2 = string.atof(line[i])
        i += 1
        y2 = string.atof(line[i])
        i += 1
        z2 = string.atof(line[i])
        i += 1
        h2 = string.atof(line[i])
        i += 1
        p2 = string.atof(line[i])
        i += 1
        r2 = string.atof(line[i])
        i += 1
        state = line[i]
        return (type, props,
                Point3(x, y, z),
        ...
        if tcfFilename:
            ...
            for t in list(self.avatarDict.values()):
                ...
                if type == 't':
                    ...
                    style = t.style
                    styleStr = f"{style.body}*{style.dept}*{style.name}"
                pos = t.getPos()
                hpr = t.getHpr()
                ...
                state = t.state
                f.write(f"{type}*{styleStr}*{pose}*{startPose}*{endPose}*{state}\n")
            f.close()
    ...
    def nextAnim(self, modifiers=0):
        ...
        if self.selectedToon and modifiers == 0:
            ...
            try:
                i = self.animPanel.actorControlList[0]['animList'].index(
                    currState)
            ...
            if (newIndex) < len(ToonAnimList):
                self.setToonAnimState(
                    self.animPanel.actorControlList[0]['animList'][newIndex])
            else:
                self.setToonAnimState(
                    self.animPanel.actorControlList[0]['animList'][0])
...
class RobotToonControlPanel(AppShell):
    ...
    def appInit(self):
        ...
        self.maleBottomsNames = [BoyBottomsDict[x]
                                 for x in self.maleBottomsKeys]
        ...
        self.femaleSkirtsNames = [GirlBottomsDict[x]
                                  for x in self.femaleSkirtsKeys]
        ...
        self.femaleShortsNames = [GirlBottomsDict[x]
                                  for x in self.femaleShortsKeys]
    ...
    def createInterface(self):
        ...
        for prop in list(globalPropPool.propTypes.keys()):
            self.props.add_command(
                label=prop, command=lambda selectProp=prop: self.useProp(selectProp))
        ...
        for anim in DoodleAnimList:
            if (animIndex % 10) == 0:
                ...
                self.doodleAnimMenu.add_cascade(
                    label=DoodleAnimList[animIndex], menu=subMenu)
            subMenu.add_command(
                label=anim,
    ...
    def toggleSnow(self):
        if self.snow:
            ...
            # start the snow slightly above the camera
    ...
    def firework(self):
        ...
        show = FireworkShows.getShow(
            holidays[self.holiday.get()], self.fireworkType.get())
    ...
    def createTestToon(self):
        ...
        dna.newToonFromProperties(
            "dsl", "ms", "m", "m", 15, 0, 15, 15, 0, 0, 0, 0, 0, 0,)
        self.rtm.makeToonFromProperties(
            dna.asTuple(), pos=Point3(0), hpr=Point3(
                180, 0, 0), startPos=Point3(0), startHpr=Point3(0), endPos=Point3(
                0, 1, 0), endHpr=Point3(0), state='neutral')
        self.updateToonInfo()
    ...
    def resizeWindow(self):
        text = askstring(
            'New window size',
            'Type new window size such as 800x600\n',
            parent=self.component('hull'))
        tokens = text.split('x')
        if len(tokens) != 2:
            return
