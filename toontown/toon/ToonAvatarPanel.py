...
class ToonAvatarPanel(AvatarPanelBase.AvatarPanelBase):
    ...
    def __init__(self, avatar, playerId=None):
        ...
        # The friends button is diabled for transient (player only) friends
        # that are not in our cr
        if base.cr.playerFriendsManager.askTransientFriend(
                self.avId) and self.avId not in base.cr.doId2do:
    ...
    def __handleToPet(self):
        ...
        # if we have the friend in our cr it's more up to date and we should
        # get the pet from that.
    ...
    def __petDetailsLoaded(self, avatar):
        ...
        if avatar:
            self.notify.debug(
                f"Looking at someone's pet. pet doId = {avatar.doId}")
    ...
    def __handleSecrets(self):
        base.localAvatar.chatMgr.noWhisper()
        # grep keywords: BothSecrets AvatarSecret AccountSecret secretType
        # switchboard
    ...
    def __updateHp(self, hp, maxHp, quietly=0):
        if self.laffMeter is not None and hp is not None and maxHp is not None:
    ...
    def handleKick(self):
        # Show the confirm dialog only if the toon is not already in the
        # elevator.
    ...
    def __checkGroupStatus(self):
        ...
        if hasattr(self, "avatar"):
            if self.avatar and (hasattr(self.avatar, "getZoneId")) and (
                    localAvatar.getZoneId() == self.avatar.getZoneId()):
    ...
    def handleReadInfo(self, task=None):
        ...
        self.boardingInfoText = TTDialog.TTDialog(
            ...
            text=(
                TTLocalizer.BoardingPartyInform %
    ...
    def __makePetGui(self, avatar):
        ...
        self.petButton = DirectButton(
            ...
            relief=None,
            pos=(0.22, -0.2, -0.475),
            text=("", TTLocalizer.AvatarPanelPet,
