...
# unlikely, but just in case, he won 30 awards and their still in his
# onAwardOrder list
P_OnAwardOrderListFull = -25
P_AwardMailboxFull = -24  # the award mailbox is full and can't take more
# trying to give a pet trick award but the toon has it in his pet tricks
P_ItemInPetTricks = -23
# trying to give a speed chat award but the toon has it in his My Phrases
P_ItemInMyPhrases = -22
P_ItemOnAwardOrder = -21  # trying to give an award but the toon has it onAwardOrder,
# trying to give an award but the toon has it in his award mailbox
P_ItemInAwardMailbox = -20
# trying to give an award but the toon is already wearing the clothing item
P_ItemAlreadyWorn = -19
P_ItemInCloset = -18  # trying to give an award but the toon has it in his closet
# trying to give an award but the toon has it in the gift order queue
...
MinigamePlayerMatrix = {
    ...
    # Technically pattern game can be single player, but it is not nearly as
    # fun, especially for demos
...
# Note - these have gone up since the credit is based on the size of the
# building now
TrophyStarLevels = (
    10,  # A bronze star
...
# Pumpkins will disappear as soon as the holiday ends
TOT_REWARD_END_OFFSET_AMOUNT = 0
# TOT_REWARD_END_OFFSET_AMOUNT = 60*60*24*7 # a week's worth of seconds to
# be added to the end time of the holiday
...
# first number is time to go down, 2nd is time to go up, 3rd number is how
# long it stays down
...
# change relative heading where the gavel lands, but give it a pattern so
# the observant player can predict where it lands
...
# when the lawyers throw stuff on the prosecution pan, how much damage to
# heal the boss
...
# ammo count, how many gavels, how many lawyers, how much toonup, do
# jurors influence evidence weight, cog bonus subtractor (e.g. if this
# value is 1 and you seated 2 jurors), 2-1 leads to 1 as your bonus weight
...
# num tables, diners per table, level of diners, table unflatten time,
# hungry duration, eating duration
