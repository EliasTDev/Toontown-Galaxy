...
class TeaserPanel(DirectObject):
    ...
    def showPage(self, pageName):
        if pageName not in PageOrder:
            self.notify.error(f"unknown page '{pageName}'")

        # log velvet rope hits
        base.cr.centralLogger.writeClientEvent(f'velvetRope: {pageName}')
        ...
        if ConfigVariableBool('want-teaser-scroll-keys', 0).value:
