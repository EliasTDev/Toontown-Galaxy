...
class ToontownLoadingBlocker(TTDialog.TTDialog):
    ...
    def __hideBlocker(self):
        ...
        """
        self.hide()
        # Show the smaller loading text at the left bottom corner when the
        # blocker panel is hidden.
    ...
    def __showBlocker(self):
        ...
        """
        self.show()
        # Hide the smaller loading text at the left bottom corner when the
        # blocker panel is up.
        if self.__isValidDownloadBar():
    ...
    def __createLoadingText(self):
        ...
        # Start a task to change the loading text every loadingTextChangeTimer
        # seconds.
    ...
    def __shouldShowBlocker(self, avList):
        ...
        if hasPlayableToon(avList):
            ...
            if not (base.launcher.getPhaseComplete(3.5)
                    and base.launcher.getPhaseComplete(4)):
                return True
        return False
    ...
    def __shrinkLoadingBar(self):
        if self.__isValidDownloadBar():
            ...
            barPosIval = LerpPosInterval(
                self.bar, ivalDuration, (-0.81, 0, -0.96))
