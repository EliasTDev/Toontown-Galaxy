...
class DataStore:
    ...
    def __init__(self, filepath, writePeriod=300, writeCountTrigger=100):
        ...
        if self.wantAnyDbm:
            ...
            self.notify.debug(
                f'anydbm default module used: {dbm._defaultmod.__name__} ')
        ...
        if self.wantAnyDbm:
            try:
                if os.path.exists(self.filepath):
                    ...
                    self.notify.debug(
                        f'Opening existing anydbm database at: {self.filepath}.')
                else:
            ...
            try:
                ...
                try:
                    ...
                    file = None
                    self.notify.debug(
                        f'New pickle data file will be written to {self.filepath}.')
            if file:
    ...
    def writeDataToFile(self):
        ...
        if self.data is not None:
            self.notify.debug(f'Data is now synced with disk at {self.filepath}')
            if self.wantAnyDbm:
                self.data.sync()
            else:
                try:
    ...
    def close(self):
        ...
        if self.data is not None:
            ...
            if self.wantAnyDbm:
                self.data.close()
            taskMgr.remove(f'{self.className}-syncTask')
            self.data = None
    ...
    def open(self):
        ...
        taskMgr.remove(f'{self.className}-syncTask')
        t = taskMgr.add(self.syncTask, f'{self.className}-syncTask')
        t.timeElapsed = 0.0
            ...
            if os.path.exists(self.filepath):
                try:
                    ...
                    uber.air.writeServerEvent(
                        ...
                        0,
                        f'Unable to create backup of file: {tail} ')
            else:
                    ...
                    if file.find(tail) > -1:
                        ...
                        try:
                            ...
                            uber.air.writeServerEvent(
                                'Uberdog data store Info',
                                0,
                                f'Unable to create backup of file: {newFileName} ' +
                                ext)
        else:
