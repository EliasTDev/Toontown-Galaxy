    ...
    def finish(self):
        ...
        if self.gotAvatars:
            ...
            for doId in pirateAvatarsIds:
                if doId:
                    name = self.neededObjects.get(f"setName-{doId}")
                    dna = self.neededObjects.get(f"setDNAString-{doId}")
            ...
            for avatarId in pirateAvatarsIds:
                if avatarId:
                    self.gotAvatars = 1
                    self.neededObjects[f"setName-{avatarId}"] = None
                    self.neededObjects[f"setDNAString-{avatarId}"] = None
            if self.gotAvatars:
                ...
                for avatarId in pirateAvatarsIds:
                    if avatarId and hasattr(self, "air"):
                        self.askForObjectField(
                            ...
                            "setName", avatarId, f"setName-{avatarId}")
                        self.askForObjectField(
                            "DistributedPlayerPirateUD",
                            "setDNAString",
                            avatarId,
                            f"setDNAString-{avatarId}")
                return
            else:
