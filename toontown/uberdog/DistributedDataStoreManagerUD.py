...
class DistributedDataStoreManagerUD(DistributedObjectGlobalUD):
    ...
    enableDestroyStore = simbase.config.GetBool(
        'enable-destroy-data-stores', True)
    ...
    def __getFilePath(self, storeId):
        ...
        return f'{self.serverDataFolder}/TUDS-{repr(storeId)}'

    def __str__(self):
        outStr = self.__class__.__name__ + ' - ' + \
            repr(len(self.stores)) + ' Stores\n'
        outStr += '-' * 40 + '\n'
    ...
    def startStore(self, storeId):
        ...
        if not self.stores.get(storeId, None):
            if storeClass is not None:
                ...
                self.notify.debug(
                    'DataStore type \'%d\' not found DataStoreGlobals. Store not started.' %
                    (storeId,))
        else:
            self.notify.debug(f'{storeClass.__name__} already present on uberdog.')
    ...
    def stopStore(self, storeId):
        ...
        if store:
            if self.enableDestroyStore:
                self.notify.debug('Destroying %s.' %
                                  repr(DataStoreGlobals.getStoreClass(storeId)))
                store.destroy()
            else:
                self.notify.debug('Closing %s.' %
                                  repr(DataStoreGlobals.getStoreClass(storeId)))
                store.close()
        else:
            self.notify.debug('%s not present on uberdog.' %
                              repr(DataStoreGlobals.getStoreClass(storeId)))
    ...
    def queryStore(self, storeId, query, retry=False):
        ...
        if store:
            ...
            if retry:
                ...
                if storeClass:
                    self.notify.debug(
                        '%s not present on uberdog. Query dropped.' %
                        (storeClass.__name__,))
                else:
                    self.notify.debug(
                        'Store of typeId %s not defined in DataStoreGlobals. Query dropped.' %
                        (storeId,))
            else:
