...
class AddGiftRequestFR(AsyncRequest):
    ...
    def finish(self):
        ...
        giftItem = CatalogItemList.CatalogItemList(
            self.newGift, store=CatalogItem.Customization | CatalogItem.DeliveryDate)
        giftList = CatalogItemList.CatalogItemList(
            giftBlob, store=CatalogItem.Customization | CatalogItem.DeliveryDate)
        giftList.append(giftItem[0])
        giftBlob = giftList.getBlob(
            CatalogItem.Customization | CatalogItem.DeliveryDate)
        self.distObj.writeGiftFR(
            self.avatarId,
            giftBlob,
            self.replyToChannelId,
            self.senderId,
            self.context,
            self.retcode)
        AsyncRequest.finish(self)
...
def AccumRATBeans(newGift, GiftListBlob):
    giftList = CatalogItemList.CatalogItemList(
        GiftListBlob, store=CatalogItem.Customization | CatalogItem.DeliveryDate)
    ...
    if newGift.giftCode == ToontownGlobals.GIFT_RAT:
        ...
        for index in range(len(giftList)):
            if(giftList[index].giftCode == ToontownGlobals.GIFT_RAT and found == 0):
                found = 1
                giftList[index].beanAmount = numBeans + \
                    giftList[index].beanAmount
...
class DeliverGiftRequest(AsyncRequest):
    ...
    def finish(self):
        ...
        giftBlob = remaining.getBlob(
            CatalogItem.Customization | CatalogItem.DeliveryDate)
        self.distObj.writeGiftField(
            self.avatarId, giftBlob, self.replyToChannelId)
        AsyncRequest.finish(self)
...
class PurchaseGiftRequest(AsyncRequest):
    ...
    def checkCatalog(self, retcode):
        ...
        if self.item in sAv.monthlyCatalog:
            ...
            self.air.writeServerEvent(
                'suspicious',
                sAv.doId,
                f'purchaseItem {self.item} not in catalog')
            self.notify.warning(
                "Avatar %s attempted to purchase %s, not on catalog." %
                (sAv.doId, self.item))
            self.notify.warning(
                f"Avatar {sAv.doId} weekly: {sAv.weeklyCatalog}")
    ...
    def checkMoney(self, retcode):
        ...
        if price > sAv.getTotalMoney():
            self.air.writeServerEvent(
                'suspicious',
                sAv.doId,
                f'purchaseItem {self.item} not enough money')
            self.notify.warning(
                "Avatar %s attempted to purchase %s, not enough money." %
                (sAv.doId, self.item))
    ...
    def checkMailbox(self, retcode):
        sAv = self.neededObjects[self.senderId]
        rAv = self.neededObjects[self.receiverId]
        if len(rAv.mailboxContents) + \
                len(rAv.onGiftOrder) >= ToontownGlobals.MaxMailboxContents:
    ...
    def finish(self):
        ...
        sAv = self.neededObjects[self.senderId]
        rAv = self.neededObjects[self.receiverId]
        self.item = CatalogItem.getItem(
            self.itemBlob, store=CatalogItem.Customization)
        retcode = None
        # put neat stuff here
        # -----------------------------------------------------------------------------
        ...
        if (retcode is not None):
            self.distObj.sendUpdateToChannel(
                self.replyToChannelId, "receiveRejectPurchaseGift", [
...
class DistributedDeliveryManagerUD(DistributedObjectGlobalUD):
    ...
    def __init__(self, air):
        ...
        # {} #cache for names from the database
        self.avatarIdToName = LRUlist(8192)
        # {} #cache for gifts from the database
        self.avatarIdToGifts = LRUlist(8192)
    ...
    def giveRecruitAToonPayment(self, receiverId, amount=1):
        ...
        itemBlob = testItem.getBlob(store=CatalogItem.Customization)
        testBlob = CatalogItemList.CatalogItemList(
            itemBlob, store=CatalogItem.Customization)
        #import pdb; pdb.set_trace()
        myGiveItem = GiveItem(self, replyToChannel, receiverId, itemBlob)

    def receiveRequestPurchaseGift(
            self,
            giftBlob,
            receiverId,
            senderId,
        ...
        # this is where the message gets sent back to, in this case it is the
        # calling Client
        replyToChannelAI = self.air.getSenderReturnChannel()
        # senderId = self.air.getAvatarIdFromSender()
        myPGR = PurchaseGiftRequest(
            self,
            replyToChannelAI,
            senderId,
            receiverId,
    ...
    def addGiftFR(
        ...
        giftItem = CatalogItemList.CatalogItemList(
            newGift, store=CatalogItem.Customization | CatalogItem.DeliveryDate)
        self.air.writeServerEvent(
            "Adding Gift", doId, "sender %s receiver %s gift %s" %
            (senderId, doId, giftItem[0].getName()))
        if giftBlob is None:
            # if not in our cache
            myAddGiftRequestFR = AddGiftRequestFR(
                self, replyToChannelId, doId, newGift, senderId, context, retcode)
        else:
    ...
    def addGift(self, doId, newGift, replyToChannelId):
        ...
        giftItem = CatalogItemList.CatalogItemList(
            newGift, store=CatalogItem.Customization | CatalogItem.DeliveryDate)
        self.air.writeServerEvent(
            "Adding Server Gift", doId, "receiver %s gift %s" %
            (doId, giftItem[0].getName()))
        if giftBlob is None:
            # if not in our cache
    ...
    def deliverGifts(self, avId, time):
        ...
        if giftListBlob is None:
            # if not in our cache
            myAddGiftRequest = DeliverGiftRequest(
                self, replyToChannelId, avId, time)
        else:
            giftList = CatalogItemList.CatalogItemList(
                giftListBlob, store=CatalogItem.Customization | CatalogItem.DeliveryDate)
    ...
    def writeGiftField(self, avatarId, giftBlob, replyToChannelId):
        ...
        """
        if 1 == 1:  # case where it works
            self.avatarIdToGifts.putData(
                avatarId, giftBlob)  # update the cache
            self.air.sendUpdateToDoId(
                "DistributedToon",
                "setGiftSchedule", avatarId, [giftBlob])
            # update the database
            self.sendUpdateToChannel(
                replyToChannelId, "receiveAcceptDeliverGifts", [
                    avatarId, "rADG"])
            # return an Accept message to the AI caller
        else:
            self.sendUpdateToChannel(
                replyToChannelId, "receiveRejectDeliverGifts", [
                    avatarId, "rRDG"])
            # return a Reject message to the AI caller
