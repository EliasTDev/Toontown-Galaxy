...
# ===============================================================================
    ...
    def requestPartyZone(self, avId, zoneId, callback):
        assert(
            self.notify.debug(
                f"requestPartyZone : avId={avId} zoneId={zoneId}"))
        if zoneId < 0:
            zoneId = 0
    ...
    def removeGuest(self, ownerId, avId):
        self.notify.debug(
            f"removeGuest ownerId = {ownerId}, avId = {avId}")
    ...
    def sendAvatarToParty(self, hostId):
        ...
        DistributedPartyManager.notify.debug(
            f"sendAvatarToParty hostId = {hostId}")
    ...
    def sendShardIdZoneIdToAvatar(self, shardId, zoneId):
        ...
        """
        DistributedPartyManager.notify.debug(
            f"sendShardIdZoneIdToAvatar shardId = {shardId}  zoneId = {zoneId}")
