...
class ttInviteDb:
    ...
    def __init__(self, host, port, user, passwd, db):
        ...
        try:
            ...
            self.notify.warning(
                "Failed to connect to MySQL db=%s at %s:%d.  ttInvitedb DB is disabled." %
                (db, host, port))
        ...
        self.notify.info(
            "Connected to invitedb=%s at %s:%d." %
            (db, host, port))
        ...
        try:
            ...
            if not cursor.rowcount:
                # we know the ttInviteStatus table doesn't exist, create it
                # again
                cursor.execute("""
            ...
            if __debug__:
                self.notify.info(
                    "Table ttInvite did not exist, created a new one!")
        except MySQLdb.OperationalError as e:
            pass
        ...
        try:
            ...
            self.notify.debug(
                f"{self.dbname} database not found, ttInvite not active.")
            self.sqlAvailable = False
    ...
    def getInvites(self, avatarId, isRetry=False):
        ...
        try:
            ...
            res = cursor.fetchall()
            self.notify.debug(
                f"Select was successful in ttInvitedb, returning {str(res)}")
            return res

        except MySQLdb.OperationalError as e:
            if isRetry:
                self.notify.warning(
                    f"Error on getInvites retry, giving up:\n{str(e)}")
                return ()
                ...
                self.notify.warning(
                    f"Unknown error in getInvites, retrying:\n{str(e)}")
                self.reconnect()
                return self.getInvites(avatarId, True)
        except Exception as e:
            self.notify.warning(
                f"Unknown error in getInvites, giving up:\n{str(e)}")
            return ()
    ...
    def putInvite(self, partyId, inviteeId, isRetry=False):
        ...
        try:
            ...
            if isRetry:
                self.notify.warning(
                    f"Error on putInvite retry, giving up:\n{str(e)}")
                return
            elif e[0] == SERVER_GONE_ERROR or e[0] == SERVER_LOST:
                self.reconnect()
                ...
                self.notify.warning(
                    f"Unknown error in putInvite, retrying:\n{str(e)}")
                self.reconnect()
                self.putInvite(partyId, inviteeId, True)
        except Exception as e:
            self.notify.warning(
                f"Unknown error in putInvite, giving up:\n{str(e)}")
            return
    ...
    def deleteInviteByParty(self, partyId, isRetry=False):
        ...
        try:
            ...
            if isRetry:
                self.notify.warning(
                    f"Error in deleteInviteByParty retry, giving up:\n{str(e)}")
                return
            elif e[0] == SERVER_GONE_ERROR or e[0] == SERVER_LOST:
                self.reconnect()
                ...
                self.notify.warning(
                    f"Unnown error in deleteInviteByParty, retrying:\n{str(e)}")
                self.reconnect()
                self.deleteMail(accountId, messageId, True)
        except Exception as e:
            self.notify.warning(
                f"Unknown error in deleteInviteByParty, giving up:\n{str(e)}")
            return
    ...
    def getReplies(self, partyId, isRetry=False):
        ...
        try:
            ...
            if isRetry:
                self.notify.warning(
                    f"Error on getReplies retry, giving up:\n{str(e)}")
                return ()
            elif e[0] == SERVER_GONE_ERROR or e[0] == SERVER_LOST:
                self.reconnect()
                ...
                self.notify.warning(
                    f"Unknown error in getReplies, retrying:\n{str(e)}")
                self.reconnect()
                return self.getReplies(partyId, True)
        except Exception as e:
            self.notify.warning(
                f"Unknown error in getReplies, giving up:\n{str(e)}")
            return ()
    ...
    def getOneInvite(self, inviteKey, isRetry=False):
        ...
        try:
            ...
            if isRetry:
                self.notify.warning(
                    f"Error on getOneInvite retry, giving up:\n{str(e)}")
                return ()
            elif e[0] == SERVER_GONE_ERROR or e[0] == SERVER_LOST:
                self.reconnect()
                ...
                self.notify.warning(
                    f"Unknown error in getOneInvite, retrying:\n{str(e)}")
                self.reconnect()
                return self.getOneInvite(partyId, True)
        except Exception as e:
            self.notify.warning(
                f"Unknown error in getOneInvite, giving up:\n{str(e)}")
            return ()
    ...
    def updateInvite(self, inviteKey, newStatus, isRetry=False):
        ...
        try:
            ...
            if isRetry:
                self.notify.warning(
                    f"Error on updateInvite retry, giving up:\n{str(e)}")
                return ()
            elif e[0] == SERVER_GONE_ERROR or e[0] == SERVER_LOST:
                self.reconnect()
                ...
                self.notify.warning(
                    f"Unknown error in updateInvite, retrying:\n{str(e)}")
                self.reconnect()
                return self.updateInvite(newStatus, inviteKey, True)
        except Exception as e:
            self.notify.warning(
                f"Unknown error in updateInvite, giving up:\n{str(e)}")
            return ()
    ...
    def getInviteesOfParty(self, inviteKey, isRetry=False):
        ...
        try:
            ...
            if isRetry:
                self.notify.warning(
                    f"Error on getInviteesOfParty retry, giving up:\n{str(e)}")
                return ()
            elif e[0] == SERVER_GONE_ERROR or e[0] == SERVER_LOST:
                self.reconnect()
                ...
                self.notify.warning(
                    f"Unknown error in getInviteesOfParty, retrying:\n{str(e)}")
                self.reconnect()
                return self.getInviteesOfParty(partyId, True)
        except Exception as e:
            self.notify.warning(
                f"Unknown error in getInviteesOfParty, giving up:\n{str(e)}")
            return ()
